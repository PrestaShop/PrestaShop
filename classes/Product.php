	...
	*/
	public static function getNewProducts($id_lang, $page_number = 0, $nb_products = 10, $count = false, $order_by = null, $order_way = null, Context $context = null)
	{
		...
		$sql->select(
			...
			product_shop.`date_add` > "'.date('Y-m-d', strtotime('-'.(Configuration::get('PS_NB_DAYS_NEW_PRODUCT') ? (int)Configuration::get('PS_NB_DAYS_NEW_PRODUCT') : 20).' DAY')).'" as new '.(Configuration::get('PS_COMBINATION_FEATURE_ACTIVE')==1 ? ", product_attribute_shop.minimal_quantity AS product_attribute_minimal_quantity" : "")
		);
