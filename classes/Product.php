...
class ProductCore extends ObjectModel
{
    ...
    */
    public static function getDefaultAttribute($id_product, $minimum_quantity = 0, $reset = false)
    {
        ...
        $result_no_filter = Db::getInstance()->getValue('
    ...
    */
        ...
        $only_active = false, Context $context = null)
    {
        ...
        if ($order_by == 'price') {
            Tools::orderbyPrice($results, $order_way);
        }
    ...
    public function generateMultipleCombinations($combinations, $attributes)
    {
        ...
        foreach ($combinations as $key => $combination) {
            ...
            $object = new Combination($id_combination);

            if ($id_combination) {
                $object->minimal_quantity = 1;
                $object->available_date = '0000-00-00';
            }
            ...
            if (!$id_combination) {
                ...
                foreach ($attributes[$key] as $id_attribute) {
                    $attribute_list[] = array(
                        'id_product_attribute' => (int)$object->id,
                        'id_attribute' => (int)$id_attribute
                    );
                }

                $result &= Db::getInstance()->insert('product_attribute_combination', $attribute_list);
            }
        }
    ...
    */
        ...
        $id_images, $reference, $id_supplier, $ean13, $default, $location = null, $upc = null, $minimal_quantity = 1, array $id_shop_list = array(), $available_date = null)
    {
        ...
        return (!$id_product_attribute || !$result) ? false : $id_product_attribute;
    }
    ...
    */
    ...
    public static function updateDefaultAttribute($id_product)
    {
        ...
        return ($result && $id_default_attribute) ? $id_default_attribute : $result;
    }
    ...
    */
    public function deleteCustomization()
    {
        ...
        return $result && Db::getInstance()->execute('
            DELETE `'._DB_PREFIX_.'customization_field_lang`
            FROM `'._DB_PREFIX_.'customization_field_lang`
            LEFT JOIN `'._DB_PREFIX_.'customization_field`
    ...
    public function deleteFeatures()
    {
        ...
        $features = Db::getInstance()->executeS('
            ...
            LEFT JOIN `'._DB_PREFIX_.'feature_value` as f
                ON (f.`id_feature_value` = p.`id_feature_value`)
            WHERE `id_product` = '.(int)$this->id);
    ...
    */
    public function getAttributesResume($id_lang, $attribute_value_separator = ' - ', $attribute_separator = ', ')
    {
        ...
        $lang = Db::getInstance()->executeS('
            ...
            GROUP_CONCAT(agl.`name`, \''.pSQL($attribute_value_separator).'\',al.`name`
    ...
    */
    public function getAttributeCombinations($id_lang)
    {
        ...
        $results = Db::getInstance()->executeS('
            SELECT pa.*, product_attribute_shop.*, ag.`id_attribute_group`, ag.`is_color_group`, agl.`name` AS group_name, al.`name` AS attribute_name, a.`id_attribute`
            FROM `'._DB_PREFIX_.'product_attribute` pa
            '.Shop::addSqlAssociation('product_attribute', 'pa').'
        ...
        foreach ($results as $key => $row) {
            ...
            if (Cache::isStored($cache_key)) {
                $results[$key]['quantity'] = Cache::retrieve($cache_key);
            } else {
                $quantities = StockAvailable::getQuantityAvailableByProduct($row['id_product'], $row['id_product_attribute']);

                Cache::store($cache_key, $quantities);

                $results[$key]['quantity'] = $quantities;
            }
        }
    ...
    */
    public function getAttributeCombinationsById($id_product_attribute, $id_lang)
    {
        ...
        foreach ($result as $key => $row) {
            ...
            if (Cache::isStored($cache_key)) {
                $result[$key]['quantity'] = Cache::retrieve($cache_key);
            } else {
                $quantities = StockAvailable::getQuantityAvailableByProduct($row['id_product'], $row['id_product_attribute']);

                Cache::store($cache_key, $quantities);

                $result[$key]['quantity'] = $quantities;
            }
        }
    ...
    */
    public static function getRandomSpecial($id_lang, $beginning = false, $ending = false, Context $context = null)
    {
        ...
        if ($product_reductions) {
            ...
            Db::getInstance()->execute('
                CREATE TEMPORARY TABLE `'._DB_PREFIX_.'product_reductions`
                (id_product INT UNSIGNED NOT NULL DEFAULT 0, id_product_attribute INT UNSIGNED NOT NULL DEFAULT 0) ENGINE=MEMORY', false);

            if ($ids_products) {
                Db::getInstance()->execute('
                    INSERT INTO `'._DB_PREFIX_.'product_reductions`
                    VALUES '.$ids_products, false);
            }

            $groups = FrontController::getCurrentCustomerGroups();

            $sql_groups = ' AND EXISTS(
                SELECT 1
                FROM `'._DB_PREFIX_.'category_product` cp
            ...
            $result = Db::getInstance()->getRow('
                SELECT product_shop.id_product, IFNULL(product_attribute_shop.id_product_attribute,0) id_product_attribute
                FROM
                ...
                WHERE p.id_product=pr.id_product
                    ...
                    OR product_attribute_shop.id_product_attribute = pr.id_product_attribute)
            ...
            $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow('
                SELECT p.*, product_shop.*, stock.`out_of_stock` out_of_stock, pl.`description`, pl.`description_short`,
    ...
    */
        ...
        $order_by = null, $order_way = null, $beginning = false, $ending = false, Context $context = null)
    {
        ...
        foreach ($ids_product as $product) {
            $tab_id_product[] = is_array($product) ? (int)$product['id_product'] : (int)$product;
        }
        ...
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            WHERE product_shop.`active` = 1
                ...
                '.((!$beginning && !$ending) ? ' AND p.`id_product` IN ('.((is_array($tab_id_product) && !empty($tab_id_product)) ? implode(', ', $tab_id_product) : 0).')' : '').'
            '.$sql_groups.'
    ...
    public static function getProductCategoriesFull($id_product = '', $id_lang = null)
    {
        ...
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN `'._DB_PREFIX_.'category_lang` cl
                ON (cp.`id_category` = cl.`id_category`'.Shop::addSqlRestrictionOnLang('cl').')
    ...
    public function setCarriers($carrier_list)
    {
        ...
        // Since PHP 5.2.9
        $unique_array = array_unique($data, SORT_REGULAR);
    ...
    */
        ...
        $use_customer_price = true)
    {
        ...
        // Initializations
        $id_group = $id_customer ? Customer::getDefaultGroupId((int)$id_customer) : null;
        ...
        if ((int)$id_cart) {
            ...
            if (!Cache::isStored($cache_id) || ($cart_quantity = (Cache::retrieve($cache_id) != (int)$quantity))) {
                $cart_quantity = (int)Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('
                    SELECT SUM(`quantity`)
        ...
        $id_customer = 0, $use_customer_price = true, $id_cart = 0, $real_quantity = 0)
    {
        ...
        if (($only_reduc || $use_reduc) && $specific_price) {
            if ($specific_price['reduction_type'] == 'amount') {
                ...
                } elseif ($use_tax && !$specific_price['reduction_tax']) {
        ...
        self::$_prices[$cache_id] = ($price < 0) ? 0 : $price;
    ...
    public static function isDiscounted($id_product, $quantity = 1, Context $context = null)
    {
        ...
        $cart_quantity = $context->cart ? Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('
            SELECT SUM(`quantity`)
            FROM `'._DB_PREFIX_.'cart_product`
            WHERE `id_product` = '.(int)$id_product.'
    ...
    */
    ...
    public function checkQty($qty)
    {
        ...
        $available = StockAvailable::getQuantityAvailableByProduct($this->id, $id_product_attribute);
    ...
    public function checkDefaultAttributes()
    {
        ...
        return ObjectModel::updateMultishopTable('Combination', array('default_on' => 1), 'a.id_product_attribute = '.(int)$mini['id_attr']);
    }
    ...
    public function getAccessories($id_lang, $active = true)
    {
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN `'._DB_PREFIX_.'product_attribute_shop` product_attribute_shop
                ON (p.`id_product` = product_attribute_shop.`id_product`
                    AND product_attribute_shop.`default_on` = 1
                    AND product_attribute_shop.id_shop='.(int)$this->id_shop.')
            LEFT JOIN `'._DB_PREFIX_.'product_lang` pl ON (
                p.`id_product` = pl.`id_product`
                AND pl.`id_lang` = '.(int)$id_lang.Shop::addSqlRestrictionOnLang('pl').'
            )
            ...
            LEFT JOIN `'._DB_PREFIX_.'image_shop` image_shop
                ON (image_shop.`id_product` = p.`id_product`
                    AND image_shop.cover=1
                    AND image_shop.id_shop='.(int)$this->id_shop.')
            LEFT JOIN `'._DB_PREFIX_.'image_lang` il
                ON (image_shop.`id_image` = il.`id_image`
                    AND il.`id_lang` = '.(int)$id_lang.')
            LEFT JOIN `'._DB_PREFIX_.'manufacturer` m
    ...
    public static function getAccessoryById($accessory_id)
    {
        return Db::getInstance()->getRow('
            ...
            WHERE `id_product` = '.(int)$accessory_id);
    }
    ...
    */
    public static function searchByName($id_lang, $query, Context $context = null)
    {
        ...
        $where = 'pl.`name` LIKE \'%'.$psql_query.'%\'
            OR p.`ean13` LIKE \'%'.$psql_query.'%\'
            OR p.`upc` LIKE \'%'.$psql_query.'%\'
            OR p.`reference` LIKE \'%'.$psql_query.'%\'
            OR p.`supplier_reference` LIKE \'%'.$psql_query.'%\'
            OR EXISTS(SELECT * FROM `'._DB_PREFIX_.'product_supplier` sp WHERE sp.`id_product` = p.`id_product` AND `product_supplier_reference` LIKE \'%'.$psql_query.'%\')';

        $sql->orderBy('pl.`name` ASC');

        if (Combination::isFeatureActive()) {
            $where .= ' OR EXISTS(SELECT * FROM `'._DB_PREFIX_.'product_attribute` `pa` WHERE pa.`id_product` = p.`id_product` AND (pa.`reference` LIKE \'%'.$psql_query.'%\'
                OR pa.`supplier_reference` LIKE \'%'.$psql_query.'%\'
                OR pa.`ean13` LIKE \'%'.$psql_query.'%\'
                OR pa.`upc` LIKE \'%'.$psql_query.'%\'))';
        }
    ...
    public static function duplicateAttributes($id_product_old, $id_product_new)
    {
        ...
        foreach ($result as $row) {
            ...
            if (!isset($combinations[$id_product_attribute_old])) {
                ...
                $sub_result = Db::getInstance()->executeS('
                    SELECT *
            ...
            if (!isset($combinations[$id_product_attribute_old])) {
                ...
                foreach ($sub_result as $sub_row) {
                    $sub_row['id_product_attribute'] = $id_product_attribute_new;
                    $return &= Db::getInstance()->insert('product_attribute_combination', $sub_row);
                }
            } else {
            ...
            foreach ($sub_result as $sub_row) {
                unset($sub_row['id_product_supplier']);

                $sub_row['id_product'] = $id_product_new;
                $sub_row['id_product_attribute'] = $id_product_attribute_new;

                $return &= Db::getInstance()->insert('product_supplier', $sub_row);
            }
        }

        $impacts = self::getAttributesImpacts($id_product_old);

        if (is_array($impacts) && !empty($impacts)) {
        ...
        return $return ? $combination_images : false;
    }
    ...
    */
    ...
    {
        $results = Db::getInstance()->executeS('
            SELECT `display_filename`, `filename`, `date_add`, `date_expiration`, `nb_days_accessible`, `nb_downloadable`, `active`, `is_shareable`
    ...
    public static function duplicateAttachments($id_product_old, $id_product_new)
    {
        ...
        $result = Db::getInstance()->insert('product_attachment', $data);
    ...
    */
    ...
    protected static function _getCustomizationFieldsNLabels($product_id, $id_shop = null)
    {
        ...
        $customizations['fields'] = Db::getInstance()->executeS('
        ...
        $customization_labels = Db::getInstance()->executeS('
            SELECT `id_customization_field`, `id_lang`, `name`
            FROM `'._DB_PREFIX_.'customization_field_lang`
            WHERE `id_customization_field` IN ('.implode(', ', $customization_field_ids).')'.($id_shop ? '
                AND cfl.`id_shop` = '.$id_shop : '').'
            ORDER BY `id_customization_field`');

        if (!$customization_labels) {
            return false;
        }
    ...
    */
    ...
    public function getTags($id_lang)
    {
        ...
        return implode(', ', $this->tags[$id_lang]);
    }
    ...
    public static function getProductProperties($id_lang, $row, Context $context = null)
    {
        ...
        // Pack management
        $row['pack'] = isset($row['cache_is_pack']) ? (int)$row['cache_is_pack'] : Pack::isPack($row['id_product']);
        ...
        if (isset($row['customizable']) && $row['customizable'] && Customization::isFeatureActive()) {
            if (!empty(Product::getRequiredCustomizableFieldsStatic((int)$row['id_product']))) {
    ...
    public static function getTaxesInformations($row, Context $context = null)
    {
        ...
        $id_tax_rules_group = Product::getIdTaxRulesGroupByIdProduct((int)$row['id_product'], $context);
    ...
    public static function getProductsProperties($id_lang, $query_result)
    {
        ...
        if (is_array($query_result)) {
            foreach ($query_result as $row) {
                if ($roperties = Product::getProductProperties($id_lang, $row)) {
                    $results_array[] = $roperties;
                }
            }
        }
    ...
    */
    ...
    public static function getAttachmentsStatic($id_lang, $id_product)
    {
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN '._DB_PREFIX_.'attachment a
    ...
    */
    ...
    protected function _createLabel($languages, $type)
    {
        ...
        // Label insertion
        if (!$result || !$id_customization_field = (int)Db::getInstance()->Insert_ID()) {
            return false;
        }
    ...
    public function updateLabels()
    {
                ...
                if (Shop::isFeatureActive()) {
                    foreach (Shop::getContextListShopID() as $id_shop) {
                        $result = Db::getInstance()->execute('
                            ...
                            VALUES ('.(int)$tmp[2].', '.(int)$tmp[3].', '.$id_shop.', \''.pSQL($value).'\')
                    ...
                    VALUES ('.(int)$tmp[2].', '.(int)$tmp[3].', \''.pSQL($value).'\')
                    ON DUPLICATE KEY UPDATE `name` = \''.pSQL($value).'\'');
        ...
        return (bool)$this->_deleteOldLabels();
    }
    ...
    public function getCustomizationFields($id_lang = false, $id_shop = null)
    {
        ...
        $result = Db::getInstance()->executeS('
            ...
            WHERE cf.`id_product` = '.(int)$this->id.($id_lang ? '
    ...
    public static function checkAccessStatic($id_product, $id_customer)
    {
        ...
        if (!$id_customer) {
            ...
            SELECT ctg.`id_group`
            FROM `'._DB_PREFIX_.'category_product` cp
            INNER JOIN `'._DB_PREFIX_.'category_group` ctg
                ON (ctg.`id_category` = cp.`id_category`)
            WHERE cp.`id_product` = '.(int)$id_product.'
    ...
    public static function getUrlRewriteInformations($id_product)
    {
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN `'._DB_PREFIX_.'category_lang` cl
                ...
                AND cl.`id_lang` = pl.`id_lang`'.Shop::addSqlRestrictionOnLang('cl').')
            WHERE p.`id_product` = '.(int)$id_product.'
                AND l.`active` = 1
        ');
    }
    ...
    */
    public function getWsAccessories()
    {
        return Db::getInstance()->executeS('
            ...
            LEFT JOIN `'._DB_PREFIX_.'product` p
                ON (p.id_product = a.id_product_2)
            '.Shop::addSqlAssociation('product', 'p').'
    ...
    public function setWsAccessories($accessories)
    {
        ...
        foreach ($accessories as $accessory) {
            Db::getInstance()->execute('
                ...
                VALUES ('.(int)$this->id.', '.(int)$accessory['id'].')');
        }
        ...
        foreach ($to_add as $id) {
            ...
            if (in_array($id, $all_ids)) {
                Db::getInstance()->execute('
                    ...
                    SET id_product = '.(int)$this->id.'
                    WHERE id_product_attribute='.$id);
            } else {
                Db::getInstance()->execute('
                    INSERT INTO `'._DB_PREFIX_.'product_attribute` (`id_product`)
                    VALUES ('.$this->id.')');
            }
        }
    ...
    */
    public function getWsPositionInCategory()
    {
        ...
        return empty($result) ? '' : $result[0]['position'];
    }
    ...
    */
        ...
        if (Module::isInstalled('blocklayered') && Module::isEnabled('blocklayered')) {
            $nb_custom_values = Db::getInstance()->executeS('
                ...
                LEFT JOIN `'._DB_PREFIX_.'layered_indexable_attribute_lang_value` la
                    ON (la.`id_attribute` = a.`id_attribute`
                        AND la.`id_lang` = '.(int)$id_lang.')
    ...
    public static function getAttributesInformationsByProduct($id_product)
    {
        ...
        if (Module::isInstalled('blocklayered') && Module::isEnabled('blocklayered')) {
            $nb_custom_values = Db::getInstance()->executeS('
                ...
                LEFT JOIN `'._DB_PREFIX_.'layered_indexable_attribute_lang_value` la
                    ON (la.`id_attribute` = a.`id_attribute`
                        AND la.`id_lang` = '.(int)Context::getContext()->language->id.')
                WHERE la.`url_name` IS NOT NULL
                    AND la.`url_name` != \'\'
                    AND pa.`id_product` = '.(int)$id_product);
    ...
    public function getAnchor($id_product_attribute, $with_id = false)
    {
        ...
        foreach ($attributes as &$attribute) {
            foreach ($attribute as &$attribute_link) {
                $attribute_link = str_replace($separator, '_', Tools::link_rewrite($attribute_link));
            }

            if ($with_id && isset($attribute['id_attribute']) && $attribute['id_attribute']) {
                $anchor .= '/'.(int)$attribute['id_attribute'].$separator.$attribute['group'].$separator.$attribute['name'];
            } else {
                $anchor .= '/'.$attribute['group'].$separator.$attribute['name'];
            }
        }
    ...
    */
    ...
    {
        return Db::getInstance()->executeS('
            SELECT id_product_item as id, quantity
            ...
            AND EXISTS (
                ...
                JOIN '._DB_PREFIX_.'product_attribute_combination pac
                    ON (pac.id_product_attribute
                        AND product_attribute_shop.id_product_attribute)
    ...
    public static function setPackStockType($id_product, $pack_stock_type)
    {
        return Db::getInstance()->execute('
            ...
            '.Shop::addSqlAssociation('product', 'p').'
            SET product_shop.pack_stock_type = '.(int)$pack_stock_type.'
            WHERE p.`id_product` = '.(int)$id_product);
    }
}
