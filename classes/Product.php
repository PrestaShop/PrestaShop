...
class ProductCore extends ObjectModel
{
    ...
    public static $definition = [
        ...
        'fields' => [
            ...
            'date_new' => ['type' => self::TYPE_DATE, 'shop' => true, 'validate' => 'isDate'],
    ...
    public function add($autodate = true, $null_values = false)
    {
        ...
        $this->date_new = date('Y-m-d H:i:s');
    ...
    public static function isNewStatic($idProduct)
    {
        ...
        $query = 'SELECT COUNT(p.id_product)
            ...
            AND DATEDIFF("' . date('Y-m-d') . ' 00:00:00", product_shop.`date_new`) < ' . $nbDaysNewProduct;
    ...
    public static function getNewProducts($id_lang, $page_number = 0, $nb_products = 10, $count = false, $order_by = null, $order_way = null, Context $context = null)
    {
        ...
        if ($count) {
            $sql = 'SELECT COUNT(p.`id_product`) AS nb
                    ...
                    AND DATEDIFF(product_shop.`date_new`, DATE_SUB("' . $now . '", INTERVAL ' . $nb_days_new_product . ' DAY)) > 0
                    ' . ($front ? ' AND product_shop.`visibility` IN ("both", "catalog")' : '') . '
                    ' . $sql_groups;
        ...
        $sql->select(
            ...
            (DATEDIFF(product_shop.`date_new`,
                DATE_SUB(
                    "' . $now . '",
        ...
        $sql->where('DATEDIFF(product_shop.`date_new`,
            DATE_SUB(
                "' . $now . '",
                INTERVAL ' . $nb_days_new_product . ' DAY
            )
