...
abstract class ObjectModelCore implements \PrestaShop\PrestaShop\Core\Foundation\Database\EntityInterface
{
    ...
    public function __construct($id = null, $id_lang = null, $id_shop = null, $translator = null)
    {
        ...
        if (null !== $id_lang) {
            $this->id_lang = (false !== Language::getLanguage($id_lang)) ? $id_lang : Configuration::get('PS_LANG_DEFAULT');
        }
    ...
    protected function formatFields($type, $id_lang = null)
    {
        ...
        foreach ($this->def['fields'] as $field => $data) {
            ...
            if ((self::FORMAT_LANG == $type && empty($data['lang']))
                || (self::FORMAT_SHOP == $type && empty($data['shop']))
                || (self::FORMAT_COMMON == $type && ((!empty($data['shop']) && 'both' != $data['shop']) || !empty($data['lang'])))) {
                continue;
            }

            if (is_array($this->update_fields)) {
                if ((!empty($data['lang']) || (!empty($data['shop']) && 'both' != $data['shop'])) && (empty($this->update_fields[$field]) || (self::FORMAT_LANG == $type && empty($this->update_fields[$field][$id_lang])))) {
                    continue;
                }
            }
            ...
            $value = $this->$field;
            if (self::FORMAT_LANG == $type && $id_lang && is_array($value)) {
            ...
            $purify = (isset($data['validate']) && 'iscleanhtml' == Tools::strtolower($data['validate'])) ? true : false;
            // Format field value
    ...
    public function add($auto_date = true, $null_values = false)
    {
        ...
        if (Shop::checkIdShopDefault($this->def['table'])) {
            $this->id_shop_default = (true == in_array(Configuration::get('PS_SHOP_DEFAULT'), $id_shop_list, true)) ? Configuration::get('PS_SHOP_DEFAULT') : min($id_shop_list);
        }
        ...
        if (!empty($this->def['multilang'])) {
            ...
            if ($fields && is_array($fields)) {
                ...
                foreach ($fields as $field) {
                    ...
                    if (false !== $asso && 'fk_shop' == $asso['type']) {
    ...
    public function update($null_values = false)
    {
        ...
        // Automatically fill dates
        if (array_key_exists('date_add', $this) && null == $this->date_add) {
            $this->date_add = date('Y-m-d H:i:s');
            if (isset($this->update_fields) && is_array($this->update_fields) && count($this->update_fields)) {
    ...
    public function validateField($field, $value, $id_lang = null, $skip = array(), $human_errors = false)
    {
        ...
        if (null === $ps_lang_default) {
            $ps_lang_default = Configuration::get('PS_LANG_DEFAULT');
        }

        if (null === $ps_allow_html_iframe) {
        ...
        if (!in_array('validate', $skip, true) && !empty($data['validate'])) {
            ...
            if (!empty($value)) {
                $res = true;
                if ('iscleanhtml' == Tools::strtolower($data['validate'])) {
    ...
    public static function displayFieldName($field, $class = __CLASS__, $htmlentities = true, Context $context = null)
    {
        ...
        if (null === $_FIELDS && file_exists(_PS_TRANSLATIONS_DIR_.$context->language->iso_code.'/fields.php')) {
    ...
    public function validateController($htmlentities = true)
    {
        ...
        foreach ($this->def['fields'] as $field => $data) {
            ...
            if (isset($data['required']) && $data['required'] && empty($value) && '0' !== $value) {
            ...
            if (!empty($value) || '0' === $value || ('postcode' == $field && '0' == $value)) {
    ...
    public function getWebserviceParameters($ws_params_attribute_name = null)
    {
        ...
        if (null === $ws_params_attribute_name) {
        ...
        foreach ($this->def['fields'] as $field_name => $details) {
            ...
            if ((isset($details['required']) && true === $details['required']) || in_array($field_name, $required_fields, true)) {
    ...
    public static function updateMultishopTable($classname, $data, $where = '')
    {
        ...
        foreach ($data as $field => $value) {
            ...
            if (!empty($def['fields'][$field]['shop'])) {
                if (null === $value && !empty($def['fields'][$field]['allow_null'])) {
    ...
    public static function getDefinition($class, $field = null)
    {
        ...
        if (null !== $field || !Cache::isStored($cache_id)) {
