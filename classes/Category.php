...
class CategoryCore extends ObjectModel
{
    ...
    public function delete()
    {
        ...
        $deleted_children = $categories = $this->getAllChildren();
        $categories[] = $this;
    ...
    public static function regenerateEntireNtree()
    {
        ...
        if (isset($categories_array[0]) && $categories_array[0]['subcategories']) {
            Category::_subTree($categories_array, $categories_array[0]['subcategories'][0], $nested);
        }
    }

    protected static function _subTree(&$categories, $id_category, &$nested)
    {
        $left = $nested++;

        if (isset($categories[(int)$id_category]['subcategories'])) {
            foreach ($categories[(int)$id_category]['subcategories'] as $id_subcategory) {
                Category::_subTree($categories, (int)$id_subcategory, $nested);
            }
        }
    ...
    public static function duplicateProductCategories($id_old, $id_new)
    {
        ...
        if ($result) {
            foreach ($result as $value) {
                $row[] = '('.implode(', ', array((int)$id_new, $value['id_category'], '(SELECT tmp.max + 1 FROM (
					SELECT MAX(cp.`position`) AS max
					FROM `'._DB_PREFIX_.'category_product` cp
					WHERE cp.`id_category`='.(int)$value['id_category'].') AS tmp)'
                )).')';
            }
        }
    ...
    public static function checkBeforeMove($id_category, $id_parent)
    {
        ...
        while (42) {
            ...
            if (!isset($result['id_parent'])) {
                return false;
            } elseif ($result['id_parent'] == $id_category) {
                return false;
            } elseif ($result['id_parent'] == Configuration::get('PS_HOME_CATEGORY')) {
                return true;
            }
...
* 	 * @param bool $method_to_create a category
    ...
    public function getParentsCategories($id_lang = null)
    {
        ...
        while (true) {
            ...
            if (Shop::isFeatureActive() && (Shop::getContext() == Shop::CONTEXT_SHOP)) {
                ...
                if (!Tools::isSubmit('id_category') || ((int)Tools::getValue('id_category') == (int)$root_category->id) || ((int)$root_category->id == (int)$context->shop->id_category)) {
    ...
    */
    ...
    public function addGroupsIfNoExist($id_group)
    {
        ...
        return (in_array((int)$id_group, $groups)) ? false : $this->addGroups(array((int)$id_group));
    }
        ...
        if (!$id_customer) {
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('
    			...
    			WHERE ctg.`id_category` = '.(int)$this->id.'
                    AND ctg.`id_group` = '.(int)Group::getCurrent()->id);
        } else {
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('
    			SELECT ctg.`id_group`
    			FROM '._DB_PREFIX_.'category_group ctg
    			INNER JOIN '._DB_PREFIX_.'customer_group cg
                    ON (cg.`id_group` = ctg.`id_group`
                    AND cg.`id_customer` = '.(int)$id_customer.')
    			WHERE ctg.`id_category` = '.(int)$this->id);
        }

        Cache::store($cache_id, (bool)$result);
    ...
    public function updateGroup($list)
    {
        ...
        if (empty($list)) {
            $list = Configuration::getMultiple(array('PS_UNIDENTIFIED_GROUP', 'PS_GUEST_GROUP', 'PS_CUSTOMER_GROUP'));
        }
    ...
    public static function setNewGroupForHome($id_group)
    {
        if ((int)$id_group) {
            ...
            return Db::getInstance()->execute('
        		...
        		VALUES ('.(int)$category.', '.(int)$id_group.')');
        }
    ...
    public function updatePosition($way, $position)
    {
        ...
        $result = Db::getInstance()->execute('
			...
			SET category_shop.`position`= category_shop.`position` '.($way ? '-' : '+').' 1
			WHERE category_shop.`position` '.($way
    ...
    public function getWsNbProductsRecursive()
    {
        ...
        return ($nb_product_recursive) ? $nb_product_recursive : -1;
    }
    ...
    public static function getCategoryInformations($ids_category, $id_lang = null)
    {
        ...
        if (!is_array($ids_category) || empty($ids_category)) {
            return;
        }
            ...
            if (Shop::getContext() != Shop::CONTEXT_SHOP) {
                foreach (Shop::getContextListShopID() as $id_shop) {
                    $return &= Db::getInstance()->execute('
                        ...
                        VALUES ('.(int)$this->id.', '.(int)$id_shop.', '.(int)$position.')
