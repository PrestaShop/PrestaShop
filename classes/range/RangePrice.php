...
class RangePriceCore extends ObjectModel
{
    ...
    public static function getRanges($id_carrier)
    {
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            SELECT *
            FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'range_price') . '
    ...
    public static function rangeExist($id_carrier, $delimiter1, $delimiter2, $id_reference = null)
    {
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue('
            SELECT count(*)
            FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'range_price') . ' rp' .
            (null === $id_carrier && $id_reference ? '
            INNER JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'carrier') . ' c on (rp.id_carrier = c.id_carrier)' : '') . '
            WHERE' .
