...
class OrderDeliveryDetailCore extends ObjectModel
{
	...
	public static $definition = array(
		...
		'fields' => array(
			...
			'delivery_id' => 			array('type' => self::TYPE_INT, 'validate' => 'isUnsignedId'),
	...
	/**
	 * Create an order delivery detail liable to an id_order
	 * @param object $order
	 * @param object $cart
	 * @param array $product
	...
	/**
	 * Create a list of order delivery detail for a specified id_order using cart
	 * @param object $order
	 * @param object $cart
	 * @param int $id_order_status
	 * @param int $id_order_invoice
	...
	*/
	...
	public function saveTaxCalculator(Order $order, $replace = false)
	{
		...
		if ($replace)
			Db::getInstance()->execute('DELETE FROM `'._DB_PREFIX_.'order_delivery_detail_tax` WHERE id_order_detail='.(int)$this->id);

		$values = rtrim($values, ',');
		$sql = 'INSERT INTO `'._DB_PREFIX_.'order_delivery_detail_tax` (id_order_detail, id_tax, unit_amount, total_amount)
				VALUES '.$values;
