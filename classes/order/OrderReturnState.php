...
class OrderReturnStateCore extends ObjectModel
{
    ...
    public static function getOrderReturnStates($id_lang)
    {
        ...
        SELECT *
        FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'order_return_state') . ' ors
        LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'order_return_state_lang') . ' orsl ON (ors.id_order_return_state = orsl.id_order_return_state AND orsl.id_lang = ' . (int) $id_lang . ')
