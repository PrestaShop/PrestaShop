...
class OrderDeliveryCore extends ObjectModel
{
	...
	{
		$qty = 0;
		$qty_array = Db::getInstance()->executeS('
		...
		if (isset($qty_array[0]))
			$qty = $qty_array[0]['product_quantity'];
		return $qty;
	}
	...
	public function createDelivery($delivery_number, $order, $product, $qty, $id_warehouse)
	{
		...
		if (Configuration::get('PS_EDS_INVOICE_DELIVERED'))
		{
			$id_order_invoice = 0; // Reset it to 0, since we should create new invoice
			$free_shipping = false;
	...
	public function createDeliveryDetail($order, $product_table, $qty, $delivery_id, $id_warehouse, $order_invoice = false)
	{
		...
		$order_delivery_detail->createDetailList($order, $cart, $order->getCurrentOrderState(), $cart->getProducts(),
			(isset($order_invoice) ? $order_invoice->id : 0), $use_taxes, $id_warehouse, $delivery_id);
	...
	public function updateDeliveryDetail($order, $product_table, $qty, $delivery_id, $id_warehouse)
	{
		...
		$order_delivery_detail->updateDetailList($order, $cart, $order->getCurrentOrderState(), $cart->getProducts(),
			(isset($order_invoice) ? $order_invoice->id : 0), $use_taxes, $id_warehouse, $delivery_id);

		Db::getInstance()->update('order_delivery', array('date_add' => date('Y-m-d H:i:s') ), '`delivery_id` = '.$delivery_id ); // update delivery date when adding product
	}

	public function getOrderDetailId($product_id, $product_attribute_id, $delivery_id, $id_order_invoice, $id_order)
	{
