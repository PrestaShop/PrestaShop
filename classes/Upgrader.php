...
class UpgraderCore
{
    ...
    protected function addChangedFile($path)
    {
        ...
        if (false !== mb_strpos($path, 'mails/')) {
            $this->changed_files['mail'][] = $path;
        } elseif (
            false !== mb_strpos($path, '/en.php')
            || false !== mb_strpos($path, '/fr.php')
            || false !== mb_strpos($path, '/es.php')
            || false !== mb_strpos($path, '/it.php')
            || false !== mb_strpos($path, '/de.php')
            || false !== mb_strpos($path, 'translations/')
        ) {
