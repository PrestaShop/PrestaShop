    ...
    public function getStatsSales($id_product, $employee)
    {
        ...
        if ($implode) {
                if (Configuration::get('REFERER_SHIPPING')) {
                        $sql = 'SELECT COUNT(id_order) AS orders, (SUM('.(Configuration::get('REFERER_TAX')? 'total_paid_tax_excl':'total_paid_real').' / conversion_rate)
                                                -SUM('.(Configuration::get('REFERER_TAX')? 'total_shipping_tax_excl':'total_shipping_tax_incl').' / conversion_rate)) AS sales
                                                FROM '._DB_PREFIX_.'orders
                                                WHERE id_order IN ('.implode($implode, ',').')
                                                        '.Shop::addSqlRestriction(Shop::SHARE_ORDER).'
                                                        AND valid = 1';
                } else {
                        $sql = 'SELECT COUNT(id_order) AS orders, SUM('.(Configuration::get('REFERER_TAX')? 'total_paid_tax_excl':'total_paid_real').' / conversion_rate) AS sales
                                                FROM '._DB_PREFIX_.'orders
					        WHERE id_order IN ('.implode($implode, ',').')
                                                        '.Shop::addSqlRestriction(Shop::SHARE_ORDER).'
                                                        AND valid = 1';
