...
class ToolsCore
{
    ...
    ** For other purposes use the smarty function instead */
    ...
    public static function generateHtaccess($path = null, $rewrite_settings = null, $cache_control = null, $specific = '', $disable_multiviews = null, $medias = false, $disable_modsec = null)
    {
        ...
        if ($specific_before) {
            fwrite($write_fd, trim($specific_before) . PHP_EOL . PHP_EOL);
        }

        $domains = self::getDomains();

        // Write data in .htaccess file
        fwrite($write_fd, '# ~~start~~ Do not remove this comment, Prestashop will keep automatically the code outside this comment when .htaccess will be generated again' . PHP_EOL);
        fwrite($write_fd, '# .htaccess automaticaly generated by PrestaShop e-commerce open-source solution' . PHP_EOL);
        fwrite($write_fd, '# http://www.prestashop.com - http://www.prestashop.com/forums' . PHP_EOL . PHP_EOL);

        if ($disable_modsec) {
            fwrite($write_fd, '<IfModule mod_security.c>' . PHP_EOL . 'SecFilterEngine Off' . PHP_EOL . 'SecFilterScanPOST Off' . PHP_EOL . '</IfModule>' . PHP_EOL . PHP_EOL);
        }

        // RewriteEngine
        fwrite($write_fd, '<IfModule mod_rewrite.c>' . PHP_EOL . PHP_EOL);

        // Ensure HTTP_MOD_REWRITE variable is set in environment
        fwrite($write_fd, '  RewriteEngine on' . PHP_EOL . PHP_EOL);
        fwrite($write_fd, '  <IfModule mod_env.c>' . PHP_EOL);
        fwrite($write_fd, '    SetEnv HTTP_MOD_REWRITE On' . PHP_EOL);
        fwrite($write_fd, '  </IfModule>' . PHP_EOL . PHP_EOL);

        // If multiviews is disabled
        if ($disable_multiviews) {
            fwrite($write_fd, '  # Disable Multiviews' . PHP_EOL . '  Options -Multiviews' . PHP_EOL . PHP_EOL);
        }
        ...
        if (Configuration::get('PS_WEBSERVICE_CGI_HOST')) {
            fwrite($write_fd, '  RewriteCond %{HTTP:Authorization} ^(.*)' . PHP_EOL . 'RewriteRule . - [E=HTTP_AUTHORIZATION:%1]' . PHP_EOL . PHP_EOL);
        }
        ...
        foreach ($domains as $domain => $list_uri) {
            ...
            foreach ($list_uri as $uri) {
                fwrite($write_fd, '  # Domain: ' . $domain . PHP_EOL . PHP_EOL);

                // ENV
                fwrite($write_fd, '  # ENV' . PHP_EOL . '  RewriteRule . - [E=REWRITEBASE:' . $uri['physical'] . ']' . PHP_EOL . PHP_EOL);

                // API
                fwrite($write_fd, '  # API' . PHP_EOL . '  RewriteRule ^api/(.*)$ %{ENV:REWRITEBASE}webservice/dispatcher.php?url=$1 [QSA,L]' . PHP_EOL . PHP_EOL);
                ...
                if ($uri['virtual']) {
                    if (!$rewrite_settings) {
                        ...
                        fwrite($write_fd, 'RewriteRule ^' . trim($uri['virtual'], '/') . '/?$ ' . $uri['physical'] . $uri['virtual'] . 'index.php [L,R]' . PHP_EOL);
                    } else {
                        fwrite($write_fd, $media_domains);
                        fwrite($write_fd, 'RewriteRule ^' . trim($uri['virtual'], '/') . '$ ' . $uri['physical'] . $uri['virtual'] . ' [L,R]' . PHP_EOL);
                    }
                    fwrite($write_fd, $media_domains);
                    fwrite($write_fd, 'RewriteRule ^' . ltrim($uri['virtual'], '/') . '(.*) ' . $uri['physical'] . '$1 [L]' . PHP_EOL . PHP_EOL);
                }

                if ($rewrite_settings) {
                    // Rewrite URL of product images
                    fwrite($write_fd, $media_domains);
                    fwrite($write_fd, '  # Product URL' . PHP_EOL . '  RewriteRule ^([0-9])([0-9]*(?:\-[_a-z0-9-]*)?(?:-[0-9]+)?)\/(.+)\.(jpe?g|gif|png|webp|svg)(?:\/img\/p\/((?:[0-9]\/)+)(.*))?$ $2/$3.$4/img/p/$5$1/$6$1 [NC,N,L]' . PHP_EOL . '  RewriteRule ^(?:\-[_a-z0-9-]*)?(?:-[0-9]+)?\/.+\.(jpe?g|gif|png|webp|svg)\/(img\/p\/([0-9]\/)+.*)$ %{ENV:REWRITEBASE}$2.$1 [NC,L]' . PHP_EOL . PHP_EOL);

                    // Rewrite URL of category image
                    fwrite($write_fd, $media_domains);
                    fwrite($write_fd, '  # Category URL' . PHP_EOL . '  RewriteRule ^c\/([a-z_-]+|[0-9]+-[\.*_a-z0-9-]*(?:-[0-9]+)?)\/.+\.(jpe?g|gif|png|webp|svg)$ %{ENV:REWRITEBASE}img/c/$1.$2 [NC,L]' . PHP_EOL . PHP_EOL);
                }

                fwrite($write_fd, '  # AlphaImageLoader for IE and fancybox' . PHP_EOL);
                fwrite($write_fd, '  ReWriteRule ^images_ie\/?([^\/]+)\.(jpe?g|gif|png|webp|svg)$ %{ENV:REWRITEBASE}js/jquery/plugins/fancybox/images/$1.$2 [NC,L]' . PHP_EOL . PHP_EOL);

                // Redirections to dispatcher
                fwrite($write_fd, '  # Dispatcher' . PHP_EOL);
                fwrite($write_fd, '  RewriteCond %{REQUEST_FILENAME} -s [OR]' . PHP_EOL);
                fwrite($write_fd, '  RewriteCond %{REQUEST_FILENAME} -l [OR]' . PHP_EOL);
                fwrite($write_fd, '  RewriteCond %{REQUEST_FILENAME} -d' . PHP_EOL);
                fwrite($write_fd, '  RewriteRule ^.*$ - [NC,L]' . PHP_EOL . PHP_EOL);

                fwrite($write_fd, '  # Index' . PHP_EOL . '  RewriteRule ^.*$ %{ENV:REWRITEBASE}index.php [NC,L]' . PHP_EOL . PHP_EOL);
            }
        }
        fwrite($write_fd, '</IfModule>' . PHP_EOL . PHP_EOL);

        fwrite($write_fd, 'AddType application/vnd.ms-fontobject .eot' . PHP_EOL);
        fwrite($write_fd, 'AddType font/ttf .ttf' . PHP_EOL);
        fwrite($write_fd, 'AddType font/otf .otf' . PHP_EOL);
        fwrite($write_fd, 'AddType application/font-woff .woff' . PHP_EOL);
        fwrite($write_fd, 'AddType font/woff2 .woff2' . PHP_EOL . PHP_EOL);

        fwrite($write_fd, '<IfModule mod_headers.c>' . PHP_EOL);
        fwrite($write_fd, '  <FilesMatch "\.(ttf|ttc|otf|eot|woff|woff2|svg)$">' . PHP_EOL);
        fwrite($write_fd, '    Header set Access-Control-Allow-Origin "*"' . PHP_EOL);
        fwrite($write_fd, '  </FilesMatch>' . PHP_EOL);
        fwrite($write_fd, '</IfModule>' . PHP_EOL . PHP_EOL);
...
<IfModule mod_deflate.c>
  <IfModule mod_filter.c>
    AddOutputFilterByType DEFLATE text/html text/css text/javascript application/javascript application/x-javascript font/ttf application/x-font-ttf font/otf application/x-font-otf font/opentype image/svg+xml application/json
  </IfModule>
        ...
        fwrite($write_fd, "# If rewrite mod isn't enabled" . PHP_EOL);

        // Do not remove ($domains is already iterated upper)
        reset($domains);
        $domain = current($domains);
        fwrite($write_fd, 'ErrorDocument 404 ' . $domain[0]['physical'] . 'index.php?controller=404' . PHP_EOL . PHP_EOL);
