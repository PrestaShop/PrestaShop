...
class ToolsCore
{
    ...
    ** For other purposes use the smarty function instead */
    ...
    public static function generateHtaccess($path = null, $rewrite_settings = null, $cache_control = null, $specific = '', $disable_multiviews = null, $medias = false, $disable_modsec = null)
    {
        ...
        if ($specific_before) {
            $ht[] = trim($specific_before) . PHP_EOL;
        }
        ...
        $ht = array();
        $ht[] = '# ~~start~~ Do not remove this comment, Prestashop will keep automatically the code outside this comment when .htaccess will be generated again';
        $ht[] = '# .htaccess automaticaly generated by PrestaShop e-commerce open-source solution';
        $ht[] = '# http://www.prestashop.com - http://www.prestashop.com/forums' . PHP_EOL;

        if ($disable_modsec) {
            $ht[] = '<IfModule mod_security.c>';
            $ht[] = '    SecFilterEngine Off';
            $ht[] = '    SecFilterScanPOST Off';
            $ht[] = '</IfModule>' . PHP_EOL;
        }
        ...
        $ht[] = '        SetEnv HTTP_MOD_REWRITE On';
        $ht[] = '    </IfModule>' . PHP_EOL;

        // If multiviews is disabled
        if ($disable_multiviews) {
            $ht[] = '    # Disable Multiviews';
            $ht[] = '    Options -Multiviews' . PHP_EOL;
        }
        ...
        if (Configuration::get('PS_WEBSERVICE_CGI_HOST')) {
            ...
            $ht[] = '    RewriteRule . - [E=HTTP_AUTHORIZATION:%1]' . PHP_EOL;
        }

        foreach ($domains as $domain => $list_uri) {
            // As we use regex in the .htaccess file, IPv6 surrounded by brackets must be escaped
            $domain = str_replace(['[', ']'], ['\[', '\]'], $domain);

            foreach ($list_uri as $uri) {
                $ht[] = '    # Domain: ' . $domain . PHP_EOL;

                // ENV
                $ht[] = '    # ENV';
                $ht[] = '    RewriteRule . - [E=REWRITEBASE:' . $uri['physical'] . ']' . PHP_EOL;
                ...
                if ($uri['virtual']) {
                    if (!$rewrite_settings) {
                        ...
                        $ht[] = 'RewriteRule ^' . trim($uri['virtual'], '/') . '/?$ ' . $uri['physical'] . $uri['virtual'] . 'index.php [L,R]';
                    } else {
                        if (!empty($media_domains)) {
                            $ht[] = $media_domains;
                        }
                        $ht[] = 'RewriteRule ^' . trim($uri['virtual'], '/') . '$ ' . $uri['physical'] . $uri['virtual'] . ' [L,R]';
                    }
                    if (!empty($media_domains)) {
                        $ht[] = $media_domains;
                    }
                    $ht[] = 'RewriteRule ^' . ltrim($uri['virtual'], '/') . '(.*) ' . $uri['physical'] . '$1 [L]';
                }

                if ($rewrite_settings) {
                    // Rewrite URL of product images
                    if (!empty($media_domains)) {
                        $ht[] = $media_domains;
                    }
                    $ht[] = '    # Product URL';
                    $ht[] = '    RewriteRule ^([0-9])([0-9]*(?:\-[_a-z0-9-]*)?(?:-[0-9]+)?)\/(.+)\.(bmp|cur|gif|ico|jpe?g|png|svgz?|webp)(?:\/img\/p\/((?:[0-9]\/)+)(.*))?$ $2/$3.$4/img/p/$5$1/$6$1 [NC,N,L]';
                    $ht[] = '    RewriteRule ^(?:\-[_a-z0-9-]*)?(?:-[0-9]+)?\/.+\.(bmp|cur|gif|ico|jpe?g|png|svgz?|webp)\/(img\/p\/([0-9]\/)+.*)$ %{ENV:REWRITEBASE}$2.$1 [NC,L]' . PHP_EOL;

                    // Rewrite URL of category images
                    if (!empty($media_domains)) {
                        $ht[] = $media_domains;
                    }
                    $ht[] = '    # Category URL';
                ...
                $ht[] = '    # AlphaImageLoader for IE and fancybox';
                $ht[] = '    ReWriteRule ^images_ie\/?([^\/]+)\.(bmp|cur|gif|ico|jpe?g|png|svgz?|webp)$ %{ENV:REWRITEBASE}js/jquery/plugins/fancybox/images/$1.$2 [NC,L]' . PHP_EOL;

                // Redirections to dispatcher
                $ht[] = '    # Dispatcher';
                $ht[] = '    RewriteCond %{REQUEST_FILENAME} -s [OR]';
                $ht[] = '    RewriteCond %{REQUEST_FILENAME} -l [OR]';
                $ht[] = '    RewriteCond %{REQUEST_FILENAME} -d';
                $ht[] = '    RewriteRule ^.*$ - [NC,L]' . PHP_EOL;

                // Rewrite index
                $ht[] = '    # Index';
                $ht[] = '    RewriteRule ^.*$ %{ENV:REWRITEBASE}index.php [NC,L]';
            }
        }
        $ht[] = '</IfModule>' . PHP_EOL;
        ...
        if ($cache_control) {
            // MIME types
            $ht[] = '<IfModule mod_mime.c>';

            // Data interchange
            $ht[] = '    AddType application/atom+xml atom';
            $ht[] = '    AddType application/json json map topojson';
            $ht[] = '    AddType application/ld+json jsonld';
            $ht[] = '    AddType application/rss+xml rss';
            $ht[] = '    AddType application/geo+json geojson';
            $ht[] = '    AddType application/rdf+xml rdf';
            $ht[] = '    AddType application/xml xml';

            // JavaScript
            $ht[] = '    AddType text/javascript js mjs';

            // Manifest files
            $ht[] = '    AddType application/manifest+json webmanifest';
            $ht[] = '    AddType application/x-web-app-manifest+json webapp';
            $ht[] = '    AddType text/cache-manifest appcache';

            // Media files
            $ht[] = '    AddType audio/mp4 f4a f4b m4a';
            $ht[] = '    AddType audio/ogg oga ogg opus';
            $ht[] = '    AddType image/bmp bmp';
            $ht[] = '    AddType image/svg+xml svg svgz';
            $ht[] = '    AddType image/webp webp';
            $ht[] = '    AddType video/mp4 f4v f4p m4v mp4';
            $ht[] = '    AddType video/ogg ogv';
            $ht[] = '    AddType video/webm webm';
            $ht[] = '    AddType video/x-flv flv';
            $ht[] = '    AddType image/x-icon cur ico';

            // WebAssembly
            $ht[] = '    AddType application/wasm wasm';

            // Web fonts
            $ht[] = '    AddType font/woff woff';
            $ht[] = '    AddType font/woff2 woff2';
            $ht[] = '    AddType application/vnd.ms-fontobject eot';
            $ht[] = '    AddType font/ttf ttf';
            $ht[] = '    AddType font/collection ttc';
            $ht[] = '    AddType font/otf otf';

            // Other
            $ht[] = '    AddType application/octet-stream safariextz';
            $ht[] = '    AddType application/x-bb-appworld bbaw';
            $ht[] = '    AddType application/x-chrome-extension crx';
            $ht[] = '    AddType application/x-opera-extension oex';
            $ht[] = '    AddType application/x-xpinstall xpi';
            $ht[] = '    AddType text/calendar ics';
            $ht[] = '    AddType text/markdown markdown md';
            $ht[] = '    AddType text/vcard vcard vcf';
            $ht[] = '    AddType text/vnd.rim.location.xloc xloc';
            $ht[] = '    AddType text/vtt vtt';
            $ht[] = '    AddType text/x-component htc';
            $ht[] = '</IfModule>' . PHP_EOL;
            ...
            $ht[] = '<IfModule mod_headers.c>';
            $ht[] = '    <FilesMatch "\.(eot|otf|tt[cf]|woff2?)$">';
            $ht[] = '        Header set Access-Control-Allow-Origin "*"';
            $ht[] = '    </FilesMatch>';
            $ht[] = '</IfModule>' . PHP_EOL;

            // Compress content

            $ht[] = '<IfModule mod_filter.c>';
            $ht[] = '    AddOutputFilterByType DEFLATE "application/atom+xml" \\';
            $ht[] = '                                  "application/javascript" \\';
            $ht[] = '                                  "application/json" \\';
            $ht[] = '                                  "application/ld+json" \\';
            $ht[] = '                                  "application/manifest+json" \\';
            $ht[] = '                                  "application/rdf+xml" \\';
            $ht[] = '                                  "application/rss+xml" \\';
            $ht[] = '                                  "application/schema+json" \\';
            $ht[] = '                                  "application/geo+json" \\';
            $ht[] = '                                  "application/vnd.ms-fontobject" \\';
            $ht[] = '                                  "application/wasm" \\';
            $ht[] = '                                  "application/x-font-ttf" \\';
            $ht[] = '                                  "application/x-javascript" \\';
            $ht[] = '                                  "application/x-web-app-manifest+json" \\';
            ...
            $ht[] = '                                  "image/bmp" \\';
            $ht[] = '                                  "image/svg+xml" \\';
            $ht[] = '                                  "image/vnd.microsoft.icon" \\';
            $ht[] = '                                  "text/cache-manifest" \\';
            $ht[] = '                                  "text/calendar" \\';
            $ht[] = '                                  "text/css" \\';
            $ht[] = '                                  "text/html" \\';
            $ht[] = '                                  "text/javascript" \\';
            ...
            $ht[] = '                                  "text/markdown" \\';
            $ht[] = '                                  "text/vcard" \\';
            $ht[] = '                                  "text/vnd.rim.location.xloc" \\';
            $ht[] = '                                  "text/vtt" \\';
            $ht[] = '                                  "text/x-component" \\';
            $ht[] = '                                  "text/x-cross-domain-policy" \\';
            $ht[] = '                                  "text/xml"';
            $ht[] = '</IfModule>' . PHP_EOL;

            $ht[] = '<IfModule mod_mime.c>';
            $ht[] = '    AddEncoding gzip svgz';
            $ht[] = '</IfModule>' . PHP_EOL;

            // Remove ETags
            $ht[] = '<IfModule mod_headers.c>';
            $ht[] = '    Header unset ETag';
            $ht[] = '</IfModule>' . PHP_EOL;
            $ht[] = 'FileETag None' . PHP_EOL;
            ...
            // CSS
            $ht[] = '    ExpiresByType text/css "access plus 1 year"';

            // Data interchange
            $ht[] = '    ExpiresByType application/atom+xml "access plus 1 hour"';
            $ht[] = '    ExpiresByType application/rdf+xml "access plus 1 hour"';
            $ht[] = '    ExpiresByType application/rss+xml "access plus 1 hour"';
            $ht[] = '    ExpiresByType application/json "access plus 0 seconds"';
            $ht[] = '    ExpiresByType application/ld+json "access plus 0 seconds"';
            $ht[] = '    ExpiresByType application/schema+json "access plus 0 seconds"';
            $ht[] = '    ExpiresByType application/geo+json "access plus 0 seconds"';
            $ht[] = '    ExpiresByType application/xml "access plus 0 seconds"';
            $ht[] = '    ExpiresByType text/calendar "access plus 0 seconds"';
            $ht[] = '    ExpiresByType text/xml "access plus 0 seconds"';

            // Favicon (cannot be renamed!) and cursor images
            $ht[] = '    ExpiresByType image/vnd.microsoft.icon "access plus 1 week"';
            $ht[] = '    ExpiresByType image/x-icon "access plus 1 week"';

            // HTML
            $ht[] = '    ExpiresByType text/html "access plus 0 seconds"';

            // JavaScript
            $ht[] = '    ExpiresByType application/javascript "access plus 1 year"';
            $ht[] = '    ExpiresByType application/x-javascript "access plus 1 year"';
            $ht[] = '    ExpiresByType text/javascript "access plus 1 year"';

            // Manifest files
            $ht[] = '    ExpiresByType application/manifest+json "access plus 1 week"';
            $ht[] = '    ExpiresByType application/x-web-app-manifest+json "access plus 0 seconds"';
            $ht[] = '    ExpiresByType text/cache-manifest "access plus 0 seconds"';

            // Markdown
            $ht[] = '    ExpiresByType text/markdown "access plus 0 seconds"';

            // Media files
            $ht[] = '    ExpiresByType audio/ogg "access plus 1 month"';
            $ht[] = '    ExpiresByType image/bmp "access plus 1 month"';
            $ht[] = '    ExpiresByType image/gif "access plus 1 month"';
            $ht[] = '    ExpiresByType image/jpeg "access plus 1 month"';
            $ht[] = '    ExpiresByType image/png "access plus 1 month"';
            $ht[] = '    ExpiresByType image/svg+xml "access plus 1 month"';
            $ht[] = '    ExpiresByType image/webp "access plus 1 month"';
            $ht[] = '    ExpiresByType video/mp4 "access plus 1 month"';
            $ht[] = '    ExpiresByType video/ogg "access plus 1 month"';
            $ht[] = '    ExpiresByType video/webm "access plus 1 month"';

            // WebAssembly
            $ht[] = '    ExpiresByType application/wasm "access plus 1 year"';

            // Collection
            $ht[] = '    ExpiresByType font/collection "access plus 1 month"';

            // Embedded OpenType (EOT)
            $ht[] = '    ExpiresByType application/vnd.ms-fontobject "access plus 1 month"';
            $ht[] = '    ExpiresByType font/eot "access plus 1 month"';

            // OpenType
            $ht[] = '    ExpiresByType font/opentype "access plus 1 month"';
            $ht[] = '    ExpiresByType font/otf "access plus 1 month"';

            // TrueType
            $ht[] = '    ExpiresByType application/x-font-ttf "access plus 1 month"';
            $ht[] = '    ExpiresByType font/ttf "access plus 1 month"';

            // Web Open Font Format (WOFF) 1.0
            $ht[] = '    ExpiresByType application/font-woff "access plus 1 month"';
            $ht[] = '    ExpiresByType application/x-font-woff "access plus 1 month"';
            $ht[] = '    ExpiresByType font/woff "access plus 1 month"';

            // Web Open Font Format (WOFF) 2.0
            $ht[] = '    ExpiresByType application/font-woff2 "access plus 1 month"';
            $ht[] = '    ExpiresByType font/woff2 "access plus 1 month"';
        ...
        $ht[] = '# If rewrite_mod isn\'t enabled';

        // Do not remove ($domains is already iterated upper)
        reset($domains);
        $domain = current($domains);
        $ht[] = 'ErrorDocument 404 ' . $domain[0]['physical'] . 'index.php?controller=404' . PHP_EOL;

        $ht[] = '# ~~end~~ Do not remove this comment, Prestashop will keep automatically the code outside this comment when .htaccess will be generated again';

        $htacces_content = implode(PHP_EOL, $ht);
        fwrite($write_fd, $htacces_content);

        if ($specific_after) {
            fwrite($write_fd, PHP_EOL . PHP_EOL . trim($specific_after));
        }
        fclose($write_fd);
