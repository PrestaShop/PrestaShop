...
class ConnectionsSourceCore extends ObjectModel
{
    ...
    public static function getOrderSources($idOrder)
    {
		...
		INNER JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'guest') . ' g ON g.id_customer = o.id_customer
		INNER JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'connections') . ' co  ON co.id_guest = g.id_guest
		INNER JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'connections_source') . ' cos ON cos.id_connections = co.id_connections
