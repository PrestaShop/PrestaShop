    ...
    public function getTaxCalculator()
    {
        ...
        if (!Cache::isStored($cache_id)) {
            $rows = Db::getInstance()->executeS('
				SELECT tr.*
				FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'tax_rule') . ' tr
				JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'tax_rules_group') . ' trg ON (tr.id_tax_rules_group = trg.id_tax_rules_group)
				WHERE trg.active = 1
