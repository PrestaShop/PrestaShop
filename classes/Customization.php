...
class CustomizationCore extends ObjectModel
{
    ...
    public static function getReturnedCustomizations($idOrder)
    {
        if (($result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
			...
			FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'order_return') . ' ore
			INNER JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'order_return_detail') . ' ord ON (ord.id_order_return = ore.id_order_return AND ord.cancelled = 0)
    ...
    {
        if (!$result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('SELECT id_customization, quantity FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customization') . ' WHERE id_cart = ' . (int) $idCart)) {
            return false;
        }
        $customizations = [];
        foreach ($result as $row) {
    ...
    public static function getCustomizationPrice($idCustomization)
    {
        ...
        return (float) Db::getInstance()->getValue(
            '
            SELECT SUM(price) FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customized_data') . '
    ...
    public static function getCustomizationWeight($idCustomization)
    {
        ...
        return (float) Db::getInstance()->getValue(
            '
            SELECT SUM(weight) FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customized_data') . '
            WHERE id_customization = ' . (int) $idCustomization
        );
    }
    ...
    public static function getLabel($idCustomization, $idLang, $idShop = null)
    {
        ...
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
            '
		SELECT name
		FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customization_field_lang') . '
        ...
        if (!empty($inValues)) {
            $results = Db::getInstance()->executeS(
                'SELECT id_customization, id_product, quantity, quantity_refunded, quantity_returned
							 FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customization') . '
    ...
    public static function isCurrentlyUsed($table = null, $hasActiveColumn = false)
    {
        return (bool) Db::getInstance()->getValue('
			...
			FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customization_field') . '
		');
    }
    ...
    public function getWsCustomizedDataTextFields()
    {
        if (!$results = Db::getInstance()->executeS('
			...
			FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customization_field') . ' cf
			LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customized_data') . ' cd ON (cf.id_customization_field = cd.' . Db::quoteIdentifier('index') . ')
    ...
    public function setWsCustomizedDataTextFields($values)
    {
		...
		DELETE FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customized_data') . '
		WHERE id_customization = ' . (int) $this->id . '
		AND type = ' . (int) Product::CUSTOMIZE_TEXTFIELD);
        foreach ($values as $value) {
            $query = 'INSERT INTO ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customized_data') . ' (id_customization, type, ' . Db::quoteIdentifier('index') . ', value)
    ...
    {
        $return = Db::getInstance()->execute('DELETE FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customization_field_lang') . '
