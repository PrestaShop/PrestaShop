...
class TabCore extends ObjectModel
{
    ...
    public static function getIdFromClassName($className)
    {
        ...
        if (null === self::$_getIdFromClassName) {
    ...
    public function move($direction)
    {
        ...
        if ('l' == $direction && $this->position <= 1) {
            return false;
        }
        if ('r' == $direction && $this->position >= $nbTabs) {
            return false;
        }

        $newPosition = ('l' == $direction) ? $this->position - 1 : $this->position + 1;
    ...
    public static function checkTabRights($idTab)
    {
        ...
        if (_PS_ADMIN_PROFILE_ == Context::getContext()->employee->id_profile) {
            return true;
        }
        ...
        if (isset($tabAccesses[(int) $idTab]['view'])) {
            return '1' === $tabAccesses[(int) $idTab]['view'];
        }
