...
class PackCore extends Product
{
    ...
    public static function getItems($id_product, $id_lang)
    {
        ...
        foreach ($result as $row) {
            $product = new Product($row['id_product_item'], false, $id_lang);
            $product->loadStockData();
            $product->pack_quantity = $row['quantity'];
            $product->id_pack_product_attribute = (isset($row['id_product_attribute_item']) && $row['id_product_attribute_item']) ? $row['id_product_attribute_item'] : 0;
            ...
            if (isset($row['id_product_attribute_item']) && $row['id_product_attribute_item']) {
                ...
                foreach ($combinations as $combination) {
                    $product->name .= ' '.$combination['group_name'].'-'.$combination['attribute_name'];
                }
            }
    ...
    public static function getItemTable($id_product, $id_lang, $full = false)
    {
        ...
        $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
            ...
            LEFT JOIN `'._DB_PREFIX_.'product` p
                ON p.id_product = a.id_product_item
        ...
        foreach ($result as &$line) {
            if (Combination::isFeatureActive() && isset($line['id_product_attribute_item']) && $line['id_product_attribute_item']) {
                ...
                $attr_name = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS('
                    SELECT agl.`name` AS group_name, al.`name` AS attribute_name,  pai.`id_image` AS id_product_attribute_image
                    FROM `'._DB_PREFIX_.'product_attribute` pa
                    '.Shop::addSqlAssociation('product_attribute', 'pa').'
                    LEFT JOIN `'._DB_PREFIX_.'product_attribute_combination` pac
                        ON pac.`id_product_attribute` = '.$line['id_product_attribute_item'].'
                    LEFT JOIN `'._DB_PREFIX_.'attribute` a
                        ON a.`id_attribute` = pac.`id_attribute`
                    LEFT JOIN `'._DB_PREFIX_.'attribute_group` ag
                        ON ag.`id_attribute_group` = a.`id_attribute_group`
                    ...
                    LEFT JOIN `'._DB_PREFIX_.'product_attribute_image` pai
                        ON ('.$line['id_product_attribute_item'].' = pai.`id_product_attribute`)
                    WHERE pa.`id_product` = '.(int)$line['id_product'].'
                        AND pa.`id_product_attribute` = '.$line['id_product_attribute_item'].'
                    GROUP BY pa.`id_product_attribute`, ag.`id_attribute_group`
                    ORDER BY pa.`id_product_attribute`');
    ...
    public static function addItem($id_product, $id_item, $qty, $id_attribute_item = 0)
    {
        ...
        $result = Db::getInstance()->update('product', array('cache_is_pack' => 1), 'id_product = '.(int)$id_product);

        $result &= Db::getInstance()->insert('pack', array(
                'id_product_pack' => (int)$id_product,
                'id_product_item' => (int)$id_item,
                'id_product_attribute_item' => (int)$id_attribute_item,
                'quantity' => (int)$qty));

        return $result && Configuration::updateGlobalValue('PS_PACK_FEATURE_ACTIVE', '1');
    }
    ...
    public static function duplicate($id_product_old, $id_product_new)
    {
        Db::getInstance()->execute('
            ...
            (
                SELECT '.(int)$id_product_new.', `id_product_item`, `id_product_attribute_item`, `quantity`
                FROM `'._DB_PREFIX_.'pack`
                WHERE `id_product_pack` = '.(int)$id_product_old.'
            )');
