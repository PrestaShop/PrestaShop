...
class CarrierCore extends ObjectModel
{
    ...
    public function __construct($id = null, $id_lang = null)
    {
         ...
         * @deprecated 1.5.5
         */
        if (Carrier::SHIPPING_METHOD_DEFAULT == $this->shipping_method) {
    ...
    public static function getCarriersForOrder($id_zone, $groups = null, $cart = null, &$error = array())
    {
        ...
        foreach ($result as $k => $row) {
            ...
            if (Carrier::SHIPPING_METHOD_FREE != $shipping_method) {
                ...
                if ((Carrier::SHIPPING_METHOD_WEIGHT == $shipping_method && false === $carrier->getMaxDeliveryPriceByWeight($id_zone))) {
                    $error[$carrier->id] = Carrier::SHIPPING_WEIGHT_EXCEPTION;
                    unset($result[$k]);
                    continue;
                }
                if ((Carrier::SHIPPING_METHOD_PRICE == $shipping_method && false === $carrier->getMaxDeliveryPriceByPrice($id_zone))) {
                ...
                if ($row['range_behavior']) {
                    ...
                    if (Carrier::SHIPPING_METHOD_PRICE == $shipping_method
        ...
        $prices = array();
        if (Carrier::SORT_BY_PRICE == Configuration::get('PS_CARRIER_DEFAULT_SORT')) {
            foreach ($results_array as $r) {
                $prices[] = $r['price'];
            }
            if (Carrier::SORT_BY_ASC == Configuration::get('PS_CARRIER_DEFAULT_ORDER')) {
    ...
    public function addDeliveryPrice($price_list, $delete = false)
    {
        ...
        foreach ($price_list as $values) {
            if (!isset($values['id_shop'])) {
                $values['id_shop'] = (Shop::CONTEXT_SHOP == Shop::getContext()) ? Shop::getContextShopID() : null;
            }
            if (!isset($values['id_shop_group'])) {
                $values['id_shop_group'] = (Shop::CONTEXT_ALL != Shop::getContext()) ? Shop::getContextShopGroupID() : null;
            }
            ...
            if ($delete) {
                Db::getInstance()->execute(
                    ...
                    (null !== $values['id_range_price'] ? ' AND id_range_price='.(int) $values['id_range_price'] : ' AND (ISNULL(`id_range_price`) OR `id_range_price` = 0)').
                    (null !== $values['id_range_weight'] ? ' AND id_range_weight='.(int) $values['id_range_weight'] : ' AND (ISNULL(`id_range_weight`) OR `id_range_weight` = 0)').'
        ...
        foreach (array('range_price', 'range_weight') as $range) {
            ...
            if (count($res)) {
                foreach ($res as $val) {
                    ...
                    $range_price_id = ('range_price' == $range) ? $range_id : 'NULL';
                    $range_weight_id = ('range_weight' == $range) ? $range_id : 'NULL';
    ...
    public function getRangeTable()
    {
        ...
        if (Carrier::SHIPPING_METHOD_WEIGHT == $shipping_method) {
            return 'range_weight';
        } elseif (Carrier::SHIPPING_METHOD_PRICE == $shipping_method) {
            return 'range_price';
        }
    ...
    public function getRangeSuffix($currency = null)
    {
        ...
        if (Carrier::SHIPPING_METHOD_PRICE == $this->getShippingMethod()) {
    ...
    public static function getAvailableCarrierList(Product $product, $id_warehouse, $id_address_delivery = null, $id_shop = null, $cart = null, &$error = array())
    {
        ...
        $id_address = (int) ((!is_null($id_address_delivery) && 0 != $id_address_delivery) ? $id_address_delivery : $cart->id_address_delivery);
        if ($id_address) {
