...
class ConfigurationCore extends ObjectModel
{
    ...
    public static function get($key, $idLang = null, $idShopGroup = null, $idShop = null, $default = false)
    {
        ...
        if (null === self::$_new_cache_shop) {
            $idShop = 0;
        } else {
            if (null === $idShop || !Shop::isFeatureActive()) {
                $idShop = Shop::getContextShopID(true);
            }
        }

        if (null === self::$_new_cache_group) {
            $idShopGroup = 0;
        } else {
            if (null === $idShopGroup || !Shop::isFeatureActive()) {
                $idShopGroup = Shop::getContextShopGroupID(true);
            }
        }
    ...
    public static function hasContext($key, $idLang, $context)
    {
        ...
        if (Shop::CONTEXT_SHOP == $context && Configuration::hasKey($key, $idLang, null, $idShop)) {
            return true;
        } elseif (Shop::CONTEXT_GROUP == $context && Configuration::hasKey($key, $idLang, $idShopGroup)) {
            return true;
        } elseif (Shop::CONTEXT_ALL == $context && Configuration::hasKey($key, $idLang)) {
            return true;
        }
    ...
    public static function isOverridenByCurrentContext($key)
    {
        if (Configuration::isLangKey($key)) {
            ...
            foreach (Language::getIDs(false) as $idLang) {
                if ((Shop::CONTEXT_SHOP == Shop::getContext() && Configuration::hasContext($key, $idLang, Shop::CONTEXT_SHOP))
                    || (Shop::CONTEXT_GROUP == Shop::getContext() && Configuration::hasContext($key, $idLang, Shop::CONTEXT_GROUP))) {
                    $testContext = true;
                }
            }
        } else {
            $testContext = ((Shop::CONTEXT_SHOP == Shop::getContext() && Configuration::hasContext($key, null, Shop::CONTEXT_SHOP))
                            || (Shop::CONTEXT_GROUP == Shop::getContext() && Configuration::hasContext($key, null, Shop::CONTEXT_GROUP))) ? true : false;
        }

        return Shop::isFeatureActive() && Shop::CONTEXT_ALL != Shop::getContext() && $testContext;
    }
    ...
    public static function isCatalogMode()
    {
        if (is_a(Context::getContext()->controller, 'FrontController')) {
            $isCatalogMode =
                ...
                (Country::GEOLOC_CATALOG_MODE == Context::getContext()->controller->getRestrictedCountry());
        } else {
            $isCatalogMode =
                Configuration::get('PS_CATALOG_MODE') ||
                !Configuration::showPrices();
