...
class TranslateCore
{
	...
	*/
	...
	/**
	 * Flag for marking tainted execution - when we do not want to recreate files (afer removal generally)
	 * @var boolean
	 */
	...
	public static function getAdminTranslation($string, $class = 'AdminTab', $addslashes = false, $htmlentities = true, $sprintf = null)
	{
		if (!self::$l2_save_guard)
			self::initL2Cache();
		$l2_cache_key = md5(sprintf('%s|%s|%s|%s|%d', $module instanceof Module ? $module->name : (string)$module, $string, $source, $sprintf ? json_encode($sprintf) : '', (int)$js));
	...
	*/
	...
	public static function clearL2Cache($prefix = null){
		var_dump(glob(self::getTranslationL2CacheDir().($prefix !==null && trim($prefix, 'a..zA..Z0..9') =='' ? (string)$prefix : ''). '*'));
		var_dump(array_map('unlink', glob(self::getTranslationL2CacheDir().($prefix !==null && trim($prefix, 'a..zA..Z0..9') =='' ? (string)$prefix : ''). '*')));
	}
	...
	public static function saveTranslationToL2Cache(){
		if (!self::$l2_do_not_save && !empty(self::$l2_cache) && (!empty(self::$l2_miss) || !file_exists(self::getTranslationL2CacheFileName())))
		{
			...
			{
				return rename($tmpfile, self::getTranslationL2CacheFileName());
			}
		}
		return false;
	}
	...
	public static function getTranslateDisplayError($string, $htmlentities, Context $context = null)
	{
		...
		$l2_cache_key = md5(sprintf('%s|%d|%s', $string, (int)$htmlentities, $context->language->iso_code));
	...
	public static function getTranslateDisplayFieldName($field, $class, $htmlentities = true, Context $context = null)
	{
		...
		$l2_cache_key = md5(sprintf('%s|%s|%d|%s', $field, $class, (int)$htmlentities, $context->language->iso_code));
