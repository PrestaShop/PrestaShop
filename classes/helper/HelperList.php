...
class HelperListCore extends Helper
{
    ...
    public function displayEnableLink($token, $id, $value, $active, $id_category = null, $id_product = null, $ajax = false)
    {
        ...
        $tpl_enable->assign(array(
            ...
            'url_enable' => $this->currentIndex.'&'.$this->identifier.'='.$id.'&'.$active.$this->table.($ajax ? '&action='.$active.$this->table.'&ajax='.(int) $ajax : '').
                ((int) $id_category && (int) $id_product ? '&id_category='.(int) $id_category : '').($this->page && $this->page > 1 ? '&page='.(int) $this->page : '').'&token='.(null != $token ? $token : $this->token),
        ));
    ...
    public function displayListContent()
    {
        ...
        foreach ($this->_list as $index => $tr) {
            ...
            foreach ($this->fields_list as $key => $params) {
                ...
                } elseif (isset($params['type']) && 'float' == $params['type']) {
    ...
    public function displayDuplicateLink($token, $id, $name = null)
    {
        ...
        if (('product' == $this->table) && !Image::hasImages($this->context->language->id, (int) $id)) {
            $confirm = '';
        }

        $tpl->assign(array(
            'href' => $this->currentIndex.'&'.$this->identifier.'='.$id.'&view'.$this->table.'&token='.(null != $token ? $token : $this->token),
            'action' => self::$cache_lang['Duplicate'],
            'confirm' => $confirm,
            'location_ok' => $duplicate.'&token='.(null != $token ? $token : $this->token),
    ...
    public function displayDetailsLink($token, $id, $name = null)
    {
        ...
        $tpl->assign(array(
            'id' => $id,
            'href' => $this->currentIndex.'&'.$this->identifier.'='.$id.'&details'.$this->table.'&token='.(null != $token ? $token : $this->token),
    ...
    public function displayEditLink($token, $id, $name = null)
    {
        ...
        $href = $this->currentIndex.'&'.$this->identifier.'='.$id.'&update'.$this->table.($this->page && $this->page > 1 ? '&page='.(int) $this->page : '').'&token='.(null != $token ? $token : $this->token);
        ...
        switch ($this->currentIndex) {
            ...
            case 'index.php?tab=AdminProducts':
                ...
                if (!$redirectLegacy && 'id_product' == $this->identifier) {
                    $href = Context::getContext()->link->getAdminLink('AdminProducts', true, array('id_product' => $id, 'updateproduct' => 1));
                }
                break;
            default:
        }
    ...
    public function displayDeleteLink($token, $id, $name = null)
    {
        ...
        $href = $this->currentIndex.'&'.$this->identifier.'='.$id.'&delete'.$this->table.'&token='.(null != $token ? $token : $this->token);
        ...
        switch ($this->currentIndex) {
            ...
            case 'index.php?tab=AdminProducts':
                ...
                $redirectLegacy = $pagePreference->getTemporaryShouldUseLegacyPage('product');
                if (!$redirectLegacy && 'id_product' == $this->identifier) {
                    $href = Context::getContext()->link->getAdminLink('AdminProducts', true, array('id_product' => $id, 'deleteproduct' => 1));
                }
                break;
            default:
        }
    ...
    public function displayDefaultLink($token, $id, $name = null)
    {
        ...
        $tpl->assign(array(
            'href' => $this->currentIndex.'&'.$this->identifier.'='.(int) $id.'&default'.$this->table.'&token='.(null != $token ? $token : $this->token),
    ...
    public function displayListHeader()
    {
        ...
        if ($this->position_identifier && ('position' == $this->orderBy && 'DESC' != $this->orderWay)) {
            $table_dnd = true;
        }
        ...
        foreach ($this->fields_list as $key => $params) {
            ...
            if ('active' == $key && false !== mb_strpos($key, '!')) {
        ...
        foreach ($this->fields_list as $key => $field) {
            if (isset($field['value']) && false !== $field['value'] && '' !== $field['value']) {
            ...
            if (!(isset($field['search']) && false === $field['search'])) {
        ...
        $this->header_tpl->assign(array_merge(array(
            ...
            'action' => $action,
            'is_order_position' => $this->position_identifier && 'position' == $this->orderBy,
            'order_way' => $this->orderWay,
            'order_by' => $this->orderBy,
            'fields_display' => $this->fields_list,
