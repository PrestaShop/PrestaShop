...
class LocalizationPackCore
{
    ...
    public function loadLocalisationPack($file, $selection, $install_mode = false, $iso_localization_pack = null)
    {
        ...
        if (empty($selection)) {
            $result &= $this->_installStates($xml);
            $result &= $this->_installTaxes($xml);
            $result &= $this->_installCurrencies($xml, $install_mode);
            $result &= $this->installConfiguration($xml);
            $result &= $this->installModules($xml);
            $result &= $this->updateDefaultGroupDisplayMethod($xml);

            if (($result || $install_mode) && isset($this->iso_code_lang)) {
            ...
            if (!Language::isInstalled(Language::getIsoById($id_lang))) {
                $result &= $this->_installLanguages($xml, $install_mode);
                $result &= $this->_installUnits($xml);
            }

            if ($install_mode && $result && isset($this->iso_currency)) {
                Cache::clean('Currency::getIdByIsoCode_*');
                $result &= Configuration::updateValue('PS_CURRENCY_DEFAULT', (int)Currency::getIdByIsoCode($this->iso_currency));
                Currency::refreshCurrencies();
            }
        } else {
            foreach ($selection as $selected) {
                // No need to specify the install_mode because if the selection mode is used, then it's not the install
                $result &= Validate::isLocalizationPackSelection($selected) ? $this->{'_install'.$selected}($xml) : false;
            }
        }
    ...
    protected function _installTaxes($xml)
    {
        if (isset($xml->taxes->tax)) {
            ...
            foreach ($xml->taxes->taxRulesGroup as $group) {
                ...
                $tax_rule_group->name = $group['name'];
                $tax_rule_group->active = 1;

                if (!$tax_rule_group->save()) {
                ...
                foreach ($group->taxRule as $rule) {
                    ...
                    $tax_rule->id_tax_rules_group = $tax_rule_group->id;
                    $tax_rule->id_country = $id_country;
                    $tax_rule->id_state = $id_state;
                    $tax_rule->id_county = $id_county;
                    $tax_rule->zipcode_from = $zipcode_from;
                    $tax_rule->zipcode_to = $zipcode_to;
                    $tax_rule->behavior = $behavior;
                    $tax_rule->description = '';
                    $tax_rule->id_tax = $assoc_taxes[strval($rule_attributes['id_tax'])];
                    $tax_rule->save();
                }
            }
        }
    ...
    protected function _installLanguages($xml, $install_mode = false)
    {
        ...
        if (empty($this->_errors) && $install_mode && isset($attributes['iso_code']) && (count($xml->languages->language) == 1)) {
