        ...
        if (isset($xml->currencies->currency)) {
            foreach ($xml->currencies->currency as $data) {
                ...
                if (!Currency::exists($currency->iso_code)) {
                    if (!$currency->add()) {
                        $this->_errors[] = Context::getContext()->getTranslator()->trans('An error occurred while importing the currency: %s', [strval($attributes['name'])], 'Admin.International.Notification');
