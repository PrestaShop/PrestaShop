...
class LinkCore
{
    ...
    public function __construct($protocolLink = null, $protocolContent = null)
    {
        ...
        if (null === Link::$category_disable_rewrite) {
    ...
    public function getCMSCategoryLink(
        ...
        if (!is_object($cmsCategory)) {
            if (null !== $alias && !$dispatcher->hasKeyword('cms_category_rule', $idLang, 'meta_keywords', $idShop) && !$dispatcher->hasKeyword('cms_category_rule', $idLang, 'meta_title', $idShop)) {
    ...
    public function getCMSLink(
        ...
        $dispatcher = Dispatcher::getInstance();
        if (!is_object($cms)) {
            if (null !== $alias && !$dispatcher->hasKeyword('cms_rule', $idLang, 'meta_keywords', $idShop) && !$dispatcher->hasKeyword('cms_rule', $idLang, 'meta_title', $idShop)) {
    ...
    public function getManufacturerLink(
        ...
        if (!is_object($manufacturer)) {
            if (null !== $alias && !$dispatcher->hasKeyword('manufacturer_rule', $idLang, 'meta_keywords', $idShop) && !$dispatcher->hasKeyword('manufacturer_rule', $idLang, 'meta_title', $idShop)) {
    ...
    public function getAdminLink($controller, $withToken = true, $sfRouteParams = array(), $params = array())
    {
        ...
        switch ($controller) {
            ...
            case 'AdminTranslations':
                ...
                if (isset($params['selected-emails']) && 'body' === $params['selected-emails']) {
                    break;
                }

                // In case of modules translations get a link to legacy controller
                if (isset($params['type']) && 'modules' === $params['type'] && isset($params['module'])) {
                    break;
                }
    ...
    public function getImageLink($name, $ids, $type = null)
    {
        ...
        if (($psLegacyImages
            ...
            || ($notDefault = false !== mb_strpos($ids, 'default'))) {
            if (1 == $this->allow && !$notDefault) {
    ...
    public function getPageLink($controller, $ssl = null, $idLang = null, $request = null, $requestUrlEncode = false, $idShop = null, $relativeProtocol = false)
    {
        ...
        if ('cart' === $controller && (!empty($request['add']) || !empty($request['delete'])) && Configuration::get('PS_TOKEN_ENABLE')) {
    ...
    public function goPage($url, $p)
    {
        ...
        return $url.(1 == $p ? '' : (!mb_strstr($url, '?') ? '?' : '&').'p='.(int) $p);
    }
    ...
    public function getPaginationLink($type, $idObject, $nb = false, $sort = false, $pagination = false, $array = false)
    {
        ...
        foreach ($_GET as $k => $value) {
            if ($k != 'id_'.$type && 'controller' != $k) {
                if (Configuration::get('PS_REWRITING_SETTINGS') && ('isolang' == $k || 'id_lang' == $k)) {
                    continue;
                }
        ...
        if (!$array) {
            if (count($vars)) {
                return $url.(!mb_strstr($url, '?') && (1 == $this->allow || $url == $this->url) ? '?' : '&').http_build_query($vars, '', '&');
            }
    ...
    public function getBaseLink($idShop = null, $ssl = null, $relativeProtocol = false)
    {
        ...
        if (Configuration::get('PS_MULTISHOP_FEATURE_ACTIVE') && null !== $idShop) {
            $shop = new Shop($idShop);
        } else {
            $shop = Context::getContext()->shop;
        }
