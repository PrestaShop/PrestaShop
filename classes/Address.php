...
class AddressCore extends ObjectModel
{
    ...
    public function delete()
    {
        ...
        if (!$this->isUsed()) {
            ...
            // Update the cache
            if (isset(self::$addressExists[$this->id])) {
                self::$addressExists[$this->id] = false;
            }
    ...
    public static function addressExists($id_address, bool $useCache = false)
    {
        ...
        if ($useCache && isset(self::$addressExists[$id_address])) {
            return self::$addressExists[$id_address];
        }

        self::$addressExists[$id_address] = (bool) Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
            'SELECT `id_address`
            FROM ' . _DB_PREFIX_ . 'address a
            WHERE a.`id_address` = ' . (int) $id_address,
            false
