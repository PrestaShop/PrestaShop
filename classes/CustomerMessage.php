...
class CustomerMessageCore extends ObjectModel
{
    ...
    public static function getMessagesByOrderId($idOrder, $private = true)
    {
        return Db::getInstance()->executeS('
			...
			FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customer_message') . ' cm
			LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customer_thread') . ' ct
    ...
    public static function getTotalCustomerMessages($where = null)
    {
        if (null === $where) {
            return (int) Db::getInstance()->getValue(
				...
				LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customer_thread') . ' ct ON (cm.id_customer_thread = ct.id_customer_thread)
