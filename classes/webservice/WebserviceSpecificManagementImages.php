...
class WebserviceSpecificManagementImagesCore implements WebserviceSpecificManagementInterface
{
    ...
    protected function manageGeneralImages()
    {
        ...
        switch ($this->wsObject->urlSegment[2]) {
            ...
            case 'header':
                if (in_array($this->wsObject->method, array('GET', 'HEAD', 'PUT'), true)) {
        ...
        switch ($this->wsObject->method) {
            ...
            case 'PUT':
                if ($this->writePostedImageOnDisk($path, null, null)) {
                    if ('header' == $this->wsObject->urlSegment[2]) {
    ...
    protected function manageDefaultDeclinatedImages($directory, $normal_image_sizes)
    {
        ...
        if ('' == $this->wsObject->urlSegment[3] && 'GET' == $this->wsObject->method) {
    ...
    protected function manageEntityDeclinatedImages($directory, $normal_image_sizes)
    {
        ...
        if ('products' == $this->imageType) {
            ...
            if ('' != $this->wsObject->urlSegment[3]) {
                if ('bin' == $this->wsObject->urlSegment[3]) {
                    $current_product = new Product($object_id);
                    $this->wsObject->urlSegment[3] = $current_product->getCoverWs();
                }
                if (!Validate::isUnsignedId($object_id) || !in_array($this->wsObject->urlSegment[3], $available_image_ids, true)) {
            ...
            } elseif ('GET' == $this->wsObject->method || 'HEAD' == $this->wsObject->method) {
        ...
        if ('' != $this->output) {
            ...
            if ('products' == $this->imageType && 'bin' == $image_size) {
                $filename = $directory . $object_id . '-' . $image_id . '.jpg';
            } elseif (!in_array($image_size, $normal_image_size_names, true)) {
    ...
    protected function manageCustomizationImages()
    {
        ...
        if (empty($this->wsObject->urlSegment[2])) {
            ...
            if ('POST' == $this->wsObject->method) {
                $customizations = $this->getCustomizations();
                if (!in_array((int) $this->wsObject->urlSegment[3], $customizations, true)) {
        ...
        switch ($this->wsObject->method) {
            ...
            case 'DELETE':
                ...
                if ('products' == $this->imageType) {
                    ...
                    if (in_array($this->imageType, array('categories', 'manufacturers', 'suppliers', 'stores'), true)) {
    ...
    protected function writeImageOnDisk($base_path, $new_path, $dest_width = null, $dest_height = null, $image_types = null, $parent_path = null)
    {
        ...
        if ($width_diff > 1 && $height_diff > 1) {
            ...
            if (2 == (int) (Configuration::get('PS_IMAGE_GENERATION_METHOD')) || (0 == (int) (Configuration::get('PS_IMAGE_GENERATION_METHOD')) && $width_diff > $height_diff)) {
                $next_height = $dest_height;
                $next_width = (int) (($source_width * $next_height) / $source_height);
                $dest_width = (0 == (int) (Configuration::get('PS_IMAGE_GENERATION_METHOD')) ? $dest_width : $next_width);
            } else {
                $next_width = $dest_width;
                $next_height = (int) ($source_height * $dest_width / $source_width);
                $dest_height = (0 == (int) (Configuration::get('PS_IMAGE_GENERATION_METHOD')) ? $dest_height : $next_height);
            }
        }
        ...
        switch ($this->imgExtension) {
            case 'gif':
                ...
                break;
            case 'png':
                $quality = (false === Configuration::get('PS_PNG_QUALITY') ? 7 : Configuration::get('PS_PNG_QUALITY'));
                $imaged = imagepng($dest_image, $new_path, (int) $quality);
                break;
            case 'jpeg':
            default:
                $quality = (false === Configuration::get('PS_JPEG_QUALITY') ? 90 : Configuration::get('PS_JPEG_QUALITY'));
                $imaged = imagejpeg($dest_image, $new_path, (int) $quality);
                if ('customizations' == $this->wsObject->urlSegment[1]) {
    ...
    protected function writePostedImageOnDisk($reception_path, $dest_width = null, $dest_height = null, $image_types = null, $parent_path = null)
    {
        if ('PUT' == $this->wsObject->method) {
            if (isset($_FILES['image']['tmp_name']) && $_FILES['image']['tmp_name']) {
                ...
                if (empty($mime_type) || 'regular file' == $mime_type) {
                    $mime_type = $file['type'];
                }
                if (false !== ($pos = mb_strpos($mime_type, ';'))) {
                    $mime_type = mb_substr($mime_type, 0, $pos);
                }

                // Check mime content type
                if (!$mime_type || !in_array($mime_type, $this->acceptedImgMimeTypes, true)) {
        ...
        } elseif ('POST' == $this->wsObject->method) {
            if (isset($_FILES['image']['tmp_name']) && $_FILES['image']['tmp_name']) {
                $file = $_FILES['image'];
                if ($file['size'] > $this->imgMaxUploadSize) {
                    throw new WebserviceException(sprintf('The image size is too large (maximum allowed is %d KB)', ($this->imgMaxUploadSize / 1000)), array(72, 400));
                ...
                if (isset($file['tmp_name']) && null != $file['tmp_name']) {
                    if ('products' == $this->imageType) {
                    ...
                    } elseif (in_array($this->imageType, array('categories', 'manufacturers', 'suppliers', 'stores'), true)) {
                        if (!($tmp_name = tempnam(_PS_TMP_IMG_DIR_, 'PS')) || !move_uploaded_file($file['tmp_name'], $tmp_name)) {
                            throw new WebserviceException('An error occurred during the image upload', array(76, 400));
