...
class FeatureCore extends ObjectModel
{
    ...
    public function delete()
    {
        ...
        Db::getInstance()->execute('
            DELETE `'._DB_PREFIX_.'feature_value_lang`
            ...
            WHERE `'._DB_PREFIX_.'feature_value`.`id_feature` = '.(int)$this->id.'
        ');
    ...
    public function update($nullValues = false)
    {
        ...
        foreach ($fields as $field) {
            ...
            if ($mode) {
                $result &= Db::getInstance()->update($this->def['table'].'_lang', $field,
    ...
    public static function nbFeatures($id_lang)
    {
        return Db::getInstance()->getValue('
            ...
            LEFT JOIN `'._DB_PREFIX_.'feature_lang` agl
                ON (ag.`id_feature` = agl.`id_feature`
    ...
    */
    public static function addFeatureImport($name, $position = false)
    {
        ...
        if (empty($result)) {
            ...
            return $feature->id;
        } elseif (isset($result['id_feature']) && $result['id_feature']) {
            if (is_numeric($position) && ($feature = new Feature((int)$result['id_feature']))) {
    ...
    public static function getFeaturesForComparison($list_ids_product, $id_lang)
    {
        ...
        return Db::getInstance()->executeS('
            ...
            LEFT JOIN `'._DB_PREFIX_.'feature_lang` fl
                ON f.`id_feature` = fl.`id_feature`
            WHERE fp.`id_product` IN ('.implode(',', $list_ids_product).')
                AND `id_lang` = '.(int)$id_lang.'
