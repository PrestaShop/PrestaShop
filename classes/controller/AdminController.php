...
class AdminControllerCore extends Controller
{
    ...
    public function __construct($forceControllerName = '', $default_theme_name = 'default')
    {
        ...
        // Fix for homepage
        if ('AdminDashboard' == $this->controller_name) {
        ...
        if (defined('_PS_HOST_MODE_') && _PS_HOST_MODE_) {
            if (isset($this->context->cookie->is_contributor) && 1 === (int) $this->context->cookie->is_contributor) {
                $this->context->mode = Context::MODE_HOST_CONTRIB;
            } else {
                $this->context->mode = Context::MODE_HOST;
            }
        } elseif (isset($this->context->cookie->is_contributor) && 1 === (int) $this->context->cookie->is_contributor) {
        ...
        if (is_array($import_access) && isset($import_access['view']) && 1 == $import_access['view']) {
        ...
        if ($this->filter && is_array($this->fields_list)) {
            ...
            foreach ($this->fields_list as $field => $t) {
                ...
                if (($val = Tools::getValue($this->table . 'Filter_' . $field)) || $val = $this->context->cookie->{$this->getCookieFilterPrefix() . $this->table . 'Filter_' . $field}) {
                    if (!is_array($val)) {
                        $filter_value = '';
                        if (isset($t['type']) && 'bool' == $t['type']) {
                            $filter_value = ((bool) $val) ? $this->l('yes') : $this->l('no');
                        } elseif (isset($t['type']) && 'date' == $t['type'] || isset($t['type']) && 'datetime' == $t['type']) {
    ...
    public function processFilter()
    {
        ...
        if (isset($this->list_id)) {
            foreach ($_POST as $key => $value) {
                ...
                } elseif (0 === mb_stripos($key, $this->list_id . 'Filter_')) {
                    $this->context->cookie->{$prefix . $key} = !is_array($value) ? $value : json_encode($value);
                } elseif (0 === mb_stripos($key, 'submitFilter')) {
                    $this->context->cookie->$key = !is_array($value) ? $value : json_encode($value);
                }
            }

            foreach ($_GET as $key => $value) {
                if (0 === mb_stripos($key, $this->list_id . 'Filter_')) {
                    $this->context->cookie->{$prefix . $key} = !is_array($value) ? $value : json_encode($value);
                } elseif (0 === mb_stripos($key, 'submitFilter')) {
                    $this->context->cookie->$key = !is_array($value) ? $value : json_encode($value);
                }
                if (0 === mb_stripos($key, $this->list_id . 'Orderby') && Validate::isOrderBy($value)) {
                    if ('' === $value || $value == $this->_defaultOrderBy) {
                        unset($this->context->cookie->{$prefix . $key});
                    } else {
                        $this->context->cookie->{$prefix . $key} = $value;
                    }
                } elseif (0 === mb_stripos($key, $this->list_id . 'Orderway') && Validate::isOrderWay($value)) {
        ...
        foreach ($filters as $key => $value) {
            ...
            if (null != $value && !strncmp($key, $prefix . $this->list_id . 'Filter_', 7 + Tools::strlen($prefix . $this->list_id))) {
                ...
                if ($field = $this->filterToField($key, $filter)) {
                    ...
                    if (is_array($value)) {
                        ...
                        if ('int' == $type || 'bool' == $type) {
                            $sql_filter .= (($check_key || '`active`' == $key) ? $alias . '.' : '') . pSQL($key) . ' = ' . (int) $value . ' ';
                        } elseif ('decimal' == $type) {
        ...
        try {
            if ($this->ajax) {
                ...
                if (!empty($action) && method_exists($this, 'ajaxProcess' . Tools::toCamelCase($action))) {
                    ...
                    return $return;
                } elseif (!empty($action) && 'AdminModules' == $this->controller_name && Tools::getIsset('configure')) {
                ...
                if ($this->filter && 'reset_filters' != $this->action) {
                    $this->processFilter();
                }
    ...
    public function processExport($text_delimiter = '"')
    {
        ...
        foreach ($this->_list as $i => $row) {
            ...
            foreach ($this->fields_list as $key => $params) {
                ...
                if ('image' == $key) {
                    if ('p' != $params['image'] || Configuration::get('PS_LEGACY_IMAGES')) {
    ...
    public function processAdd()
    {
        ...
        if (count($this->errors) <= 0) {
            ...
            if (method_exists($this->object, 'add') && !$this->object->add()) {
                ...
                if (empty($this->redirect_after) && false !== $this->redirect_after && Tools::isSubmit('submitAdd' . $this->table . 'AndStay')) {
                    $this->redirect_after = self::$currentIndex . '&' . $this->identifier . '=' . $this->object->id . '&conf=3&update' . $this->table . '&token=' . $this->token;
                }
                // Save and back to parent
                if (empty($this->redirect_after) && false !== $this->redirect_after && Tools::isSubmit('submitAdd' . $this->table . 'AndBackToParent')) {
                    $this->redirect_after = self::$currentIndex . '&' . $this->identifier . '=' . $parent_id . '&conf=3&token=' . $this->token;
                }
                // Default behavior (save and back)
                if (empty($this->redirect_after) && false !== $this->redirect_after) {
    ...
    public function processStatus()
    {
        if (Validate::isLoadedObject($object = $this->loadObject())) {
            if ($object->toggleStatus()) {
                $matches = array();
                if (false !== preg_match('/[\?|&]controller=([^&]*)/', (string) $_SERVER['HTTP_REFERER'], $matches)
    ...
    public function processResetFilters($list_id = null)
    {
        ...
        foreach ($filters as $cookie_key => $filter) {
            if (0 == strncmp($cookie_key, $prefix . $list_id . 'Filter_', 7 + Tools::strlen($prefix . $list_id))) {
    ...
    protected function processUpdateOptions()
    {
        ...
        foreach ($this->fields_options as $category_data) {
            ...
            foreach ($fields as $field => $values) {
                if (isset($values['type']) && 'selectLang' == $values['type']) {
            ...
            foreach ($fields as $field => $values) {
                ...
                if ((!Shop::isFeatureActive() && isset($values['required']) && $values['required'])
                    ...
                    if (isset($values['type']) && 'textLang' == $values['type']) {
                        foreach ($languages as $language) {
                            if (false == ($value = Tools::getValue($field . '_' . $language['id_lang'])) && '0' != (string) $value) {
                                $this->errors[] = $this->trans('field %s is required.', array($values['title']), 'Admin.Notifications.Error');
                            }
                        }
                    } elseif (false == ($value = Tools::getValue($field)) && '0' != (string) $value) {
                        $this->errors[] = $this->trans('field %s is required.', array($values['title']), 'Admin.Notifications.Error');
                    }
                }
                ...
                // Set default value
                if (false === Tools::getValue($field) && isset($values['default'])) {
            ...
            if (!count($this->errors)) {
                foreach ($fields as $key => $options) {
                    ...
                    if (method_exists($this, $method_name)) {
                        ...
                        Configuration::updateValue($key, $list, isset($values['validation'], $options['validation']) && 'isCleanHtml' == $options['validation'] ? true : false);
                    } else {
    ...
    public function display()
    {
        ...
        foreach ($template_dirs as $template_dir) {
            if (file_exists($template_dir . DIRECTORY_SEPARATOR . $tpl_action) && 'view' != $this->display && 'options' != $this->display) {
    ...
    private function getTabs($parentId = 0, $level = 0)
    {
        ...
        foreach ($tabs as $index => $tab) {
            if (!Tab::checkTabRights($tab['id_tab'])
                || ('AdminStock' == $tab['class_name'] && 0 == Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT'))
                || 'AdminCarrierWizard' == $tab['class_name']) {
                unset($tabs[$index]);
    ...
    public function initContent()
    {
        ...
        if ('edit' == $this->display || 'add' == $this->display) {
            ...
            if ('AdminCarriersController' != get_class($this)) {
    ...
    public function initFooter()
    {
        ...
        $this->context->smarty->assign(array(
            ...
            'iso_is_fr' => 'FR' == mb_strtoupper($this->context->language->iso_code),
    ...
    public function renderForm()
    {
        ...
        if ($this->fields_form && is_array($this->fields_form)) {
            ...
            $helper->show_cancel_button = (isset($this->show_form_cancel_button)) ? $this->show_form_cancel_button : ('add' == $this->display || 'edit' == $this->display);
    ...
    public function renderOptions()
    {
        ...
        if ($this->fields_options && is_array($this->fields_options)) {
            if (isset($this->display) && 'options' != $this->display && 'list' != $this->display) {
    ...
    protected function l($string, $class = null, $addslashes = false, $htmlentities = true)
    {
        ...
        } elseif ('controller' == mb_strtolower(mb_substr($class, -10))) {
    ...
    public function init()
    {
        ...
        if ('AdminLogin' != $this->controller_name && (!isset($this->context->employee) || !$this->context->employee->isLoggedBack())) {
            ...
            Tools::redirectAdmin($this->context->link->getAdminLink('AdminLogin') . ((!isset($_GET['logout']) && 'AdminNotFound' != $this->controller_name && Tools::getValue('controller')) ? '&redirect=' . $this->controller_name : '') . ($email ? '&email=' . $email : ''));
        }
        // Set current index
    ...
    public function initShopContext()
    {
        ...
        if (!($this->multishop_context & Shop::getContext())) {
            if (Shop::CONTEXT_SHOP == Shop::getContext() && !($this->multishop_context & Shop::CONTEXT_SHOP)) {
                Shop::setContext(Shop::CONTEXT_GROUP, Shop::getContextShopGroupID());
            }
            if (Shop::CONTEXT_GROUP == Shop::getContext() && !($this->multishop_context & Shop::CONTEXT_GROUP)) {
                Shop::setContext(Shop::CONTEXT_ALL);
            }
        }
    ...
    public function initProcess()
    {
        ...
        if (Tools::isSubmit('submitFilter' . $this->list_id)
            || false !== $this->context->cookie->{'submitFilter' . $this->list_id}
                ...
                if (Tools::isSubmit('submitBulk' . $bulk_action . $this->table) || Tools::isSubmit('submitBulk' . $bulk_action)) {
                    if ('delete' === $bulk_action) {
                        if ($this->access('delete')) {
                            ...
                            if (empty($this->boxes) && 'attribute' == $this->table) {
    ...
    public function getList(
        ...
        if ($this->multishop_context && Shop::isTableAssociated($this->table) && !empty($this->className)) {
            if (Shop::CONTEXT_ALL != Shop::getContext() || !$this->context->employee->isSuperAdmin()) {
        ...
        do {
            ...
            if ($this->explicitSelect) {
                foreach ($this->fields_list as $key => $array_value) {
                    ...
                    } elseif ('image' != $key && !preg_match('/' . preg_quote($key, '/') . '/i', $this->_select)) {
    ...
    {
        $sql_table = 'order' == $this->table ? 'orders' : $this->table;
    ...
    protected function getLanguageJoinClause($idLang, $idLangShop)
    {
        ...
        if ($this->lang) {
            ...
            if ($idLangShop) {
                ...
                } elseif (Shop::CONTEXT_SHOP == Shop::getContext()) {
            ...
            if (isset($fieldset['form']['input'])) {
                foreach ($fieldset['form']['input'] as $input) {
                    if (!isset($this->fields_value[$input['name']])) {
                        if (isset($input['type']) && 'shop' == $input['type']) {
                            if ($obj->id) {
                            ...
                            if (false === $field_value && isset($input['default_value'])) {
                                $field_value = $input['default_value'];
                            }
    ...
    public function validateRules($class_name = false)
    {
        ...
        foreach ($definition['fields'] as $field => $def) {
            ...
            if (isset($def['lang']) && $def['lang']) {
                ...
                foreach ($languages as $language) {
                    ...
                    if (!empty($value)) {
                        if (true !== ($error = $object->validateField($field, $value, $language['id_lang'], $skip, true))) {
                            $this->errors[$field . '_' . $language['id_lang']] = $error;
                        }
                    }
                }
            } elseif (true !== ($error = $object->validateField($field, Tools::getValue($field), null, $skip, true))) {
        ...
        if (isset($rules['validateLang']) && is_array($rules['validateLang'])) {
            foreach ($rules['validateLang'] as $field_lang => $function) {
                foreach ($languages as $language) {
                    if (false !== ($value = Tools::getValue($field_lang . '_' . $language['id_lang'])) && !empty($value)) {
                        if ('iscleanhtml' == Tools::strtolower($function) && Configuration::get('PS_ALLOW_HTML_IFRAME')) {
    ...
    public function displayModuleOptions($module, $output_type = 'link', $back = null, $install_source_tracking = false)
    {
        ...
        if (isset($module->preferences, $module->preferences['favorite']) && 1 == $module->preferences['favorite']) {
