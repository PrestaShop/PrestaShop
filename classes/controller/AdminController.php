...
class AdminControllerCore extends Controller
{
    ...
    public function initFooter()
    {
        ...
        if ($this->context->language->is_rtl) {
            $this->addJS(_PS_JS_DIR_.'rtl.js', true);
            $css = __PS_BASE_URI__.$this->admin_webpath.'/themes/'.$this->bo_theme.'/css/'.$this->context->language->iso_code.'.css';
            if (file_exists($css)) {
                $this->addCSS($css, 'all', false);
            }
        }
