...
class ImageCore extends ObjectModel
{
    ...
    public static function getImages($id_lang, $id_product, $id_product_attribute = null)
    {
        ...
        $sql .= ' WHERE i.`id_product` = '.(int)$id_product.'
                AND il.`id_lang` = '.(int)$id_lang.$attribute_filter.'
    ...
    public static function deleteCover($id_product)
    {
        ...
        return ($result &&
            Db::getInstance()->execute('
    			...
    			WHERE image_shop.id_shop IN ('.implode(',', array_map('intval', Shop::getContextListShopID())).')
                    AND image_shop.id_image = i.id_image
                    AND i.`id_product` = '.(int)$id_product));
    }
    ...
    public static function duplicateProductImages($id_product_old, $id_product_new, $combination_images)
    {
        ...
        foreach ($result as $row) {
            ...
            if ($image_new->add()) {
                $path = $image_new->getPathForCreation();

                foreach ($images_types as $type) {
                    if (file_exists(_PS_PROD_IMG_DIR_.$image_old->getExistingImgPath().'-'.$type['name'].'.jpg')) {
                        if (!Configuration::get('PS_LEGACY_IMAGES')) {
                            $image_new->createImgFolder();
                        }

                        copy(_PS_PROD_IMG_DIR_.$image_old->getExistingImgPath().'-'.$type['name'].'.jpg',
                        $path.'-'.$type['name'].'.jpg');

                        if (Configuration::get('WATERMARK_HASH')) {
                            copy(_PS_PROD_IMG_DIR_.$image_old->getExistingImgPath().'-'.$type['name'].'-'.Configuration::get('WATERMARK_HASH').'.jpg',
                            $path.'-'.$type['name'].'-'.Configuration::get('WATERMARK_HASH').'.jpg');
                        }
                    }
                }

                if (file_exists(_PS_PROD_IMG_DIR_.$image_old->getExistingImgPath().'.jpg')) {
                    copy(_PS_PROD_IMG_DIR_.$image_old->getExistingImgPath().'.jpg', $path.'.jpg');
                }
    ...
    public function positionImage($position, $direction)
    {
        ...
        Db::getInstance()->execute('
    		UPDATE `'._DB_PREFIX_.'image`
    		SET `position` = `position`'.($direction ? '-' : '+').' 1
    ...
    public function deleteImage($force_delete = false)
    {
        ...
        foreach ($image_types as $type) {
            $files_to_delete[] = $this->image_dir.$this->getExistingImgPath().'-'.$type['name'].'.'.$this->image_format;

            if (Configuration::get('WATERMARK_HASH')) {
                $files_to_delete[] = $this->image_dir.$this->getExistingImgPath().'-'.$type['name'].'-'.Configuration::get('WATERMARK_HASH').'.'.$this->image_format;
            }
        }
    ...
    {
        $testfilesystem = _PS_PROD_IMG_DIR_.'testfilesystem/';
        $testsubfolder = $testfilesystem.'testsubfolder/';

        // check if folders are already existing from previous failed test
        if (file_exists($testsubfolder)) {
            @rmdir($testsubfolder);
            @rmdir($testfilesystem);
        }
        ...
        @mkdir($testsubfolder, self::$access_rights, true);
        @chmod($testsubfolder, self::$access_rights);

        if (!is_writeable($testsubfolder)) {
            return false;
        }
        ...
        return !file_exists($testfilesystem);
    }
