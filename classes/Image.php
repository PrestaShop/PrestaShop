...
class ImageCore extends ObjectModel
{
    ...
    {
        $confWatermarkHash = Configuration::get('WATERMARK_HASH');
        if (isset(Context::getContext()->employee->id)) { // true when call from the back-office
        ...
        if ($confWatermarkHash) {
            $filesToDelete[] = $this->image_dir . $this->getExistingImgPath() . '-' . $imageType['name'] . '-' . $confWatermarkHash . '.' . $imageFormat;
        }
