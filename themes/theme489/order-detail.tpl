...
{/if}
{if $order->gift}
	<p><img src="{$img_dir}icon/gift.png" alt="" class="icon" />&nbsp;{l s='You requested gift-wrapping for your order.'}</p>
...
<ul class="bordercolor address item{if $order->isVirtual()}full_width{/if}">
...
<ul class="bordercolor address alternate_item" {if $order->isVirtual()}style="display:none;"{/if}>
						...
						<td class="bold">
							<label for="cb_{$product.id_order_detail|intval}">
								{if $product.download_hash && $invoice && $product.display_filename != '' && $product.product_quantity_refunded == 0 && $product.product_quantity_return == 0}
									...
									{/if}
										<img src="{$img_dir}icon/download_product.png" class="icon" alt="{l s='Download product'}" />
									</a>
									{if isset($is_guest) && $is_guest}
...
{if !$is_guest}
	...
	{/if}
	<form action="{$link->getPageLink('order-detail', true)}" method="post" id="sendOrderMessage">
...
{else}
<p><img src="{$img_dir}icon/infos.png" alt="" class="icon" />&nbsp;{l s='You cannot make a merchandise return with a guest account'}</p>
{/if}
