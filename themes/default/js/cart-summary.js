...
$(document).ready(function () {
	...
	$('.cart_quantity_delete').unbind('click').live('click', function () {
	...
	$('.cart_address_delivery').live('change', function () {
...
function updateQty(val, cart, el) {
	...
	if (exp.test(val) === true) {
		...
		var QtyToUp = parseInt(input, 10) - parseInt(hidden, 10);

		if (parseInt(QtyToUp, 10) > 0) {
			upQuantity(id.replace('quantity_', ''), QtyToUp);
		} else if (parseInt(QtyToUp, 10) < 0) {
...
function deleteProductFromSummary(id) {
	...
	var ids = 0;
	ids = id.split('_');
	productId = parseInt(ids[0], 10);
	if (typeof(ids[1]) !== 'undefined') {
		productAttributeId = parseInt(ids[1], 10);
	}
	if (typeof(ids[2]) !== 'undefined' && ids[2] !== 'nocustom') {
		customizationId = parseInt(ids[2], 10);
	}
	if (typeof(ids[3]) !== 'undefined') {
		id_address_delivery = parseInt(ids[3], 10);
	}
	$.ajax({
		type : 'POST',
		headers : {
		...
		success : function (jsonData) {
			if (jsonData.hasError) {
				...
				if (parseInt(jsonData.summary.products.length, 10) === 0) {
					...
					for (i = 0; i < jsonData.summary.products.length; i++) {
						if (jsonData.summary.products[i].id_product === productId
							 ...
							 && (parseInt(jsonData.summary.products[i].customization_quantity, 10) > 0)) {
							exist = true;
						}
					}
...
function updateCartSummary(json) {
	...
	for (i in product_list) {
		...
		nbrProducts += parseInt(product_list[i].quantity, 10);
	}
...
function updateCustomizedDatas(json) {
	// @TODO: The body the four for in should be wrapped in an if statement to filter unwanted properties from the prototype
	for (var i in json) {
		for (var j in json[i]) {
