...
<div class="addresses">
	...
	{foreach from=$multipleAddresses item=address name=myLoop}
		<ul class="address {if $smarty.foreach.myLoop.last}last_item{elseif $smarty.foreach.myLoop.first}first_item{/if} {if $smarty.foreach.myLoop.index % 2}alternate_item{else}item{/if}">
			...
			{/foreach}
			<li class="address_update"><a href="{$link->getPageLink('address', true)}id_address={$address.object.id|intval}" title="{l s='Update'}">&raquo; {l s='Update'}</a></li>
			<li class="address_delete"><a href="{$link->getPageLink('address', true)}id_address={$address.object.id|intval}&amp;delete" onclick="return confirm('{l s='Are you sure?'}');" title="{l s='Delete'}">&raquo; {l s='Delete'}</a></li>
		</ul>
	{/foreach}
	</div>
	<p class="clear" />
