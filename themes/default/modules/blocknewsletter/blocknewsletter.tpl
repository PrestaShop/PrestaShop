		...
		<form action="{$link->getPageLink('index')}" method="post">
			<p>
				...
				<input type="text" name="email" size="18" 
					...
					class="inputNew" />
				<!--<select name="action">
					<option value="0"{if isset($action) && $action == 0} checked="checked"{/if}>{l s='Subscribe' mod='blocknewsletter'}</option>
					<option value="1"{if isset($action) && $action == 1} checked="checked"{/if}>{l s='Unsubscribe' mod='blocknewsletter'}</option>
				</select>-->
