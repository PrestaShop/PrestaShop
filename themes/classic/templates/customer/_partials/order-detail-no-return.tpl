<table id="order-products" class="table table-bordered">
  ...
  {foreach from=$order.products item=product}
    <tr>
      ...
      <td class="_align-right">{$product.product_quantity}</td>
  ...
  <tfoot>
    {if $priceDisplay && $use_tax}
      <tr>
        ...
        <td colspan="4" class="_align-right">{$order.data.total_products}</td>
      </tr>
    {/if}
    <tr>
      <td>{l s='Items'} {if $use_tax}{l s='(tax incl.)'}{/if}</td>
      <td colspan="4" class="_align-right">{$order.data.total_products_wt}</td>
    </tr>
    {if $order.data.total_discounts}
      <tr>
        <td>{l s='Total vouchers'}</td>
        <td colspan="4" class="_align-right">{$order.data.total_discounts}</td>
      </tr>
    {/if}
    {if $order.data.total_wrapping}
    <tr>
      <td>{l s='Total gift wrapping cost'}</td>
      <td colspan="4" class="_align-right">{$order.data.total_wrapping}</td>
    </tr>
    {/if}
    <tr>
      <td>{l s='Shipping & handling'} {if $use_tax}{l s='(tax incl.)'}{/if}</td>
      <td colspan="4" class="_align-right">{$order.data.total_shipping}</td>
    </tr>
    <tr>
      <td>{l s='Total'}</td>
      <td colspan="4" class="_align-right">{$order.data.total_paid}</td>
    </tr>
  </tfoot>
</table>
