...
/******/
__webpack_require__.p="",__webpack_require__(__webpack_require__.s=4)}([/* 0 */
/***/
...
,/* 4 */
/***/
function(module,exports,__webpack_require__){__webpack_require__(5),__webpack_require__(6),__webpack_require__(7),__webpack_require__(8),__webpack_require__(9),__webpack_require__(10),__webpack_require__(11),__webpack_require__(12),module.exports=__webpack_require__(13)},/* 5 */
/***/
 ...
 */
global.displayFieldsManager=function(){var managedVirtualProduct,typeProduct=$("#form_step1_type_product"),showVariationsSelector=$("#show_variations_selector"),combinationsBlock=$("#combinations");return{init:function(virtualProduct){managedVirtualProduct=virtualProduct,/** Type product fields display management */
...
//default display
$('#form_step6_suppliers input[name="form[step6][suppliers][]"]').map(function(){supplierInputManage($(this))})}}}();/**
 * Supplier combination collection management
 */
global.supplierCombinations=function(){var id_product=$("#form_id_product").val(),collectionHolder=$("#supplier_combination_collection");return{refresh:function(){var suppliers=$('#form_step6_suppliers input[name="form[step6][suppliers][]"]:checked').map(function(){return $(this).val()}).get(),url=collectionHolder.attr("data-url").replace(/refresh-product-supplier-combination-form\/\d+\/\d+/,"refresh-product-supplier-combination-form/"+id_product+(suppliers.length>0?"/"+suppliers.join("-"):""));$.ajax({url:url,success:function(response){collectionHolder.empty().append(response)}})}}}();/**
 * Quantities management
 */
var stock={init:function(){/** Update qty_0 and shortcut qty_0 field on change */
 ...
 * Form management
 */
global.PsProductForm=function(){function send(redirect,target,callBack){
...
$("#form_step3_attributes").on("tokenfield:removedtoken",function(e){$("#attribute-generator-"+e.attrs.value).remove()})})},send:function(redirect,target,callBack){send(redirect,target,callBack)},switchLanguage:function(iso_code){switchLanguage(iso_code)}}}();/**
 * Custom field collection management
 */
var customFieldCollection=function(){var collectionHolder=$("ul.customFieldCollection");return{init:function(){/** Click event on the add button */
...
check:function(value){defaultCategoryForm.find('input[value="'+value+'"]').prop("checked",!0)},isChecked:function(value){return defaultCategoryForm.find('input[value="'+value+'"]').is(":checked")},/**
     ...
     */
reset:function(){var firstInput=defaultCategoryForm.find("input:first-child");firstInput.prop("checked",!0);var categoryId=firstInput.val();this.checkDefaultCategory(categoryId)}}})().init()},/* 10 */
/***/
function(module,exports,__webpack_require__){/* WEBPACK VAR INJECTION */
(function(global){/**
...
$(function(){$("#combination_form_"+contentElem.attr("data")).find("img").each(function(){title=$(this).attr("src").split("/").pop(),$(this).attr("title",title)})}),$("#form-nav, #form_content").hide()}).on("click","#form .combination-form .btn-back",function(e){e.preventDefault(),$(this).closest(".combination-form").hide(),$("#form-nav, #form_content").show()}).on("click","#form .combination-form .nav a",function(e){e.preventDefault(),$(".combination-form").hide(),$('#accordion_combinations .combination[data="'+$(this).attr("data")+'"] .btn-open').click()})}}}();/**
 * Refresh bulk actions combination number after creating or deleting combinations
 *
 * @param {number} sign
 * @param {number} number
 */
global.refreshTotalCombinations=function(sign,number){var $bulkCombinationsTotal=$("#js-bulk-combinations-total"),currentNumber=parseInt($bulkCombinationsTotal.text())+sign*number;$bulkCombinationsTotal.text(currentNumber)},combinations.init()}).call(exports,__webpack_require__(0))},/* 11 */
/***/
