    ...
    if ($('#product_rule_type_' + product_rule_group_id).val() != 0)
        $.get(
            'ajax-tab.php',
            {
                ...
                newProductRule: 1,
                product_rule_type: $(
                    '#product_rule_type_' + product_rule_group_id,
                ).val(),
            ...
            function(content) {
                if (content != '')
                    $('#product_rule_table_' + product_rule_group_id).append(
                        content,
                    );
            },
        );
    ...
    .autocomplete('ajax-tab.php', {
        ...
        parse: function(data) {
            ...
            for (var i = 0; i < data.length; i++)
                mytab[mytab.length] = {
                    data: data[i],
                    value: (data[i].reference + ' ' + data[i].name).trim(),
                };
            return mytab;
        },
        extraParams: {
...
$('#cart_rule_link_' + currentFormTab)
    .parent()
    .addClass('active');
        ...
        success: function(res) {
            ...
            if (res.found) {
                ...
                $.each(res.products, function() {
                    ...
                    attributes_html +=
                        '<select class="id_product_attribute" id="ipa_' +
                        this.id_product +
                        '" name="ipa_' +
                        this.id_product +
                        '" style="display:none">';
                ...
                $('#gift_attributes_list #gift_attributes_list_select').html(
                    attributes_html,
                );
...
function displayProductAttributes() {
    if (
        $('#ipa_' + $('#gift_product option:selected').val() + ' option')
            .length === 0
    )
...
$(document).ready(function() {
    ...
    if (
        $('#cart_rule_select_1').length > 0 &&
        $('#cart_rule_select_2').length > 0
    ) {
        $('#cart_rule_select_1')
            .jscroll()
            .data('jscrollapi')
            .load_scroll(baseHref + '&type=unselected&search=');
        $('#cart_rule_select_2')
            .jscroll()
            .data('jscrollapi')
            .load_scroll(baseHref + '&type=selected&search=');
