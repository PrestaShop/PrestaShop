...
{/if}
	<div class="message-body">
		...
		{if isset($message.file_name)} 
			{foreach $message.file_name as $att}
				<span class="message-attachment">&nbsp;<i class="icon-link"></i> <a href="{$att.filename}" target="_blank">{if isset($att.name)}{$att.name}{else}{l s="Attachment"}{/if}</a>
			{/foreach}
                {/if}
