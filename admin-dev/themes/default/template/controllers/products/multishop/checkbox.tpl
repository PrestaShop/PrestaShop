...
<!-- 					<div class="multishop_lang_{$language.id_lang}" style="{if !$language.is_default}display: none;{/if}">
 -->						<input
 								style="{if !$language.is_default}display: none;{/if}"
 								class="multishop_lang_{$language.id_lang}"
								type="checkbox"
								name="multishop_check[{$field}][{$language.id_lang}]"
								value="1"
								onclick="ProductMultishop.checkField(this.checked, '{$field}_{$language.id_lang}', '{$type}')" {if !empty($multishop_check[$field][$language.id_lang])}checked="checked"{/if} />
<!-- 					</div>
 -->				{/foreach}
			<!-- </div> -->
		{/if}
