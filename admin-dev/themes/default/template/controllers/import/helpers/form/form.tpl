...
<div class="row">
	<div class="col-lg-8">
		...
		<div class="panel">
			...
			<hr />
			<form id="preview_import" action="{$current}&amp;token={$token|escape:'html':'UTF-8'}" method="post" enctype="multipart/form-data" class="form-horizontal">
				...
				<div class="form-group" id="csv_file_uploader">
					...
					<div class="col-lg-8">
						<input id="file" type="file" name="file" data-url="{$current}&amp;token={$token|escape:'html':'UTF-8'}&amp;ajax=1&amp;action=uploadCsv" class="hide" />
							...
							<tr >
								...
								<td>
									<div class="btn-group pull-right">
										...
										<ul class="dropdown-menu" role="menu">
											<li>
												<a href="{$current}&amp;token={$token|escape:'html':'UTF-8'}&amp;csvfilename={$filename|@urlencode}" target="_blank">
													<i class="icon-download"></i>
													{l s='Download'}
												</a>
											</li>
											<li class="divider"></li>
											<li>
												<a href="{$current}&amp;token={$token|escape:'html':'UTF-8'}&amp;csvfilename={$filename|@urlencode}&amp;delete=1">
													<i class="icon-trash"></i>
													{l s='Delete'}
												</a>
											</li>
				...
				if (data.result) {
					if (typeof data.result.file !== 'undefined') {
						if (typeof data.result.file.error !== 'undefined' && data.result.file.error != '')
							...
							row.find('a.csv-download-link').attr('href','{$current}&token={$token|escape:'html':'UTF-8'}&csvfilename='+filename);
							row.find('a.csv-delete-link').attr('href','{$current}&token={$token|escape:'html':'UTF-8'}&csvfilename='+filename+'&delete=1');
