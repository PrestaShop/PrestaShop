    ...
    var customer_search = $('#customer').val();
        ...
        if (res.found) {
          ...
          $.each(res.customers, function () {
            ...
            html += '<button type="button" data-customer="' + this.id_customer + '" class="setup-customer btn btn-default pull-right"><i class="icon-arrow-right"></i> {l s='Choose' d='Admin.Actions' js=1}</button>';
            html += '</div>';
            html += '</div>';
            html += '</div>';
          });
