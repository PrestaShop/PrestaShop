		...
		<div id="currencyCronjobLiveExchangeRate" class="panel">
			<div class="panel-heading">
				...
				<div class="pull-right checkbox titatoggle unchecked-red checkbox-slider--b-flat">
					<label>
						<input type="checkbox" {(0 != Configuration::get('PS_ACTIVE_CRONJOB_EXCHANGE_RATE'))?'checked="checked"':''}><span></span>
					</label>
				</div>
				<div class="clearfix"></div>
			</div>
			<span class="status disabled {(0 == Configuration::get('PS_ACTIVE_CRONJOB_EXCHANGE_RATE'))?'':'hide'}">{l s="The exchange rates aren't automaticly updated"}</span>
			<span class="status enabled {(0 != Configuration::get('PS_ACTIVE_CRONJOB_EXCHANGE_RATE'))?'':'hide'}">{l s="The exchange rates are automaticly updated"}</span>
		</div>
		{/if}
		<div class="panel">
