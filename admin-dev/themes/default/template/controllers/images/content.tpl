...
{if isset($display_move) && $display_move}
    ...
    {else}
        <form action="{$current}&amp;token={$token|escape:'html':'UTF-8'}" method="post" class="form-horizontal">
            <div class="panel">
                <h3>
...
{if isset($display_regenerate)}
	...
	<form class="form-horizontal" action="{$current}&amp;token={$token|escape:'html':'UTF-8'}" method="post">
		<div class="panel">
			<h3>
                <i class="icon-picture"></i>
