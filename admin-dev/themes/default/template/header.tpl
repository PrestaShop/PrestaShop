...
{/if}
		...
		var errorLogin = '{l|escape s='PrestaShop was unable to log in to Addons. Please check your credentials and your Internet connection.' js=1 d='Admin.Notifications.Warning'}';
      ...
      <ul id="header_employee_box" class="component">
        <li id="employee_infos" class="dropdown">
          ...
          <ul id="employee_links" class="dropdown-menu dropdown-menu-right">
            ...
            {foreach from=$displayBackOfficeEmployeeMenu item=$menuItemModule name=displayBackOfficeEmployeeMenu}
              {foreach from=$menuItemModule item=$menuItem}
                <a class="dropdown-item" href="{$menuItem.link}" target="_blank" rel="noopener noreferrer nofollow">
                  {if isset($menuItem.icon)}<i class="material-icons">{$menuItem.icon}</i> {/if}{$menuItem.title}
                </a>
              {/foreach}
              <p class="divider"></p>
            {/foreach}
