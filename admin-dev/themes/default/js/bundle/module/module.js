...
var AdminModuleController = function() {
        ...
        $.ajax({
            ...
            .done(function(response) {
                if (response.status === true) {
                    ...
                    var requiredSelectorCombination =
                        moduleGlobalSelector + ', ' + moduleSortingSelector;
        ...
        for (var i = 0; i < this.modulesList.length; i++) {
            ...
            if (currentModule.display == this.currentDisplay) {
                ...
                if (this.currentRefCategory !== null) {
                    isVisible &=
                        currentModule.categories === this.currentRefCategory;
                }
            ...
            self.animateEndUpload(function() {
                if (result.status === true) {
                    if (result.is_configurable === true) {
                        var configureLink = moduleURLs.configurationPage.replace(
                            '1',
                            result.module_name,
                        );
    ...
    this.getNotificationsCount = function() {
        ...
        $.getJSON(urlToCall, this.updateNotificationsCount).fail(function() {
    ...
    this.initCategorySelect = function() {
        ...
        body.on('click', this.categoryItemSelector, function() {
            // Get data from li DOM input
            self.currentRefCategory = $(this)
                .attr('data-category-ref')
                .toLowerCase();
            var categorySelectedDisplayName = $(this).attr(
