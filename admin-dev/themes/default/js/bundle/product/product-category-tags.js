...
var productCategoriesTags = (function() {
    ...
    return {
        ...
        getTags: function() {
            var categoriesForm = $('#form_step1_categories');
            var inputs = categoriesForm
                .find('label > input[type=checkbox]:checked')
                .toArray();
        ...
        manageTagsOnInput: function() {
            var categoriesForm = $('#form_step1_categories');
            var that = this;
            categoriesForm.on('change', 'input[type=checkbox]', function(
                event,
            ) {
                var input = $(this);
        ...
        manageTagsOnTags: function() {
            ...
            tagsContainer.on('click', 'a.pstaggerClosingCross', function(
                event,
            ) {
        ...
        createTag: function(category) {
            ...
            var isTagExist = tagsContainer.find(
            ...
            if (0 == isTagExist.length) {
                tagsContainer.append(
                    '<span class="pstaggerTag">' +
                        ...
                        category.name +
                        '</span>' +
                        '<a class="pstaggerClosingCross" href="#" data-id="' +
                        category.id +
                        '">x</a>' +
                        '</span>',
                );
                ...
                if (0 == $(optionId).length) {
                    defaultCategoryForm.append(
                        '<div class="radio">' +
                            '<label class="required">' +
                            '<input type="radio"' +
                            'id="form_step1_id_category_default_' +
                            category.id +
                            '" name="form[step1][id_category_default]" required="required" value="' +
                            category.id +
                            '">' +
                            category.name +
                            '</label>' +
            ...
            searchBox
                .autocomplete({
                    ...
                    select: function(event, ui) {
                        ...
                        var categoriesForm = $('#form_step1_categories');
                        categoriesForm
                            .find('input[value="' + categoryId + '"].category')
                            .prop('checked', true);
                        $(this).val('');
                    },
                })
...
BOEvent.on(
    'Product Categories Management started',
