...
$(document).ready(function() {
    ...
    $('#form-nav').on(
        'click',
        '.nav-item',
        function clearTooltipsAndPopovers() {
    ...
    return {
        init: function(virtualProduct) {
            ...
            $('a#tax_rule_shortcut_opener').on('click', function() {
                ...
                if (duplicate.length == 0) {
                    ...
                    duplicate = origin
                        .clone(false)
                        .attr('id', 'form_step2_id_tax_rules_group_shortcut');
        ...
        initVisibilityRule: function() {
            ...
            var applyVisibilityRule = function applyVisibilityRule() {
                var $availableForOrder = $(
                    availableForOrderSelector + ' input',
                );
            ...
            $(availableForOrderSelector + ' .checkbox').on(
                'click',
                applyVisibilityRule,
            );
            applyVisibilityRule();
        ...
        refresh: function() {
            ...
            $('#form-nav a[href="#step3"]').text(
                translate_javascripts['Quantities'],
            );
            ...
            if (typeProduct.val() === '1') {
                ...
                $(
                    '#virtual_product, #pack_stock_type, #js_form_step1_inputPackItems',
                ).hide();
                $('#form-nav a[href="#step4"]').show();
                ...
                if (typeProduct.val() === '2') {
                    ...
                    showVariationsSelector
                        ...
                        .attr('checked', true);
                    $('#form-nav a[href="#step3"]').text(
                        translate_javascripts['Virtual product'],
                    );
                } else {
            ...
            if (
                shouldDestroyVirtualProduct &&
                managedVirtualProduct !== undefined
            ) {
            ...
            if (
                ...
                $('#accordion_combinations tr:not(#loading-attribute)').length >
                    0
            ) {
                combinationsBlock.show();

                $('#form-nav a[href="#step3"]').text(
                    translate_javascripts['Combinations'],
                );
            ...
            if (
                $('input[name="show_variations"][value="1"]:checked').length >=
                1
            ) {
        ...
        checkAccessVariations: function() {
            if (
                (showVariationsSelector.find('input:checked').val() === '1' ||
                    $('#accordion_combinations tr:not(#loading-attribute)')
                        .length > 0) &&
                ...
                var errorMessage =
                    "You can't create " +
                    typeOfProduct +
                    ' product with variations. Are you sure to disable variations ? they will all be deleted.';
                modalConfirmation
                    .create(translate_javascripts[errorMessage], null, {
                        onCancel: function() {
                            typeProduct.val(0).change();
                            /* else the radio bouton is not display even if checked attribute is true */
                            $(
                                '#show_variations_selector input[value="1"]',
                            ).click();
                        },
                        onContinue: function() {
                            $.ajax({
                                type: 'GET',
                                url: $('#accordion_combinations')
                                    .attr('data-action-delete-all')
                                    .replace(
                                        /delete-all\/\d+/,
                                        'delete-all/' +
                                            $('#form_id_product').val(),
                                ...
                                success: function() {
                                    $(
                                        '#accordion_combinations .combination',
                                    ).remove();
                                    displayFieldsManager.refresh();
                                },
...
var formCategory = (function() {
    ...
    function send(form) {
        $.ajax({
            ...
            data: {
                'form[category][name]': $(
                    '#form_step1_new_category_name',
                ).val(),
                'form[category][id_parent]': $(
            ...
            success: function(response) {
                ...
                var html =
                    ...
                    '<label>' +
                    '<input type="checkbox" name="form[step1][categories][tree][]" checked value="' +
                    response.category.id +
                    '"> ' +
                    response.category.name[1] +
                    '<input type="radio" value="' +
                    response.category.id +
                    '" name="ignore" class="default-category">' +
                    '</label>' +
                    '</div>' +
                    '</li>';

                var parentElement = $(
                    '#form_step1_categories input[value=' +
                        response.category.id_parent +
                        ']',
                )
                    .parent()
                    .parent();
                ...
                $('#form_step1_new_category_id_parent').append(
                    '<option value="' +
                        response.category.id +
                        '">' +
                        response.category.name[1] +
            ...
            error: function(response) {
                $.each(jQuery.parseJSON(response.responseText), function(
                    key,
                    errors,
                ) {
            ...
            complete: function() {
                $('#form_step1_new_category button.submit').removeAttr(
                    'disabled',
                );
            },
        });
...
var featuresCollection = (function() {
    ...
    function add() {
        var newForm = collectionHolder
            .attr('data-prototype')
            .replace(/__name__/g, maxCollectionChildren);
    ...
    return {
        init: function() {
            ...
            $(document).on(
                'change',
                '.feature-collection select.feature-selector',
                function(event) {
            ...
            $featuresContainer.on(
                'change',
                '.row select, .row input[type="text"]',
                function onChange(event) {
                    var that = event.currentTarget;
                    var $row = $($(that).parents('.row')[0]);
                    var $definedValueSelector = $row.find(
...
var supplier = (function() {
    var supplierInputManage = function(input) {
        var supplierDefaultInput = $(
            '#form_step6_suppliers input[name="form[step6][default_supplier]"][value=' +
                $(input).val() +
                ']',
        );
        if ($(input).is(':checked')) {
    ...
    return {
        init: function() {
            ...
            var supplierInput = $(
                '#form_step6_suppliers input[name="form[step6][suppliers][]"]',
            );
...
var supplierCombinations = (function() {
    ...
    return {
        refresh: function() {
            var suppliers = $(
                '#form_step6_suppliers input[name="form[step6][suppliers][]"]:checked',
            )
                .map(function() {
                    return $(this).val();
                })
                .get();
            var url = collectionHolder
                .attr('data-url')
                .replace(
                    /refresh-product-supplier-combination-form\/\d+\/\d+/,
                    'refresh-product-supplier-combination-form/' +
                        id_product +
...
var stock = (function() {
    return {
        init: function() {
            ...
            $('#form_step1_qty_0_shortcut, #form_step3_qty_0').keyup(
                function() {
            ...
            $('#form_step3_advanced_stock_management').on('change', function(
                e,
            ) {
                if (e.target.checked) {
            ...
            $(
                '#form_step3_depends_on_stock_0, #form_step3_depends_on_stock_1, #form_step3_advanced_stock_management',
...
var nav = (function() {
    return {
        init: function() {
            ...
            if (hash) {
                formNav
                    .find("a[href='" + hash.replace(prefix, '') + "']")
                    .tab('show');
            }
...
var warehouseCombinations = (function() {
    ...
    return {
        init: function() {
            ...
            $(document).on(
                'click',
                'div[id^="warehouse_combination_"] button.check_all_warehouse',
                function() {
                    var checkboxes = $(this)
                        .closest('div[id^="warehouse_combination_"]')
            ...
            $(document).on(
                'change',
                'div[id^="warehouse_combination_"] input[id^="form_step4_warehouse_combination_"][id$="_activated"]',
                function() {
                    var checked = $(this).prop('checked');
                    var location = $(this)
                        .closest('div.form-group')
                        .find(
                            'input[id^="form_step4_warehouse_combination_"][id$="_location"]',
                        );
        ...
        refresh: function() {
            var show =
                $('input#form_step3_advanced_stock_management:checked').length >
                0;
            if (show) {
                var url = collectionHolder
                    .attr('data-url')
                    .replace(/\/\d+(?=\?.*)/, '/' + id_product);
                $.ajax({
                    url: url,
                    success: function(response) {
                        collectionHolder.empty().append(response);
...
var form = (function() {
    ...
    function send(redirect, target, callBack) {
        ...
        var data = $('input, textarea, select', elem)
        ...
        if (target == '_blank' && redirect) {
            ...
            openBlank.document.write(
                '<p style="text-align: center;">' +
                    ...
                    baseAdminDir +
                    '/themes/default/img/spinner.gif">' +
                    '</p>',
            );
        }
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if (typeof response.customization_fields_ids != 'undefined') {
                    $.each(response.customization_fields_ids, function(k, v) {
                        $(
                            '#form_step6_custom_fields_' +
                                k +
                                '_id_customization_field',
                        ).val(v);
                    });
                }
            ...
            error: function(response) {
                ...
                $.each(tabsWithErrors, function(key, tabIndex) {
                    ...
                    $('#form-nav li a[href="#' + tabIndex.split('_')[0] + '"]')
                        .parent()
                        .addClass('has-error');
                });

                if (
                    $('div[class*="translation-label-"].has-danger').length > 0
                ) {
                    var regexLabel = 'translation-label-';

                    var translationLabelClass = $.grep(
                    ...
                    if (translationLabelClass) {
                        var selectValue = translationLabelClass.replace(
                            regexLabel,
                            '',
                        );
                        ...
                        if (
                            $(
                                '#form_switch_language option[value="' +
                                    selectValue +
                                    '"]',
                            ).length > 0
                        ) {
    ...
    function switchLanguage(iso_code) {
        $(
            'div.translations.tabbable > div > div.translation-field:not(.translation-label-' +
                iso_code +
                ')',
        ).removeClass('show active');
        $(
            'div.translations.tabbable > div > div.translation-field.translation-label-' +
                iso_code,
    ...
    return {
        init: function() {
            ...
            jwerty.key('alt+shift+V', function(e) {
                ...
                send(
                    productFooter.attr('data-redirect'),
                    productFooter.attr('target'),
                );
            });
            ...
            $('#form-loading').fadeIn(function() {
                ...
                var filter = function(suggestions) {
                    var selected = [];
                    $('#attributes-generator input.attribute-generator').each(
                        function() {
                    ...
                    return $.grep(suggestions, function(suggestion) {
                        return (
                            $.inArray(suggestion.value, selected) === -1 &&
                            $.inArray(
                                'group-' + suggestion.data.id_group,
                                selected,
                            ) === -1
                        );
                    });
                ...
                $('#form_step3_attributes').on(
                    'tokenfield:createtoken',
                    function(e) {
                        if (
                            !e.attrs.data &&
                            e.handleObj.origType !== 'tokenfield:createtoken'
                        ) {
                            return false;
                        }
                    },
                ...
                $('#form_step3_attributes').on(
                    ...
                    function(e) {
                        if (e.attrs.data) {
                            $('#attributes-generator').append(
                                '<input type="hidden" id="attribute-generator-' +
                                    e.attrs.value +
                                    '" class="attribute-generator" value="' +
                                    e.attrs.value +
                                    '" name="options[' +
                                    e.attrs.data.id_group +
                                    '][' +
                                    e.attrs.value +
                                    ']" />',
                            );
                        } else if (
                            e.handleObj.origType == 'tokenfield:createdtoken'
                        ) {
                            $('#attributes-generator').append(
                                '<input type="hidden" id="attribute-generator-' +
                                    $(
                                        '.js-attribute-checkbox[data-value="' +
                                            e.attrs.value +
                                            '"]',
                                    ).data('value') +
                                    '" class="attribute-generator" value="' +
...
var customFieldCollection = (function() {
    ...
    return {
        init: function() {
            ...
            $(document).on(
                'click',
                'ul.customFieldCollection .delete',
                function(e) {
                    e.preventDefault();
                    var _this = $(this);
...
var virtualProduct = (function() {
    ...
    var getOnDeleteVirtualProductFileHandler = function($deleteButton) {
        return $.ajax({
            ...
            url: $deleteButton
                ...
                .replace(/\/\d+(?=\?.*)/, '/' + id_product),
            success: function() {
                $('#form_step3_virtual_product_file_input')
                    .removeClass('hide')
                    .addClass('show');
                $('#form_step3_virtual_product_file_details')
                    .removeClass('show')
                    .addClass('hide');
            },
        });
    ...
    return {
        init: function() {
            $(document).on(
                'change',
                'input[name="form[step3][virtual_product][is_virtual_file]"]',
                function() {
                    if ($(this).val() === '1') {
                        ...
                        var url = $('#virtual_product')
                            .attr('data-action-remove')
                            .replace(/remove\/\d+/, 'remove/' + id_product);
                        ...
                        $.ajax({
                            ...
                            success: function() {
                                ...
                                $(
                                    '#form_step3_virtual_product_nb_downloadable',
                                ).val(0);
                                $(
                                    '#form_step3_virtual_product_expiration_date',
                                ).val('');
            ...
            if (
                $(
                    'input[name="form[step3][virtual_product][is_virtual_file]"]:checked',
                ).val() === '1'
            ) {
                $('#virtual_product_content').show();
            } else {
            ...
            $('#form_step3_virtual_product_file_details .delete').click(
                function(e) {
                    e.preventDefault();
                    ...
                    modalConfirmation
                        .create(
                            ...
                            {
                                onContinue: function() {
                                    getOnDeleteVirtualProductFileHandler(
                                        $deleteButton,
                                    );
                                },
                            },
            ...
            $('#form_step3_virtual_product_save').click(function() {
                ...
                if ($('#form_step3_virtual_product_file')[0].files[0]) {
                    data.append(
                        'product_virtual[file]',
                        $('#form_step3_virtual_product_file')[0].files[0],
                    );
                }
                data.append(
                    'product_virtual[is_virtual_file]',
                    $(
                        'input[name="form[step3][virtual_product][is_virtual_file]"]:checked',
                    ).val(),
                );
                data.append(
                    'product_virtual[name]',
                    $('#form_step3_virtual_product_name').val(),
                );
                data.append(
                    'product_virtual[nb_downloadable]',
                    $('#form_step3_virtual_product_nb_downloadable').val(),
                );
                data.append(
                    'product_virtual[expiration_date]',
                    $('#form_step3_virtual_product_expiration_date').val(),
                );
                data.append(
                    'product_virtual[nb_days]',
                    $('#form_step3_virtual_product_nb_days').val(),
                );
                ...
                $.ajax({
                    ...
                    url: $('#virtual_product')
                        .attr('data-action')
                        .replace(/save\/\d+/, 'save/' + id_product),
                    data: data,
                    contentType: false,
                    processData: false,
                    beforeSend: function() {
                    ...
                    success: function(response) {
                        ...
                        if (response.file_download_link) {
                            $(
                                '#form_step3_virtual_product_file_details a.download',
                            ).attr('href', response.file_download_link);
                            $('#form_step3_virtual_product_file_input')
                                .removeClass('show')
                                .addClass('hide');
                            $('#form_step3_virtual_product_file_details')
                    ...
                    error: function(response) {
                        $.each(
                            ...
                            function(key, errors) {
                                ...
                                $('#form_step3_virtual_product_' + key)
                                    .parent()
                                    .append(html);
                                $('#form_step3_virtual_product_' + key)
                                    .parent()
        ...
        destroy: function() {
            var fileDetailsSelector =
                '#form_step3_virtual_product_file_details';
            var fileAssociationExists = !$(fileDetailsSelector).hasClass(
                'hide',
            );
            ...
            var associatedFileCheckboxSelectorPrefix =
                '#form_step3_virtual_product_is_virtual_file_';
            $(associatedFileCheckboxSelectorPrefix + '0').prop(
                'checked',
                false,
            );
    ...
    return {
        init: function() {
            ...
            $('#product-attachment-files-check').change(function() {
                if ($(this).is(':checked')) {
                    $('#product-attachment-file input[type="checkbox"]').prop(
                        'checked',
                        true,
                    );
                } else {
            ...
            $('#form_step6_attachment_product_add').click(function() {
                ...
                if ($('#form_step6_attachment_product_file')[0].files[0]) {
                    data.append(
                        ...
                        $('#form_step6_attachment_product_file')[0].files[0],
                    );
                }
                data.append(
                    'product_attachment[name]',
                    $('#form_step6_attachment_product_name').val(),
                );
                data.append(
                ...
                $.ajax({
                    type: 'POST',
                    url: replaceEndingIdFromUrl(
                        $('#form_step6_attachment_product').attr('data-action'),
                        id_product,
                    ),
                    data: data,
                    contentType: false,
                    ...
                    success: function(response) {
                        ...
                        if (response.id) {
                            var row =
                                '<tr>\
                <td class="col-md-3"><input type="checkbox" name="form[step6][attachments][]" value="' +
                                response.id +
                                '" checked="checked"> ' +
                                response.real_name +
                                '</td>\
                ...
                <td class="col-md-2">' +
                            ...
                            $('.js-options-with-attachments').removeClass(
                                'hide',
                            );
                        }
                    },
                    error: function(response) {
                        $.each(
                            ...
                            function(key, errors) {
                                ...
                                $('#form_step6_attachment_product_' + key)
                                    .parent()
                                    .append(html);
                                $('#form_step6_attachment_product_' + key)
                                    .parent()
...
var imagesProduct = (function() {
    ...
    return {
        ...
        init: function() {
            ...
            var dropzoneOptions = {
                ...
                init: function() {
                    ...
                    if (
                        dropZoneElem.find('.dz-preview:not(.openfilemanager)')
                            .length
                    ) {
        ...
        updateDisplayCover: function(id_image) {
            ...
            $(
                '#product-images-dropzone .dz-preview[data-id="' +
                    id_image +
                    '"]',
            ).append(
                '<div class="iscover">' +
                        ...
                        onContinue: function() {
                            $.ajax({
                                ...
                                complete: function() {
                                    ...
                                    var wasCover = !!dropZoneElem.find(
                                        '.dz-preview[data-id="' +
                                            id +
                                            '"] .iscover',
                                    ).length;
                                    dropZoneElem
                                        .find(
                                            '.dz-preview[data-id="' + id + '"]',
                                        )
                                        .remove();
                                    $(
                                        '.images .product-combination-image [value=' +
                                            id +
                                            ']',
                                    )
                                        .parent()
...
var priceCalculation = (function() {
    ...
    var displayPricePrecision = priceHTElem.attr(
        'data-display-price-precision',
    );
    ...
    function addTaxes(price, rates, computationMethod) {
        ...
        if (computationMethod === '0') {
            for (i in rates) {
                price_with_taxes *= 1.0 + parseFloat(rates[i]) / 100.0;
                break;
            }
            ...
            price_with_taxes *= 1.0 + parseFloat(rate) / 100.0;
    ...
    return {
        init: function() {
            ...
            $('#form_step1_price_shortcut, #form_step2_price').keyup(
                function() {
            ...
            $('#form_step1_price_ttc_shortcut, #form_step2_price_ttc').keyup(
                function() {
                    var price = priceCalculation.normalizePrice($(this).val());

                    if (
                        $(this).attr('id') === 'form_step1_price_ttc_shortcut'
                    ) {
                        $('#form_step2_price_ttc')
                            .val(price)
                            .change();
                    } else {
                        $('#form_step1_price_ttc_shortcut')
                            .val(price)
                            .change();
                    }
            ...
            $('#form_step2_price, #form_step2_price_ttc').change(function() {
                var taxExcludedPrice = priceCalculation.normalizePrice(
                    $('#form_step2_price').val(),
                );
                var taxIncludedPrice = priceCalculation.normalizePrice(
                    $('#form_step2_price_ttc').val(),
                );
            ...
            $(document).on(
                'keyup',
                '.combination-form .attribute_priceTE',
                function() {
            ...
            $(document).on(
                'keyup',
                '.combination-form .attribute_priceTI',
                function() {
        ...
        addCurrentTax: function(price) {
            ...
            var computation_method = taxElem
                .find('option:selected')
                .attr('data-computation-method');
            var priceWithTaxes = Number(
                ps_round(
        ...
        impactTaxInclude: function(obj) {
            ...
            var targetInput = obj
                .closest('div[id^="combination_form_"]')
                .find('input.attribute_priceTI');
            var newPrice = 0;
        ...
        impactTaxExclude: function(obj) {
            ...
            if (!isNaN(price)) {
                ...
                var computation_method = taxElem
                    .find('option:selected')
                    .attr('data-computation-method');
                newPrice = removeTaxes(
                    ps_round(price, displayPricePrecision),
                    rates,
                    computation_method,
                );
                newPrice = truncateDecimals(newPrice, 6);
...
var seo = (function() {
    ...
    function updateRemoteUrl() {
        switch (redirectTypeElem.val()) {
            ...
            case '302-category':
                productRedirect
                    .find('label')
                    .html(redirectTypeElem.attr('data-labelcategory'));
                productRedirect
                    .find('input')
                    .attr(
                        'placeholder',
                        redirectTypeElem.attr('data-placeholdercategory'),
                    );
                productRedirect
                    .find('.typeahead-hint')
                    .text(redirectTypeElem.attr('data-hintcategory'));
                break;
            default:
                productRedirect
                    .find('label')
                    .html(redirectTypeElem.attr('data-labelproduct'));
                productRedirect
                    .find('input')
                    .attr(
                        'placeholder',
                        redirectTypeElem.attr('data-placeholderproduct'),
                    );
    ...
    var updateFriendlyUrl = function(elem) {
        ...
        $('#form_step5_link_rewrite_' + id_lang).val(
            str2url(elem.val(), 'UTF-8'),
        );
    };
    ...
    return {
        init: function() {
            ...
            redirectTypeElem.change(function() {
                productRedirect
                    .find('#form_step5_id_type_redirected-data')
                    .html('');
                hideShowRedirectToProduct();
            });

            /** On product title change, update friendly URL*/
            $('#form_step1_names.friendly-url-force-update input').keyup(
                function() {
        ...
        onSave: function() {
            ...
            $(
                'input[id^="form_step5_link_rewrite_"]',
                '#form_step5_link_rewrite',
            ).each(function() {
                var elem = $(this);
...
var recommendedModules = (function() {
    return {
        init: function() {
            this.moduleActionMenuLinkSelectors =
                'button.module_action_menu_install, button.module_action_menu_enable, ' +
            ...
            $(this.moduleActionMenuLinkSelectors).on(
                'module_card_action_event',
                this.saveProduct,
            );
        },
