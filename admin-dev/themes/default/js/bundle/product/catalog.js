...
$(document).ready(function() {
    ...
    $(
        'div#product_catalog_category_tree_filter div.radio > label > input:radio',
    ).change(function() {
        if ($(this).is(':checked')) {
     ...
     */
    $('tr.column-filters input:text, tr.column-filters select', form).on(
        'change input',
        function() {
    ...
    $('body').on(
        'mousedown',
        'tbody.sortable [data-uniturl] td.placeholder',
        function() {
            var trParent = $(this).closest('tr');
            trParent
                .find('input:checkbox[name="bulk_action_selected_products[]"]')
                .attr('checked', true);
        },
    );
    ...
    form.submit(function(e) {
        e.preventDefault();
        $('#filter_column_id_product', form).val(
            $('#filter_column_id_product', form).attr('sql'),
        );
        $('#filter_column_price', form).val(
            $('#filter_column_price', form).attr('sql'),
        );
        $('#filter_column_sav_quantity', form).val(
            $('#filter_column_sav_quantity', form).attr('sql'),
        );
     ...
     */
    $('#catalog_sql_query_modal button[value="sql_manager"]').on(
        'click',
        function() {
...
function productOrderTable(orderBy, orderWay) {
    ...
    var url = form
        .attr('orderingurl')
        .replace(/name/, orderBy)
        .replace(/asc/, orderWay);
    window.location.href = url;
}
...
function updateBulkMenu() {
    var selectedCount = $(
        'form#product_catalog_list input:checked[name="bulk_action_selected_products[]"][disabled!="disabled"]',
    ).length;
...
function updateFilterMenu() {
    ...
    columnFilters
        .find('input[type="text"][sql!=""][sql], input[type="text"]:visible')
        .each(function() {
            if ($(this).val() !== '') {
                count++;
            }
        });
    var filtersNotUpdatedYet =
        count === 0 && productCatalogFilterChanged === false;
    $('button[name="products_filter_submit"]').prop(
        'disabled',
        filtersNotUpdatedYet,
    );
...
function categoryFilterButtons() {
    ...
    catTreeSiblingDivs
        .find('button[name="product_catalog_category_tree_filter_collapse"]')
        .toggle(!catTreeList.filter(':visible').length);
    catTreeSiblingDivs
        .find('button[name="product_catalog_category_tree_filter_expand"]')
        .toggle(!catTreeList.filter(':hidden').length);
    catTreeSiblingDivs
        .find('button[name="product_catalog_category_tree_filter_reset"]')
...
function bulkModalAction(allItems, postUrl, redirectUrl, action) {
    ...
    var details = targetModal.find(
        '#catalog_' + action + '_progression .progress-details-text',
    );
    var progressBar = targetModal.find(
        '#catalog_' + action + '_progression .progress-bar',
    );
    ...
    var bulkCall = function(items, successCallback, errorCallback) {
        ...
        details.html(
            details.attr('default-value').replace(/\.\.\./, '') +
                ' (#' +
                item0 +
                ')',
        );
        $.ajax({
            ...
            success: function(data, status) {
                progressBar.css(
                    'width',
                    (currentItemIdx * 100) / itemsCount + '%',
                );
                progressBar
                    .find('span')
                    .html(currentItemIdx + ' / ' + itemsCount);
...
function bulkProductAction(element, action) {
    ...
    var items = $(
        'input:checked[name="bulk_action_selected_products[]"]',
        form,
    );
    if (items.length === 0) {
        return false;
    ...
    switch (action) {
        case 'delete_all':
            ...
            $('#catalog_deletion_modal button[value="confirm"]').on(
                'click',
                function() {
...
function unitProductAction(element, action) {
    ...
    var redirectionInput = $('<input>')
        .attr('type', 'hidden')
        .attr('name', 'redirect_url')
        .val(redirectUrlHandler.attr('redirecturl'));
...
function showBulkProductEdition(show) {
    ...
    if ($('a#pagination_next_url[href]').length === 0) {
        $('#bulk_edition_save_next')
            .prop('disabled', true)
            .removeClass('btn-primary');
        $('#bulk_edition_save_keep')
            .attr('type', 'submit')
...
function bulkProductEdition(element, action) {
    ...
    */
        case 'sort':
            showBulkProductEdition(true);
            $(
                'input#bulk_action_select_all, input:checkbox[name="bulk_action_selected_products[]"]',
                form,
            ).prop('disabled', true);
            $('#bulk_edition_toolbar input:submit').attr(
                'editionaction',
                action,
            );
            break;
...
function showLastSqlQuery() {
    $('#catalog_sql_query_modal_content textarea[name="sql"]').val(
        $('tbody[last_sql]').attr('last_sql'),
    );
