...
var combinations = (function() {
                    ...
                    onContinue: function() {
                        ...
                        $.ajax({
                            ...
                            beforeSend: function() {
                                ...
                                $(
                                    '#create-combinations, #apply-on-combinations, #submit, .btn-submit',
    ...
    function updateFinalPrice(tableRow) {
        if (!tableRow.is('tr')) {
            throw new Error(
                'Structure of table has changed, this function needs to be updated.',
            );
        }
        ...
        var previousImpactOnPrice = Tools.parseFloatFromString(
            priceImpactInput.attr('value'),
        );

        var currentFinalPrice = Tools.parseFloatFromString(
            finalPriceLabel.data('price'),
            true,
        );
        var finalPrice =
            currentFinalPrice - previousImpactOnPrice + impactOnPrice;
    ...
    return {
        init: function() {
            ...
            $(document)
                ...
                .on(
                    'keyup',
                    'input[id^="combination"][id$="_attribute_quantity"]',
                    function() {
                        var attributeId = $(this)
                            .closest('.combination-form')
                            .attr('data');
                        var input = getCombinationRow(attributeId).find(
                ...
                .on('keyup', '.attribute-quantity input', function() {
                    var attributeId = $(this)
                        .closest('.combination')
                        .attr('data');
                    var input = getCombinationForm(attributeId).find(
                        'input[id^="combination"][id$="_attribute_quantity"]',
                    );
                ...
                .on('click', 'input.attribute-default', function() {
                    ...
                    getCombinationForm(attributeId)
                        .find(
                            'input[id^="combination"][id$="_attribute_default"]',
                        )
                        .prop('checked', true);
                })
                ...
                .on('click', '#accordion_combinations .btn-open', function(e) {
                    ...
                    var prevCombinationId = $(
                        '#accordion_combinations tr[data="' +
                            id_attribute +
                            '"]',
                    )
                        .prev()
                        .attr('data');
                    var nextCombinationId = $(
                        '#accordion_combinations tr[data="' +
                            id_attribute +
                            '"]',
                    )
                    ...
                    if (prevCombinationId) {
                        navElem
                            .find('.prev')
                            .attr('data', prevCombinationId)
                            .show();
                    }
                    if (nextCombinationId) {
                        navElem
                            .find('.next')
                            .attr('data', nextCombinationId)
                            .show();
                    }

                    /** init combination tax include price */
                    priceCalculation.impactTaxInclude(
                        contentElem.find('.attribute_priceTE'),
                    );

                    contentElem
                        .insertBefore('#form-nav')
                        .removeClass('hide')
                        .show();

                    contentElem
                        .find('.datepicker input[type="text"]')
                        .datetimepicker({
                            locale: iso_user,
                            format: 'YYYY-MM-DD',
                        });
                    ...
                    number.text(
                        countSelectedProducts() + '/' + allProductCombination,
                    );

                    $(document).on(
                        'click',
                        '.tabs .product-combination-image',
                        function() {
                            number.text(
                    ...
                    $(function() {
                        $('#combination_form_' + contentElem.attr('data'))
                            .find('img')
                            .each(function() {
                                title = $(this)
                                    .attr('src')
                ...
                .on('click', '#form .combination-form .nav a', function(e) {
                    ...
                    $(
                        '#accordion_combinations .combination[data="' +
                            $(this).attr('data') +
                            '"] .btn-open',
                    ).click();
                });
...
BOEvent.on(
    'Product Combinations Management started',
    function initCombinationsManagement() {
        combinations.init();
    },
    'Back office',
);
