    ...
    var DateRangePicker = function(element, options) {
        ...
        if (typeof options.start !== 'undefined') {
            if (options.start.constructor === String) {
                start = DPGlobal.parseDate(
                    options.start,
                    DPGlobal.parseFormat('Y-m-d'),
                ).getTime();
        ...
        this.format = DPGlobal.parseFormat(
            options.format || this.element.data('date-format') || 'Y-m-d',
        );
    ...
    DateRangePicker.prototype = {
        ...
        setStartCompare: function(date) {
            if (date === null) {
                ...
                startCompare = DPGlobal.parseDate(
                    date,
                    DPGlobal.parseFormat('Y-m-d'),
                ).getTime();
            } else if (date.constructor === Number) {
        ...
        fill: function() {
            ...
            this.picker
                .find('.daterangepicker-days th:eq(1)')
                .text(year + ' / ' + DPGlobal.dates.months[month])
                .append('&nbsp;<small><i class="icon-angle-down"></i><small>');
            ...
            this.picker
                .find('.daterangepicker-days tbody')
                .empty()
                .append(html.join(''));
            var currentYear = this.date.getFullYear();

            var months = this.picker
                .find('.daterangepicker-months')
                .find('th:eq(1)')
                .text(year)
                .end()
                .find('span')
                        ...
                        if (target.is('.day') && !target.is('.disabled')) {
                            ...
                            if (start && end) {
                                if (!compare) {
                                    ...
                                    $('.start-selected').removeClass(
                                        'start-selected',
                                    );
                                    $('.end-selected').removeClass(
                                        'end-selected',
                                    );
                                }
                            }
                            if (click === 2) {
                                ...
                                if (compare) {
                                    ...
                                    $('.date-input')
                                        ...
                                        .removeClass('input-complete');
                                    $('.range-compare').removeClass(
                                        'range-compare',
                                    );
                                } else {
                                    $('td.day')
                                        .removeClass('start-selected')
                                        .removeClass('end-selected');
                                    $('.date-input')
                                        .removeClass('input-selected')
                                        .removeClass('input-complete');
                                    $('.range').removeClass('range');
                            ...
                            if (!click || switched === true) {
                                ...
                                if (!switched) {
                                    if (compare) {
                                        ...
                                        target
                                            .addClass('start-selected-compare')
                                            .addClass('end-selected-compare');
                                    } else {
                                        $('#date-end')
                                            .val(null)
                                            .focus()
                                            .addClass('input-selected');
                                        target
                                            .addClass('start-selected')
                                            .addClass('end-selected');
                                    }
                                }
                            ...
                            //define end
                                ...
                                if (compare) {
                                    $('.end-selected-compare').removeClass(
        ...
        mouseoverRange: function() {
            ...
            $('#datepicker .day').each(function() {
                ...
                if (compare) {
                    if (
                        !endCompare &&
                        date_val > startCompare &&
                        date_val < over
                    ) {
                        $(this)
                            .not('.old')
                            .not('.new')
                            .addClass('range-compare');
                    } else if (
                        !startCompare &&
                        date_val > over &&
                        date_val < endCompare
        ...
        mouseover: function(e) {
            ...
            if (click === 1 && over) {
                if (compare) {
                    $('#datepicker .range-compare').removeClass(
                        'range-compare',
                    );
                    ...
                    if (startCompare && over < startCompare) {
                        ...
                        $('#date-start-compare')
                            ...
                            .focus()
                            .addClass('input-selected');
                        $('#datepicker .start-selected-compare')
                            .removeClass('start-selected-compare')
                            .addClass('end-selected-compare');
                        startCompare = null;
                        switched = true;
                        ...
                        $('#date-end-compare')
                            ...
                            .focus()
                            .addClass('input-selected');
                        $('#datepicker .end-selected-compare')
                            .removeClass('end-selected-compare')
                            .addClass('start-selected-compare');
                        endCompare = null;
                        switched = false;
        ...
        showMode: function(dir) {
            ...
            this.picker
                ...
                .hide()
                .filter(
                    '.daterangepicker-' + DPGlobal.modes[this.viewMode].clsName,
                )
                .show();
        },
    };
            ...
            if (!data) {
                $this.data(
                    ...
                    (data = new DateRangePicker(
                        this,
                        $.extend({}, $.fn.daterangepicker.defaults, options),
                    )),
                );
            }
