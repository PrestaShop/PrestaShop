...
$(function() {
    ...
    $('.toolbarBox a.btn-help').on('click', function(e) {
        e.preventDefault();
        if (
            !$('#main').hasClass('helpOpen') &&
            document.body.clientWidth > 1200
        ) {
            if (storage) storage.setItem('helpOpen', true);
            $('.toolbarBox a.btn-help i')
                .removeClass('process-icon-help')
                .addClass('process-icon-loading');
            initHelp();
        } else if (
            !$('#main').hasClass('helpOpen') &&
            document.body.clientWidth < 1200
            ...
            $('#help-container').html('');
            $('.toolbarBox a.btn-help i')
                .removeClass('process-icon-close')
                .addClass('process-icon-help');
            if (storage) storage.setItem('helpOpen', false);
        }
    });
    ...
    //toc
    var toc = [];
    var lang = [['en', '19726802'], ['fr', '20840479']];

    // change help icon
    function iconCloseHelp() {
        $('.toolbarBox a.btn-help i')
            .removeClass('process-icon-loading')
            .addClass('process-icon-close');
    }
    ...
    function initNavigation() {
        var d = new $.Deferred();
        var request = encodeURIComponent(
            'api/content/' + home + '/child?expand=page',
        );
        $.ajax({
            url: '//help.prestashop.com/api/?request=' + request,
            jsonp: 'callback',
            ...
            success: function(data) {
                for (var i = 0; i < data.page.results.length; i++) {
                    if (
                        isCleanHtml(
                            data.page.results[i].id +
                                data.page.results[i].title,
                        )
                    )
                        $('#help-container #main-nav').append(
                            '<a href="//help.prestashop.com/' +
                                data.page.results[i].id +
                                '?version=' +
                                _PS_VERSION_ +
                                '" data-target="' +
    ...
    function initToc() {
        var getLinksByLang = function(item) {
            ...
            var request = encodeURIComponent(
                'api/content/' +
                    item[1] +
                    '/child/page?expand=children.page&limit=100',
            );
            $.ajax({
                url: '//help.prestashop.com/api/?request=' + request,
                jsonp: 'callback',
        ...
        return $.when.apply(null, lang.map(getLinksByLang)).then(function() {
            ...
            $("#help-container a[href^='/pages/viewpage.action?pageId=']").on(
                'click',
                function(e) {
                    e.preventDefault();
                    var pageId = $(this)
    ...
    function initSearch() {
        ...
        $('#help-container #search-box').on('submit', function(e) {
            ...
            var searchUrl = encodeURIComponent(
                'searchv3/1.0/search?where=PS16&type=page&queryString=',
            );
            ...
            $.ajax({
                url:
                    '//help.prestashop.com/api/?request=' +
                    searchUrl +
                    searchTerm,
                jsonp: 'callback',
                dataType: 'jsonp',
                        ...
                        if (
                            isCleanHtml(
                                data.results[i].id +
                                    ...
                                    data.results[i].bodyTextHighlights,
                            )
                        ) {
                            $('#search-results')
                                .removeClass('hide')
                                .append(
                                    '<div class="result-item"><i class="fa fa-file-o"></i> <a href="//help.prestashop.com/' +
                                        data.results[i].id +
                                        '?version=' +
                                        _PS_VERSION_ +
                                        '" data-target="' +
    ...
    function initFeedback() {
        ...
        $('#help-container .helpful-labels li').on('click', function() {
            ...
            $('#help-container .helpful-labels li')
                .addClass('disabled')
                .off();
            $(this)
        ...
        $('#help-container .feedback-reason .radio label').on(
            'click',
            function() {
                var reasonMap = {
                    1: 'Not related',
                ...
                arr_feedback.reason =
                    reasonMap[$('input[name=lowrating-reason]:checked').val()];
            },
        );
