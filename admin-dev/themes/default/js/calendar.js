...
function setPreviousPeriod() {
    startDate = Date.parseDate(
        $('#date-start').val(),
        $('#date-start').data('date-format'),
    ).subDays(1);
    endDate = Date.parseDate(
        $('#date-end').val(),
        $('#date-end').data('date-format'),
    ).subDays(1);

    diff = endDate - startDate;
    startDateCompare = new Date(startDate - diff);

    $('#date-end-compare').val(
        startDate.format($('#date-end-compare').data('date-format')),
    );
    $('#date-start-compare').val(
        startDateCompare.format($('#date-start-compare').data('date-format')),
    );
}
...
function setPreviousYear() {
    ...
    $('#date-start-compare').val(
        startDate.format($('#date-start').data('date-format')),
    );
    $('#date-end-compare').val(
        endDate.format($('#date-start').data('date-format')),
    );
}

$(document).ready(function() {
    //Instanciate datepickers
    datepickerStart = $('.datepicker1')
        .daterangepicker({
        ...
        .on('changeDate', function(ev) {
            if (ev.date.valueOf() >= datepickerEnd.date.valueOf()) {
                datepickerEnd.setValue(
                    ev.date.setMonth(ev.date.getMonth() + 1),
                );
            }
        })
        .data('daterangepicker');

    datepickerEnd = $('.datepicker2')
        .daterangepicker({
            dates: translated_dates,
            weekStart: 1,
            start: $('#date-start').val(),
