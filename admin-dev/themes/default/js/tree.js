...
Tree.prototype = {
    ...
    init: function() {
        ...
        var name = this.$element
            ...
            .first()
            .attr('name');
        var idTree = this.$element
            .parent()
            .find('.cattree.tree')
            .first()
            .attr('id');
        this.$element
            .find('label.tree-toggler, .icon-folder-close, .icon-folder-open')
            .unbind('click');
        this.$element
            .find('label.tree-toggler, .icon-folder-close, .icon-folder-open')
            .click(function() {
                if (
                    ...
                    $(this)
                        .parent()
                        .children('.icon-folder-close')
                        .removeClass('icon-folder-close')
                        .addClass('icon-folder-open');

                    var load_tree =
                        typeof idTree != 'undefined' &&
                        $(this)
                            .parent()
                            .closest('.tree-folder')
            ...
            if ($('select#id_category_default').length) {
                ...
                this.$element.find(':input[type=checkbox]').click(function() {
                    if ($(this).prop('checked')) addDefaultCategory($(this));
                        ...
                        $(
                            'select#id_category_default option[value=' +
                                $(this).val() +
                                ']',
                        ).remove();
                        if (
                            $('select#id_category_default option').length == 0
                        ) {
    ...
    collapse: function(elem, $speed) {
        elem.find('label.tree-toggler').each(function() {
            $(this)
                .parent()
                .children('.icon-folder-open')
                .removeClass('icon-folder-open')
    ...
    collapseAll: function($speed) {
        this.$element.find('label.tree-toggler').each(function() {
            $(this)
                .parent()
                .children('.icon-folder-open')
                .removeClass('icon-folder-open')
    ...
    expandAll: function($speed) {
        ...
        if (
            typeof idTree != 'undefined' &&
            !$('#' + idTree).hasClass('full_loaded')
        ) {
            var selected = [];
            that = this;
            $('#' + idTree)
                .find('.tree-selected input')
                .each(function() {
            ...
            $.get(
                ...
                function(content) {
                    ...
                    organizeTree();
                    $('#' + idTree).tree('init');
                    that.$element.find('label.tree-toggler').each(function() {
                        $(this)
                            .parent()
                            .children('.icon-folder-close')
                            .removeClass('icon-folder-close')
