...
$(document).ready(function() {
    ...
    function mobileNav() {
        ...
        if ($('#employee_links').length > 0) {
            $employee = $('<a class="employee_avatar">').attr(
                'href',
                $('#employee_infos > a.employee_name').attr('href'),
            );
            $employee.append($('#employee_links .employee_avatar').clone());
            $employee.append(
                $('<span></span>').append(
                    $('#employee_links > .username').text(),
                ),
            );
        }
        ...
        $('.main-menu').prepend(
            `<li class="link-levelone">${$employee.prop('outerHTML')}</li>`,
        );
        $('.collapse').collapse({
            toggle: false,
        });
        if ($('#employee_links').length === 0) {
            $('.employee_avatar .material-icons, .employee_avatar span').wrap(
    ...
    $('#currencyCronjobLiveExchangeRate input').change(function() {
        ...
        $.ajax({
            ...
            cache: false,
            data:
                'ajax=1&action=cronjobLiveExchangeRate&tab=AdminCurrencies&enable=' +
                enable,
        });
    });
    ...
    $(document).on('click', '.edit_shipping_link', function(e) {
        ...
        $(
            '#shipping_carrier option[value=' +
                $(this).data('id-carrier') +
                ']',
        ).prop('selected', true);
