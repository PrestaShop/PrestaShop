        ...
        success: function(json) {
            if (json) {
                ...
                let notifications_total =
                    nbOrders + nbCustomers + nbCustomerMessages;
        ...
        $.each(results, function(property, value) {
            ...
            eltAppendTo.children('.notification-elements').append(
                tpl
                    ...
                    .replace(
                        /order_url/g,
                        `${baseAdminDir}index.php?tab=AdminOrders&token=${token_admin_orders}&vieworder&id_order=${
                            value.id_order
                        }`,
                    )
                    .replace(
                        /customer_url/g,
                        `${baseAdminDir}index.php?tab=AdminCustomers&token=${token_admin_customers}&viewcustomer&id_customer=${
                            value.id_customer
                        }`,
                    )
                    .replace(
                        /message_url/g,
                        `${baseAdminDir}index.php?tab=AdminCustomerThreads&token=${token_admin_customer_threads}&viewcustomer_thread&id_customer_thread=${
                            value.id_customer_thread
                        }`,
                    ),
            );
        });
