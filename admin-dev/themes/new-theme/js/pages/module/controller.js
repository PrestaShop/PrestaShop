...
class AdminModuleController {
    ...
    constructor(moduleCardController) {
        ...
        this.bulkActionCheckboxListSelector =
            '.module-checkbox-bulk-list input';
    ...
    initFilterStatusDropdown() {
        ...
        body.on('click', self.statusItemSelector, function() {
            ...
            $(self.statusSelectorLabelSelector).text(
                $(this)
                    .find('a:first')
                    .text(),
            );
    ...
    initBulkDropdown() {
        ...
        body.on(
            'click',
            self.bulkItemSelector,
            function initializeBodyChange() {
                if ($(self.getBulkCheckboxesCheckedSelector()).length === 0) {
                    $.growl.warning({
                        message:
                ...
                if (response.status === true) {
                    ...
                    if (stylesheet.insertRule) {
                        stylesheet.insertRule(
                            requiredSelectorCombination + stylesheetRule,
                    ...
                    $(self.placeholderGlobalSelector).fadeOut(800, () => {
                        $(self.placeholderFailureMsgSelector).text(
                            response.msg,
                        );
        ...
        $('.module-short-list').each(function setShortListVisibility() {
            ...
            if (
                (self.currentRefCategory &&
                    self.currentRefCategory !==
                        String(container.find('.modules-list').data('name'))) ||
                (self.currentRefStatus !== null &&
    ...
    updateModuleVisibility() {
        ...
        for (let i = 0; i < modulesListLength; i += 1) {
            ...
            if (currentModule.display === self.currentDisplay) {
                ...
                if (self.currentTagsList.length) {
                    ...
                    $.each(self.currentTagsList, (index, value) => {
                        ...
                        tagExists |=
                            ...
                            currentModule.description.indexOf(newValue) !==
                                -1 ||
                 ...
                 */
                if (
                    self.currentDisplay === self.DISPLAY_LIST &&
                    !self.currentTagsList.length
                ) {
                    if (
                        self.currentCategoryDisplay[moduleCategory] ===
                        undefined
                    ) {
                ...
                if (isVisible) {
                    if (
                        ...
                        $(self.recentlyUsedSelector).append(
                            currentModule.domObject,
                        );
                    } else {
        ...
        if (self.currentTagsList.length) {
            $('.modules-list').append(
                this.currentDisplay === self.DISPLAY_GRID
                    ? this.addonsCardGrid
                    : this.addonsCardList,
            );
        }
    ...
    initAddonsConnect() {
        ...
        if ($(self.addonsConnectModalBtnSelector).attr('href') === '#') {
            ...
            $(self.addonsConnectModalBtnSelector).attr(
                'data-target',
                self.addonsConnectModalSelector,
            );
        }

        if ($(self.addonsLogoutModalBtnSelector).attr('href') === '#') {
            $(self.addonsLogoutModalBtnSelector).attr('data-toggle', 'modal');
            $(self.addonsLogoutModalBtnSelector).attr(
                'data-target',
                self.addonsLogoutModalSelector,
            );
        }
        ...
        $('body').on(
            ...
            function initializeBodySubmit(event) {
                event.preventDefault();
                event.stopPropagation();
    ...
    initDropzone() {
        ...
        body.on(
            'click',
            `.dropzone:not(${this.moduleImportSelectFileManualSelector}, ${
                this.moduleImportSuccessConfigureBtnSelector
            })`,
            (event, manualSelect) => {
        ...
        body.on(
            'click',
            this.moduleImportSuccessConfigureBtnSelector,
            function initializeBodyClickOnModuleImport(event) {
                event.stopPropagation();
                event.preventDefault();
        ...
        self.animateEndUpload(() => {
            if (result.status === true) {
                if (result.is_configurable === true) {
                    const configureLink = window.moduleURLs.configurationPage.replace(
                        /:number:/,
                        result.module_name,
                    );
    ...
    displayPrestaTrustStep(result) {
        const self = this;
        const modal = self.moduleCardController._replacePrestaTrustPlaceholders(
            result,
        );
        ...
        $('body').on(
            ...
            () => {
                ...
                window.open(
                    `${
                        self.baseAddonsUrl
                    }search.php?search_query=${searchQuery}`,
                    '_blank',
                );
            },
        );
    ...
    initCategoriesGrid() {
        ...
        $('body').on(
            'click',
            this.categoryGridItemSelector,
            function initilaizeGridBodyClick(event) {
                event.stopPropagation();
                event.preventDefault();
                ...
                const menuCategoryToTrigger = $(
                    `${
                        self.categoryItemSelector
                    }[data-category-ref="${refCategory}"]`,
                );

                if (!menuCategoryToTrigger.length) {
                    console.warn(
                        `No category with ref (${refCategory}) seems to exist!`,
                    );
                    return false;
                }
    ...
    initCurrentDisplay() {
        this.currentDisplay =
            this.currentDisplay === '' ? this.DISPLAY_LIST : this.DISPLAY_GRID;
    }
    ...
    doBulkAction(requestedBulkAction) {
        ...
        if (typeof bulkActionToUrl[requestedBulkAction] === 'undefined') {
            $.growl.error({
                message: window.translate_javascripts[
    ...
    initCategorySelect() {
        ...
        body.on(
            ...
            function initializeCategorySelectClick() {
                ...
                self.currentRefCategory = self.currentRefCategory
                    ? String(self.currentRefCategory).toLowerCase()
                    : null;
                // Change dropdown label to set it to the current category's displayname
                $(self.categorySelectorLabelSelector).text(
                    $(this).data('category-display-name'),
                );
        ...
        body.on(
            ...
            function initializeCategoryResetButtonClick() {
                const rawText = $(self.categorySelector).attr(
                    'aria-labelledby',
                );
    ...
    initSearchBlock() {
        ...
        self.pstaggerInput = $('#module-search-bar').pstagger({
            ...
            inputPlaceholder:
                window.translate_javascripts['Search - placeholder'],
            closingCross: true,
            context: self,
        });
    ...
    initializeSeeMore() {
        ...
        $(`${self.moduleShortList} ${self.seeMoreSelector}`).on(
            'click',
            function seeMore() {
        ...
        $(`${self.moduleShortList} ${self.seeLessSelector}`).on(
            'click',
            function seeMore() {
        ...
        if ($shortLists.length > 0) {
            ...
            if (modulesCount === 0) {
                $('.module-addons-search-link').attr(
                    ...
                    `${
                        this.baseAddonsUrl
                    }search.php?search_query=${encodeURIComponent(
                        this.currentTagsList.join(' '),
                    )}`,
                );
            }
        }
