...
class SpecificPriceFormHandler {
    ...
    loadAndDisplayExistingSpecificPricesList() {
        ...
        $.ajax({
            ...
            var specificPricesList = this.renderSpecificPricesListingAsHtml(
                specificPrices,
            );
    ...
    renderSpecificPricesListingAsHtml(specificPrices) {
        ...
        $.each(specificPrices, (index, specificPrice) => {
            var deleteUrl = $('#js-specific-price-list')
    ...
    renderSpecificPriceRow(specificPrice, deleteUrl) {
        ...
        var row =
            '<tr>' +
            '<td>' +
            specificPrice.rule_name +
            '</td>' +
            '<td>' +
            specificPrice.attributes_name +
            '</td>' +
            '<td>' +
            specificPrice.currency +
            '</td>' +
            '<td>' +
            ...
            '</td>' +
            '<td>' +
            specificPrice.fixed_price +
            '</td>' +
            '<td>' +
            specificPrice.impact +
            '</td>' +
            '<td>' +
            specificPrice.period +
            '</td>' +
            '<td>' +
            specificPrice.from_quantity +
            '</td>' +
            '<td>' +
            (specificPrice.can_delete
                ? '<a href="' +
                  deleteUrl +
                  '" class="js-delete delete btn tooltip-link delete pl-0 pr-0"><i class="material-icons">delete</i></a>'
                : '') +
            '</td>' +
            '<td>' +
            (specificPrice.can_edit
                ? '<a href="#" data-specific-price-id="' +
                  specificPriceId +
                  '" class="js-edit edit btn tooltip-link delete pl-0 pr-0"><i class="material-icons">edit</i></a>'
                : '') +
            '</td>' +
            '</tr>';
    ...
    configureEditPriceModalBehavior() {
        $(document).on('click', '#js-specific-price-list .js-edit', event => {
            ...
            var specificPriceId = $(event.currentTarget).data(
                'specificPriceId',
            );
    ...
    submitCreatePriceForm() {
        ...
        $.ajax({
            ...
            .done(response => {
                showSuccessMessage(
                    translate_javascripts['Form update success'],
                );
                this.resetCreatePriceFormDefaultValues();
                $('#specific_price_form').collapse('hide');
                this.loadAndDisplayExistingSpecificPricesList();
     ...
     */
    submitEditPriceForm() {
        const baseUrl = $('#edit-specific-price-modal-form').attr(
            'data-action',
        );
        ...
        $('#edit-specific-price-modal-form .js-save').attr(
            'disabled',
            'disabled',
        );
        ...
        $.ajax({
            ...
            .done(response => {
                ...
                this.closeEditPriceModalAndRemoveForm();
                this.loadAndDisplayExistingSpecificPricesList();
                $('#edit-specific-price-modal-form .js-save').removeAttr(
                    'disabled',
                );
            })
            .fail(errors => {
    ...
    loadAndFillOptionsForSelectCombinationInput(usePrefixForCreate) {
        ...
        $.ajax({
            ...
            if (inputField.data('selectedAttribute') != '0') {
                inputField
                    .val(inputField.data('selectedAttribute'))
                    .trigger('change');
            }
        });
    }
    ...
    resetCreatePriceFormDefaultValues() {
        ...
        $('#specific_price_form')
            ...
            .each((index, value) => {
                $(value)
                    .val(previouslyStoredValues[$(value).attr('id')])
                    .change();
            });
