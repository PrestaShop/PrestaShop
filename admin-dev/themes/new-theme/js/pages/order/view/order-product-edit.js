...
export default class OrderProductEdit {
  ...
  setupListener() {
    this.quantityInput.on('change keyup', (event) => {
      ...
      const available = parseInt($(event.currentTarget).data('stock'), 10) - (this.quantity - this.quantityInput.data('previousQuantity'));
  ...
  displayProduct(product) {
    ...
    this.quantityInput.val(product.quantity).data('stock', product.availableQuantity).data('previousQuantity', product.quantity);
