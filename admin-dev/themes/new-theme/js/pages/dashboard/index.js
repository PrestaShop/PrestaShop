    ...
    $('#page-header-desc-configuration-switch_demo').tooltip().click(function(e) {
        $.ajax({
            ...
            data : {
                ...
                PS_DASHBOARD_SIMULATION: $(this).find('i').hasClass('toggle_on') ? 0 : 1
            },
            success : function(result) {
                let $icon =  $('#page-header-desc-configuration-switch_demo i');
                if ($icon.hasClass('toggle_on')) {
                    $icon.removeClass('toggle_on').addClass('toggle_off');
                } else {
                    $icon.removeClass('toggle_off').addClass('toggle_on');
                }
                refreshDashboard(false, false);
            }
        });
