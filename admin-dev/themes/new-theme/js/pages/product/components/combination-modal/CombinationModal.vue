...
<template>
  <div id="combination-edit-modal">
    ...
    >
      ...
      >
        <template #body>
          <p>{{ $t('modal.history.confirmBody', {'%combinationName%': selectedCombinationName}) }}</p>
        </template>
      </modal>
    </div>
  </div>
  ...
  export default {
    ...
    methods: {
      ...
      onFrameLoaded() {
        ...
        this.applyIframeStyling();
        this.selectedCombinationName = iframeBody.querySelector(ProductMap.combinations.combinationName).innerHTML;
