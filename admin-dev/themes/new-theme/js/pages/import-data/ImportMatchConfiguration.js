...
export default class ImportMatchConfiguration {
     ...
     */
    loadEvents() {
        $(document).on('click', '.js-save-import-match', event =>
            this.save(event),
        );
        $(document).on('click', '.js-load-import-match', event =>
            this.load(event),
        );
        $(document).on('click', '.js-delete-import-match', event =>
            this.delete(event),
        );
    }
        ...
        $.ajax({
            ...
            if (
                ...
                for (let key in response.matches) {
                    let $existingMatch = $dataMatchesDropdown.find(
                        `option[value=${
    ...
    delete(event) {
        ...
        $.ajax({
            ...
            $dataMatchesDropdown
                .find(`option[value=${selectedMatchId}]`)
                .remove();
        });
    }
