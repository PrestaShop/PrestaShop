...
<template>
    <tr :class="{ 'low-stock': lowStock }">
        ...
        <td> {{ reference }} </td> <td> {{ product.supplier_name }} </td>
    ...
    computed: {
        ...
        availableQtyUpdated() {
            return (
                ...
                Number(this.product.qty)
            );
        },
        physical() {
            const productAvailableQty = Number(
                this.product.product_available_quantity,
            );
            const productReservedQty = Number(
                this.product.product_reserved_quantity,
            );
        ...
        lowStockLevel() {
            return `<div class="text-sm-left">
                  ...
                  <p><strong>${this.trans('product_low_stock_level')} ${
                this.product.product_low_stock_threshold
            }</strong></p>
                </div>`;
        },
        lowStockAlert() {
            return `<div class="text-sm-left">
                  <p><strong>${this.trans('product_low_stock_alert')} ${
                this.product.product_low_stock_alert
            }</strong></p>
                </div>`;
        },
        id() {
            return `product-${this.product.product_id}${
