...
export default function() {
    ...
    function updateMissingTranslationsWarning(domainActions) {
        ...
        $(missingTranslations).each(function(index, element) {
            totalMissingTranslations =
                totalMissingTranslations +
                parseInt($(element).attr('data-missing-translations'), 10);
        });

        if (totalMissingTranslations > 0) {
            let missingTranslationsWarning = domainActions.find(
    ...
    $('.domain-first-part').each((index, domainToggler) => {
        ...
        allDomainsMissingTranslations =
            allDomainsMissingTranslations +
            updateMissingTranslationsWarning(domainActions);
        ...
        $(domainToggler).click(event => {
            ...
            $(domainTitle.nextAll().filter('.subdomains')[0]).toggleClass(
                'hide',
            );
    ...
    (function(totalTranslations, totalRemainingTranslations) {
        let totalTranslationsTemplate = $('.summary .total-translations').attr(
            'data-template',
        );
        let remainingTranslationsTemplate = $(
            '.summary .total-remaining-translations',
        ).attr('data-template');

        if (totalRemainingTranslations > 0) {
            let remainingTranslationsMessage = remainingTranslationsTemplate.replace(
                '%d',
                totalRemainingTranslations,
            );
            $('.total-remaining-translations').text(
                remainingTranslationsMessage,
            );
            $('.summary .separator').removeClass('hide');
        }

        if (totalTranslationsTemplate) {
            let totalTranslationsMessages = totalTranslationsTemplate.replace(
                '%d',
                totalTranslations,
            );
