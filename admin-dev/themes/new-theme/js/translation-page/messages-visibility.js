...
export default function(callback) {
    ...
    function hideCurrentNavigationBar(navigationContainer) {
        ...
        if (currentNavigationBar.length > 0) {
            ...
            let hiddenNavigationBarContainer = $(
    ...
    function updateDomainTitle(editTranslationForms) {
        let domainPart = editTranslationForms
            .parents('.translation-domain')
            .prev();
        let missingTranslationWarning = domainPart.find(
    ...
    function updateMissingTranslationsMessages(title) {
        let missingTranslationsMessage = title.find(
    ...
    function updateNavigationBar(translationDomain, editTranslationForms) {
        ...
        navigation
            .parent()
            .attr('data-navigation-parent-of', editTranslationForms.attr('id'));
    ...
    function updateEditTranslationForms(formsContainer, editTranslationForms) {
        ...
        $(formsContainer.find('.translation-forms form')).removeClass(
            hideClass,
        );
    }
    ...
    (() => {
        $('.show-' + buttonSuffix).each((buttonIndex, button) => {
            $(button).click(event => {
                ...
                let editTranslationForms = translationDomain.find(
                    '.translation-forms',
                );
                ...
                updateEditTranslationForms(
                    formsContainer,
                    editTranslationForms,
                );
        ...
        $('.domain-part .delegate-toggle-messages').each(
            (togglerIndex, toggler) => {
                $(toggler).click(event => {
                    ...
                    let showMessagesButton = translationDomain.find(
                        '.show-' + buttonSuffix,
                    );
