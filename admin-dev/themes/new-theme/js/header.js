...
export default class Header {
            ...
            if ((method === 'add' && name) || method === 'remove') {
                ...
                $.ajax({
                    ...
                    success: data => {
                        ...
                        if (
                            typeof data['has_errors'] !== 'undefined' &&
                            data['has_errors']
                        )
                            $.each(data, index => {
                            ...
                            $('#header_quick ul.dropdown-menu').html(
                                quicklink_list,
                            );
    ...
    initNotificationsToggle() {
        ...
        $('body').on('click', function(e) {
            if (
                ...
                $('div.notification-center.dropdown').has(e.target).length ===
                    0 &&
    ...
    updateEmployeeNotifications() {
        $.post(baseAdminDir + 'ajax.php', {
            updateElementEmployee: '1',
            updateElementEmployeeType: $(
                '.notification-center .nav-link.active',
            ).attr('data-type'),
        });
    }
    ...
    initContentDivOffset() {
        const onToolbarResize = function() {
            ...
            if (toolbar.length && header.length && content.length) {
                content.css(
                    'padding-top',
                    toolbar.outerHeight() + header.outerHeight() + spacing,
                );
            }
        };
