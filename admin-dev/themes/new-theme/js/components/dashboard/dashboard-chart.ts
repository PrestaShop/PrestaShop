...
/**
 * Dashboard chart contract (#41971): a `<canvas data-dashboard-chart id="X">` mounts from a
 ...
 *
 *   <canvas id="dashtrends-sales" data-dashboard-chart></canvas>
...
export default class DashboardChart {
  ...
  /**
   * Scans `root` for chart canvases and (re)mounts them. Exposed so a caller that injects new
   * dashboard HTML after the initial page load (e.g. an AJAX date-range reload) can re-run it:
   * `window.prestashop.instance.dashboardChart.mount(root)`.
   */
  mount(root: ParentNode = document): void {
    const {Chart} = window as unknown as {Chart: typeof ChartClass};

    root
      .querySelectorAll<HTMLCanvasElement>(ComponentsMap.dashboard.chartCanvas)
      .forEach((canvas) => this.mountChart(canvas, Chart));
  }

  private mountChart(canvas: HTMLCanvasElement, ChartCtor: typeof ChartClass): void {
    if (!canvas.id) {
      console.warn('[dashboard chart] a chart canvas is missing an id, skipping');
      return;
    }

    const config = this.readConfig(canvas, ChartCtor);
  ...
  private readConfig(canvas: HTMLCanvasElement, ChartCtor: typeof ChartClass): ChartConfiguration | null {
    ...
    if (!this.isChartConfiguration(payload, ChartCtor)) {
  ...
  private isChartConfiguration(value: unknown, ChartCtor: typeof ChartClass): value is ChartConfiguration {
