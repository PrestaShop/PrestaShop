...
class ContextualNotification {
  ...
  displayNotification(message: string, key: string): void {
    ...
    const contentMessageBox = document.getElementById(ComponentsMap.contextualNotification.messageBox);
...
export default function initContextualNotification(key: string): void {
  const multistoreHeader = document.querySelector(ComponentsMap.contextualNotification.headerMultiShop);
