...
export default function() {
    $(document).ready(function() {
        ...
        const idsProductAttribute = $jsCombinationsList
            .data('ids-product-attribute')
            .toString()
            .split(',');
        const refreshImagesUrl = $jsCombinationsList
            ...
            .replace(
                /form-images\/\d+/,
                'form-images/' + $jsCombinationsList.data('id-product'),
            );
        ...
        $(
            ...
            if (
                ...
                $('#product_combination_bulk_impact_on_price_te')
                    .val(priceCalculation.removeCurrentTax(price))
                    .change();
            } else {
                $('#product_combination_bulk_impact_on_price_ti')
                    .val(priceCalculation.addCurrentTax(price))
                    .change();
            }
        });
        ...
        const getCombinations = combinationsImages => {
            ...
            $.get($combinationsUrl).then(function(resp) {
                ...
                refreshImagesCombination(
                    combinationsImages,
                    idsProductAttribute.slice(
                        currentCount,
                        currentCount + step,
                    ),
                );
        ...
        const getCombinationsUrl = () => {
            const $numbers = idsProductAttribute
                .slice(currentCount, currentCount + step)
                .join('-');
            if ($numbers.length === 0) {
                return false;
            }
    ...
    const refreshImagesCombination = (
        combinationsImages,
        idsProductAttribute,
    ) => {
        $.each(idsProductAttribute, function(index, value) {
            ...
            $.each(combinationsImages[value], function(key, image) {
                html += `<div class="product-combination-image ${
                    image.id_image_attr ? 'img-highlight' : ''
                }">
          <input type="checkbox" name="combination_${$index}[id_image_attr][]" value="${
                    image.id
                }" ${image.id_image_attr ? 'checked="checked"' : ''}>
          <img src="${image.base_image_url}-small_default.${
                    image.format
                }" alt="" />
        </div>`;
            });
    ...
    const refreshDefaultImage = () => {
        const productCoverImageElem = $('#product-images-dropzone').find(
            '.iscover',
        );
        ...
        if (productCoverImageElem.length === 1) {
            ...
            if (imgElem.find('img').length) {
                ...
                productDefaultImageUrl = imgElem
        ...
        $.each($('#form .combination-form'), function(key, elem) {
            ...
            const defaultImageElem = $(elem).find(
                '.product-combination-image input:checked:first',
            );
            if (defaultImageElem.length === 1) {
                defaultImageUrl = defaultImageElem
                    .parent()
                    .find('img')
                    .attr('src');
            }
    ...
    const generate = () => {
        $.ajax({
            ...
            data: $(
                '#attributes-generator input.attribute-generator, #form_id_product',
            ).serialize(),
            beforeSend: function() {
                $('#create-combinations, #submit, .btn-submit').attr(
                    'disabled',
                    'disabled',
                );
            },
            success: function(response) {
                refreshTotalCombinations(
                    1,
                    $(response.form).filter('.combination.loaded').length,
                );
                $('#accordion_combinations').append(response.form);
                displayFieldsManager.refresh();
                const url = $('.js-combinations-list')
                    .attr('data-action-refresh-images')
                    .replace(
                        /form-images\/\d+/,
                        'form-images/' +
                            $('.js-combinations-list').data('id-product'),
                    );
                $.get(url).then(function(combinationsImages) {
                    refreshImagesCombination(
                        combinationsImages,
                        response.ids_product_attribute,
                    );
                });
            ...
            complete: function() {
                $('#create-combinations, #submit, .btn-submit').removeAttr(
                    'disabled',
                );
                activateCombinationsBulk();
