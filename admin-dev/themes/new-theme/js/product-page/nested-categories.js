...
export default function() {
    ...
    return {
        init: function() {
            ...
            nestedCategoriesForm.on(
                'change',
                'input.default-category',
                function updateDefaultCategory() {
                     ...
                     */
                    var category = nestedCategoriesForm.find(
                        'input[value="' + categoryId + '"].category',
                    );
                    var defaultcat = nestedCategoriesForm.find(
                        'input[value="' + categoryId + '"].default-category',
                    );

                    if (
                        false === category.is(':checked') &&
                        true === defaultcat.is(':checked')
                    ) {
        ...
        removeDefaultIfNeeded: function() {
            ...
            nestedCategoriesForm.on(
                'change',
                'input.category',
                function removeDefaultCategoryIfNotSelected() {
                    const findClosestCheckedCategory = function(category) {
                        const parent = category.closest('ul').parent();
                        const parentCategory = parent.find(
                            '> div > label > input',
                        );

                        if (!parent.is('li')) {
                            return nestedCategoriesForm
                    ...
                    if (
                        ...
                        const newCategory = findClosestCheckedCategory(
                            category,
                        );

                        defaultCategory.check(newCategory.val());
                        productCategoriesTags.checkDefaultCategory(
                            newCategory.val(),
                        );
                    }
                },
