    ...
    return {
        init: function init() {
            ...
            $('#toggle-all-combinations').on('change', event => {
                $(
                    '#accordion_combinations td:first-child input[type="checkbox"]',
                ).each(function() {
                    $(this).prop(
                        'checked',
            ...
            $(document).on('change', '.js-combination', () => {
                if (
                    $('.bulk-action').attr('aria-expanded') === 'false' ||
                    !$('.js-combination').is(':checked')
                ) {
                    $('.js-collapse').collapse('toggle');
                }
                $('span.js-bulk-combinations').text(
                    $('input.js-combination:checked').length,
                );
            });
        },
        getSelectedCombinations: function getSelectedCombinations() {
            var combinations = [];
            var selectedCombinations = Array.from(
                $(
            ...
            selectedCombinations.forEach(combination => {
                ...
                combinations.push(
                    new Combination(combinationId, combinationIndex),
                );
            });
        ...
        deleteCombinations: function deleteCombinations() {
            ...
            modalConfirmation
                .create(
                    ...
                    {
                        onContinue: function() {
                            var deletionURL = $(deleteCombinationsBtn).attr(
                                'data',
                            );
                            $.ajax({
                                type: 'DELETE',
        ...
        getFormValues: function getFormValues() {
            ...
            $(bulkForm)
                ...
                .each(function() {
                    if (
                        $(this).val() !== '' &&
                        $(this).attr('id') !== 'product_combination_bulk__token'
                    ) {
                        values.push({
                            id: $(this).attr('id'),
                            value: $(this).val(),
        ...
        syncToPricingTab: function syncToPricingTab() {
            ...
            $('tr.combination')
                ...
                .forEach(item => {
                    ...
                    // calculate new price
                    var newFinalPrice =
                        new Number(newPrice) + new Number(impactOnPrice);
...
class Combination {
    ...
    convertInput(bulkInput) {
        ...
        switch (bulkInput) {
            ...
            case 'cost_price':
                convertedInput =
                    this.inputPattern + 'attribute_wholesale_price';
                break;
    ...
    syncValues(values) {
        values.forEach(valueObject => {
            ...
            if (syncedProperties.indexOf(valueId) !== -1) {
                ...
                var input = $(
                    `#attribute_${this.domId} .attribute-${valueId} input`,
                );
                input.val(value);
                input.change();
            }
