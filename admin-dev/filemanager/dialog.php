...
if (isset($_POST['submit'])) {
    ...
    if (isset($_GET['fldr'])
        && !empty($_GET['fldr'])
        && 0 === preg_match('/\.{1,2}[\/|\\\]?/', urldecode($_GET['fldr']))
    ) {
    ...
    if ('' == $subdir) {
        if (!empty($_COOKIE['last_position'])
            && false === mb_strpos($_COOKIE['last_position'], '.')
        ) {
    ...
    $subfolder = '';
    if (!empty($_SESSION['subfolder']) && false === mb_strpos($_SESSION['subfolder'], '../')
        && false === mb_strpos($_SESSION['subfolder'], './') && 0 !== mb_strpos($_SESSION['subfolder'], '/')
        && false === mb_strpos($_SESSION['subfolder'], '.')
    ) {
    ...
    if (isset($_GET['descending'])) {
        $descending = $_SESSION['descending'] = 'true' === fix_filename($_GET['descending'], $transliteration);
    } else {
    ...
    if (1 == $_GET['type']) {
        ...
        $apply = 'apply_link';
    } elseif (0 == $_GET['type'] && '' == $_GET['field_id']) {
        $apply = 'apply_none';
    } elseif (3 == $_GET['type']) {
        $apply = 'apply_video';
    } else {
	...
	<div class="navbar navbar-fixed-top">
        ...
        echo ' btn-inverse';
									...
									<i class="icon-fire <?php if (2 == $view) {
        echo 'icon-white';
    } ?>"></i></button>
							</div>
        ...
        ?><label id="filter" class="btn"><i class="icon-play"></i></label><?php
								...
								<label id="ff-item-type-all" title="<?php echo Tools::safeOutput(lang_All); ?>" <?php if (1 == Tools::getValue('type') || 3 == Tools::getValue('type')) {
    ...
    $jplayer_ext = array(
        ...
        foreach ($files as $file_array) {
            ...
            if ('.' == $file || (isset($file_array['extension']) && lang_Type_dir != $file_array['extension']) || ('..' == $file && '' == $subdir) || in_array($file, $hidden_folders, true) || ('' != $filter && '..' != $file && false === mb_strpos($file, $filter))) {
                continue;
            }
		...
		<li data-name="<?php echo Tools::safeOutput($file); ?>" <?php if ('..' == $file) {
			...
			<figure data-name="<?php echo Tools::safeOutput($file); ?>" class="<?php if ('..' == $file) {
                ...
                ?>
                    ...
                    echo 'rename-folder';
                } ?>" title="<?php echo lang_Rename; ?>" data-path="<?php echo Tools::safeOutput($subfolder.$subdir.$file); ?>" data-thumb="<?php echo Tools::safeOutput($subdir.$file); ?>">
							<i class="icon-pencil <?php if (!$rename_folders) {
                    echo 'icon-white';
                } ?>"></i></a>
						<a href="javascript:void('')" class="tip-left erase-button <?php if ($delete_folders) {
                    echo 'delete-folder';
                } ?>" title="<?php echo lang_Erase; ?>" data-confirm="<?php echo lang_Confirm_Folder_del; ?>" data-path="<?php echo Tools::safeOutput($subfolder.$subdir.$file); ?>" data-thumb="<?php echo Tools::safeOutput($subdir.$file); ?>">
	...
	<?php
        ...
        foreach ($files as $nu => $file_array) {
            ...
            if ('.' == $file || '..' == $file || is_dir($current_path.$subfolder.$subdir.$file) || in_array($file, $hidden_files, true) || !in_array(fix_strtolower($file_array['extension']), $ext, true) || ('' != $filter && false === mb_strpos($file, $filter))) {
                continue;
            }
            ...
            if ((!(1 == Tools::getValue('type') && !$is_img) && !((3 == Tools::getValue('type') && !$is_video) && (3 == Tools::getValue('type') && !$is_audio))) && $class_ext > 0) {
                ?>
		...
		<figure data-name="<?php echo Tools::safeOutput($file); ?>" data-type="<?php if ($is_img) {
			...
			<figcaption>
				<form action="force_download.php" method="post" class="download-form" id="form<?php echo Tools::safeOutput($nu); ?>">
                    ...
                    echo 'delete-file';
                } ?>" title="<?php echo lang_Erase; ?>" data-confirm="<?php echo lang_Confirm_del; ?>" data-path="<?php echo Tools::safeOutput($subfolder.$subdir.$file); ?>" data-thumb="<?php echo Tools::safeOutput($subdir.$file); ?>">
