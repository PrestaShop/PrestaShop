...
$(document).ready(function() {
                    ...
                    callback: function(key, options) {
                        switch (key) {
                            case 'copy_url':
                                ...
                                bootbox.alert(
                                    'URL:<br/><br/><input type="text" style="height:30px; width:100%;" value="' +
                                        m +
                                        '" />',
                                );
                                break;
                            case 'unzip':
                                var m =
                                    $('#sub_folder').val() +
                                    $('#fldr_value').val() +
                                    $trigger.find('a.link').attr('data-file');
                                $.ajax({
                                    type: 'POST',
                            ...
                            case 'edit_img':
                                ...
                                $('#aviary_img')
                                    .attr('src', full_path)
                                    .load(
                                        launchEditor('aviary_img', full_path),
                                    );

                                break;
                            case 'duplicate':
                                var old_name = $trigger
                                    .find('h4')
                                    .text()
                                    .trim();
                                bootbox.prompt(
                                    ...
                                    function(name) {
                                        if (name !== null) {
                                            ...
                                            if (name != old_name) {
                                                var _this = $trigger.find(
                                                    '.rename-file',
                                                );
                                                execute_action(
                                                    'duplicate_file',
                                                    _this.attr('data-path'),
                                                    _this.attr('data-thumb'),
                                                    name,
                                                    _this,
    ...
    if (
        parseInt($('#file_number').val()) >
        parseInt($('#file_number_limit_js').val())
    )
        var js_script = false;
    ...
    $('#info').on('click', function() {
        bootbox.alert(
            '<center><img src="img/logo.png" alt="responsive filemanager"/><br/><br/><p><strong>RESPONSIVE filemanager v.' +
                version +
    ...
    $('.close-uploader').on('click', function() {
        $('.uploader').hide(500);
        window.location.href =
            $('#refresh').attr('href') + '&' + new Date().getTime();
    });
        ...
        bootbox.prompt(
            ...
            function(name) {
                if (name !== null) {
                    name = fix_filename(name).replace('.', '');
                    var folder_path =
                        $('#sub_folder').val() + $('#fldr_value').val() + name;
    ...
    $('.view-controller button').on('click', function() {
        ...
        if (typeof $('ul.grid')[0] !== 'undefined' && $('ul.grid')[0])
            $('ul.grid')[0].className = $('ul.grid')[0].className.replace(
                /\blist-view.*?\b/g,
                '',
            );
        if (
            typeof $('.sorter-container')[0] !== 'undefined' &&
            $('.sorter-container')[0]
        )
            $('.sorter-container')[0].className = $(
                '.sorter-container',
        ...
        $('figure').bind('mouseover', function() {
            if ($('#view').val() == 0) {
                $(this)
                    ...
                    .animate({ top: '-30px' }, { queue: false, duration: 300 });
            }
        });
    ...
    $('ul.grid').on('click', '.link', function() {
        ...
        window[_this.attr('data-function')](
            _this.attr('data-file'),
            _this.attr('data-field_id'),
        );
    });
});
...
function apply_link(file, external) {
    ...
    if (external != '') {
        ...
        close_window();
    } else apply_any(base_url + path, file);
}
...
function apply_file_duplicate(container, name) {
    ...
    li_container.after(
        "<li class='" +
            li_container.attr('class') +
            "' data-name='" +
            li_container.attr('data-name') +
            "'>" +
            li_container.html() +
            '</li>',
    );
...
function apply_file_rename(container, name) {
    ...
    if (typeof file !== 'undefined' && file) {
        link2.attr(
            'data-url',
            file.replace(
                encodeURIComponent(old_name),
                encodeURIComponent(name + '.' + extension),
            ),
        );
    }
...
function apply_folder_rename(container, name) {
    ...
    var new_url = url.replace(
        'fldr=' + fldr + encodeURIComponent(old_name),
        'fldr=' + fldr + encodeURIComponent(name),
    );
    link.each(function() {
...
function sortUnorderedList(ul, sortDescending, sort_field) {
    ...
    $.each(lis_file, function(index) {
        ...
        if ($.isNumeric(value)) {
            value = parseFloat(value);
            while (
                typeof vals_file[value] !== 'undefined' &&
                vals_file[value]
            ) {
                value = parseFloat(parseFloat(value) + parseFloat(0.001));
            }
