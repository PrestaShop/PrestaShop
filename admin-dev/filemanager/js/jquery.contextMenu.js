...
*/
        ...
        defaults = {
            ...
            determinePosition: function($menu) {
                ...
                if ($.ui && $.ui.position) {
                    ...
                    offset.left +=
                        this.outerWidth() / 2 - $menu.outerWidth() / 2;
                    $menu.css(offset);
                }
            },
            // position menu
                ...
                switch (e.keyCode) {
                    ...
                    case 13: // enter
                        ...
                        if (opt.isInput) {
                            if (
                                opt.$selected &&
                                !opt.$selected.is('textarea, select')
                            ) {
                                e.preventDefault();
                                return;
                            }
                    ...
                    if (typeof item == 'string') {
                        ...
                        switch (item.type) {
                            ...
                            case 'sub':
                                ...
                                op.create(item, root);
                                $t.data('contextMenu', item).addClass(
            ...
            case 'destroy':
                ...
                if (_hasContext) {
                    ...
                    $.each(menus, function(ns, o) {
                        ...
                        $visibleMenu = $('.context-menu-list').filter(
                            ':visible',
                        );
                        if (
                            $visibleMenu.length &&
                    ...
                    if (
                        ...
                        $visibleMenu
                            .data()
                            .contextMenuRoot.$trigger.is(o.selector)
                    ) {
                        $visibleMenu.trigger('contextmenu:hide', {
                            force: true,
                        });
    ...
    $.contextMenu.setInputValues = function(opt, data) {
        ...
        $.each(opt.inputs, function(key, item) {
            switch (item.type) {
                ...
                case 'radio':
                    item.selected =
                        (data[item.radio] || '') == item.value ? true : false;
                    break;
    ...
    function inputLabel(node) {
        return (
            (node.id && $('label[for="' + node.id + '"]').val()) || node.name
        );
    }
