...
if (0 !== preg_match('/\.{1,2}[\/|\\\]/', $_POST['path_thumb'])
    ...
    || ($realPath && 0 !== strpos($realPath, realpath($current_path)))
    || ($realPathThumb && 0 !== strpos($realPathThumb, realpath($thumbs_base_path)))
) {
    die('wrong path');
}
...
$info = pathinfo($path);
if (isset($info['extension']) && !(isset($_GET['action']) && 'delete_folder' == $_GET['action']) && !in_array(strtolower($info['extension']), $ext)) {
    die('wrong extension');
}
