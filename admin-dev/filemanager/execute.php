...
if (0 !== preg_match('/\.{1,2}[\/|\\\]/', $_POST['path_thumb'])
    || 0 !== preg_match('/\.{1,2}[\/|\\\]/', $_POST['path'])
    || ($realPath && 0 !== mb_strpos($realPath, realpath($current_path)))
    || ($realPathThumb && 0 !== mb_strpos($realPathThumb, realpath($thumbs_base_path)))
) {
    die('wrong path');
}
...
if (isset($_POST['name'])) {
    $name = $_POST['name'];
    if (0 !== preg_match('/\.{1,2}[\/|\\\]/', $name)) {
        die('wrong name');
    }
}

$info = pathinfo($path);
if (isset($info['extension']) && !(isset($_GET['action']) && 'delete_folder' == $_GET['action']) && !in_array(mb_strtolower($info['extension']), $ext, true)) {
    die('wrong extension');
}
