...
(function(Popcorn) {
    var JQUERY_SCRIPT =
        ...
        JPLAYER_SCRIPT =
            'http://www.jplayer.org/2.4.0/js/jquery.jplayer.min.js', // Used if jPlayer not already present.
        ...
        format = {
            // Duplicate of jPlayer 2.4.0 object, to avoid always requiring jQuery and jPlayer to be loaded before performing the _canPlayType() test.
            mp3: {
        ...
        getSupplied = function(url) {
            ...
            if (DEBUG)
                console.log(
                    'getSupplied(): Generated: supplied = "' + supplied + '"',
                );
            return supplied;
        };
        ...
        _canPlayType: function(containerType, url) {
            ...
            if (cType !== 'video' && cType !== 'audio') {
                ...
                if (isObject(srcObj) && isObject(srcObj.media)) {
                    ...
                    var solution = srcObj.options.solution
                            .toLowerCase()
                            .split(','), // Create the solution array, with prority based on the order of the solution string.
                        supplied = srcObj.options.supplied
                            .toLowerCase()
                            .split(','); // Create the supplied formats array, with prority based on the order of the supplied formats string.

                    for (var sol = 0; sol < solution.length; sol++) {
                        var solutionType = solution[sol].replace(
                                /^\s+|\s+$/g,
                                '',
                            ), //trim
                        ...
                        for (var fmt = 0; fmt < supplied.length; fmt++) {
                            ...
                            if (format[mediaType]) {
                                ...
                                if (!mediaElem && checkingHtml) {
                                    mediaElem = document.createElement(
                                        format[mediaType].media,
                                    );
                                }
                                ...
                                var htmlCanPlay = !!(
                                        ...
                                        mediaElem.canPlayType(
                                            format[mediaType].codec,
                                        )
                                    ),
                                    htmlWillPlay = htmlCanPlay && checkingHtml,
                                    flashWillPlay =
                                        format[mediaType].flashCanPlay &&
                                        checkingFlash;
            ...
            var media = this,
                ...
                dispatchDurationChange = function() {
                    if (ready) {
                        if (DEBUG)
                            console.log(
                                'Dispatched event : durationchange : ' +
                                    duration,
                            );
                ...
                jPlayerInit = function() {
                    (function($) {
                        ...
                        if (typeof media.src === 'string') {
                            ...
                            jpMedia = isObject(media.src.media)
                                ? media.src.media
                                : {};
                            jpOptions = isObject(media.src.options)
                                ? media.src.options
                                : {};
                            jpOptions.solution = jpOptions.solution || SOLUTION;
                            jpOptions.supplied =
                                jpOptions.supplied ||
                                getSupplied(media.src.media);
                        }
                        ...
                        jpOptions.swfPath =
                            jpOptions.swfPath || JPLAYER_SWFPATH;
                        ...
                        var bindEvent = function(name) {
                            myPlayer.bind($.jPlayer.event[name], function(
                                event,
                            ) {
                                if (DEBUG)
                                    console.log(
                            ...
                            if (DEBUG)
                                console.log(
                                    'Created event handler for: ' + name,
                                );
                        };
                        ...
                        for (var eventName in $.jPlayer.event) {
                            if ($.jPlayer.event.hasOwnProperty(eventName)) {
                                ...
                                if (nativeEvent) {
                                    ...
                                    if (DEBUG)
                                        console.log(
                                            'Skipped auto event handler creation for: ' +
                                                eventName,
                                        );
                                }
                            }
                        ...
                        myPlayer.bind($.jPlayer.event.loadeddata, function(
                            event,
                        ) {
                            if (DEBUG)
                                console.log(
                        ...
                        if (DEBUG)
                            console.log(
                                'Created CUSTOM event handler for: loadeddata',
                            );

                        myPlayer.bind($.jPlayer.event.durationchange, function(
                            event,
                        ) {
                            duration = event.jPlayer.status.duration;
                            dispatchDurationChange();
                        ...
                        if (DEBUG)
                            console.log(
                                'Created CUSTOM event handler for: durationchange',
                            );
                        ...
                        if (DEBUG)
                            console.log(
                                'Created CUSTOM event handler for: error',
                            );
