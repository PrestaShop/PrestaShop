...
<?php foreach ($this->tests_render as $type => $categories) { ?>
    ...
    <?php foreach ($categories as $category) { ?>
        <li class="title <?php if (1 == $category['success']) { ?>ok<?php } ?>"><?php echo $category['title']; ?></li>
        <?php $i = 0; foreach ($category['checks'] as $id => $lang) { ?>
            <li class="required <?php if (0 == $i) { ?>first<?php } ?> <?php echo isset($this->tests[$type]['checks'][$id]) ? $this->tests[$type]['checks'][$id] : 'fail'; ?>">
                <?php echo $lang; ?>
            </li>
        <?php ++$i; } ?>
    <?php } ?>
