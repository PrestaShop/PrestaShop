...
class ProductGridDefinitionFactory extends AbstractGridDefinitionFactory
{
    ...
    protected function getBulkActions()
    {
        ...
        return (new BulkActionCollection())
            ...
            // WHY the values are strings: the JavaScript router treats a numeric 0 in the last path
            // segment as empty and moves it to the query string, where no route matches it.
            ->add($this->buildAjaxBulkAction(
                ...
                $routeParams + ['outOfStockType' => (string) OutOfStockType::OUT_OF_STOCK_NOT_AVAILABLE]
            ))
            ->add($this->buildAjaxBulkAction(
                ...
                $this->trans('Updating %done% / %total% products', [], 'Admin.Actions'),
                'add_shopping_cart',
                $routeParams + ['outOfStockType' => (string) OutOfStockType::OUT_OF_STOCK_AVAILABLE]
            ))
            ->add($this->buildAjaxBulkAction(
                ...
                $routeParams + ['outOfStockType' => (string) OutOfStockType::OUT_OF_STOCK_DEFAULT]
            ))
        ;
    }
