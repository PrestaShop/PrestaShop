...
/**
 * Defines actions that can be performed on product quantity in cart
 */
final class QuantityAction
{
     ...
     * @var string
     */
    private $quantityAction;

    /**
     * @param string $quantityAction
     */
    public function __construct($quantityAction)
    {
        ...
        if (!in_array($quantityAction, $availableQuantityActions)) {
            throw new CartException(
                sprintf(
                    'Undefined quantity action %s supplied. Available quantity actions are: %s',
                    var_export($quantityAction, true),
                    implode(', ', $availableQuantityActions)
                )
            );
        }
