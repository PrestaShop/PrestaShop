...
class MultistoreConfigurationTypeExtension extends AbstractTypeExtension
{
    ...
    public function buildForm(FormBuilderInterface $builder, array $options): void
    {
        ...
        $builder->addEventListener(FormEvents::PRE_SUBMIT, static function (FormEvent $event) use ($checkboxEnabler) {
            $form = $event->getForm();
            $checkboxEnabler->addMultistoreElements($form, $event->getData());
        });
    }
    ...
    public function configureOptions(OptionsResolver $resolver): void
    {
        ...
        $resolver->setNormalizer('attr', function (Options $options, array $value): array {
            $value['class'] = trim('js-multishop-form-row ' . ($value['class'] ?? ''));
