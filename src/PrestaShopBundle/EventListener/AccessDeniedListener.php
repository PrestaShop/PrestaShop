...
class AccessDeniedListener
{
    ...
    public function onKernelException(GetResponseForExceptionEvent $event)
    {
        ...
        $betterSecurity = $security[0];
        $this->throwNotificationMessage($betterSecurity);
        $url = $this->computeUrl($betterSecurity);
     ...
     * @param BetterSecurity $betterSecurity
     *
     * @return string
     */
    private function computeUrl(BetterSecurity $betterSecurity)
    {
        $route = $betterSecurity->getRoute();
        if ($route !== null) {
            return $this->router->generate($route);
        }

        return $betterSecurity->getUrl();
    }
