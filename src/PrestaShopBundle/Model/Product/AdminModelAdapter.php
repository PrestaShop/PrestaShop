...
class AdminModelAdapter extends \PrestaShopBundle\Model\AdminModelAdapter
{
    ...
    public function getModelData($form_data, $isMultiShopContext = false)
    {
        ...
        $form_data = array_merge(
            array('id_product' => $form_data['id_product']),
            $form_data['step1'],
            $form_data['step2'],
            $form_data['step3'],
            $form_data['step4'],
        ...
        $form_data['submitted_tabs'] = array('Shipping');
        ...
        if (ProductInterface::REDIRECT_TYPE_NOT_FOUND != $form_data['redirect_type']) {
        ...
        $new_form_data['specificPricePriority'] = array(
    ...
    private function mapStep1FromData(Product $product)
    {
        return array(
                ...
                'active' => 0 == $product->active ? false : true,
                'price_shortcut' => $product->price,
                'qty_0_shortcut' => $product->getQuantity($product->id),
                'categories' => array('tree' => $product->getCategories()),
    ...
    private function mapStep2FormData(Product $product)
    {
        return array(
                ...
                'unit_price' => 0 != $product->unit_price_ratio
                    ? $product->price / $product->unit_price_ratio
                    : 0,
                'unity' => $product->unity,
                'specific_price' => array( // extra form to be saved separately. Here this is the default form values.
                    'sp_from_quantity' => 1,
                    'sp_reduction' => 0,
                    'sp_reduction_tax' => 1,
                    'leave_bprice' => true,
    ...
    private function mapStep5FormData(Product $product)
    {
        return array(
                ...
                'id_type_redirected' => array(
                    'data' => array($product->id_type_redirected),
                ),
        );
    }
    ...
    private function getVirtualProductData(Product $product)
    {
        ...
        if ($id_product_download) {
            ...
            $dateValue = '0000-00-00 00:00:00' == $download->date_expiration
                ? ''
    ...
    private function getCustomFields(Product $product)
    {
        ...
        foreach ($customizationFields as $customizationField) {
            $baseObject = array(
                ...
                'require' => 1 == $customizationField[$this->locales[0]['id_lang']]['required'] ? true : false,
            );
    ...
    private function getDataSuppliersCombinations(Product $product)
    {
        ...
        $dataSuppliersCombinations = array();
        ...
        foreach ($this->supplierAdapter->getProductSuppliers($product->id) as $supplier) {
            foreach ($combinations as $combination) {
                ...
                $dataSuppliersCombinations['supplier_combination_'.$supplier->id_supplier][] = array(
    ...
    private function getDataWarehousesCombinations(Product $product)
    {
        ...
        $dataWarehousesCombinations = array();
        ...
        foreach ($this->warehouseAdapter->getWarehouses() as $warehouse) {
            ...
            foreach ($combinations as $combination) {
                ...
                $dataWarehousesCombinations['warehouse_combination_'.$warehouseId][] = array(
                    'label' => $combination['attribute_designation'],
