...
class DatabaseCatalogueProvider
{
    /**
    ...
    public function getCatalogue(string $locale, ?string $themeName = null): MessageCatalogue
    {
        $databaseCatalogue = new MessageCatalogue($locale);
        foreach ($this->translationDomains as $translationDomain) {
            $domainCatalogue = $this->databaseTranslationLoader->load(null, $locale, $translationDomain, $themeName);
