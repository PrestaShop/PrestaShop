...
class XmlLoader
{
            ...
            foreach ($entities as $key => $entity) {
                if (isset($dependencies[$entity])) {
                    ...
                    foreach ($dependencies[$entity] as $item) {
                        if (false !== ($key = array_search($item, $sort_entities, true))) {
                            $min = min($min, $key);
                        }
                    }
                    if (0 == $min) {
    ...
    public function populateEntity($entity)
    {
        ...
        if ('.' == mb_substr($entity, 0, 1) || '_' == mb_substr($entity, 0, 1)) {
            return;
        }
    ...
    {
        if ('en' === $this->getFallBackToDefaultLanguage($iso)) {
            return 'en';
        }
    ...
    public function createEntity($entity, $identifier, $classname, array $data, array $data_lang = array())
    {
        ...
        if ($classname) {
            ...
            if (!$xml->fields['primary']) {
                $primary = 'id_' . $entity;
            } elseif (false === mb_strpos((string) $xml->fields['primary'], ',')) {
            ...
            if ($data_lang) {
                ...
                if (false !== $entity_asso && 'shop' == $entity_asso['type']) {
