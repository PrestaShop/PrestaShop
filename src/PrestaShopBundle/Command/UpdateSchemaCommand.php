                ...
                if (preg_match_all('/([^\s,]*?) CHANGE (.+?) (.+?)(,|$)/', $sql, $matches)) {
                    foreach ($matches[2] as $matchKey => $fieldName) {
                        ...
                        if (null !== $oldDefaultValue
                            && false === mb_strpos($oldDefaultValue, 'CURRENT_TIMESTAMP')) {
                        ...
                        if (!('NO' == $results[0]['Null'] && null === $results[0]['Default'])
                            && !('NULL' === $oldDefaultValue
                                && false !== mb_strpos($matches[0][$matchKey], 'NOT NULL'))
                            && (false === mb_strpos($matches[0][$matchKey], 'BLOB'))
                            && (false === mb_strpos($matches[0][$matchKey], 'TEXT'))
                        ) {
