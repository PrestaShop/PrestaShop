...
/**
 * Computes the refreshed value for the "Installed Modules" KPI.
 */
class InstalledModulesKpiRefreshProvider implements KpiRefreshProviderInterface
{
    ...
    public function getValue(array $requestParameters = []): KpiRefreshValue
    {
        ...
        $value = $this->statsRepository->countInstalledModules($shopIds);

        $this->configuration->set('INSTALLED_MODULES', $value);
        $this->configuration->set('INSTALLED_MODULES_EXPIRE', strtotime('+2 min'));
