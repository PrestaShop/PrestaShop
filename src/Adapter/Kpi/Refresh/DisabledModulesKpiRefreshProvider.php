...
/**
 * Computes the refreshed value for the "Disabled Modules" KPI.
 */
class DisabledModulesKpiRefreshProvider implements KpiRefreshProviderInterface
{
    ...
    public function getValue(array $requestParameters = []): KpiRefreshValue
    {
        ...
        $value = $this->statsRepository->countDisabledModules($shopIds);

        $this->configuration->set('DISABLED_MODULES', $value);
        $this->configuration->set('DISABLED_MODULES_EXPIRE', strtotime('+2 min'));
