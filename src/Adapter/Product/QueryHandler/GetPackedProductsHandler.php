...
class GetPackedProductsHandler implements GetPackedProductsHandlerInterface
{
    ...
    {
        $results = $this->productRepository->getPackedProducts($query->getProductId(), $query->getLanguageId());
        ...
        foreach ($results as $result) {
            ...
            $relatedProducts[] = new PackedProductDetails(
                $productId,
                (int) $result['quantity'],
                (int) $result['combinationId'],
                $result['name'],
                $result['reference'],
                $imagePath
            );
