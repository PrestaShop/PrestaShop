...
class BulkToggleStateStatusHandler implements BulkToggleStateStatusHandlerInterface
{
    ...
    public function handle(BulkToggleStateStatusCommand $command): void
    {
        foreach ($command->getStateIds() as $stateId) {
            ...
            if (0 >= $state->id) {
                throw new StateNotFoundException(sprintf('State object with id "%d" has been not found for status changing', $stateId->getValue()));
            }
            ...
            try {
                ...
                throw new StateException(sprintf('An error occurred while updating state status with id "%d"', $stateId->getValue()));
            }
        }
    }
}
