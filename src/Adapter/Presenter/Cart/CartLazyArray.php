...
class CartLazyArray extends AbstractLazyArray
{
    ...
    public function getSubtotals(): array
    {
        ...
        $total_discount = $this->cart->gift ?
            $this->cart->getDiscountSubtotalWithoutGifts($this->cartPresenter->includeTaxes())
            : $this->cart->getOrderTotal($this->cartPresenter->includeTaxes(), Cart::ONLY_DISCOUNTS);
        $subtotals['products'] = [
            'type' => 'products',
