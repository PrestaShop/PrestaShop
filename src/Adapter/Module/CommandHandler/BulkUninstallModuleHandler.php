...
use PrestaShop\PrestaShop\Core\Domain\Module\Exception\CannotUninstalledModuleException;
...
class BulkUninstallModuleHandler implements BulkUninstallModuleHandlerInterface
{
    ...
    public function handle(BulkUninstallModuleCommand $command): void
    {
        foreach ($command->getModules() as $moduleName) {
            ...
            if (!$result) {
                throw new CannotUninstalledModuleException('Technical error occurred while uninstalling module.');
            }
        }
    }
}
