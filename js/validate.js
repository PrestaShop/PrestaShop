...
var unicode_hack = (function() {
    ...
    */
    ...
    for (var p in unicodeCategories) {
        if (firstLetters[p[0]])
            firstLetters[p[0]] =
                unicodeCategories[p].substring(
                    0,
                    unicodeCategories[p].length - 1,
    ...
    return function(regexpString, classes) {
        ...
        regexpString = regexpString.replace(/\\p\{(..?)\}/g, function(
            match,
            group,
        ) {
            var unicode_categorie = unicodeCategories[group];
            if (!classes)
                unicode_category = unicode_categorie.replace(
                    /\[(.*?)\]/g,
                    '$1',
                );
...
function validate_field(that) {
    if ($(that).hasClass('is_required') || $(that).val().length) {
        if ($(that).attr('data-validate') == 'isPostCode') {
            ...
            if (
                typeof countriesNeedZipCode[id_country] != 'undefined' &&
                typeof countries[id_country] != 'undefined'
            )
                var result = window[
                    'validate_' + $(that).attr('data-validate')
                ](
                    $(that).val(),
                    countriesNeedZipCode[id_country],
                    countries[id_country]['iso_code'],
                );
        } else if ($(that).attr('data-validate'))
            var result = window['validate_' + $(that).attr('data-validate')](
                $(that).val(),
            );
