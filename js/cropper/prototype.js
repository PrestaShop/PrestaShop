...
var Enumerable = {
    ...
    partition: function(iterator) {
        var trues = [],
            falses = [];
        this.each(function(value, index) {
            ((iterator || Prototype.K)(value, index) ? trues : falses).push(
                value,
            );
        });
        return [trues, falses];
...
Ajax.Request.prototype = Object.extend(new Ajax.Base(), {
    ...
    request: function(url) {
        ...
        try {
            ...
            var body = this.options.postBody
        ...
        if (event == 'Complete') {
            try {
                (this.options['on' + this.transport.status] ||
                    this.options[
                        'on' +
                            (this.responseIsSuccess() ? 'Success' : 'Failure')
                    ] ||
...
Selector.prototype = {
    ...
    buildMatchExpression: function() {
        ...
        if ((clause = params.tagName))
            conditions.push(
                'element.tagName.toUpperCase() == ' + clause.inspect(),
            );
        ...
        if ((clause = params.attributes)) {
            clause.each(function(attribute) {
                var value =
                    'element.getAttribute(' + attribute.name.inspect() + ')';
        ...
        .map(function(expression) {
            return expression
                ...
                .inject([null], function(results, expr) {
                    ...
                    return results
                        .map(selector.findElements.bind(selector))
                        .flatten();
                });
        })
        .flatten();
