...
Draggable.prototype = {
    initialize: function(element) {
        var options = Object.extend(
            {
                ...
                reverteffect: function(element, top_offset, left_offset) {
                    var dur =
                        Math.sqrt(
                            Math.abs(top_offset ^ 2) +
                                Math.abs(left_offset ^ 2),
                        ) * 0.02;
                ...
                endeffect: function(element) {
                    var toOpacity =
                        typeof element._opacity == 'number'
                            ? element._opacity
                            : 1.0;
                    new Effect.Opacity(element, {
                        duration: 0.2,
        ...
        if (options.handle && typeof options.handle == 'string') {
            var h = Element.childrenWithClassName(
                this.element,
                options.handle,
                true,
            );
    ...
    startDrag: function(event) {
        ...
        if (this.options.zindex) {
            this.originalZ = parseInt(
                Element.getStyle(this.element, 'z-index') || 0,
            );
            ...
            if (typeof this.options.snap == 'function') {
                ...
                if (this.options.snap instanceof Array) {
                    p = p.map(
                        function(v, i) {
                            return (
                                Math.round(v / this.options.snap[i]) *
                                this.options.snap[i]
                            );
                        }.bind(this),
                    );
                    ...
                    p = p.map(
                        function(v) {
                            return (
                                Math.round(v / this.options.snap) *
                                this.options.snap
                            );
                        }.bind(this),
                    );
    ...
    scroll: function() {
        ...
        if (this.options.scroll == window) {
            ...
            this.options.scroll.scrollLeft +=
                (this.scrollSpeed[0] * delta) / 1000;
            this.options.scroll.scrollTop +=
                (this.scrollSpeed[1] * delta) / 1000;
        }
...
var Sortable = {
    ...
    onEmptyHover: function(element, dropon, overlap) {
        ...
        if (!Element.isParent(dropon, element)) {
            ...
            if (children) {
                var offset =
                    Element.offsetSize(dropon, droponOptions.overlap) *
                    (1.0 - overlap);
    ...
    mark: function(dropon, position) {
        ...
        if (position == 'after')
            if (sortable.overlap == 'horizontal')
                Sortable._marker.style.left =
                    offsets[0] + dropon.clientWidth + 'px';
            else
                Sortable._marker.style.top =
...
Element.findChildren = function(element, only, recursive, tagName) {
    ...
    $A(element.childNodes).each(function(e) {
        if (
            ...
            elements.push(e);
        if (recursive) {
            var grandchildren = Element.findChildren(
                e,
                only,
                recursive,
                tagName,
