...
if (!id_language) var id_language = Number(1);
...
function copy2friendlyURL() {
    if (typeof $('#link_rewrite_' + id_language).val() == 'undefined') return;
    if (typeof id_product == 'undefined') id_product = false;

    if (
        ps_force_friendly_product ||
        !$('#link_rewrite_' + id_language).val().length ||
        !id_product
    ) {
        //check if user didn't type anything in rewrite field, to prevent overwriting
        $('#link_rewrite_' + id_language).val(
            str2url(
                $.trim(
                    $('#name_' + id_language)
                        .val()
...
function updateFriendlyURLByName() {
    $('#link_rewrite_' + id_language).val(
        str2url($('#name_' + id_language).val(), 'UTF-8'),
    );
    $('#friendly-url_' + id_language).html(
        $('#link_rewrite_' + id_language).val(),
    );
}
...
function updateFriendlyURL() {
    ...
    if (link[0]) {
        $('#friendly-url_' + id_language).text(
            str2url($('#link_rewrite_' + id_language).val(), 'UTF-8'),
        );
    }
}
...
function updateLinkRewrite() {
    ...
    $('#link_rewrite_' + id_language).val(
        $.trim($('#link_rewrite_' + id_language).val()),
    );
    var link = $('#link_rewrite_' + id_language);
    if (link[0]) {
...
function toggleLanguageFlags(elt) {
    $(elt)
        .parents('.displayed_flag')
        .siblings('.language_flags')
        .toggle();
}
...
function changeLanguage(field, fieldsString, id_language_new, iso_code) {
    ...
    for (var i = 0; i < fields.length; ++i) {
        ...
        $('#' + 'language_current_' + fields[i]).attr(
            'src',
...
function changeFormLanguage(id_language_new, iso_code, employee_cookie) {
    $('.translatable').each(function() {
        $(this)
            ...
            .show()
            .siblings('div:not(.displayed_flag):not(.clear)')
            .hide();
        $(this)
            .find('.language_current')
            .attr('src', '../img/l/' + id_language_new + '.jpg');
    });

    // For multishop checkboxes
    $('.multishop_lang_' + id_language_new)
        .show()
        .siblings("div[class^='multishop_lang_']")
        .hide();
    id_language = id_language_new;
            ...
            if (!$(this).find('.displayed_flag').length > 0) {
                ...
                if (
                    ...
                    $(this)
                        .find('p:last-child')
                        .before(displayFlags)
                        .before(languagesFlags);
                else
                    $(this)
                        .append(displayFlags)
                        .append(languagesFlags);
            }
        });
    }
}
...
function viewTemplates(id_select, lang, ext) {
    ...
    if (loc != 0) {
        ...
        var url_preview = $('option[value=' + loc + ']', id_select).data(
            'preview',
        );
        tpl_viewing_window = window.open(
            url_preview + lang + loc + ext,
            'tpl_viewing',
...
function orderDeleteProduct(txtConfirm, txtExplain) {
    ret = true;
    $('table#cancelProducts input[type=checkbox]:checked').each(function() {
        totalCancel = parseInt(
            $(this)
                .parent()
                .parent()
                .find('td.cancelQuantity input[type=text]')
                .val(),
        );
        totalQty = parseInt(
            $(this)
                .parent()
                .find('input#totalQty[type=hidden]')
                .val(),
        );
        totalQtyReturn = parseInt(
            $(this)
                .parent()
                .find('input#totalQtyReturn[type=hidden]')
                .val(),
        );
        productName = $(this)
            .parent()
            .find('input#productName[type=hidden]')
            .val();
...
function toggleShippingCost() {
    ...
    } else $('#spanShippingBack').css('display', 'block');
}
...
function orderOverwriteMessage(sl, text) {
    ...
    if (sl_value != '0') {
        if ($zone.val().length > 0 && !confirm(text)) return;
        $zone.val(sl_value);
    }
...
function hookCheckboxes(id, opt, champ) {
    if (opt == 1 && champ.checked == false)
        ...
        if (champ.checked) $('.hook' + id).attr('checked', 'checked');
        else $('.hook' + id).attr('checked', false);
    }
}
...
function changeCMSActivationAuthorization() {
    ...
    else getE('PS_CONDITIONS_CMS_ID').disabled = 'disabled';
}
...
function disableZipFormat() {
    if ($('#need_zip_code_on').prop('checked') == false) {
        ...
        $('#zip_code_format').val('');
    } else $('.zip_code_format').show();
}

function disableTAASC() {
    if ($('#iso_code').val() == 'US') $('#TAASC').show();
    else $('#TAASC').hide();
}

function spreadFees(id_range) {
    newVal = $('#fees_all_' + id_range)
        .val()
        .replace(/,/g, '.');
...
function showRedirectProductOptions(show) {
    if (show) $('.redirect_product_options').fadeIn();
    else $('.redirect_product_options').fadeOut();
...
function redirectSelectChange() {
    ...
    else showRedirectProductSelectOptions(true);
}

function addRelatedProduct(id_product_to_add, product_name) {
    if (!id_product_to_add || id_product == id_product_to_add) return;
...
function showRedirectProductSelectOptions(show) {
    if (show) $('.redirect_product_options_product_choise').show();
    else {
...
function showOptions(show) {
    if (show) $('tr#product_options').show();
    else $('tr#product_options').hide();
}

function submitAddProductAndPreview() {
    $('#fakeSubmitAddProductAndPreview').attr(
        'name',
...
function checkAllMultishopDefaultValue(item) {
    $(item)
        .parent()
        .find("input[name^='multishopOverrideOption']")
        .each(function(k, v) {
...
function checkMultishopDefaultValue(obj, key) {
    ...
    $('#conf_id_' + key + ' .preference_default_multishop input').prop(
        'disabled',
        false,
    );
}
...
function toggleMultishopDefaultValue(obj, key) {
    ...
    $('#conf_id_' + key + " input[name^='multishopOverrideOption']").attr(
        'disabled',
        false,
    );
}
...
function doAdminAjax(data, success_func, error_func) {
    $.ajax({
        ...
        success: function(data) {
            if (success_func) return success_func(data);
        ...
        error: function(data) {
            if (error_func) return error_func(data);
...
$(document).ready(function() {
    if (
        typeof helper_tabs != 'undefined' &&
        typeof unique_field_id != 'undefined'
    ) {
        $.each(helper_tabs, function(index) {
            $(
                '#' + unique_field_id + 'fieldset_' + index + ' .form-wrapper',
            ).prepend('<div class="tab-content panel" />');
            $(
                '#' + unique_field_id + 'fieldset_' + index + ' .form-wrapper',
            ).prepend('<ul class="nav nav-tabs" />');
            $.each(helper_tabs[index], function(key, value) {
                ...
                $(
                    '#' +
                        ...
                        index +
                        ' .tab-content',
                ).append('<div id="' + key + '" class="tab-pane" />');
                var elemts = $(
                    '#' + unique_field_id + 'fieldset_' + index,
            ...
            $('#' + unique_field_id + 'fieldset_' + index + ' .tab-content div')
                .first()
                .addClass('active');
            $('#' + unique_field_id + 'fieldset_' + index + ' .nav-tabs li')
                .first()
                .addClass('active');
        });
    }
    ...
    if (
        typeof formToMove != 'undefined' &&
        typeof formDestination != 'undefined'
    ) {
        $('<hr style="margin 24px 0;" />').appendTo('#' + formDestination);
        $('#configuration_fieldset_' + formToMove + ' .form-wrapper').appendTo(
            '#' + formDestination,
        );
    }
    ...
    $('.isInvisible input, .isInvisible select, .isInvisible textarea').attr(
        'disabled',
        true,
    );
    ...
    $('.preference_default_multishop input[type=checkbox]').each(function(
        k,
        v,
    ) {
        var key = $(v).attr('name');
    ...
    $("[name='submitFilter']").click(function(event) {
        ...
        $(document.body)
            .find("input[name*='" + list_id + "Filter']")
            .each(function() {
                if ($(this).val() != '') {
                    empty_filters = false;
                    return false;
        ...
        $(document.body)
            .find("select[name*='" + list_id + "Filter']")
            .each(function() {
                empty_filters = false;
                return false;
            });
    ...
    view.bind('scroll resize', function(e) {
        message.each(function(el) {
            if (message.length) {
                ...
                window_is_more_than_twice_the_toolbar =
                    view.height() > message.parent().height() * 2;
                if (
                    !$(this).hasClass('fix-toolbar') &&
                    (window_is_more_than_twice_the_toolbar &&
    ...
    $(document).on('click', '.untrustedaddon', function(e) {
        ...
        var addonsSearchLink =
            'https://addons.prestashop.com/en/search?search_query=' +
            encodeURIComponent(moduleDisplayName) +
            '&utm_source=back-office&utm_medium=addons-certified&utm_campaign=back-office-' +
            iso_user.toUpperCase();
    ...
    $('.swap-container').each(function() {
        ...
        if (
            typeof $('.addSwap', this) !== undefined &&
            typeof $('.removeSwap', this) !== undefined &&
            typeof $('.selectedSwap', this) !== undefined &&
            typeof $('.availableSwap', this) !== undefined
        ) {
    ...
    if (typeof host_mode !== 'undefined' && host_mode) {
        ...
        sp.components({
            success: function(data) {
                $('.status-page-description').text(
                    status_map[
                        data.components[components_map[host_cluster]].status
                    ],
                );
                $('.status-page-dot').addClass(
                    data.components[components_map[host_cluster]].status,
                );
            },
        });
...
function bindSwapSave(context) {
    ...
    else $('.availableSwap option', context).attr('selected', 'selected');
}

function bindSwapButton(
    prefix_button,
    prefix_select_remove,
    prefix_select_add,
    context,
) {
    $('.' + prefix_button + 'Swap', context).on('click', function(e) {
        e.preventDefault();
        $('.' + prefix_select_remove + 'Swap option:selected', context).each(
            function() {
                $('.' + prefix_select_add + 'Swap', context).append(
                    "<option value='" +
                        $(this).val() +
                        "'>" +
                        $(this).text() +
    ...
    $('.action_tab_module').each(function() {
        $(this).click(function() {
            ...
            if ($(option).data('onclick') != '') {
                var f = eval(
                    '(function(){ ' + $(option).data('onclick') + '})',
                );
                if (f.call()) window.location.href = $(option).data('href');
            } else window.location.href = $(option).data('href');
            return false;
        });
    });
}
...
function display_action_details(row_id, controller, token, action, params) {
    ...
    if (!current_element.data('dataMaped')) {
        ...
        $.ajax({
            ...
            success: function(data) {
                if (
                    typeof data.use_parent_structure == 'undefined' ||
                    data.use_parent_structure == true
                ) {
                    if (
                        current_element
                            .parent()
                    ...
                    current_element
                        ...
                        .after(
                            $(
                                ...
                                $(
                                    '<td style="border:none!important;" class="empty"></td>',
                                ).attr(
                                    'colspan',
                                    current_element
                                        .parent()
                    ...
                    $.each(data.data, function(it, row) {
                        ...
                        var content = $(
                            '<tr class="action_details details_' +
                                id +
                                ' ' +
                                (alt_row ? 'alt_row' : '') +
                                '"></tr>',
                    ...
                    if (
                        ...
                        var content = $(
                            '<tr class="details_' + id + ' alt_row"></tr>',
                        );
                    else
...
function sendBulkAction(form, action) {
    String.prototype.splice = function(index, remove, string) {
        return (
            this.slice(0, index) + string + this.slice(index + Math.abs(remove))
        );
    };
    ...
    if (form_action.indexOf('#') == -1)
        ...
        $(form).attr(
            'action',
            form_action.splice(form_action.lastIndexOf('&'), 0, '&' + action),
        );
...
function getControllerActionMap(force_action) {
    ...
    for (i = 0; i < vars.length; i++) {
        ...
        if (pair[0] == 'controller') controller = pair[1];

        if (pair.length == 1) {
            if (pair[0].indexOf('add') != -1) action = 'new';
            else if (pair[0].indexOf('view') != -1) action = 'view';
            else if (
                pair[0].indexOf('edit') != -1 ||
                pair[0].indexOf('modify') != -1 ||
                pair[0].indexOf('update') != -1
            )
                action = 'edit';
            else if (pair[0].indexOf('delete') != -1) action = 'delete';
        }
    }

    if (force_action !== undefined) action = force_action;

    if (typeof help_class_name != 'undefined') controller = help_class_name;
        ...
        try {
            resAjax = $.ajax({
                ...
                beforeSend: function(xhr) {
                    $('#addons_loading').html(
                        '<img src="../img/loader.gif" alt="" border="0" />',
                    );
                },
                success: function(data) {
                    if (data == 'OK') {
                        ...
                        window.location.href =
                            currentIndex + '&token=' + token + '&conf=32';
                    } else
                        $('#addons_loading').html(
        ...
        try {
            resAjax = $.ajax({
                ...
                success: function(data) {
                    ...
                    } else $('#addons_loading').html(errorLogin);
                },
            });
        } catch (e) {}
        return false;
...
function ajaxStates(id_state_selected) {
    $.ajax({
        ...
        data:
            'token=' +
            state_token +
            '&ajax=1&action=states&tab=AdminStates&no_empty=0&id_country=' +
            $('#id_country').val() +
            '&id_state=' +
            $('#id_state').val(),
        success: function(html) {
            if (html == 'false') {
                ...
                $('#id_state option[value=' + id_state_selected + ']').attr(
                    'selected',
                    'selected',
                );
            }
    ...
    if (module_dir && vat_number) {
        $.ajax({
            ...
            url:
                ...
                module_dir +
                'vatnumber/ajax.php?id_country=' +
                $('#id_country').val(),
            success: function(isApplicable) {
                if (isApplicable == 1) $('#vat_area').show();
                else $('#vat_area').hide();
            },
        });
    }
...
function check_for_all_accesses(tabsize, tabnumber) {
    ...
    while (i != parseInt(tabsize) + 1) {
        ...
        else $('#all' + i).attr('checked', 'checked');
        i++;
    }
    right = 0;
    while (right != 5) {
        ...
        i = 0;
        while (i != tabsize) {
            if ($('#' + rights[right] + i).prop('checked') == true) res++;
            i++;
        }
        if (res == tabnumber - 1)
            $('#' + rights[right] + 'all').attr('checked', 'checked');
        else $('#' + rights[right] + 'all').attr('checked', false);
        right++;
    }
}

function perfect_access_js_gestion(
    src,
    action,
    id_tab,
    tabsize,
...
function verifyMail(testMsg, testSubject) {
    $('#mailResultCheck')
        ...
        .html('<img src="../img/admin/ajax-loader.gif" alt="" />');
    ...
    if (!($('#testEmail').val().length > 0)) {
        ...
        $.ajax({
            ...
            data: {
                mailMethod:
                    $('input[name=PS_MAIL_METHOD]:checked').val() == 2
                        ? 'smtp'
                        : 'native',
            ...
            function(ret) {
                ...
                if (ret.status == 'ok') {
                    ...
                    message =
                        ...
                        ' ' +
                        content['version'] +
                        ' <a href="http://www.prestashop.com/download/lang_packs/gzip/' +
                        content['version'] +
                        '/' +
                        $('#iso_code')
                            .val()
                            .toLowerCase() +
                        '.gzip" target="_blank" class="link">' +
                        download +
                        '</a><br />' +
                        langPackInfo;
                    $('#lang_pack_msg').html(message);
                    $('#lang_pack_msg').show();
                } else showErrorMessage(ret.error);
            },
        );
    }
}
...
function createSqlQueryName() {
    ...
    else if (container) name = container;
    else if (current) name = current;
...
function confirm_link(
    ...
    jConfirm(display_text, head_text, function(confirm) {
        if (confirm === true) document.location = confirm_link;
        else document.location = cancel_link;
    });
}
