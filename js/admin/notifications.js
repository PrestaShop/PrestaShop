...
$(document).ready(function() {
    ...
    $('body').on('click', function(e) {
        if (
            !$('#notification.dropdown').is(e.target) &&
    ...
    if (
        parseInt(show_new_orders) ||
        parseInt(show_new_customers) ||
        parseInt(show_new_messages)
    ) {
        getPush();
    }
});
...
function updateEmployeeNotifications() {
    $.post(baseAdminDir + 'ajax.php', {
        ...
        updateElementEmployeeType: $('.notifications .nav-item.active a').attr(
            'data-type',
        ),
    });
}
            ...
            if (json) {
                ...
                $.each(json.order.results, function(property, value) {
                    html +=
                        "<a class='notif' href='" +
                        baseAdminDir +
                        'index.php?tab=AdminOrders&token=' +
                        token_admin_orders +
                        '&vieworder&id_order=' +
                        parseInt(value.id_order) +
                        "'>";
                ...
                $('#orders-notifications')
                    .children('.notification-elements')
                    .empty();
                ...
                $.each(json.customer.results, function(property, value) {
                    html +=
                        "<a class='notif' href='" +
                        baseAdminDir +
                        'index.php?tab=AdminCustomers&token=' +
                        token_admin_customers +
                        '&viewcustomer&id_customer=' +
                        parseInt(value.id_customer) +
                        "'>";
                    html +=
                        '#' +
                        value.id_customer +
                ...
                if (parseInt(json.customer.total) > 0) {
                    ...
                    $('#customers-notifications')
                        .children('.notification-elements')
                        .append(html);
                    $('#customers_notif_value')
                        .text(' (' + nbCustomers + ')')
                        .attr('data-nb', nbCustomers);
                } else {
                ...
                html = '';
                $.each(json.customer_message.results, function(
                    property,
                    value,
                ) {
                    html +=
                        "<a class='notif' href='" +
                        baseAdminDir +
                        'index.php?tab=AdminCustomerThreads&token=' +
                        token_admin_customer_threads +
                        '&viewcustomer_thread&id_customer_thread=' +
                        parseInt(value.id_customer_thread) +
                        "'>";
                    html +=
                ...
                if (parseInt(json.customer_message.total) > 0) {
                    ...
                    $('#messages-notifications')
                        .children('.notification-elements')
                        .append(html);
                    $('#customer_messages_notif_value')
                        .text(' (' + nbCustomerMessages + ')')
                        .attr('data-nb', nbCustomerMessages);
                } else {
