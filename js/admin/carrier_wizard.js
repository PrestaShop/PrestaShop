...
$(document).ready(function() {
    ...
    if ($('#is_free_on').prop('checked') === true) {
        $('#shipping_handling_off')
            .prop('checked', true)
            .prop('disabled', true);
        $('#shipping_handling_on')
            .prop('disabled', true)
            .prop('checked', false);
    }
...
function onFinishCallback(obj, context) {
    ...
    $.ajax({
        ...
        data:
            ...
            '&action=finish_step&ajax=1&step_number=' +
            context.fromStep,
        success: function(data) {
            if (data.has_error) {
                displayError(data.errors, context.fromStep);
                resizeWizard();
            } else window.location.href = carrierlist_url;
        },
...
function onLeaveStepCallback(obj, context) {
    if (context.toStep == nbr_steps) displaySummary();
...
function displaySummary() {
    ...
    if (!delay_text) delay_text = $('#delay_' + id_default_lang).val();

    // Carrier name
    $('#summary_name').text($('#name').val());

    // Delay and pricing
    tmp = summary_translation_meta_informations.replace(
        '%2$s',
        '<strong>' + delay_text + '</strong>',
    );
    if ($('#is_free_on').attr('checked'))
        tmp = tmp.replace('%1$s', summary_translation_free);
    else tmp = tmp.replace('%1$s', summary_translation_paid);
    $('#summary_meta_informations').html(tmp);

    // Tax and calculation mode for the shipping cost
    tmp = summary_translation_shipping_cost.replace(
        '%2$s',
        $('#id_tax_rules_group option:selected').text(),
    );
    ...
    if ($('#billing_price').attr('checked'))
        ...
        tmp = tmp.replace(
            '%1$s',
            '<strong>' + summary_translation_undefined + '</strong>',
        );
    ...
    $('#summary_range').text(
        summary_translation_range + ' ' + summary_translation_range_limit,
    );

    if ($('input[name="shipping_method"]:checked').val() == 1)
        unit = PS_WEIGHT_UNIT;
    else unit = currency_sign;
    ...
    $('tr.range_inf td input').each(function() {
        if (
            ...
            (range_inf == summary_translation_undefined ||
                parseFloat(range_inf) > parseFloat($(this).val()))
        )
            range_inf = $(this).val();
    });
    ...
    $('tr.range_sup td input').each(function() {
        if (
            ...
            (range_sup == summary_translation_undefined ||
                parseFloat(range_sup) < parseFloat($(this).val()))
        )
            range_sup = $(this).val();
    });
    ...
    $('#summary_range').html(
        $('#summary_range')
            ...
            .replace(
                '%3$s',
                '<strong>' +
                    $('#range_behavior option:selected')
                        .text()
                        .toLowerCase() +
                    '</strong>',
            ),
    );
    if ($('#is_free_on').attr('checked')) $('span.is_free').hide();
    // Delivery zones
    $('#summary_zones').html('');
...
function validateSteps(fromStep, toStep) {
    var is_ok = true;
    if (
        (multistore_enable && fromStep == 3) ||
        (!multistore_enable && fromStep == 2)
    ) {
        if (toStep > fromStep && !$('#is_free_on').attr('checked')) {
            is_ok = false;
            $('.input_zone').each(function() {
                if ($(this).prop('checked')) is_ok = true;
            });
    ...
    if (is_ok && isOverlapping()) is_ok = false;
...
function bind_inputs() {
    $('input').focus(function() {
        $(this)
            .closest('div.input-group')
            .removeClass('has-error');
        $('#carrier_wizard .actionBar a.btn')
            .not('.buttonFinish')
            ...
            if ($(this).is(':checked')) {
                $(this)
                    ...
                    .each(function() {
                        index = $(this).index();
                        if (
                            $('tr.fees_all td:eq(' + index + ')').hasClass(
                                'validated',
                            )
                        ) {
                            enableGlobalFees(index);
                            $(this)
                                .find('div.input-group input:text')
                                .removeAttr('disabled');
                        } else disabledGlobalFees(index);
                    });
            } else
                $(this)
                    .closest('tr')
    ...
    $('tr.range_sup td input:text, tr.range_inf td input:text').keypress(
        function(evn) {
            index = $(this)
                .closest('td')
                .index();
            if (evn.keyCode == 13) {
                if (validateRange(index)) enableRange(index);
                else disableRange(index);
                return false;
            }
        },
    );
    ...
    $('tr.fees_all td input:text').keypress(function(evn) {
        index = $(this)
            .parent('td')
            .index();
        if (evn.keyCode == 13) return false;
    });

    $(document.body)
        .off('change', 'tr.fees_all td input')
        .on('change', 'tr.fees_all td input', function() {
            index = $(this)
                .closest('td')
                .index();
            val = $(this).val();
            ...
            $('tr.fees').each(function() {
                $(this)
                    .find('td:eq(' + index + ') input:text:enabled')
                    .val(val);
            });
    ...
    $('input[name="shipping_method"]')
        ...
        .on('click', function() {
            $.ajax({
                ...
                data:
                    'id_carrier=' +
                    parseInt($('#id_carrier').val()) +
                    '&shipping_method=' +
                    parseInt($(this).val()) +
                    '&action=changeRanges&ajax=1',
                success: function(data) {
    ...
    $('#zones_table td input[type=text]')
        .off('change')
        .on('change', function() {
...
function is_freeClick(elt) {
    var is_free = $(elt);
    if (parseInt(is_free.val())) hideFees();
    else if (fees_is_hide) showFees();
}

function hideFees() {
    $('tr.range_inf td, tr.range_sup td, tr.fees_all td, tr.fees td').each(
        function() {
            if ($(this).index() >= 2) {
                $(this)
                    .find('input:text, button')
                    .val('')
                    .attr('disabled', 'disabled')
                ...
                if (
                    ($(tr).index() > 2 &&
                        $(tr)
                            ...
                            .attr('checked') &&
                        validate) ||
                    !$(tr).hasClass('range_sup') ||
                    !$(tr).hasClass('range_inf')
                )
                    $(this)
                        .find('div.input-group input:text')
                        .removeAttr('disabled');
...
function validateRange(index) {
    ...
    var isValid = true;
    var $currentRangeSup = $('tr.range_sup td:eq(' + index + ')').find(
        'div.input-group input:text',
    );
    var $currentRangeInf = $('tr.range_inf td:eq(' + index + ')').find(
        'div.input-group input:text',
    );
    ...
    if (isNaN(rangeSup) || rangeSup.length === 0) {
        ...
        displayError(
            [invalid_range],
            $('#carrier_wizard').smartWizard('currentStep'),
        );
        ...
        isValid &&
        (index > 2 ||
            $('tr.range_sup td').not('.range_type, .range_sign').length > 1)
    ) {
        //check range only if it's not the first range
    ...
    if (isValid) {
        $('tr.range_sup td')
            .not('.range_type, .range_sign')
            .each(function() {
                var $this = $(this);
                var currentIndex = $this.index();
                if (
                    $this.find('.has-error').length > 0 &&
                    currentIndex !== index
                ) {
...
function enableGlobalFees(index) {
    ...
    $('tr.fees_all td:eq(' + index + ')')
        ...
        .removeAttr('disabled');
    $('tr.fees_all td:eq(' + index + ')')
        .find('div.input-group .currency_sign')
        .show();
}
...
function add_new_range() {
    ...
    $('tr.range_sup td:last').after(
        '<td class="range_data"><div class="input-group fixed-width-md"><span class="input-group-addon weight_unit" style="display: none;">' +
            PS_WEIGHT_UNIT +
            '</span><span class="input-group-addon price_unit" style="display: none;">' +
            currency_sign +
            '</span><input class="form-control" name="range_sup[]" type="text" autocomplete="off" /></div></td>',
    );
    //add new rand inf input
    $('tr.range_inf td:last').after(
        '<td class="border_bottom"><div class="input-group fixed-width-md"><span class="input-group-addon weight_unit" style="display: none;">' +
            PS_WEIGHT_UNIT +
            '</span><span class="input-group-addon price_unit" style="display: none;">' +
            currency_sign +
            '</span><input class="form-control" name="range_inf[]" type="text" value="' +
            last_sup_val +
            '" autocomplete="off" /></div></td>',
    );
    $('tr.fees_all td:last').after(
        '<td class="border_top border_bottom"><div class="input-group fixed-width-md"><span class="input-group-addon currency_sign" style="display:none" >' +
            currency_sign +
            '</span><input class="form-control" style="display:none" type="text" /></div></td>',
    );
    ...
    $('tr.fees').each(function() {
        $(this)
            .find('td:last')
            .after(
                '<td><div class="input-group fixed-width-md"><span class="input-group-addon currency_sign">' +
                    currency_sign +
                    '</span><input class="form-control" disabled="disabled" name="fees[' +
                    $(this).data('zoneid') +
                    '][]" type="text" /></div></td>',
            );
    });
    $('tr.delete_range td:last').after(
        '<td><button class="btn btn-default">' + labelDelete + '</button</td>',
    );
...
function delete_new_range() {
    if (($('#new_range_form_placeholder').find('td').length = 1)) return false;
}
    ...
    $('#zones_table tr').each(function() {
        ...
        $(this)
            ...
            .each(function() {
                ...
                if (
                    $(this).index() >= 2 &&
                    $(this).find('div.input-group input')
                )
                    $(this)
                        .find('div.input-group input')
                        .attr('tabindex', j);
...
function repositionRange(current_index, new_index) {
    $(
        'tr.range_sup, tr.range_inf, tr.fees_all, tr.fees, tr.delete_range ',
    ).each(function() {
        $(this)
            .find('td:eq(' + current_index + ')')
            .each(function() {
                $(this)
                    .closest('tr')
                    .find('td:eq(' + new_index + ')')
                    .after(this.outerHTML);
                $(this).remove();
            });
    });
}
...
function checkRangeContinuity(reordering) {
    ...
    $('tr.range_sup td')
        ...
        .each(function() {
            index = $(this).index();
            if (index > 2) {
                range_sup = parseFloat(
                    $('tr.range_sup td:eq(' + index + ')')
                        .find('div.input-group input:text')
                        .val()
                        .trim(),
                );
                range_inf = parseFloat(
                ...
                prev_index = index - 1;
                prev_range_sup = parseFloat(
                    $('tr.range_sup td:eq(' + prev_index + ')')
                        .find('div.input-group input:text')
                        .val()
                        .trim(),
                );
                prev_range_inf = parseFloat(
                    $('tr.range_inf td:eq(' + prev_index + ')')
                        .find('div.input-group input:text')
                        .val()
                        .trim(),
                );
                ...
                if (range_inf < prev_range_inf || range_sup < prev_range_sup) {
                    res = false;
                    if (reordering) {
                        new_position = getCorrectRangePosistion(
                            range_inf,
                            range_sup,
                        );
                        if (new_position) repositionRange(index, new_position);
                    }
                }
            }
        });
    if (res) $('.ranges_not_follow').fadeOut();
    else $('.ranges_not_follow').fadeIn();
    resizeWizard();
}
    ...
    $('tr.range_sup td')
        ...
        .each(function() {
            ...
            if ($('tr.range_inf td:eq(' + index + 1 + ')').length)
                next_range_inf = parseFloat(
                    $('tr.range_inf td:eq(' + index + 1 + ')')
                        .find('div.input-group input:text')
                        .val()
                        .trim(),
                );
...
function isOverlapping() {
    ...
    $('tr.range_sup td')
        ...
        .each(function() {
            ...
            var currentInf = parseFloat(
                $('.range_inf td:eq(' + index + ') input').val(),
            );
            var currentSup = parseFloat(
                $('.range_sup td:eq(' + index + ') input').val(),
            );
            ...
            $('tr.range_sup td')
                ...
                .each(function() {
                    ...
                    if (testingIndex !== index && testingIndex >= index - 1) {
                        //do not test himself
                        var testingInf = parseFloat(
                            $(
                                '.range_inf td:eq(' + testingIndex + ') input',
                            ).val(),
                        );
                        var testingSup = parseFloat(
                            $(
                                '.range_sup td:eq(' + testingIndex + ') input',
                            ).val(),
                        );
                        var checkOverLapping =
                            index < testingIndex
                                ? currentInf >= testingInf ||
                                  currentInf >= testingSup ||
                                  currentSup > testingInf ||
                                  currentSup >= testingSup
                                : currentInf <= testingInf ||
                                  currentInf < testingSup ||
                                  currentSup <= testingInf ||
                                  currentSup <= testingSup;
                        ...
                        if (checkOverLapping) {
                            $(
                                'tr.range_sup td:eq(' +
                                    testingIndex +
                                    ') div.input-group, tr.range_inf td:eq(' +
                                    testingIndex +
                                    ') div.input-group',
                            ).addClass('has-error');
    ...
    if ($(elt).is(':checked')) {
        ...
        $('.fees div.input-group input:text').each(function() {
            ...
            if ($('tr.fees_all td:eq(' + index + ')').hasClass('validated')) {
                ...
                $(
                    '.fees_all td:eq(' + index + ') div.input-group input:text',
                ).removeAttr('disabled');
            }
        });
    } else {
        ...
        $(
            '.fees div.input-group input:text, .fees_all div.input-group input:text',
        ).attr('disabled', 'disabled');
    }
}
            ...
            $(this).typeWatch({
                ...
                callback: function() {
                    var index = $(this.el)
                        .closest('td')
                        .index();
                    var rangeSup = $('tr.range_sup td:eq(' + index + ')')
                        .find('div.input-group input:text')
                        .val()
                        .trim();
                    var rangeInf = $('tr.range_inf td:eq(' + index + ')')
                        .find('div.input-group input:text')
                        .val()
                        .trim();
