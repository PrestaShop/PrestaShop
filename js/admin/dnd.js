...
function initTableDnD(table) {
    if (typeof table == 'undefined') table = 'table.tableDnD';
    ...
    $(table).tableDnD({
        ...
        onDrop: function(table, row) {
            if (originalOrder != $.tableDnD.serialize()) {
                var way =
                    originalOrder.indexOf(row.id) <
                    $.tableDnD.serialize().indexOf(row.id)
                        ? 1
                        : 0;
                ...
                if (tableId == 'cms_block_0' || tableId == 'cms_block_1')
                    ...
                    tableId.indexOf('attribute') != -1 &&
                    tableId != 'attribute_group'
                ) {
                    params = {
                ...
                params['selected_pagination'] = parseInt(
                    $('input[name=selected_pagination]').val(),
                );
                ...
                $.ajax({
                    ...
                    success: function(data) {
                        var nodrag_lines = $(tableDrag).find(
                            'tr:not(".nodrag")',
                        );
                        var new_pos;
                        ...
                        nodrag_lines
                            .children('td.dragHandle')
                            .find('a')
                            .attr('disabled', false);

                        if (typeof alternate !== 'undefined' && alternate) {
                            nodrag_lines
                                .children('td.dragHandle:first')
                                .find('a:odd')
                                .attr('disabled', true);
                            nodrag_lines
                                .children('td.dragHandle:last')
                                .find('a:even')
                                .attr('disabled', true);
                        } else {
                            nodrag_lines
