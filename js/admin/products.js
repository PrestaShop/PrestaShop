...
function ProductTabsManager() {
    ...
    this.init = function() {
        for (var tab_name in this.product_tabs) {
            if (
                this.product_tabs[tab_name].onReady !== undefined &&
                this.product_tabs[tab_name] !== this.product_tabs['Pack']
            ) {
    ...
    this.display = function(tab_name, selected) {
        ...
        if (
            tab_selector.hasClass('not-loaded') &&
            !tab_selector.hasClass('loading')
        ) {
    ...
    this.displayBulk = function(stack) {
        ...
        if (this.current_request !== undefined) {
            this.current_request.complete(function(request, status) {
                ...
                var wrong_status_code = new Array(
                    400,
                    401,
                    403,
                    404,
                ...
                if (
                    (in_array(status, wrong_statuses) ||
                        in_array(request.status, wrong_status_code)) &&
                    !self.page_reloading
                ) {
                    var current_tab = '';
                    if (
                        ...
                        current_tab = $(request.responseText)
                            .filter('.product-tab')
                            .attr('id')
                            .replace('product-', '');
                    }
                    ...
                    !self.has_error_loading_tabs &&
                    self.stack_done.length === self.tabs_to_preload.length
                ) {
...
function loadPack() {
    ...
    var id_product = $('input[name=id_product]')
        .first()
        .val();
    var data;
    $.ajax({
...
product_tabs['Combinations'] = new function() {
            ...
            $.ajax({
                ...
                success: function(data) {
                    ...
                    for (i = 0; i < data.length; i++) {
                        product_att_list.push(
                            data[i]['group_name'] +
                                ' : ' +
                                data[i]['attribute_name'],
                        );
                ...
                if (data.status == 'ok') {
                    ...
                    if (data.id_product_attribute)
                        if (data.attribute) {
                            ...
                            $('#qty_' + data.id_product_attribute)
                                .parent()
                                .hide();
                } else showErrorMessage(data.message);
            },
    ...
    this.removeButtonCombination = function(item) {
        $('#add_new_combination').show();
        $('#desc-product-newCombination')
            .children('i')
            .first()
            .removeClass('process-icon-new');
        $('#desc-product-newCombination')
            .children('i')
            .first()
            .addClass('process-icon-minus');
        $('#desc-product-newCombination')
            .children('span')
            .first()
            .html(msg_cancel_combination);
    ...
    this.addButtonCombination = function(item) {
        ...
        $('#desc-product-newCombination')
            .children('i')
            .first()
            .removeClass('process-icon-minus');
        $('#desc-product-newCombination')
            .children('i')
            .first()
            .addClass('process-icon-new');
        $('#desc-product-newCombination')
            .children('span')
            .first()
            .html(msg_new_combination);
    };
    ...
    this.fillCombination = function(
        ...
        if (default_attribute == 1) getE('attribute_default').checked = true;
        else getE('attribute_default').checked = false;

        if (price_impact < 0) {
            getE('attribute_price_impact').options[
                getE('attribute_price_impact').selectedIndex
            ].value = -1;
        ...
        if (weight_impact < 0) {
            getE('attribute_weight_impact').options[
                getE('attribute_weight_impact').selectedIndex
            ].value = -1;
        ...
        if (unit_impact < 0) {
            getE('attribute_unit_impact').options[
                getE('attribute_unit_impact').selectedIndex
            ].value = -1;
        ...
        combinationImages = $('#id_image_attr').find(
            'input[id^=id_image_attr_]',
        );
        ...
        if (typeof combination_images[id_product_attribute] != 'undefined')
            for (
                i = 0;
                i < combination_images[id_product_attribute].length;
                i++
            )
                $(
                    '#id_image_attr_' +
                        combination_images[id_product_attribute][i],
                ).attr('checked', true);
        check_impact();
        check_weight_impact();
        check_unit_impact();
    ...
    this.init_elems = function() {
        ...
        if (elem.length)
            for (i = elem.length - 1; i >= 0; i--) if (elem[i]) elem.remove(i);
...
function handleSaveButtons(e) {
    ...
    else msg[i++] = handleSaveButtonsForSimple();

    // common for all products
    $('#disableSaveMessage').remove();

    if (
        $('#name_' + id_lang_default).val() == '' &&
        (!display_multishop_checkboxes ||
            $(
                "input[name='multishop_check[name][" + id_lang_default + "]']",
            ).prop('checked'))
    )
        msg[i++] = empty_name_msg;
    // check friendly_url_[defaultlangid] only if name is ok
    else if (
        $('#link_rewrite_' + id_lang_default).val() == '' &&
        (!display_multishop_checkboxes ||
            $("input[name='link_rewrite[name][" + id_lang_default + "]']").prop(
                'checked',
            ))
    )
        ...
        for (var key in msg) {
            if (msg != '') {
                if (do_not_save == false) {
                    $('.leadin').append(
                        '<div id="disableSaveMessage" class="alert alert-danger"></div>',
                    );
...
function handleSaveButtonsForPack() {
    ...
    if ($('#inputPackItems').val() == '') return empty_pack_msg;
    return '';
}
...
product_tabs['Seo'] = new function() {
    ...
    this.onReady = function() {
        ...
        if (display_multishop_checkboxes) ProductMultishop.checkAllSeo();
    };
}();
...
product_tabs['Prices'] = new function() {
    ...
    this.toggleSpecificPrice = function() {
        $('#show_specific_price').click(function() {
            ...
            $('#add_specific_price').append(
                '<input type="hidden" name="submitPriceAddition"/>',
            );
        ...
        $('#hide_specific_price').click(function() {
            $('#add_specific_price').slideToggle();
            $('#add_specific_price')
                .find('input[name=submitPriceAddition]')
                .remove();
    ...
    this.onReady = function() {
        ...
        if (display_multishop_checkboxes) ProductMultishop.checkAllPrices();
    };
}();
...
product_tabs['Associations'] = new function() {
    ...
    this.initAccessoriesAutocomplete = function() {
        $('#product_autocomplete_input')
            .autocomplete(
                'ajax_products_list.php?exclude_packs=0&excludeVirtuals=0',
                {
                    minChars: 1,
                    autoFill: true,
                    max: 20,
    ...
    this.getAccessoriesIds = function() {
        if ($('#inputAccessories').val() === undefined) return id_product;
        return (
            id_product +
            ',' +
            $('#inputAccessories')
    ...
    this.addAccessory = function(event, data, formatted) {
        ...
        $divAccessories.html(
            $divAccessories.html() +
                '<div class="form-control-static"><button type="button" class="delAccessory btn btn-default" name="' +
                productId +
    ...
    this.delAccessory = function(id) {
        ...
        if (inputCut.length != nameCut.length) return jAlert('Bad size');
        ...
        for (i in inputCut) {
            // If empty, error, next
            if (!inputCut[i] || !nameCut[i]) continue;
        ...
        $.ajax({
            ...
            error: function(XMLHttpRequest, textStatus, errorThrown) {
                $('select#id_manufacturer').replaceWith(
                    '<p id="id_manufacturer">[TECHNICAL ERROR] ajaxProductManufacturers: ' +
                        textStatus +
                        '</p>',
                );
            },
...
product_tabs['Attachments'] = new function() {
    ...
    this.bindAttachmentEvents = function() {
        $('#addAttachment').on('click', function() {
            $('#selectAttachment2 option:selected').each(function() {
                ...
                for (var i = 0; i < tab.length; i++)
                    if (tab[i] == $(this).val()) return false;
                $('#arrayAttachments').val(val + $(this).val() + ',');
            });
            return !$('#selectAttachment2 option:selected')
                .remove()
                .appendTo('#selectAttachment1');
        });
        $('#removeAttachment').on('click', function() {
            ...
            return !$('#selectAttachment1 option:selected')
                .remove()
                .appendTo('#selectAttachment2');
        });
...
product_tabs['Shipping'] = new function() {
    ...
    this.bindCarriersEvents = function() {
        $('#addCarrier').on('click', function() {
            ...
            else $('#no-selected-carries-alert').hide();
        });
...
product_tabs['Informations'] = new function() {
    ...
    this.bindAvailableForOrder = function() {
        $('#available_for_order').click(function() {
            if (
                $(this).is(':checked') ||
                ($("input[name='multishop_check[show_price]']").length &&
                    !$("input[name='multishop_check[show_price]']").prop(
                        'checked',
                    ))
            ) {
        ...
        } else showRedirectProductOptions(true);
        ...
        $('#related_product_autocomplete_input')
            .autocomplete(
                'ajax_products_list.php?exclude_packs=0&excludeVirtuals=0&excludeIds=' +
                    id_product,
                {
                    minChars: 1,
                    autoFill: true,
            ...
            .result(function(e, i) {
                if (i != undefined) addRelatedProduct(i[1], i[0]);
                $(this).val('');
            });
    ...
    this.bindTagImage = function() {
        function changeTagImage() {
            ...
            var tag =
                '[img-' + smallImage + '-' + leftRight + '-' + imageTypes + ']';
            $('#resultImage').val(tag);
        }
        changeTagImage();
        ...
        $('input[name="type_product"]').on('click', function(e) {
            ...
            if (product_type == product_type_pack) {
                if (has_combinations) {
                    ...
                    $('#product-tab-content-Quantities').bind(
                        'loaded',
                        function() {
                ...
                if (has_combinations) {
                    ...
                    tabs_manager.onLoad('Informations', function() {
                        $('#condition').attr('disabled', true);
                        $('#condition option[value=refurbished]').removeAttr(
                            'selected',
                        );
                        $('#condition option[value=used]').removeAttr(
                            'selected',
                        );
                    });
                }
    ...
    this.onReady = function() {
        ...
        if (display_multishop_checkboxes) {
            ...
            var active_click = function() {
                if (
                    !$("input[name='multishop_check[active]']").prop('checked')
                ) {
                    $('.draft').hide();
                    showOptions(true);
                } else {
...
product_tabs['Pack'] = new function() {
    ...
    this.bindPackEvents = function() {
        ...
        function productFormatResult(item) {
            ...
            itemTemplate +=
                "<img class='media-object' width='40' src='" +
                item.image +
                "' alt='" +
                item.name +
                "'>";
                        ...
                        if (data) {
                            for (var i = data.length - 1; i >= 0; i--) {
                                ...
                                for (var j = 0; j < excludeIds.length; j++) {
                                    if (
                                        data[i].id == excludeIds[j][0] &&
                                        (typeof data[i].id_product_attribute ==
                                            'undefined' ||
                                            data[i].id_product_attribute ==
                                                excludeIds[j][1])
                                    )
                                        is_in = 1;
                                }
                                if (!is_in) returnIds.push(data[i]);
                            }
                            return {
                                results: returnIds,
                            };
        ...
        function addPackItem() {
            if (selectedProduct) {
                ...
                if (
                    selectedProduct.id == '' ||
                    (selectedProduct.name == '' && $('#curPackItemQty').valid())
                ) {
                    error_modal(error_heading_msg, msg_select_one);
                    return false;
                } else if (
                    selectedProduct.qty == '' ||
                    !$('#curPackItemQty').valid() ||
                    isNaN($('#curPackItemQty').val())
                ) {
                ...
                divContent +=
                    '<li class="product-pack-item media-product-pack" data-product-name="' +
                    ...
                    selectedProduct.id_product_attribute +
                    '">';
                divContent +=
                    '<img class="media-product-pack-img" src="' +
                ...
                divContent +=
                    ...
                    '</span>';
                divContent +=
                    '<span class="media-product-pack-ref">REF: ' +
                    selectedProduct.ref +
                    '</span>';
                divContent +=
                    '<span class="media-product-pack-quantity"><span class="text-muted">x</span> ' +
                    selectedProduct.qty +
                    '</span>';
                divContent +=
                    '<button type="button" class="btn btn-default delPackItem media-product-pack-action" data-delete="' +
                    selectedProduct.id +
                    '" data-delete-attr="' +
                    selectedProduct.id_product_attribute +
                ...
                var line =
                    ...
                    selectedProduct.id_product_attribute;
                var lineDisplay =
                    selectedProduct.qty + 'x ' + selectedProduct.name;

                $('#divPackItems').html(divContent);
                $('#inputPackItems').val(
                    $('#inputPackItems').val() + line + '-',
                );
                $('#namePackItems').val(
                    $('#namePackItems').val() + lineDisplay + '¤',
                );
        ...
        function delPackItem(id, id_attribute) {
            ...
            var elem = $(
                '.product-pack-item[data-product-id="' +
                    id +
                    '"][data-product-id-attribute="' +
                    id_attribute +
                    '"]',
...
product_tabs['Quantities'] = new function() {
    ...
    this.onReady = function() {
        ...
        if (display_multishop_checkboxes) ProductMultishop.checkAllQuantities();

        $('.pack_stock_type').click(function(e) {
            self.refreshQtyAvailabilityForm();
            self.ajaxCall({
                actionQty: 'pack_stock_type',
                value: $(this).val(),
            });
        });
...
product_tabs['VirtualProduct'] = new function() {
    ...
    this.onReady = function() {
        ...
        else $('#is_virtual_file_product').hide();

        $('input[name=is_virtual_file]').on('change', function(e) {
            if ($(this).val() == 1) $('#is_virtual_file_product').show();
            else $('#is_virtual_file_product').hide();
        });

        // Bind file deletion
        $('#product-tab-content-VirtualProduct').delegate(
            'a.delete_virtual_product',
            'click',
            function(e) {
                e.preventDefault();
...
var ProductMultishop = new function() {
        ...
        switch (type) {
            case 'tinymce':
                ...
                if (typeof self.load_tinymce[id] == 'undefined')
                    ...
                    if (checked) tinyMCE.get(id).hide();
                    else tinyMCE.get(id).show();
                }
                break;
            case 'radio':
    ...
    this.checkAllInformations = function() {
        ProductMultishop.checkField(
            $("input[name='multishop_check[active]']").prop('checked'),
            'active',
            'radio',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[visibility]']").prop('checked'),
            'visibility',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[available_for_order]']").prop(
                'checked',
            ),
            'available_for_order',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[show_price]']").prop('checked'),
            'show_price',
            'show_price',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[online_only]']").prop('checked'),
            'online_only',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[condition]']").prop('checked'),
            'condition',
        );
        $.each(languages, function(k, v) {
            ProductMultishop.checkField(
                $(
                    "input[name='multishop_check[name][" + v.id_lang + "]']",
                ).prop('checked'),
                'name_' + v.id_lang,
            );
            ProductMultishop.checkField(
                $(
                    "input[name='multishop_check[description_short][" +
                        v.id_lang +
                        "]']",
                ).prop('checked'),
                'description_short_' + v.id_lang,
                'tinymce',
            );
            ProductMultishop.checkField(
                $(
                    "input[name='multishop_check[description][" +
                        v.id_lang +
                        "]']",
                ).prop('checked'),
                'description_' + v.id_lang,
                'tinymce',
            );
        });
    };
    ...
    this.checkAllPrices = function() {
        ProductMultishop.checkField(
            $("input[name='multishop_check[wholesale_price]']").prop('checked'),
            'wholesale_price',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[price]']").prop('checked'),
            'price',
            'price',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[id_tax_rules_group]']").prop(
                'checked',
            ),
            'id_tax_rules_group',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[unit_price]']").prop('checked'),
            'unit_price',
            'unit_price',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[on_sale]']").prop('checked'),
            'on_sale',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[ecotax]']").prop('checked'),
            'ecotax',
        );
    };
    ...
    this.checkAllSeo = function() {
        $.each(languages, function(k, v) {
            ProductMultishop.checkField(
                $(
                    "input[name='multishop_check[meta_title][" +
                        v.id_lang +
                        "]']",
                ).prop('checked'),
                'meta_title_' + v.id_lang,
            );
            ProductMultishop.checkField(
                $(
                    "input[name='multishop_check[meta_description][" +
                        v.id_lang +
                        "]']",
                ).prop('checked'),
                'meta_description_' + v.id_lang,
            );
            ProductMultishop.checkField(
                $(
                    "input[name='multishop_check[meta_keywords][" +
                        v.id_lang +
                        "]']",
                ).prop('checked'),
                'meta_keywords_' + v.id_lang,
            );
            ProductMultishop.checkField(
                $(
                    "input[name='multishop_check[link_rewrite][" +
                        v.id_lang +
                        "]']",
                ).prop('checked'),
                'link_rewrite_' + v.id_lang,
                'seo_friendly_url',
            );
        });
    };
    ...
    this.checkAllQuantities = function() {
        $.each(languages, function(k, v) {
            ProductMultishop.checkField(
                $("input[name='multishop_check[minimal_quantity]']").prop(
                    'checked',
                ),
                'minimal_quantity',
            );
            ProductMultishop.checkField(
                $("input[name='multishop_check[low_stock_threshold]']").prop(
                    'checked',
                ),
                'low_stock_threshold',
            );
            ProductMultishop.checkField(
                $("input[name='multishop_check[low_stock_alert]']").prop(
                    'checked',
                ),
                'low_stock_alert',
            );
            ProductMultishop.checkField(
                $(
                    "input[name='multishop_check[available_later][" +
                        v.id_lang +
                        "]']",
                ).prop('checked'),
                'available_later_' + v.id_lang,
            );
            ProductMultishop.checkField(
                $(
                    "input[name='multishop_check[available_now][" +
                        v.id_lang +
                        "]']",
                ).prop('checked'),
                'available_now_' + v.id_lang,
            );
            ProductMultishop.checkField(
                $("input[name='multishop_check[available_date]']").prop(
                    'checked',
                ),
                'available_date',
            );
    ...
    this.checkAllAssociations = function() {
        ProductMultishop.checkField(
            $("input[name='multishop_check[id_category_default]']").prop(
                'checked',
            ),
            'id_category_default',
        );
    ...
    this.checkAllCustomization = function() {
        ProductMultishop.checkField(
            $("input[name='multishop_check[uploadable_files]']").prop(
                'checked',
            ),
            'uploadable_files',
            'uploadable_files',
        ...
        ProductMultishop.checkField(
            $("input[name='multishop_check[text_fields]']").prop('checked'),
            'text_fields',
            'text_fields',
        );
    };
    ...
    this.checkAllCombinations = function() {
        ProductMultishop.checkField(
            $("input[name='multishop_check[attribute_wholesale_price]']").prop(
                'checked',
            ),
            'attribute_wholesale_price',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[attribute_price_impact]']").prop(
                'checked',
            ),
        ...
        ProductMultishop.checkField(
            $("input[name='multishop_check[attribute_weight_impact]']").prop(
                'checked',
            ),
        ...
        ProductMultishop.checkField(
            $("input[name='multishop_check[attribute_unit_impact]']").prop(
                'checked',
            ),
        ...
        ProductMultishop.checkField(
            $("input[name='multishop_check[attribute_ecotax]']").prop(
                'checked',
            ),
            'attribute_ecotax',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[attribute_minimal_quantity]']").prop(
                'checked',
            ),
        ...
        ProductMultishop.checkField(
            $(
                "input[name='multishop_check[attribute_low_stock_threshold]']",
            ).prop('checked'),
            'attribute_low_stock_threshold',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[attribute_low_stock_alert]']").prop(
                'checked',
            ),
            'attribute_low_stock_alert',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[available_date_attribute]']").prop(
                'checked',
            ),
            'available_date_attribute',
        );
        ProductMultishop.checkField(
            $("input[name='multishop_check[attribute_default]']").prop(
                'checked',
            ),
            'attribute_default',
        );
...
$(document).ready(function() {
    ...
    tabs_manager.init();
    updateCurrentText();
    $('#name_' + id_lang_default + ',#link_rewrite_' + id_lang_default).on(
        'change',
        function(e) {
