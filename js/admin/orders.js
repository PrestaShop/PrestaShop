...
function updateInvoice(invoices) {
    ...
    $('.edit_product_invoice').each(function() {
        ...
        for (i in invoices) {
            // Create new option
            var option = $('<option>' + invoices[i].name + '</option>').attr(
                'value',
                invoices[i].id,
            );
            if (invoices[i].id == selected) option.attr('selected', true);
...
function populateWarehouseList(warehouse_list) {
    ...
    var order_warehouse_list = $('#warehouse_list')
        .val()
        .split(',');
    ...
    $.each(warehouse_list, function() {
        if (
            ...
            $.inArray(this.id_warehouse, order_warehouse_list)
        )
            warehouse_selected = this.id_warehouse;

        $('#add_product_warehouse').append(
            $(
                '<option value="' +
                    this.id_warehouse +
                    '">' +
    ...
    if (warehouse_selected) $('#add_product_warehouse').val(warehouse_selected);
}
...
function addProductRefreshTotal() {
    ...
    else var price = parseFloat($('#add_product_product_price_tax_excl').val());
...
function editProductRefreshTotal(element) {
    ...
    if (element.hasClass('customized')) {
        var element_list = $(
            '.customized-' +
                element.find('.edit_product_id_order_detail').val(),
        );
    ...
    if (use_taxes)
        var price = parseFloat(
            element.find('td .edit_product_price_tax_incl').val(),
        );
    else
        var price = parseFloat(
            element.find('td .edit_product_price_tax_excl').val(),
        );
    ...
    if (element_list.length) {
        ...
        $.each(element_list, function(i, elm) {
            if ($(elm).find('.edit_product_quantity').length) {
                qty += parseInt(
                    $(elm)
                        .find('.edit_product_quantity')
                        .val(),
                );
                subtotal = makeTotalProductCaculation(
                    $(elm)
                        .find('.edit_product_quantity')
                        .val(),
                    price,
...
function addViewOrderDetailRow(view) {
    ...
    html.find('td').each(function() {
        if (!$(this).is('.product_invoice')) $(this).fadeIn('slow');
    });
}
...
function refreshProductLineView(element, view) {
    ...
    if (
        element
            .parent()
            .parent()
            .find('.edit_product_id_order_detail').length
    )
        var element_list = $(
            '.customized-' +
                element
                    .parent()
                    .parent()
                    .find('.edit_product_id_order_detail')
                    .val(),
        );
    if (!element_list.length) element_list = $(element.parent().parent());
...
function updateAmounts(order) {
    $('#total_products td.amount').fadeOut('slow', function() {
        formatCurrencyCldr(parseFloat(order.total_products_wt), function(
            value,
        ) {
    ...
    $('#total_discounts td.amount').fadeOut('slow', function() {
        formatCurrencyCldr(parseFloat(order.total_discounts_tax_incl), function(
            value,
        ) {
    ...
    $('#total_wrapping td.amount').fadeOut('slow', function() {
        formatCurrencyCldr(parseFloat(order.total_wrapping_tax_incl), function(
            value,
        ) {
    ...
    $('#total_shipping td.amount').fadeOut('slow', function() {
        formatCurrencyCldr(parseFloat(order.total_shipping_tax_incl), function(
            value,
        ) {
    ...
    $('#total_order td.amount').fadeOut('slow', function() {
        formatCurrencyCldr(parseFloat(order.total_paid_tax_incl), function(
            value,
        ) {
    ...
    var shippingCarrierPrice = $(
        '#shipping_table .price_carrier_' + order.id_carrier + ' span',
    );
...
function closeAddProduct() {
    ...
    if (stock_management) $('#add_product_product_stock').html('0');
...
function init() {
    ...
    $('#add_product')
        ...
        .click(function(e) {
            ...
            $(
                '.edit_product_fields, .standard_refund_fields, .partial_refund_fields, .order_action',
            ).hide();
    ...
    $('#add_product_product_name')
        ...
        .result(function(event, data, formatted) {
            if (!data) {
                $('tr#new_product input, tr#new_product select').each(
                    function() {
                        if ($(this).attr('id') != 'add_product_product_name')
                            $(
                                'tr#new_product input, tr#new_product select, tr#new_product button',
                            ).attr('disabled', true);
                    },
                );
            } else {
                ...
                if (current_product.combinations.length !== 0) {
                    ...
                    $.each(current_product.combinations, function() {
                        $('select#add_product_product_attribute_id').append(
                            '<option value="' +
                    ...
                    populateWarehouseList(
                        current_product.warehouse_list[defaultAttribute],
                    );
                } else {
                    // Reset combinations list
                    $('select#add_product_product_attribute_id').html('');
    ...
    $('select#add_product_product_attribute_id').change(function() {
        ...
        if (stock_management)
            $('#add_product_product_stock').html(
                current_product.combinations[$(this).val()].qty_in_stock,
            );
    });

    $('input#add_product_product_quantity')
        .unbind('keyup')
        .keyup(function() {
            if (stock_management) {
    ...
    $('#submitAddProduct')
        ...
        .click(function(e) {
            ...
            if (go) {
                if (
                    parseInt($('input#add_product_product_quantity').val()) >
                    parseInt($('#add_product_product_stock').html())
                )
                ...
                if (go) {
                    var query =
                        'ajax=1&token=' +
                        token +
                        '&action=addProductOnOrder&id_order=' +
                        id_order +
                        '&';

                    query += $('#add_product_warehouse').serialize() + '&';
                    query += $(
                        'tr#new_product select, tr#new_product input',
                    ).serialize();
                    if ($('select#add_product_product_invoice').val() == 0)
                        query +=
                            '&' +
                            $(
                                'tr#new_invoice select, tr#new_invoice input',
                            ).serialize();
    ...
    $('.edit_shipping_number_link')
        .unbind('click')
        .click(function(e) {
            $(this)
                .parent()
                .parent()
                .find('.shipping_number_show')
                .hide();
            $(this)
                .parent()
                .find('.shipping_number_edit')
                .show();

            $(this)
                .parent()
                .find('.edit_shipping_number_link')
                .hide();
            $(this)
                .parent()
                .find('.cancel_shipping_number_link')
                .show();
            e.preventDefault();
        });

    $('.cancel_shipping_number_link')
        .unbind('click')
        .click(function(e) {
            $(this)
                .parent()
    ...
    $('#add_product_product_invoice')
        .unbind('change')
        .change(function() {
            if ($(this).val() == '0') $('#new_invoice').slideDown('slow');
            else $('#new_invoice').slideUp('slow');
        });

    $('#add_product_product_price_tax_excl')
        .unbind('keyup')
        .keyup(function() {
            var price_tax_excl = parseFloat($(this).val());
            if (price_tax_excl < 0 || isNaN(price_tax_excl)) price_tax_excl = 0;

            var tax_rate = current_product.tax_rate / 100 + 1;
            $('#add_product_product_price_tax_incl').val(
                ps_round(price_tax_excl * tax_rate, 2),
            );
    ...
    $('#add_product_product_price_tax_incl')
        .unbind('keyup')
        .keyup(function() {
            var price_tax_incl = parseFloat($(this).val());
            if (price_tax_incl < 0 || isNaN(price_tax_incl)) price_tax_incl = 0;

            var tax_rate = current_product.tax_rate / 100 + 1;
            $('#add_product_product_price_tax_excl').val(
                ps_round(price_tax_incl / tax_rate, 2),
            );
    ...
    $('.edit_product_change_link')
        ...
        .click(function(e) {
            $(
                '.add_product_fields, .standard_refund_fields, .order_action',
            ).hide();
            ...
            $.ajax({
                ...
                data: {
                    ...
                    id_order_detail: element
                        .closest('tr.product-line-row')
                        .find('input.edit_product_id_order_detail')
                        .val(),
                    id_address: id_address,
                    id_order: id_order,
                },
                success: function(data) {
                    if (data.result) {
                        ...
                        element_list
                            .find('td.product_action')
                            .attr('colspan', 1);
                        element
                            .parent()
                            .children('.edit_product_change_link')
                            .parent()
                            .hide();
                        element
                            .parent()
                            .parent()
                            .find('button.submitProductChange')
                            .show();
                        element
                            .parent()
                            .parent()
                            .find('.cancel_product_change_link')
                            .show();
        ...
        .click(function(e) {
            ...
            if (confirm(txt_confirm)) {
                ...
                query =
                    'ajax=1&token=' +
                    token +
                    '&action=editProductOnOrder&id_order=' +
                    id_order +
                    '&';
                if (element_list.length)
                    query += element_list
                        .find(
                            'input:visible, select:visible, .edit_product_id_order_detail',
                        )
                        .serialize();
                else
                    query += element
    ...
    $('.edit_product_price_tax_excl')
        .unbind('keyup')
        .keyup(function() {
            var price_tax_excl = parseFloat($(this).val());
            if (price_tax_excl < 0 || isNaN(price_tax_excl)) price_tax_excl = 0;
            var tax_rate = current_product.tax_rate / 100 + 1;
            $('.edit_product_price_tax_incl:visible').val(
                ps_round(price_tax_excl * tax_rate, 2),
            );
            // Update total product
    ...
    $('.edit_product_price_tax_incl').keyup(function() {
        ...
        var tax_rate = current_product.tax_rate / 100 + 1;
        $('.edit_product_price_tax_excl:visible').val(
            ps_round(price_tax_incl / tax_rate, 2),
        );
        // Update total product
        editProductRefreshTotal($(this));
    ...
    $('.delete_product_line')
        .unbind('click')
        .click(function(e) {
            if (!confirm(txt_confirm)) return false;
            var tr_product = $(this).closest('.product-line-row');
            var id_order_detail = $(this)
                .closest('.product-line-row')
                .find('td .edit_product_id_order_detail')
                .val();
            var query =
                'ajax=1&action=deleteProductLine&token=' +
                token +
                '&id_order_detail=' +
                id_order_detail +
                '&id_order=' +
    ...
    $('.js-set-payment')
        ...
        .click(function(e) {
            ...
            $(
                'select[name=payment_invoice] option[value=' + id_invoice + ']',
            ).attr('selected', true);
            e.preventDefault();
        });
    ...
    $('#cancel_add_voucher')
        ...
        .click(function(e) {
            $('#voucher_form').hide();
            if (!has_voucher) $('.panel-vouchers').hide();
            $('.order_action').show();
            e.preventDefault();
        });
    ...
    $('#discount_all_invoices')
        .unbind('change')
        .change(function() {
            if ($(this).is(':checked'))
                $('select[name=discount_invoice]').attr('disabled', true);
            else $('select[name=discount_invoice]').attr('disabled', false);
        });
...
$(document).ready(function() {
    $('#desc-order-standard_refund').click(function() {
        ...
        if (order_discount_price) actualizeTotalRefundVoucher();
    });
    ...
    $('#desc-order-partial_refund').click(function() {
        ...
        if (order_discount_price) actualizeRefundVoucher();
    });
});
...
function checkPartialRefundProductQuantity(it) {
    if (
        ...
        parseInt(
            $(it)
                .closest('td')
                .find('.partialRefundProductQuantity')
                .val(),
        )
    )
        $(it).val(
...
function checkPartialRefundProductAmount(it) {
    var old_price = $(it)
        .closest('td')
        .find('.partialRefundProductAmount')
        .val();
    if (
        typeof $(it).val() !== undefined &&
        typeof new_price !== undefined &&
...
function actualizeRefundVoucher() {
    ...
    $('.edit_product_price_tax_incl.edit_product_price').each(function() {
        quantity_refund_product = parseFloat(
            $(this)
                .closest('td')
                .parent()
                .find('td.partial_refund_fields.current-edit')
                .find(
                    'input[onchange="checkPartialRefundProductQuantity(this)"]',
                )
                .val(),
        );
        if (quantity_refund_product > 0) {
            current_amount = parseFloat(
                $(this)
                    ...
                    .find('td.partial_refund_fields.current-edit')
                    .find(
                        'input[onchange="checkPartialRefundProductAmount(this)"]',
                    )
                    .val(),
            )
                ? parseFloat(
    ...
    $('#lab_refund_1').append(
        '<input type="hidden" name="order_discount_price" value=' +
    ...
    if (parseFloat(total - order_discount_price) > 0.0) {
        ...
        $('#lab_refund_2').append(
            '<span id="total_refund_2">' +
                (total - order_discount_price) +
                '</span>',
        );
        ...
        $('#lab_refund_2').append(
            '<span id="total_refund_2">' + errorRefund + '</span>',
        );
    }
}
    ...
    $('.edit_product_price_tax_incl.edit_product_price').each(function() {
        quantity_refund_product = parseFloat(
            $(this)
                .closest('td')
                .parent()
                .find('td.cancelQuantity')
                .children()
                .val(),
        );
        if (
            typeof quantity_refund_product !== 'undefined' &&
            quantity_refund_product > 0
        )
            total += $(this).val() * quantity_refund_product;
    });
    $('#total_refund_1').remove();
    $('#lab_refund_total_1').append(
        '<span id="total_refund_1">' + total + '</span>',
    );
    formatCurrencyCldr(total, function(value) {
        $('#total_refund_1').html(value);
    });
...
function setCancelQuantity(itself, id_order_detail, quantity) {
    $('#cancelQuantity_' + id_order_detail).val(
        $(itself).prop('checked') ? quantity : '',
    );
    if (order_discount_price) actualizeTotalRefundVoucher();
}
...
function checkTotalRefundProductQuantity(it) {
    $(it)
        .parent()
        .parent()
        .find('td.cancelCheck input[type=checkbox]')
        .attr('checked', true);
    if (
        parseInt($(it).val()) >
        parseInt(
