...
function refreshDashboard(module_name, use_push, extra) {
    ...
    for (var module_id in module_list) {
        if (
            use_push &&
            !$('#' + module_list[module_id]).hasClass('allow_push')
        ) {
            continue;
        }
...
function setDashboardDateRange(action) {
    ...
    var data =
        'ajax=true&action=setDashboardDateRange&submitDatePicker=true&' +
        $('#calendar_form').serialize() +
        '&' +
        action +
        '=1';
    $.ajax({
...
function data_trends(widget_name, data) {
    for (var data_id in data) {
        ...
        if (data[data_id].way === 'up') {
            this.el
                .parent()
                .removeClass('dash_trend_down')
                .removeClass('dash_trend_right')
                .addClass('dash_trend_up');
        } else if (data[data_id].way === 'down') {
            this.el
                .parent()
                .removeClass('dash_trend_up')
                .removeClass('dash_trend_right')
                .addClass('dash_trend_down');
        } else {
            this.el
                .parent()
                .removeClass('dash_trend_down')
                .removeClass('dash_trend_up')
                .addClass('dash_trend_right');
        }
        ...
        if (typeof data[data_id].body === 'string') {
            ...
            for (var body_content_id in data[data_id].body) {
                ...
                for (var body_content in data[data_id].body[body_content_id]) {
                    var body =
                        data[data_id].body[body_content_id][body_content];
                    var td =
                        '<td ' +
                        ...
                        (body.id ? ' id="' + body.id + '" ' : '') +
                        '>';
                    td += body.wrapper_start
                        ? ' ' + body.wrapper_start + ' '
                        : '';
                    td += body.value;
                    td += body.wrapper_stop
                        ? ' ' + body.wrapper_stop + ' '
                        : '';
                    td += '</td>';
                    tr += td;
...
function data_list_small(widget_name, data) {
    for (var data_id in data) {
        ...
        for (var item in data[data_id]) {
            $('#' + data_id).append(
                '<li><span class="data_label">' +
                    item +
                    '</span><span class="data_value size_s">' +
                    data[data_id][item] +
                    '</span></li>',
            );
        }
        ...
        success: function(jsonData) {
            if (
                ...
                for (var article in jsonData.rss) {
                    var article_html =
                        '<article><h4><a href="' +
                        jsonData.rss[article].link +
                        '" class="_blank" onclick="return !window.open(this.href);">' +
                        jsonData.rss[article].title +
                        '</a></h4><span class="dash-news-date text-muted">' +
    ...
    if ($('#' + widget + ' section.dash_config').hasClass('hide')) {
        $('#' + widget + ' section')
            ...
            .slideUp(500, function() {
                $('#' + widget + ' section.dash_config')
                    .fadeIn(500)
                    .removeClass('hide');
                if (window[func_name] != undefined) window[func_name]();
            });
    } else {
...
function saveDashConfig(widget_name) {
    ...
    data =
        'ajax=true&action=saveDashConfig&module=' +
        widget_name +
        configs +
        '&hook=' +
        $('#' + widget_name)
    ...
    $.ajax({
        ...
        success: function(jsonData) {
            if (!jsonData.has_errors) {
                ...
                errors_str += '</div>';
                $('section#' + widget_name + '_config header').after(
                    errors_str,
                );
                errors_str += '</div>';
            }
        ...
        .click(function(e) {
            $.ajax({
                ...
                data: {
                    ...
                    PS_DASHBOARD_SIMULATION: $(this)
                        .find('i')
                        .hasClass('process-icon-toggle-on')
                        ? 0
                        : 1,
                },
                success: function(result) {
                    if (
                        $(
                            '#page-header-desc-configuration-switch_demo i',
                        ).hasClass('process-icon-toggle-on')
                    ) {
                        $('#page-header-desc-configuration-switch_demo i')
                            .removeClass('process-icon-toggle-on')
                            .addClass('process-icon-toggle-off');
                    } else {
                        $('#page-header-desc-configuration-switch_demo i')
                            .removeClass('process-icon-toggle-off')
                            .addClass('process-icon-toggle-on');
                    }
                    refreshDashboard(false, false);
                },
            });
