...
function formatPrice(price) {
    ...
    return Math.round(fixedToSix) == fixedToSix + 0.000001
        ? fixedToSix + 0.000001
        : fixedToSix;
}
...
function calcPriceTI() {
    ...
    if (isNaN(parseFloat($('#priceTI').val()))) {
        ...
        $('#priceTI').val(
            (parseFloat($('#priceTI').val()) + getEcotaxTaxIncluded()).toFixed(
                priceDisplayPrecision,
            ),
        );
        $('#finalPrice').html(
            parseFloat($('#priceTI').val()).toFixed(priceDisplayPrecision),
        );
    }
}
...
function calcPriceTE() {
    ...
    var priceTI = parseFloat(
        document.getElementById('priceTI').value.replace(/,/g, '.'),
    );
    var newPrice = removeTaxes(
        ps_round(priceTI - getEcotaxTaxIncluded(), priceDisplayPrecision),
    );
    ...
    document.getElementById('priceTEReal').value =
        isNaN(newPrice) == true || newPrice < 0 ? 0 : ps_round(newPrice, 9);
    document.getElementById('finalPrice').innerHTML =
        isNaN(newPrice) == true || newPrice < 0
            ? ''
            : ps_round(priceTI, priceDisplayPrecision).toFixed(
                  priceDisplayPrecision,
              );
    document.getElementById('finalPriceWithoutTax').innerHTML =
        isNaN(newPrice) == true || newPrice < 0
...
function calcImpactPriceTI() {
    var priceTE = parseFloat(
        document
            .getElementById('attribute_priceTEReal')
            .value.replace(/,/g, '.'),
    );
    var newPrice = addTaxes(priceTE);
    $('#attribute_priceTI').val(
        isNaN(newPrice) == true || newPrice < 0
            ? ''
            : ps_round(newPrice, priceDisplayPrecision).toFixed(
                  priceDisplayPrecision,
    ...
    if (isNaN(total) || total < 0) $('#attribute_new_total_price').html('0.00');
    else $('#attribute_new_total_price').html(total);
}

function calcImpactPriceTE() {
    var priceTI = parseFloat(
        document.getElementById('attribute_priceTI').value.replace(/,/g, '.'),
    );
...
function calcReduction() {
    ...
    else if (parseFloat($('#reduction_percent').val()) > 0) reductionPercent();
}
...
function reductionPrice() {
    ...
    newprice.innerHTML = (
        ps_round(parseFloat(curPrice), priceDisplayPrecision) +
        getEcotaxTaxIncluded()
    ).toFixed(priceDisplayPrecision);
    var rpriceWithoutTaxes = ps_round(
        removeTaxes(rprice.value),
        priceDisplayPrecision,
    );
    newpriceWithoutTax.innerHTML = ps_round(
        priceWhithoutTaxes.value - rpriceWithoutTaxes,
        priceDisplayPrecision,
    ).toFixed(priceDisplayPrecision);
}
...
function reductionPercent() {
    ...
    if (isInReductionPeriod()) {
        ...
        if (parseFloat(rpercent.value) >= 100) rpercent.value = 100;
        if (parseFloat(rpercent.value) < 0) rpercent.value = 0;
    ...
    newpriceWithoutTax.innerHTML = ps_round(
        parseFloat(removeTaxes(ps_round(curPrice, priceDisplayPrecision))),
        priceDisplayPrecision,
    ).toFixed(priceDisplayPrecision);
}
...
function unitPriceWithTax(type) {
    var priceWithTax = parseFloat(
        document.getElementById(type + '_price').value.replace(/,/g, '.'),
    );
    var newPrice = addTaxes(priceWithTax);
    $('#' + type + '_price_with_tax').html(
        isNaN(newPrice) == true || newPrice < 0
            ? '0.00'
            : ps_round(newPrice, priceDisplayPrecision).toFixed(
                  priceDisplayPrecision,
...
function changeCurrencySpecificPrice(index) {
    ...
    if (id_currency > 0)
        $('#sp_reduction_type option[value="amount"]').text(
            $(
                '#spm_currency_' +
                    index +
                    ' option[value= ' +
    ...
    if (
        currencies[id_currency]['format'] == 2 ||
        currencies[id_currency]['format'] == 4
    ) {
        $('#spm_currency_sign_pre_' + index).html('');
        $('#spm_currency_sign_post_' + index).html(
            ' ' + currencies[id_currency]['sign'],
        );
    } else if (
        currencies[id_currency]['format'] == 1 ||
        currencies[id_currency]['format'] == 3
    ) {
        $('#spm_currency_sign_post_' + index).html('');
        $('#spm_currency_sign_pre_' + index).html(
            currencies[id_currency]['sign'] + ' ',
