...
function calcImpactPriceTI() {
	...
	var total = ps_round((parseFloat($('#attribute_priceTI').val()) * parseInt($('#attribute_price_impact').val(), 10) + parseFloat($('#finalPrice').html())), 2);
...
function reductionPrice() {
	...
	if (isInReductionPeriod()) {
		rprice = document.getElementById('reduction_price');
