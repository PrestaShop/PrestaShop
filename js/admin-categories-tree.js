...
function closeChildrenCategories(element) {
	...
	for (var i = arrayLevel.length - 1; i >= 0; i--) {
		if (arrayLevel[i] !== undefined) {
			for (var j = 0; j < arrayLevel[i].length; j++) {
...
function updateNbSubCategorySelected(category, add) {
	...
	var newValue = parseInt(parentNbSubCategorySelected, 10);
	...
	if (add) {
		newValue = parseInt(parentNbSubCategorySelected, 10) + 1;
	} else {
		newValue = parseInt(parentNbSubCategorySelected, 10) - 1;
	}
