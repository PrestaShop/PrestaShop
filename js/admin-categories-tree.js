...
function buildTreeView() {
    ...
    disabled = $('#categories-treeview li:first input[type=checkbox]').attr(
        'disabled',
    );
    ...
    $('.expandable').click(function() {
        ...
        $('#categories-treeview input[type=checkbox]').attr(
            'disabled',
            disabled,
        );
    });
...
function callbackToggle(element) {
    if (!element.is('.expandable')) return false;
...
function closeChildrenCategories(element) {
    ...
    if (element.children('ul').find('li.collapsable').length == 0) return false;
    ...
    element
        ...
        .each(function() {
            var level = $(this)
                .children('span.category_level')
                .html();
            if (arrayLevel[level] == undefined) arrayLevel[level] = new Array();
    ...
    for (i = arrayLevel.length - 1; i >= 0; i--)
        if (arrayLevel[i] != undefined)
            for (j = 0; j < arrayLevel[i].length; j++) {
                $('#categories-treeview')
                    .find('li#' + arrayLevel[i][j] + '.collapsable')
                    .children('span.category_label')
                    .trigger('click');
                $('#categories-treeview')
                    .find('li#' + arrayLevel[i][j] + '.expandable')
                    .children('ul')
                    .hide();
            }
}
...
function expandAllCategories() {
    ...
    if (!needExpandAllCategories()) return;
    // force to open main category
    if ($('li#' + id_category_root).is('.expandable'))
        $('li#' + id_category_root)
            .children('span.folder')
            .trigger('click');
    readyToExpand = true;
    if (setCategoryToExpand()) interval = setInterval(openCategory, 10);
}
...
function openCategory() {
    ...
    if (id_tree >= arrayCatToExpand.length && readyToExpand) {
        ...
        } else readyToExpand = true;
    }

    if (readyToExpand) {
        if (
            $('#categories-treeview').find(
                'li#' + arrayCatToExpand[id_tree] + '.hasChildren',
            ).length > 0
        )
            readyToExpand = false;
        $('#categories-treeview')
            .find(
                'li#' +
                    arrayCatToExpand[id_tree] +
                    '.expandable:visible span.category_label',
            )
            .trigger('click');
        id_tree++;
    }
...
function checkAllCategories() {
    if (needExpandAllCategories()) expandAllCategories();
    else {
        $('input[name="categoryBox[]"]')
            .not(':checked')
            .each(function() {
...
function clickOnCategoryBox(category) {
    if (category.is(':checked')) {
        ...
        $(
            'select#id_category_default option[value=' + category.val() + ']',
        ).remove();
...
function updateNbSubCategorySelected(category, add) {
    var currentSpan = category
        .parent()
        .parent()
        .parent()
        .children('.nb_sub_cat_selected');
    var parentNbSubCategorySelected = currentSpan
        .children('.nb_sub_cat_selected_value')
        .html();
    ...
    if (add) var newValue = parseInt(parentNbSubCategorySelected) + 1;
    else var newValue = parseInt(parentNbSubCategorySelected) - 1;
    ...
    if (newValue == 0) currentSpan.hide();
    else currentSpan.show();

    if (currentSpan.parent().children('.nb_sub_cat_selected').length != 0)
        updateNbSubCategorySelected(
            currentSpan.parent().children('input'),
            add,
        );
}
...
function searchCategory() {
    ...
    if ($('#search_cat').length) {
        $('#search_cat')
            .autocomplete('ajax.php?searchCategory=1', {
                delay: 100,
                minChars: 3,
                autoFill: true,
                max: 20,
...
function openParentCategories() {
    ...
    if (readyToExpand) {
        ...
        $('li#' + arrayCatToExpand[id_tree] + '.expandable span').trigger(
            'click',
        );
        id_tree++;
    }
...
function checkCategory() {
    if (
        $('li#' + category_to_check + ' > input[type=checkbox]').prop('checked')
    ) {
        ...
        updateNbSubCategorySelected(
            $('li#' + category_to_check + ' > input[type=checkbox]'),
            true,
        );
    }
}
