...
class TaxRulesTaxManagerCoreTest extends UnitTestCase
{
    ...
    public function testGetTaxCalculatorShouldUseFirstComputationMethodFromTaxes()
    {
        ...
        // Then
        $this->assertSame(TaxCalculator::COMBINE_METHOD, $tax_calculator->computation_method);
        ...
        foreach ($tax_calculator->taxes as $key => $tax) {
            ...
            $this->assertSame($this->tax_rows[$key]['id_tax'], $tax->id);
            $this->assertSame($this->tax_rows[$key]['rate'], $tax->rate);
        }
    }
}
