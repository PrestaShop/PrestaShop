...
class Core_Foundation_Database_EntityManager_QueryBuilder_Test extends UnitTestCase
{
    ...
    {
        $this->assertSame('escaped', $this->queryBuilder->quote(42));
        $this->assertSame('escaped', $this->queryBuilder->quote(4.2));
    }

    public function testQuoteStringQuoted()
    {
        $this->assertSame('\'escaped\'', $this->queryBuilder->quote('hello'));
    }

    public function testBuildWhereConditionsANDJustOneCondition()
    {
        $this->assertSame("name = 'escaped'", $this->queryBuilder->buildWhereConditions('AND', array(
            'name' => 'some string',
        )));
    }
    ...
    {
        $this->assertSame("name = 'escaped' AND num = escaped", $this->queryBuilder->buildWhereConditions('AND', array(
            'name' => 'some string',
            'num' => 123456,
        )));
    }
    ...
    {
        $this->assertSame("stuff IN ('escaped', escaped, escaped)", $this->queryBuilder->buildWhereConditions('AND', array(
