...
class CartControllerCore extends FrontController
{
    ...
    public function displayAjaxUpdate()
    {
        ...
        $productQuantity = $updatedProduct['quantity'] ?? false;
    ...
    protected function processDeleteProductInCart()
    {
        ...
        if (count($customization_product)) {
            ...
            if ($this->id_product_attribute > 0) {
                $minimal_quantity = (int) AttributePs::getAttributeMinimalQty($this->id_product_attribute);
            } else {
            ...
            if (!$this->errors) {
                ...
                if ($update_quantity < 0) {
                    ...
                    $minimal_quantity = ($this->id_product_attribute)
                        ? AttributePs::getAttributeMinimalQty($this->id_product_attribute)
    ...
    protected function shouldAvailabilityErrorBeRaised($product, $qtyToCheck)
    {
        if (($this->id_product_attribute)) {
            return !Product::isAvailableWhenOutOfStock($product->out_of_stock)
                && !AttributePs::checkAttributeQty($this->id_product_attribute, $qtyToCheck);
