...
class ProductControllerCore extends ProductPresentingFrontControllerCore
{
    ...
    protected $redirectionExtraExcludedKeys = array('id_product_attribute', 'rewrite');
    ...
    public function initContent()
    {
        if (!$this->errors) {
            ...
            if (!$priceDisplay || 2 == $priceDisplay) {
    ...
    protected function assignPriceAndTax()
    {
        ...
        foreach ($quantity_discounts as &$quantity_discount) {
            ...
            if (0 == (int) $quantity_discount['id_currency'] && 'amount' == $quantity_discount['reduction_type']) {
                $quantity_discount['reduction'] = Tools::convertPriceFull($quantity_discount['reduction'], null, Context::getContext()->currency);
            }
        }

        $product_price = $this->product->getPrice(PS_TAX_INC == Product::$_taxCalculationMethod, false);
    ...
    protected function assignAttributesGroups($product_for_template = null)
    {
        ...
        if (is_array($attributes_groups) && $attributes_groups) {
            ...
            foreach ($attributes_groups as $k => $row) {
                ...
                if ($row['default_on'] && -1 == $groups[$row['id_attribute_group']]['default']) {
            ...
            if (!Product::isAvailableWhenOutOfStock($this->product->out_of_stock) && 0 == Configuration::get('PS_DISP_UNAVAILABLE_ATTR')) {
                foreach ($groups as &$group) {
    ...
    protected function assignCategory()
    {
        ...
        if ((false === $this->category || !Validate::isLoadedObject($this->category) || !$this->category->inShop() || !$this->category->isAssociatedToShop()) && Category::inShopStatic($this->product->id_category_default, $this->context->shop)) {
        ...
        foreach ($specific_prices as $key => &$row) {
            ...
            if ($row['price'] >= 0) {
                ...
                if (Configuration::get('PS_DISPLAY_DISCOUNT_PRICE')) {
                    if (0 == $row['reduction_tax'] && !$row['price']) {
                ...
                if ('amount' == $row['reduction_type']) {
                    if (PS_TAX_INC == Product::$_taxCalculationMethod) {
                        $row['real_value'] = 1 == $row['reduction_tax'] ? $row['reduction'] : $row['reduction'] * (1 + $tax_rate / 100);
                    } else {
                        $row['real_value'] = 0 == $row['reduction_tax'] ? $row['reduction'] : $row['reduction'] / (1 + $tax_rate / 100);
                    }
