...
class AdminTranslationsControllerCore extends AdminController
{
    ...
    protected function writeTranslationFile($override_file = false)
    {
        ...
        $kpi_key = mb_substr(mb_strtoupper($thm_name . '_' . Tools::getValue('lang')), 0, 16);
    ...
    public function submitCopyLang()
    {
        if (!($from_lang = Tools::getValue('fromLang')) || !($to_lang = Tools::getValue('toLang'))) {
            ...
            if (false == $theme_exists['from_theme'] || false == $theme_exists['to_theme']) {
        ...
        foreach ($items as $source => $dest) {
            ...
            } elseif (mb_strpos($dest, 'modules') && basename($source) === $from_lang . '.php' && false !== $bool) {
    ...
    public function changeModulesKeyTranslation($path, $theme_from, $theme_to)
    {
        ...
        if (preg_match_all('#\$_MODULE\[\'([^\']+)\'\]#Ui', $content, $matches)) {
            ...
            $bool_flag = (false === file_put_contents($path, $content)) ? false : true;
        }
    ...
    public static function checkAndAddMailsFiles($iso_code, $files_list)
    {
        ...
        foreach ($files_list as $file) {
            if (preg_match('#^mails\/([a-z0-9]+)\/#Ui', $file['filename'], $matches)) {
                $slash_pos = mb_strrpos($file['filename'], '/');
                $mails_new_lang[] = mb_substr($file['filename'], -(mb_strlen($file['filename']) - $slash_pos - 1));
            }
        }
        ...
        foreach ($arr_mails_needed as $mail_to_add) {
            if (!in_array($mail_to_add, self::$ignore_folder, true)) {
        ...
        foreach ($modules as $module) {
            if (!in_array($module, self::$ignore_folder, true) && Tools::file_exists_cache(_PS_MODULE_DIR_ . $module . '/mails/' . $default_language . '/')) {
                $arr_files = scandir(_PS_MODULE_DIR_ . $module . '/mails/' . $default_language . '/', SCANDIR_SORT_NONE);

                foreach ($arr_files as $file) {
                    if (!in_array($file, self::$ignore_folder, true)) {
        ...
        foreach ($arr_modules_mails_needed as $file) {
            ...
            $dir_iso_code = mb_substr($file_iso_code, 0, -(mb_strlen($file_iso_code) - mb_strrpos($file_iso_code, '/') - 1));
    ...
    public function checkAndAddThemesFiles($files, $themes_selected)
    {
        foreach ($files as $file) {
            ...
            if (preg_match('#^themes\/([a-z0-9]+)\/lang\/#Ui', $file['filename'], $matches)) {
                $slash_pos = mb_strrpos($file['filename'], '/');
                $name_file = mb_substr($file['filename'], -(mb_strlen($file['filename']) - $slash_pos - 1));
    ...
    public static function checkTranslationFile($content)
    {
        ...
        foreach ($lines as $line) {
            // PHP tags
            if (in_array($line, array('<?php', '?>', ''), true)) {
                continue;
            }
            ...
            if (false != $global && preg_match('/^\$' . preg_quote($global, '/') . '\s*=\s*array\(\s*\)\s*;$/i', $line)) {
                continue;
            }
            ...
            if (Validate::isLangIsoCode($iso_code)) {
                ...
                if ($gz->extractList($files_paths, $sandbox)) {
                    foreach ($files_list as $file2check) {
                        ...
                        if ('index.php' == pathinfo($file2check['filename'], PATHINFO_BASENAME)) {
                            continue;
                        }
                ...
                foreach ($files_paths as $files_path) {
                    ...
                    if (is_dir(_PS_TRANSLATIONS_DIR_ . '../' . $path) && !is_writable(_PS_TRANSLATIONS_DIR_ . '../' . $path) && !in_array($path, $tmp_array, true)) {
                ...
                if ($error = $gz->extractList($files_paths, _PS_TRANSLATIONS_DIR_ . '../')) {
                    if (is_object($error) && !empty($error->message)) {
                        ...
                        foreach ($files_list as $file2check) {
                            if ('index.php' == pathinfo($file2check['filename'], PATHINFO_BASENAME) && file_put_contents(_PS_TRANSLATIONS_DIR_ . '../' . $file2check['filename'], Tools::getDefaultIndexContent())) {
                                continue;
                            }
                        }
                        ...
                        $cldrUpdate->fetchLocale($languageCode[0] . '-' . mb_strtoupper($languageCode[1]));
    ...
    protected function findAndWriteTranslationsIntoFile($file_name, $files, $theme_name, $module_name, $dir = false)
    {
        ...
        foreach ($files as $file) {
            if (preg_match('/^(.*)\.(tpl|php)$/', $file) && Tools::file_exists_cache($dir . $file) && !in_array($file, self::$ignore_folder, true)) {
                // Get content for this file
                $content = file_get_contents($dir . $file);

                // Get file type
                $type_file = '.tpl' == mb_substr($file, -4) ? 'tpl' : 'php';

                // Parse this content
                $matches = $this->userParseFile($content, $this->type_selected, $type_file, $module_name);

                // Write each translation on its module file
                $template_name = mb_substr(basename($file), 0, -4);

                foreach ($matches as $key) {
                    if ($theme_name) {
                        $post_key = md5(mb_strtolower($module_name) . '_' . mb_strtolower($theme_name) . '_' . mb_strtolower($template_name) . '_' . md5($key));
                        $pattern = '\'<{' . mb_strtolower($module_name) . '}' . mb_strtolower($theme_name) . '>' . mb_strtolower($template_name) . '_' . md5($key) . '\'';
                    } else {
                        $post_key = md5(mb_strtolower($module_name) . '_' . mb_strtolower($template_name) . '_' . md5($key));
                        $pattern = '\'<{' . mb_strtolower($module_name) . '}prestashop>' . mb_strtolower($template_name) . '_' . md5($key) . '\'';
                    }

                    if (array_key_exists($post_key, $_POST) && !in_array($pattern, $array_check_duplicate, true)) {
                        if ('' == $_POST[$post_key]) {
                            continue;
                        }
        ...
        if (isset($cache_file[$theme_name . '-' . $file_name]) && "<?php\n\nglobal \$_MODULE;\n\$_MODULE = array();\n" != $str_write) {
    ...
    public function clearModuleFiles($files, $type_clear = 'file', $path = '')
    {
        ...
        foreach ($files as $key => $file) {
            if ('.' === $file[0] || in_array(mb_substr($file, 0, mb_strrpos($file, '.')), $this->all_iso_lang, true)) {
                unset($files[$key]);
            } elseif ('file' === $type_clear && !in_array(mb_substr($file, mb_strrpos($file, '.')), $arr_good_ext, true)) {
                unset($files[$key]);
            } elseif ('directory' === $type_clear && (!is_dir($path . $file) || in_array($file, $arr_exclude, true))) {
                unset($files[$key]);
            }
        }
    ...
    protected function findAndFillTranslations($files, $theme_name, $module_name, $dir = false)
    {
        ...
        foreach ($files as $file) {
            if ((preg_match('/^(.*).tpl$/', $file) || preg_match('/^(.*).php$/', $file)) && Tools::file_exists_cache($file_path = $dir . $file)) {
                ...
                if (false !== mb_strpos($content, 'IGNORE_THIS_FILE_FOR_TRANSLATION')) {
                    continue;
                }
                ...
                foreach ($matches as $key) {
                    $md5_key = md5($key);
                    $module_key = '<{' . Tools::strtolower($module_name) . '}' . mb_strtolower($theme_name) . '>' . Tools::strtolower($template_name) . '_' . $md5_key;
                    $default_key = '<{' . Tools::strtolower($module_name) . '}prestashop>' . Tools::strtolower($template_name) . '_' . $md5_key;
                    // to avoid duplicate entry
                    if (!in_array($module_key, $array_check_duplicate, true)) {
    ...
    public function getInformations()
    {
        ...
        if (($type = Tools::getValue('type')) && !is_array($type)) {
            $this->type_selected = mb_strtolower(Tools::safeOutput($type));
        }
        ...
        if (Tools::getValue('lang') || Tools::getValue('iso_code')) {
            ...
            if (!Validate::isLangIsoCode($iso_code) || !in_array($iso_code, $this->all_iso_lang, true)) {
    ...
    public function renderKpis()
    {
        ...
        if (false !== ConfigurationKPI::get('ENABLED_LANGUAGES')) {
        ...
        if (false !== ConfigurationKPI::get('MAIN_COUNTRY', $this->context->language->id)) {
        ...
        if (false !== ConfigurationKPI::get('FRONTOFFICE_TRANSLATIONS')) {
...
<body>
    ...
    protected function submitTranslationsMails()
    {
        ...
        foreach ($arr_mail_content as $group_name => $all_content) {
            foreach ($all_content as $type_content => $mails) {
                if (!in_array($type_content, self::$content_type_accepted, true)) {
                ...
                foreach ($mails as $mail_name => $content) {
                    $module_name = false;
                    $module_name_pipe_pos = mb_stripos($mail_name, '|');
                    if ($module_name_pipe_pos) {
                        $module_name = mb_substr($mail_name, 0, $module_name_pipe_pos);
                        ...
                        $mail_name = mb_substr($mail_name, $module_name_pipe_pos + 1);
    ...
    public function initFormFront()
    {
        ...
        foreach ($files_by_directory as $file_type => $root_directory) {
            foreach ($root_directory as $dir => $files) {
                $prefix = '';
                if (_PS_THEME_OVERRIDE_DIR_ == $dir) {
                    $prefix = 'override_';
                }

                foreach ($files as $file) {
                    if (preg_match('/^(.*).(tpl|php)$/', $file) && (Tools::file_exists_cache($file_path = $dir . $file))) {
                        $prefix_key = $prefix . mb_substr(basename($file), 0, -4);
                        $new_lang = array();
    ...
    public function initFormBack()
    {
            ...
            foreach ($files as $file) {
                ...
                foreach ($regexSf2Php as $reg) {
                    ...
                    foreach ($matches[0] as $key => $match) {
                        $domainKey = false !== mb_strpos($match, 'trans(') ? 8 : 10;
        ...
        foreach ($files_per_directory['php'] as $dir => $files) {
            foreach ($files as $file) {
                ...
                if (preg_match('/^(.*)\.php$/', $file) && Tools::file_exists_cache($file_path = $dir . $file) && !in_array($file, self::$ignore_folder, true)) {
                    $prefix_key = basename($file);
                    // -4 becomes -14 to remove the ending "Controller.php" from the filename
                    if (false !== mb_strpos($file, 'Controller.php')) {
                        $prefix_key = basename(mb_substr($file, 0, -14));
                    } elseif (false !== mb_strpos($file, 'Helper')) {
                        $prefix_key = 'Helper';
                    }
                    ...
                    if ('PaymentModule.php' == $prefix_key) {
        ...
        foreach ($files_per_directory['specific'] as $dir => $files) {
            foreach ($files as $file) {
                if (Tools::file_exists_cache($file_path = $dir . $file) && !in_array($file, self::$ignore_folder, true)) {
                    $prefix_key = 'index';
        ...
        foreach ($files_per_directory['tpl'] as $dir => $files) {
            foreach ($files as $file) {
                if (preg_match('/^(.*).tpl$/', $file) && Tools::file_exists_cache($file_path = $dir . $file)) {
                    ...
                    $pos = mb_strrpos($prefix_key, DIRECTORY_SEPARATOR);
                    $tmp = mb_substr($prefix_key, 0, $pos);

                    if (preg_match('#controllers#', $tmp)) {
                        $parent_class = explode(DIRECTORY_SEPARATOR, str_replace('/', DIRECTORY_SEPARATOR, $tmp));
                        $override = array_search('override', $parent_class, true);
                        if (false !== $override) {
                            ...
                            $key = array_search('controllers', $parent_class, true);
                            $prefix_key = 'Admin' . ucfirst($parent_class[$key + 1]);
                        }
                    } else {
                        $prefix_key = 'Admin' . ucfirst(mb_substr($tmp, mb_strrpos($tmp, DIRECTORY_SEPARATOR) + 1, $pos));
                    }
    ...
    public function initFormErrors()
    {
        ...
        if ($modules = $this->getListModules()) {
            foreach ($modules as $module) {
                if (is_dir(_PS_MODULE_DIR_ . $module) && !in_array($module, self::$ignore_folder, true)) {
    ...
    public function initFormFields()
    {
        ...
        foreach ($files_by_directory['php'] as $dir => $files) {
            foreach ($files as $file) {
                ...
                if (!preg_match('/\.php$/', $file) || in_array($file, $exclude_files, true)) {
                    continue;
                }

                $class_name = mb_substr($file, 0, -4);
    ...
    public function getMailFiles($dir, $group_name = 'mail')
    {
        ...
        if (Tools::file_exists_cache($dir_en)) {
            ...
            foreach (scandir($dir_en, SCANDIR_SORT_NONE) as $email_file) {
                if (mb_strripos($email_file, '.html') > 0 || mb_strripos($email_file, '.txt') > 0) {
                    $email_name = mb_substr($email_file, 0, mb_strripos($email_file, '.'));
                    $type = mb_substr($email_file, mb_strripos($email_file, '.') + 1);
                ...
                if (array_key_exists('html', $mail_files) || array_key_exists('txt', $mail_files)) {
                    if (array_key_exists($mail_name, $all_subject_mail)) {
                        foreach ($all_subject_mail[$mail_name] as $subject_mail) {
                            ...
                            if (in_array($subject_key, $topic_already_displayed, true)) {
                                continue;
                            }
    ...
    {
        if (mb_stripos($content[$lang], '<body')) {
            $array_lang = 'en' != $lang ? array('en', $lang) : array($lang);
            foreach ($array_lang as $language) {
                $title[$language] = mb_substr($content[$language], 0, mb_stripos($content[$language], '<body'));
        ...
        if (array_key_exists('dir', $this->translations_informations['modules'])) {
            if ($modules_dir = scandir($this->translations_informations['modules']['dir'], SCANDIR_SORT_NONE)) {
                foreach ($modules_dir as $module_dir) {
                    if (!in_array($module_dir, self::$ignore_folder, true)) {
                        $dir = false;
        ...
        foreach ($files_by_directiories['php'] as $dir => $files) {
            if (!empty($files)) {
                foreach ($files as $file) {
                    ...
                    if (Tools::file_exists_cache($dir . $file) && !in_array($file, self::$ignore_folder, true)) {
    ...
    public function copyMailFilesForAllLanguages()
    {
        ...
        foreach ($languages as $key => $lang) {
            ...
            $modules_has_mails = $this->getModulesHasMails(true);
            foreach ($modules_has_mails as $module_name => $module_path) {
                if ($pos = mb_strpos($module_path, '/modules')) {
                    $dir_to_copy_iso[] = _PS_ROOT_DIR_ . mb_substr($module_path, $pos) . 'mails/' . $current_iso_code . '/';
                }
            }
                    ...
                    if ($scanDir = scandir($dir, SCANDIR_SORT_NONE)) {
                        foreach ($scanDir as $file) {
                            if (!in_array($file, self::$ignore_folder, true)) {
                                $files_to_copy_iso[] = array(
                                    'from' => $dir . $file,
                                    'to' => str_replace((false !== mb_strpos($dir, _PS_CORE_DIR_)) ? _PS_CORE_DIR_ : _PS_ROOT_DIR_, _PS_ROOT_DIR_ . '/themes/' . $current_theme, $dir) . $file,
                                );
                            }
                        }
                    }
    ...
    protected function getSubjectMail($dir, $file, $subject_mail)
    {
        ...
        if (is_file($dir . '/' . $file) && !in_array($file, self::$ignore_folder, true) && preg_match('/\.php$/', $file)) {
            $content = file_get_contents($dir . '/' . $file);
            $content = str_replace("\n", ' ', $content);
                    ...
                    if (is_array($tab2) && isset($tab2[1])) {
                        ...
                        foreach ($tab2 as $tab3) {
                            if (preg_match('/Mail::l\(\'' . _PS_TRANS_PATTERN_ . '\'\)/Us', $tab3 . ')', $matches)) {
                                ...
                                if (!in_array($matches[1], $subject_mail[$template], true)) {
        ...
        } elseif (!in_array($file, self::$ignore_folder, true) && is_dir($dir . '/' . $file)) {
    ...
    public function initFormPDF()
    {
        ...
        if (!isset($GLOBALS[$name_var]) || 0 == count($GLOBALS[$name_var])) {
        ...
        foreach ($files_by_directory as $type => $directories) {
            foreach ($directories as $dir => $files) {
                foreach ($files as $file) {
                    if (!in_array($file, self::$ignore_folder, true) && Tools::file_exists_cache($file_path = $dir . $file)) {
                        if ('tpl' == $type) {
    ...
    {
        if (defined('_PS_HOST_MODE_') && false !== mb_strpos($email, _PS_MAIL_DIR_)) {
            $email_file = $email;
