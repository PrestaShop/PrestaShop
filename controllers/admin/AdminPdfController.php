...
class AdminPdfControllerCore extends AdminController
{
	...
	public function processGenerateInvoicePdf()
	{
		...
		elseif (Tools::isSubmit('id_order_invoice'))
			if(Tools::isSubmit('delivery_number'))
				$this->generateInvoicePDFByIdOrderInvoice(Tools::getValue('id_order_invoice'),Tools::getValue('delivery_number') );
			else
	...
	public function processGenerateDeliverySlipPDF()
	{
		...
		elseif (Tools::isSubmit('id_order_invoice'))
			if(Tools::isSubmit('delivery_number'))
				$this->generateDeliverySlipPDFByIdOrderInvoice((int)Tools::getValue('id_order_invoice'),(int)Tools::getValue('delivery_number'));
			else
	...
	public function generateDeliverySlipPDFByIdOrderInvoice($id_order_invoice,$delivery_number = false)
	{
		$order_invoice = new OrderInvoice((int)$id_order_invoice);
		if (!Validate::isLoadedObject($order_invoice))
			throw new PrestaShopException('Can\'t load Order Invoice object');
		$this->generatePDF($order_invoice, PDF::TEMPLATE_DELIVERY_SLIP,$delivery_number);
	}
	...
	public function generateInvoicePDFByIdOrderInvoice($id_order_invoice,$delivery_number = false)
	{
		$order_invoice = new OrderInvoice((int)$id_order_invoice);
		if (!Validate::isLoadedObject($order_invoice))
		...
		$this->generatePDF($order_invoice, PDF::TEMPLATE_INVOICE,$delivery_number);
	}

	public function generatePDF($object, $template,$delivery_number = false)
	{
		$pdf = new PDF($object, $template, Context::getContext()->smarty,$delivery_number);
		$pdf->render();
	}
}
