...
class AdminModulesPositionsControllerCore extends AdminController
{
	...
	public function displayModuleExceptionList($file_list, $shop_id)
	{
		...
		$content = '<input type="text" name="exceptions['.$shop_id.']" size="94" value="'.implode(', ', $file_list).'" id="em_text_'.$shop_id.'">';
		if ($shop_id)
		{
			$shop = new Shop($shop_id);
			$content .= ' ('.$shop->name.')';
		}
		$content .= '<br /><select id="em_list_'.$shop_id.'" size="45" multiple>';
		...
		$content .= '</select> <input type="button" class="button" value="'.$this->l('Add').'" onclick="position_exception_add('.$shop_id.')" style="vertical-align:top" />
				<input type="button" class="button" value="'.$this->l('Remove').'" onclick="position_exception_remove('.$shop_id.')" style="vertical-align:top" /><br /><br />';
