...
class AdminSearchConfControllerCore extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $cron_url = Tools::getHttpHost(true, true) . __PS_BASE_URI__ . basename(_PS_ADMIN_DIR_) .
            '/searchcron.php?full=1&token=' . mb_substr(_COOKIE_KEY_, 34, 8) . (Shop::getContext() == Shop::CONTEXT_SHOP ? '&id_shop=' . (int) Context::getContext()->shop->id : '');
        ...
        $this->fields_options = array(
            'indexation' => array(
                ...
                'info' => '<p>
					...
					<a href="searchcron.php?token=' . mb_substr(_COOKIE_KEY_, 34, 8) . '&amp;redirect=1' . (Shop::getContext() == Shop::CONTEXT_SHOP ? '&id_shop=' . (int) Context::getContext()->shop->id : '') . '" class="btn-link">
						<i class="icon-external-link-sign"></i>
						' . $this->trans('Add missing products to the index', array(), 'Admin.Shopparameters.Feature') . '
					</a><br />
					<a href="searchcron.php?full=1&amp;token=' . mb_substr(_COOKIE_KEY_, 34, 8) . '&amp;redirect=1' . (Shop::getContext() == Shop::CONTEXT_SHOP ? '&id_shop=' . (int) Context::getContext()->shop->id : '') . '" class="btn-link">
						<i class="icon-external-link-sign"></i>
