...
class AdminSearchConfControllerCore extends AdminController
{
    ...
    public function __construct()
    {
        ...
        $cron_url = Tools::getHttpHost(true, true) . __PS_BASE_URI__ . basename(_PS_ADMIN_DIR_) .
            '/searchcron.php?full=1&token=' . substr(_COOKIE_KEY_, 34, 8) . (Shop::CONTEXT_SHOP == Shop::getContext() ? '&id_shop=' . (int) Context::getContext()->shop->id : '');
        ...
        $this->fields_options = array(
            'indexation' => array(
                ...
                'info' => '<p>
					...
					<a href="searchcron.php?token=' . substr(_COOKIE_KEY_, 34, 8) . '&amp;redirect=1' . (Shop::CONTEXT_SHOP == Shop::getContext() ? '&id_shop=' . (int) Context::getContext()->shop->id : '') . '" class="btn-link">
						<i class="icon-external-link-sign"></i>
						' . $this->trans('Add missing products to the index', array(), 'Admin.Shopparameters.Feature') . '
					</a><br />
					<a href="searchcron.php?full=1&amp;token=' . substr(_COOKIE_KEY_, 34, 8) . '&amp;redirect=1' . (Shop::CONTEXT_SHOP == Shop::getContext() ? '&id_shop=' . (int) Context::getContext()->shop->id : '') . '" class="btn-link">
						<i class="icon-external-link-sign"></i>
