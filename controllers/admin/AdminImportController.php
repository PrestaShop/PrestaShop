    ...
    */
    ...
    protected function categoryImportOne($info, $default_language_id, $id_lang, $force_ids, $regenerate, $shop_is_feature_active, &$cat_moved, $validateOnly = false)
    {
        ...
        if (isset($category->parent) && is_numeric($category->parent)) {
            ...
            if ($category_parent['id_category']) {
                ...
                if (($field_error = $category_to_create->validateFields(UNFRIENDLY_ERROR, true)) === true &&
                    ...
                    if (!$validateOnly) {
                        $this->errors[] = $this->trans('%category_name% (ID: %id%) cannot be saved', [
        ...
        if ((isset($category->link_rewrite) && empty($category->link_rewrite[$default_language_id])) || !$valid_link) {
            ...
            if ($category->link_rewrite == '') {
                ...
                $this->warnings[] = $this->trans('URL rewriting failed to auto-generate a friendly URL for: %category_name%', ['%category_name%' => $category->name[$default_language_id]], 'Admin.Advparameters.Notification');
            }
            $category->link_rewrite = AdminImportController::createMultiLangField($category->link_rewrite);
        }

        if (!$valid_link) {
            $this->informations[] = $this->trans('Rewrite link for %1$s (ID %2$s): re-written as %3$s.', [
                '%1$s' => $bak,
        ...
        if (($field_error = $category->validateFields(UNFRIENDLY_ERROR, true)) === true &&
            ...
            if ($category->id && $category->id == $category->id_parent) {
                $this->errors[] = sprintf(
                    $this->trans('A category cannot be its own parent. The parent category ID is either missing or unknown (ID: %1$s).', [], 'Admin.Advparameters.Notification'),
                    (isset($info['id']) && !empty($info['id'])) ? $info['id'] : 'null'
                );
        ...
        if (!$res) {
            ...
            if (!$validateOnly && isset($product->image) && is_array($product->image) && count($product->image)) {
                ...
                foreach ($product->image as $key => $url) {
                    ...
                    if ($error) {
                        $this->warnings[] = $this->trans('Product #%id%: the picture (%url%) cannot be saved.', ['%id%' => $image->id_product, '%url%' => $url], 'Admin.Advparameters.Notification');
                    }
                }
            }
            ...
            if (!$validateOnly && isset($product->advanced_stock_management)) {
                if ($product->advanced_stock_management != 1 && $product->advanced_stock_management != 0) {
                    $this->warnings[] = $this->trans('Advanced stock management has incorrect value. Not set for product %name% ', ['%name%' => $product->name[$default_language_id]], 'Admin.Advparameters.Notification');
                } elseif (!Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT') && $product->advanced_stock_management == 1) {
                    $this->warnings[] = $this->trans('Advanced stock management is not enabled, cannot enable on product %name% ', ['%name%' => $product->name[$default_language_id]], 'Admin.Advparameters.Notification');
            ...
            if (isset($product->warehouse) && $product->warehouse) {
                if (!Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT')) {
                    $this->warnings[] = $this->trans('Advanced stock management is not enabled, warehouse not set on product %name% ', ['%name%' => $product->name[$default_language_id]], 'Admin.Advparameters.Notification');
                } elseif (!$validateOnly) {
                    if (Warehouse::exists($product->warehouse)) {
                        ...
                        $this->warnings[] = $this->trans('Warehouse did not exist, cannot set on product  %name% ', ['%name%' => $product->name[$default_language_id]], 'Admin.Advparameters.Notification');
                    }
                }
            }
            ...
            if (isset($product->depends_on_stock)) {
                if ($product->depends_on_stock != 0 && $product->depends_on_stock != 1) {
                    $this->warnings[] = $this->trans('Incorrect value for "Depends on stock" for product %name% ', ['%name%' => $product->name[$default_language_id]], 'Admin.Advparameters.Notification');
                } elseif ((!$product->advanced_stock_management || $product->advanced_stock_management == 0) && $product->depends_on_stock == 1) {
                    $this->warnings[] = $this->trans('Advanced stock management is not enabled, cannot set "Depends on stock" for product %name% ', ['%name%' => $product->name[$default_language_id]], 'Admin.Advparameters.Notification');
                } elseif (!$validateOnly) {
            ...
            foreach (explode($this->multiple_value_separator, $info['attribute']) as $key => $attribute) {
                ...
                if (isset($groups_attributes[$key])) {
                    ...
                    if (!Validate::isEan13($info['ean13'])) {
                        $this->warnings[] = $this->trans('EAN13 "%ean13%" has incorrect value for product with id %id%.', ['%ean13%' => $info['ean13'], '%id%' => $product->id], 'Admin.Advparameters.Notification');
                        $info['ean13'] = '';
                    }
        ...
        if ($id_product_attribute) {
            ...
            if (isset($info['advanced_stock_management'])) {
                if ($info['advanced_stock_management'] != 1 && $info['advanced_stock_management'] != 0) {
                    $this->warnings[] = $this->trans('Advanced stock management has incorrect value. Not set for product with id %id%.', ['%id%' => $product->id], 'Admin.Advparameters.Notification');
                } elseif (!Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT') && $info['advanced_stock_management'] == 1) {
                    $this->warnings[] = $this->trans('Advanced stock management is not enabled, cannot enable on product with id %id%.', ['%id%' => $product->id], 'Admin.Advparameters.Notification');
                } elseif (!$validateOnly) {
            ...
            if (isset($info['warehouse']) && $info['warehouse']) {
                if (!Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT')) {
                    $this->warnings[] = $this->trans('Advanced stock management is not enabled, warehouse is not set on product with id %id%.', ['%id%' => $product->id], 'Admin.Advparameters.Notification');
                } else {
                    if (Warehouse::exists($info['warehouse'])) {
                        ...
                        $this->warnings[] = $this->trans('Warehouse did not exist, cannot set on product %name%.', ['%name%' => $product->name[$default_language]], 'Admin.Advparameters.Notification');
                    }
                }
            }
            ...
            if (isset($info['depends_on_stock'])) {
                if ($info['depends_on_stock'] != 0 && $info['depends_on_stock'] != 1) {
                    $this->warnings[] = $this->trans('Incorrect value for "Depends on stock" for product %name% ', ['%name%' => $product->name[$default_language]], 'Admin.Notifications.Error');
                } elseif ((!$info['advanced_stock_management'] || $info['advanced_stock_management'] == 0) && $info['depends_on_stock'] == 1) {
                    $this->warnings[] = $this->trans('Advanced stock management is not enabled, cannot set "Depends on stock" for product %name% ', ['%name%' => $product->name[$default_language]], 'Admin.Advparameters.Notification');
                } elseif (!$validateOnly) {
    ...
    protected function addressImportOne($info, $force_ids, $validateOnly = false)
    {
        ...
        if (isset($address->customer_email) && !empty($address->customer_email)) {
            if (Validate::isEmail($address->customer_email)) {
                ...
                $this->errors[] = $this->trans('"%email%" is not a valid email address.', ['%email%' => $address->customer_email], 'Admin.Advparameters.Notification');
    ...
    protected function supplierImportOne($info, $shop_is_feature_active, $regenerate, $force_ids, $validateOnly = false)
    {
        ...
        if (($field_error = $supplier->validateFields(UNFRIENDLY_ERROR, true)) === true &&
            ...
            $this->errors[] = $this->trans('Supplier is invalid', [], 'Admin.Advparameters.Notification') . ' (' . $supplier->name . ')';
            $this->errors[] = ($field_error !== true ? $field_error : '') . (isset($lang_field_error) && $lang_field_error !== true ? $lang_field_error : '');
        }
    }
    ...
    protected function aliasImportOne($info, $force_ids, $validateOnly = false)
    {
        ...
        if (($field_error = $alias->validateFields(UNFRIENDLY_ERROR, true)) === true &&
            ...
            $this->errors[] = $this->trans('Alias is invalid', [], 'Admin.Advparameters.Notification') . ' (' . $alias->name . ')';
            $this->errors[] = ($field_error !== true ? $field_error : '') . (isset($lang_field_error) && $lang_field_error !== true ? $lang_field_error : '');
        }
    }
    ...
    public function storeContactImportOne($info, $shop_is_feature_active, $regenerate, $force_ids, $validateOnly = false)
    {
        ...
        if (($field_error = $store->validateFields(UNFRIENDLY_ERROR, true)) === true &&
            ...
            $id_lang = Language::getIdByIso(Tools::getValue('iso_lang'));
            $this->errors[] = $this->trans('Store is invalid', [], 'Admin.Advparameters.Notification') . ' (' . $store->name[$id_lang] . ')';
            $this->errors[] = ($field_error !== true ? $field_error : '') . (isset($lang_field_error) && $lang_field_error !== true ? $lang_field_error : '');
        }
    }
    ...
    protected function supplyOrdersDetailsImportOne($info, &$products, &$reset, $force_ids, $current_line, $validateOnly = false)
    {
        ...
        if (array_key_exists('supply_order_reference', $info) && pSQL($info['supply_order_reference']) && SupplyOrder::exists(pSQL($info['supply_order_reference']))) {
            ...
            $this->errors[] = sprintf($this->trans('Supply Order (%s) could not be loaded (at line %d).', [], 'Admin.Advparameters.Notification'), $info['supply_order_reference'], $current_line + 1);
        }
