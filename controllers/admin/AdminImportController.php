...
class AdminImportControllerCore extends AdminController
{
    ...
    protected static function rewindBomAware($handle)
    {
        ...
        rewind($handle);
        if ("\xEF\xBB\xBF" != ($bom = fread($handle, 3))) {
            rewind($handle);
        }
    }
    ...
    protected static function split($field)
    {
        ...
        fclose($fd);
        if (false !== $uniqid_path && file_exists($uniqid_path)) {
            @unlink($uniqid_path);
        }
    ...
    public function getAvailableFields($in_array = false)
    {
        ...
        foreach ($this->available_fields as $k => $field) {
            ...
            if ('price_tin' === $k) { // Special case for Product : either one or the other. Not both.
    ...
    {
        $infos = trim($infos);
        if (isset(self::$validators[$key][1]) && 'createMultiLangField' == self::$validators[$key][1] && Tools::getValue('iso_lang')) {
        ...
        } elseif (!empty($infos) || '0' == $infos) { // ($infos == '0') => if you want to disable a product by using "0" in active because empty('0') return true
    ...
    public function categoryImport($offset = false, $limit = false, &$crossStepsVariables = false, $validateOnly = false)
    {
        ...
        $cat_moved = array();
        if (false !== $crossStepsVariables && array_key_exists('cat_moved', $crossStepsVariables)) {
    ...
    protected function categoryImportOne($info, $default_language_id, $id_lang, $force_ids, $regenerate, $shop_is_feature_active, &$cat_moved, $validateOnly = false)
    {
        ...
        if (isset($category->parent) && is_numeric($category->parent)) {
            ...
            if ($category_parent['id_category']) {
                ...
                if (true === ($field_error = $category_to_create->validateFields(UNFRIENDLY_ERROR, true)) &&
                    true === ($lang_field_error = $category_to_create->validateFieldsLang(UNFRIENDLY_ERROR, true)) &&
                    ...
                    if (true !== $field_error || isset($lang_field_error) && true !== $lang_field_error) {
                        $this->errors[] = (true !== $field_error ? $field_error : '').(isset($lang_field_error) && true !== $lang_field_error ? $lang_field_error : '').
        ...
        $res = false;
        if (true === ($field_error = $category->validateFields(UNFRIENDLY_ERROR, true)) &&
            true === ($lang_field_error = $category->validateFieldsLang(UNFRIENDLY_ERROR, true)) && empty($this->errors)) {
        ...
        if (!$res) {
            ...
            $error_tmp = (true !== $field_error ? $field_error : '').(isset($lang_field_error) && true !== $lang_field_error ? $lang_field_error : '').Db::getInstance()->getMsgError();
            if ('' != $error_tmp) {
    ...
    public function productImport($offset = false, $limit = false, &$crossStepsVariables = false, $validateOnly = false, $moreStep = 0)
    {
        ...
        $accessories = array();
        if (false !== $crossStepsVariables && array_key_exists('accessories', $crossStepsVariables)) {
    ...
    protected function productImportOne($info, $default_language_id, $id_lang, $force_ids, $regenerate, $shop_is_feature_active, $shop_ids, $match_ref, &$accessories, $validateOnly = false)
    {
        ...
        if (0 != (int) $product->id_tax_rules_group) {
        ...
        if (isset($product->manufacturer) && is_numeric($product->manufacturer) && Manufacturer::manufacturerExists((int) $product->manufacturer)) {
            ...
            if ($manufacturer = Manufacturer::getIdByName($product->manufacturer)) {
                ...
                if (true === ($field_error = $manufacturer->validateFields(UNFRIENDLY_ERROR, true)) &&
                    true === ($lang_field_error = $manufacturer->validateFieldsLang(UNFRIENDLY_ERROR, true)) &&
        ...
        if (isset($product->supplier) && is_numeric($product->supplier) && Supplier::supplierExists((int) $product->supplier)) {
            ...
            if ($supplier = Supplier::getIdByName($product->supplier)) {
                ...
                if (true === ($field_error = $supplier->validateFields(UNFRIENDLY_ERROR, true)) &&
                    true === ($lang_field_error = $supplier->validateFieldsLang(UNFRIENDLY_ERROR, true)) &&
                    !$validateOnly &&  // Do not move this condition: previous tests should be played always, but next ->add() test should not be played in validateOnly mode
                    $supplier->add()) {
        ...
        foreach (Product::$definition['fields'] as $key => $array) {
            if (Product::TYPE_FLOAT == $array['type']) {
        ...
        if (true === $field_error && true === $lang_field_error) {
            // check quantity
        ...
        if (!$res) {
            ...
            if (!$validateOnly && isset($product->image) && is_array($product->image) && count($product->image)) {
                ...
                foreach ($product->image as $key => $url) {
                    ...
                    if (!empty($url)) {
                        ...
                        if (true === ($field_error = $image->validateFields(UNFRIENDLY_ERROR, true)) &&
                            true === ($lang_field_error = $image->validateFieldsLang(UNFRIENDLY_ERROR, true)) && $image->add()) {
            ...
            if (!$validateOnly && isset($product->advanced_stock_management)) {
                if (1 != $product->advanced_stock_management && 0 != $product->advanced_stock_management) {
                ...
                } elseif (!Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT') && 1 == $product->advanced_stock_management) {
                ...
                if (1 == StockAvailable::dependsOnStock($product->id) && 0 == $product->advanced_stock_management) {
            ...
            if (isset($product->warehouse) && $product->warehouse) {
                if (!Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT')) {
                    ...
                    if (Warehouse::exists($product->warehouse)) {
                        ...
                        if (false !== WarehouseProductLocation::getProductLocation($product->id, 0, $product->warehouse)) {
            ...
            if (isset($product->depends_on_stock)) {
                if (0 != $product->depends_on_stock && 1 != $product->depends_on_stock) {
                ...
                } elseif ((!$product->advanced_stock_management || 0 == $product->advanced_stock_management) && 1 == $product->depends_on_stock) {
    ...
    public function attributeImport($offset = false, $limit = false, &$crossStepsVariables = false, $validateOnly = false)
    {
        ...
        $groups = array();
        if (false !== $crossStepsVariables && array_key_exists('groups', $crossStepsVariables)) {
        ...
        $attributes = array();
        if (false !== $crossStepsVariables && array_key_exists('attributes', $crossStepsVariables)) {
    ...
    protected function attributeImportOne($info, $default_language, &$groups, &$attributes, $regenerate, $shop_is_feature_active, $validateOnly = false)
    {
            ...
            if (is_array($info['image_url']) && count($info['image_url'])) {
                foreach ($info['image_url'] as $key => $url) {
                    ...
                    if (true === $field_error &&
                        ...
                        if (true !== $field_error || true !== $lang_field_error) {
                            $this->errors[] = (true !== $field_error ? $field_error : '').(isset($lang_field_error) && true !== $lang_field_error ? $lang_field_error : '').mysql_error();
                        }
                    }
                }
            }
        ...
        if (isset($info['group'])) {
            foreach (explode($this->multiple_value_separator, $info['group']) as $key => $group) {
                ...
                if (!isset($groups[$group])) {
                    ...
                    if (true === ($field_error = $obj->validateFields(UNFRIENDLY_ERROR, true)) &&
                        true === ($lang_field_error = $obj->validateFieldsLang(UNFRIENDLY_ERROR, true))) {
        ...
        if (isset($info['attribute'])) {
            foreach (explode($this->multiple_value_separator, $info['attribute']) as $key => $attribute) {
                ...
                if (isset($groups_attributes[$key])) {
                    ...
                    if (!isset($attributes[$group.'_'.$attribute]) && 2 == count($groups_attributes[$key])) {
        ...
        if ($id_product_attribute) {
            ...
            if (isset($info['advanced_stock_management'])) {
                if (1 != $info['advanced_stock_management'] && 0 != $info['advanced_stock_management']) {
                ...
                } elseif (!Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT') && 1 == $info['advanced_stock_management']) {
                ...
                // automaticly disable depends on stock, if a_s_m set to disabled
                if (!$validateOnly && 1 == StockAvailable::dependsOnStock($product->id) && 0 == $info['advanced_stock_management']) {
            ...
            if (isset($info['warehouse']) && $info['warehouse']) {
                if (!Configuration::get('PS_ADVANCED_STOCK_MANAGEMENT')) {
                    ...
                    if (Warehouse::exists($info['warehouse'])) {
                        ...
                        if (!$validateOnly) {
                            if (false !== WarehouseProductLocation::getProductLocation($product->id, $id_product_attribute, $info['warehouse'])) {
                                $warehouse_location_entity->update();
                            } else {
                                $warehouse_location_entity->save();
                            }
            ...
            if (isset($info['depends_on_stock'])) {
                if (0 != $info['depends_on_stock'] && 1 != $info['depends_on_stock']) {
                ...
                } elseif ((!$info['advanced_stock_management'] || 0 == $info['advanced_stock_management']) && 1 == $info['depends_on_stock']) {
    ...
    protected function customerImportOne($info, $default_language_id, $id_lang, $shop_is_feature_active, $force_ids, $validateOnly = false)
    {
        ...
        if (isset($customer->date_upd) && '0000-00-00 00:00:00' == $customer->date_upd) {
            $customer->date_upd = date('Y-m-d H:i:s');
        }

        $res = false;
        if (true === ($field_error = $customer->validateFields(UNFRIENDLY_ERROR, true)) &&
            true === ($lang_field_error = $customer->validateFieldsLang(UNFRIENDLY_ERROR, true))) {
            $res = true;
    ...
    protected function addressImportOne($info, $force_ids, $validateOnly = false)
    {
        ...
        if (isset($address->country) && is_numeric($address->country)) {
            ...
            if ($id_country = Country::getIdByName(null, $address->country)) {
                ...
                if (true === ($field_error = $country->validateFields(UNFRIENDLY_ERROR, true)) &&
                    true === ($lang_field_error = $country->validateFieldsLang(UNFRIENDLY_ERROR, true)) &&
                    !$validateOnly && // Do not move this condition: previous tests should be played always, but next ->add() test should not be played in validateOnly mode
                    $country->add()) {
        ...
        if (isset($address->state) && is_numeric($address->state)) {
            ...
            if ($id_state = State::getIdByName($address->state)) {
                ...
                if (true === ($field_error = $state->validateFields(UNFRIENDLY_ERROR, true)) &&
                    true === ($lang_field_error = $state->validateFieldsLang(UNFRIENDLY_ERROR, true)) &&
                    !$validateOnly && // Do not move this condition: previous tests should be played always, but next ->add() test should not be played in validateOnly mode
                    $state->add()) {
        ...
        $res = false;
        if (true === ($field_error = $address->validateFields(UNFRIENDLY_ERROR, true)) &&
            true === ($lang_field_error = $address->validateFieldsLang(UNFRIENDLY_ERROR, true))) {
    ...
    protected function aliasImportOne($info, $force_ids, $validateOnly = false)
    {
        ...
        $res = false;
        if (true === ($field_error = $alias->validateFields(UNFRIENDLY_ERROR, true)) &&
            true === ($lang_field_error = $alias->validateFieldsLang(UNFRIENDLY_ERROR, true))) {
    ...
    public function storeContactImportOne($info, $shop_is_feature_active, $regenerate, $force_ids, $validateOnly = false)
    {
        ...
        $res = false;
        if (true === ($field_error = $store->validateFields(UNFRIENDLY_ERROR, true)) &&
            true === ($lang_field_error = $store->validateFieldsLang(UNFRIENDLY_ERROR, true))) {
    ...
    public function supplyOrdersDetailsImport($offset = false, $limit = false, &$crossStepsVariables = false, $validateOnly = false)
    {
        ...
        $products = array();
        $reset = true;
        if (false !== $crossStepsVariables && array_key_exists('products', $crossStepsVariables)) {
            $products = $crossStepsVariables['products'];
        }
        if (false !== $crossStepsVariables && array_key_exists('reset', $crossStepsVariables)) {
    ...
    public function ajaxProcessImport()
    {
        ...
        $validateOnly = (1 == (int) Tools::getValue('validateOnly'));
