...
{
    /**
     * Portable equivalent of MySQL's LEFT(column, length) applied to a date/datetime column.
     */
    private static function sqlLeft($column, $length)
    {
        /* @phpstan-ignore-next-line */
        $cast = _DB_TYPE_ == 'pgsql' ? '::text' : '';

        return 'LEFT(' . $column . $cast . ', ' . (int) $length . ')';
    }

    /**
     * Portable equivalent of MySQL's DATEDIFF(expr1, expr2) (day-granularity difference).
     */
    private static function sqlDateDiffDays($expr1, $expr2)
    {
        /* @phpstan-ignore-next-line */
        if (_DB_TYPE_ == 'pgsql') {
            return '((' . $expr1 . ')::date - (' . $expr2 . ')::date)';
        }
    ...
    public static function getVisits($unique, $date_from, $date_to, $granularity = false)
    {
        ...
        $dateAddCol = Db::quoteIdentifier('date_add');
        $connectionsTable = Db::quoteIdentifier(_DB_PREFIX_ . 'connections');
        if ($granularity == 'day') {
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                '
            SELECT date(' . $dateAddCol . ') as date, COUNT(' . ($unique ? 'DISTINCT id_guest' : '*') . ') as visits
            FROM ' . $connectionsTable . '
            WHERE ' . $dateAddCol . ' BETWEEN \'' . pSQL($date_from) . ' 00:00:00\' AND \'' . pSQL($date_to) . ' 23:59:59\'
            ' . Shop::addSqlRestriction() . '
            GROUP BY date(' . $dateAddCol . ')'
            );
            ...
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                '
            SELECT ' . self::sqlLeft($dateAddCol, 7) . ' as date, COUNT(' . ($unique ? 'DISTINCT id_guest' : '*') . ') as visits
            FROM ' . $connectionsTable . '
            WHERE ' . $dateAddCol . ' BETWEEN \'' . pSQL($date_from) . ' 00:00:00\' AND \'' . pSQL($date_to) . ' 23:59:59\'
            ' . Shop::addSqlRestriction() . '
            GROUP BY ' . self::sqlLeft($dateAddCol, 7)
            );
            foreach ($result as $row) {
    ...
    public static function getAbandonedCarts($date_from, $date_to)
    {
		...
		FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'cart') . '
		WHERE ' . Db::quoteIdentifier('date_add') . ' BETWEEN \'' . pSQL($date_from) . '\' AND \'' . pSQL($date_to) . '\'
		AND NOT EXISTS (SELECT 1 FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'orders') . ' WHERE ' . Db::quoteIdentifier(_DB_PREFIX_ . 'orders') . '.id_cart = ' . Db::quoteIdentifier(_DB_PREFIX_ . 'cart') . '.id_cart)
		' . Shop::addSqlRestriction()
        );
    }
    ...
    public static function getInstalledModules()
    {
        return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
            '
		SELECT COUNT(DISTINCT m.' . Db::quoteIdentifier('id_module') . ')
    ...
    public static function getPercentProductStock()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
            '
		SELECT SUM(CASE WHEN COALESCE(stock.quantity, 0) > 0 THEN 1 ELSE 0 END) AS with_stock, COUNT(*) AS products
    ...
    public static function getPercentProductOutOfStock()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
            '
		SELECT SUM(CASE WHEN COALESCE(stock.quantity, 0) <= 0 THEN 1 ELSE 0 END) AS without_stock, COUNT(*) AS products
		FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'product') . ' p
		' . Shop::addSqlAssociation('product', 'p') . '
		LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'product_attribute') . ' pa ON p.id_product = pa.id_product
		' . Product::sqlStock('p', 'pa') . '
    ...
    {
        $sql = 'SELECT AVG(1 - (CASE WHEN COALESCE(product_attribute_shop.wholesale_price, 0) = 0 THEN product_shop.wholesale_price ELSE product_attribute_shop.wholesale_price END / (COALESCE(product_attribute_shop.price, 0) + product_shop.price)))
		FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'product') . ' p
		' . Shop::addSqlAssociation('product', 'p') . '
		LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'product_attribute') . ' pa ON p.id_product = pa.id_product
    ...
    {
        $invoiceDateCol = Db::quoteIdentifier('invoice_date');
        $ordersTable = Db::quoteIdentifier(_DB_PREFIX_ . 'orders');
        $orderStateTable = Db::quoteIdentifier(_DB_PREFIX_ . 'order_state');
        if ($granularity == 'day') {
            $sales = [];
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                '
			SELECT ' . self::sqlLeft($invoiceDateCol, 10) . ' AS date, SUM((total_paid_tax_excl - total_shipping_tax_excl) / o.conversion_rate) AS sales
			FROM ' . $ordersTable . ' o
			LEFT JOIN ' . $orderStateTable . ' os ON o.current_state = os.id_order_state
			WHERE ' . $invoiceDateCol . ' BETWEEN \'' . pSQL($date_from) . ' 00:00:00\' AND \'' . pSQL($date_to) . ' 23:59:59\' AND os.logable = 1
			' . Shop::addSqlRestriction(false, 'o') . '
			GROUP BY ' . self::sqlLeft($invoiceDateCol, 10)
            );
            foreach ($result as $row) {
            ...
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                '
			SELECT ' . self::sqlLeft($invoiceDateCol, 7) . ' AS date, SUM((total_paid_tax_excl - total_shipping_tax_excl) / o.conversion_rate) AS sales
			FROM ' . $ordersTable . ' o
			LEFT JOIN ' . $orderStateTable . ' os ON o.current_state = os.id_order_state
			WHERE ' . $invoiceDateCol . ' BETWEEN \'' . pSQL($date_from) . ' 00:00:00\' AND \'' . pSQL($date_to) . ' 23:59:59\' AND os.logable = 1
			' . Shop::addSqlRestriction(false, 'o') . '
			GROUP BY ' . self::sqlLeft($invoiceDateCol, 7)
            );
            foreach ($result as $row) {
    ...
    public static function get8020SalesCatalog($date_from, $date_to)
    {
		...
		LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'order_detail') . ' od ON o.id_order = od.id_order
		WHERE ' . Db::quoteIdentifier('invoice_date') . ' BETWEEN \'' . pSQL($date_from) . ' 00:00:00\' AND \'' . pSQL($date_to) . ' 23:59:59\'
		' . Shop::addSqlRestriction(false, 'o')
        );
    ...
    public static function getOrders($date_from, $date_to, $granularity = false)
    {
        ...
        if ($granularity == 'day') {
            ...
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                '
			SELECT ' . self::sqlLeft($invoiceDateCol, 10) . ' AS date, COUNT(*) AS orders
			FROM ' . $ordersTable . ' o
			LEFT JOIN ' . $orderStateTable . ' os ON o.current_state = os.id_order_state
			WHERE ' . $invoiceDateCol . ' BETWEEN \'' . pSQL($date_from) . ' 00:00:00\' AND \'' . pSQL($date_to) . ' 23:59:59\' AND os.logable = 1
			' . Shop::addSqlRestriction(false, 'o') . '
            ...
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                '
			SELECT ' . self::sqlLeft($invoiceDateCol, 7) . ' AS date, COUNT(*) AS orders
			FROM ' . $ordersTable . ' o
			LEFT JOIN ' . $orderStateTable . ' os ON o.current_state = os.id_order_state
			WHERE ' . $invoiceDateCol . ' BETWEEN \'' . pSQL($date_from) . ' 00:00:00\' AND \'' . pSQL($date_to) . ' 23:59:59\' AND os.logable = 1
			' . Shop::addSqlRestriction(false, 'o') . '
    ...
    public static function getEmptyCategories()
    {
		...
		FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'category') . ' c
		' . Shop::addSqlAssociation('category', 'c') . '
		AND c.' . Db::quoteIdentifier('id_category') . ' != ' . (int) Configuration::get('PS_ROOT_CATEGORY')
        );
        $used = (int) Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
            '
		SELECT COUNT(DISTINCT cp.id_category)
		FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'category') . ' c
		LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'category_product') . ' cp ON c.id_category = cp.id_category
		' . Shop::addSqlAssociation('category', 'c') . '
		AND c.' . Db::quoteIdentifier('id_category') . ' != ' . (int) Configuration::get('PS_ROOT_CATEGORY')
        );
    ...
    public static function getCustomerMainGender()
    {
        $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
            '
		SELECT SUM(CASE WHEN g.id_gender IS NOT NULL THEN 1 ELSE 0 END) AS total, SUM(CASE WHEN type = 0 THEN 1 ELSE 0 END) AS male, SUM(CASE WHEN type = 1 THEN 1 ELSE 0 END) AS female, SUM(CASE WHEN type = 2 THEN 1 ELSE 0 END) AS neutral
		FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customer') . ' c
		LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'gender') . ' g ON c.id_gender = g.id_gender
    ...
    {
        $idCategoryCol = Db::quoteIdentifier('id_category');
        $idProductCol = Db::quoteIdentifier('id_product');

        return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
            '
		SELECT ca.' . $idCategoryCol . '
		FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'category') . ' ca
		LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'category_product') . ' capr ON ca.' . $idCategoryCol . ' = capr.' . $idCategoryCol . '
		LEFT JOIN (
			SELECT pr.' . $idProductCol . ', t.' . Db::quoteIdentifier('totalPriceSold') . '
			FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'product') . ' pr
			LEFT JOIN (
				SELECT pr.' . $idProductCol . ',
					COALESCE(SUM(cp.' . Db::quoteIdentifier('product_quantity') . '), 0) AS totalQuantitySold,
					COALESCE(SUM(cp.' . Db::quoteIdentifier('unit_price_tax_excl') . ' * cp.' . Db::quoteIdentifier('product_quantity') . '), 0) / o.conversion_rate AS totalPriceSold
				FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'product') . ' pr
				LEFT OUTER JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'order_detail') . ' cp ON pr.' . $idProductCol . ' = cp.' . Db::quoteIdentifier('product_id') . '
				LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'orders') . ' o ON o.' . Db::quoteIdentifier('id_order') . ' = cp.' . Db::quoteIdentifier('id_order') . '
				WHERE o.invoice_date BETWEEN \'' . pSQL($date_from) . ' 00:00:00\' AND \'' . pSQL($date_to) . ' 23:59:59\'
				GROUP BY pr.' . $idProductCol . '
			) t ON t.' . $idProductCol . ' = pr.' . $idProductCol . '
		) t	ON t.' . $idProductCol . ' = capr.' . $idProductCol . '
		WHERE ca.' . Db::quoteIdentifier('level_depth') . ' > 1
		GROUP BY ca.' . $idCategoryCol . '
		ORDER BY SUM(t.' . Db::quoteIdentifier('totalPriceSold') . ') DESC'
        );
    }
    ...
    public static function getMainCountry($date_from, $date_to)
    {
		...
		FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'orders') . ' o
		LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'address') . ' a ON o.id_address_delivery = a.id_address
		WHERE ' . Db::quoteIdentifier('invoice_date') . ' BETWEEN \'' . pSQL($date_from) . ' 00:00:00\' AND \'' . pSQL($date_to) . ' 23:59:59\'
		' . Shop::addSqlRestriction()
        );
    ...
    {
        /* @phpstan-ignore-next-line */
        $zeroDateCheck = _DB_TYPE_ == 'mysql' ? " AND birthday != '0000-00-00'" : '';
        $value = Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
            '
		SELECT AVG(' . self::sqlDateDiffDays("'" . date('Y-m-d') . " 00:00:00'", 'birthday') . ')
		FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customer') . ' c
		WHERE active = 1
		AND birthday IS NOT NULL' . $zeroDateCheck . ' ' . Shop::addSqlRestriction()
        );
    ...
    public static function getAverageMessageResponseTime($date_from, $date_to)
    {
		...
		FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customer_message') . ' cm1
		INNER JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customer_message') . ' cm2 ON (cm1.id_customer_thread = cm2.id_customer_thread AND cm1.date_add < cm2.date_add)
		JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customer_thread') . ' ct ON (cm1.id_customer_thread = ct.id_customer_thread)
		WHERE cm1.' . Db::quoteIdentifier('date_add') . ' BETWEEN \'' . pSQL($date_from) . ' 00:00:00\' AND \'' . pSQL($date_to) . ' 23:59:59\'
    ...
    public static function getMessagesPerThread($date_from, $date_to)
    {
		...
		LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'customer_message') . ' cm ON (ct.id_customer_thread = cm.id_customer_thread)
		WHERE ct.' . Db::quoteIdentifier('date_add') . ' BETWEEN \'' . pSQL($date_from) . ' 00:00:00\' AND \'' . pSQL($date_to) . ' 23:59:59\'
		' . Shop::addSqlRestriction() . '
		AND STATUS = \'closed\'
    ...
    {
        $invoiceDateCol = Db::quoteIdentifier('invoice_date');
        $conversionRateCol = Db::quoteIdentifier('conversion_rate');
        $ordersTable = Db::quoteIdentifier(_DB_PREFIX_ . 'orders');
        $orderDetailTable = Db::quoteIdentifier(_DB_PREFIX_ . 'order_detail');
        $orderStateTable = Db::quoteIdentifier(_DB_PREFIX_ . 'order_state');
        if ($granularity == 'day') {
            $purchases = [];
            $result = Db::getInstance(_PS_USE_SQL_SLAVE_)->executeS(
                '
			SELECT
				' . self::sqlLeft($invoiceDateCol, 10) . ' as date,
				SUM(od.' . Db::quoteIdentifier('product_quantity') . ' * CASE WHEN
					od.' . Db::quoteIdentifier('purchase_supplier_price') . ' > 0
					THEN od.' . Db::quoteIdentifier('purchase_supplier_price') . ' / ' . $conversionRateCol . '
					ELSE od.' . Db::quoteIdentifier('original_product_price') . ' * ' . (int) Configuration::get('CONF_AVERAGE_PRODUCT_MARGIN') . ' / 100
				END) as total_purchase_price
			FROM ' . $ordersTable . ' o
			LEFT JOIN ' . $orderDetailTable . ' od ON o.id_order = od.id_order
			LEFT JOIN ' . $orderStateTable . ' os ON o.current_state = os.id_order_state
			WHERE ' . $invoiceDateCol . ' BETWEEN \'' . pSQL($date_from) . ' 00:00:00\' AND \'' . pSQL($date_to) . ' 23:59:59\' AND os.logable = 1
			' . Shop::addSqlRestriction(false, 'o') . '
			GROUP BY ' . self::sqlLeft($invoiceDateCol, 10)
            ...
            return Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
                '
			SELECT SUM(od.' . Db::quoteIdentifier('product_quantity') . ' * CASE WHEN
				od.' . Db::quoteIdentifier('purchase_supplier_price') . ' > 0
				THEN od.' . Db::quoteIdentifier('purchase_supplier_price') . ' / ' . $conversionRateCol . '
				ELSE od.' . Db::quoteIdentifier('original_product_price') . ' * ' . (int) Configuration::get('CONF_AVERAGE_PRODUCT_MARGIN') . ' / 100
			END) as total_purchase_price
    ...
    public static function getExpenses($date_from, $date_to, $granularity = false)
    {
        ...
        $orders = Db::getInstance()->executeS(
            '
		SELECT
			' . self::sqlLeft(Db::quoteIdentifier('invoice_date'), 10) . ' AS date,
		...
		FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'orders') . ' o
		LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'address') . ' a ON o.id_address_delivery = a.id_address
		LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'carrier') . ' c ON o.id_carrier = c.id_carrier
		LEFT JOIN ' . Db::quoteIdentifier(_DB_PREFIX_ . 'order_state') . ' os ON o.current_state = os.id_order_state
		WHERE ' . Db::quoteIdentifier('invoice_date') . ' BETWEEN \'' . pSQL($date_from) . ' 00:00:00\' AND \'' . pSQL($date_to) . ' 23:59:59\' AND os.logable = 1
		' . Shop::addSqlRestriction(false, 'o')
        );
        ...
        switch (Tools::getValue('kpi')) {
            ...
            case 'newsletter_registrations':
                ...
                if ($moduleManager->isInstalled('ps_emailsubscription')) {
                    $value += Db::getInstance(_PS_USE_SQL_SLAVE_)->getValue(
                        '
					SELECT COUNT(*)
					FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'emailsubscription') . '
					WHERE active = 1
					' . Shop::addSqlRestriction(Shop::SHARE_ORDER)
                    );
                }
            ...
            case 'average_order_value':
                $row = Db::getInstance(_PS_USE_SQL_SLAVE_)->getRow(
                    '
				SELECT
					COUNT(' . Db::quoteIdentifier('id_order') . ') AS orders,
					SUM(' . Db::quoteIdentifier('total_paid_tax_excl') . ' / ' . Db::quoteIdentifier('conversion_rate') . ') AS total_paid_tax_excl
				FROM ' . Db::quoteIdentifier(_DB_PREFIX_ . 'orders') . '
				WHERE ' . Db::quoteIdentifier('invoice_date') . ' BETWEEN \'' . pSQL(date('Y-m-d', strtotime('-31 day'))) . ' 00:00:00\' AND \'' . pSQL(
                        date('Y-m-d', strtotime('-1 day'))
                    ) . ' 23:59:59\'
				' . Shop::addSqlRestriction()
                );
