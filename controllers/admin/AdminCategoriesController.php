...
class AdminCategoriesControllerCore extends AdminController
{
	...
	public function renderForm()
	{
		...
		{
			$thumb = $image;
			$thumb_url = ImageManager::thumbnail($image, $this->table.'_'.(int)$obj->id.'-thumb.'.$this->imageType, 125,
	...
	public function processForceDeleteThumb()
	{
		...
		if (Validate::isLoadedObject($category))
		{
			...
			$formated_medium = ImageType::getFormatedName('medium');
			foreach ($images_types as $k => $image_type)
				if ($formated_medium == $image_type['name'] && file_exists(_PS_CAT_IMG_DIR_.$category->id.'-'.$image_type['name'].'.'.$this->imageType)
