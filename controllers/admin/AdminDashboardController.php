...
class AdminDashboardControllerCore extends AdminController
{
    ...
    protected function getOptionFields()
    {
        ...
        foreach ($modules as $module) {
            if (isset($module->tab) && 'payments_gateways' == $module->tab && $module->id) {
    ...
    public function renderView()
    {
        ...
        if (is_null($stats_compare_from) || '0000-00-00' == $stats_compare_from) {
            $stats_compare_from = null;
        }

        if (is_null($stats_compare_to) || '0000-00-00' == $stats_compare_to) {
            ...
            if ($rss) {
                foreach ($rss->channel->item as $item) {
                    if ($articles_limit > 0 && Validate::isCleanHtml((string) $item->title) && Validate::isCleanHtml((string) $item->description)
                        ...
                        if (in_array($this->context->mode, array(Context::MODE_HOST, Context::MODE_HOST_CONTRIB), true)) {
                            $utm_content = 'cloud';
                        } else {
