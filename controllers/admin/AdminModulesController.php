...
class AdminModulesControllerCore extends AdminController
{
    ...
    public function postProcessDownload()
    {
        ...
        if ($this->access('add')) {
            ...
            } elseif ('.tar' != substr($_FILES['file']['name'], -4) && '.zip' != substr($_FILES['file']['name'], -4)
                && '.tgz' != substr($_FILES['file']['name'], -4) && '.tar.gz' != substr($_FILES['file']['name'], -7)) {
                ...
                foreach ($modules as $name) {
                    ...
                    if (!($module = Module::getInstanceByName(urldecode($name)))) {
                        ...
                        if ('update' != $key && 'updateAll' != $key && 'checkAndUpdate' != $key) {
                            ...
                            if ('install' == $key && true === $echo && false === strpos(Tools::getValue('install'), '|') && method_exists($module, 'getContent')) {
    ...
    public function isModuleFiltered($module)
    {
        ...
        if (!empty($filter_name)) {
            if (false === stristr($module->name, $filter_name) && false === stristr($module->displayName, $filter_name) && false === stristr($module->description, $filter_name)) {
                return true;
            }
        ...
        if ('nativeModules' == $show_type_modules && !in_array($module->name, $this->list_natives_modules)) {
            return true;
        } elseif ('partnerModules' == $show_type_modules && !in_array($module->name, $this->list_partners_modules)) {
            return true;
            ...
            return true;
        } elseif ('otherModules' == $show_type_modules && (in_array($module->name, $this->list_partners_modules) || in_array($module->name, $this->list_natives_modules))) {
            return true;
        } elseif (false !== strpos($show_type_modules, 'authorModules[')) {
