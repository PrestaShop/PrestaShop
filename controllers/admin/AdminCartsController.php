        ...
        if ($this->access('edit')) {
            ...
            if (isset($product) && $product->id) {
                if (($id_product_attribute = Tools::getValue('id_product_attribute')) != 0) {
                    if (!Product::isAvailableWhenOutOfStock($product->out_of_stock) && !AttributePs::checkAttributeQty((int) $id_product_attribute, (int) $qty)) {
            ...
            if (!count($errors)) {
                ...
                if (!($qty_upd = $this->context->cart->updateQty($qty, $id_product, (int) $id_product_attribute, (int) $id_customization, $operator))) {
                    ...
                    $minimal_qty = $id_product_attribute ? AttributePs::getAttributeMinimalQty((int) $id_product_attribute) : $product->minimal_quantity;
