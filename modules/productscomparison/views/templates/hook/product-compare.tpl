...
{if $comparator_max_item}
	<form method="post" action="{$link->getModuleLink('productscomparison', 'compare', [], true)|escape:'html':'UTF-8'}" class="compare-form">
		<button type="submit" class="btn btn-default button button-medium bt_compare bt_compare{if isset($paginationId)}_{$paginationId}{/if}" disabled="disabled">
			<span>{l s='Compare' mod='productscomparison'} (<strong class="total-compare-val">{count($compared_products)}</strong>)<i class="icon-chevron-right right"></i></span>
		</button>
	...
	{if !isset($paginationId) || $paginationId == ''}
		{addJsDefL name=min_item}{l s='Please select at least one product' mod='productscomparison' js=1}{/addJsDefL}
		{addJsDefL name=max_item}{l s='You cannot add more than %d product(s) to the product comparison' mod='productscomparison' sprintf=$comparator_max_item js=1}{/addJsDefL}
