				...
				<li class="sf-search noBack" style="float:right">
					<form id="searchbox" action="{$link->getPageLink('search')|escape:'html'}" method="get">
						<p>
							...
							<input id="search_query_menu" type="text" name="search_query" value="{if isset($smarty.get.search_query)}{$smarty.get.search_query|escape:'html':'UTF-8'}{/if}" />
						</p>
					</form>
				</li>
			{/if}
				...
				if($(this).val().length > 0){
					...
					instantSearchQuery = $.ajax({
						...
						success: function(data){
							if($("#search_query_menu").val().length > 0)
							{
