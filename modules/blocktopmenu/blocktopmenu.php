...
<div class="margin-form">';
			...
			switch (substr($item, 0, strlen($value[1])))
			{
				...
				case 'CMS_CAT':
					...
					if (count($category))
					{
						$this->menu_items[] = array(
							...
							'label' => $category->name,
							'submenu' => $this->getCMSMenuItems($category->id),
						);
					}
					break;
				...
				case 'MAN':
					...
					if (!is_null($manufacturer->id))
					{
						...
						$this->menu_items[] = array(
							'selected' => $selected,
							'href' => $link->getManufacturerLink((int)$id, $manufacturer->link_rewrite),
				...
				case 'SUP':
					...
					if (!is_null($supplier->id))
					{
						...
						$this->menu_items[] = array(
							'selected' => $selected,
							'href' => $link->getSupplierLink((int)$id, $supplier->link_rewrite),
							'label' => $supplier->name
						);
					}
					break;
	...
	public function makeMenuDeprecated()
	{
		...
		foreach ($this->menu_items as $item)
		{
			...
			{
				$MENU .= $this->makeSubMenuDeprecated($item['submenu']);
			}
	...
	public function makeSubMenuDeprecated($items) {
		$html = '<ul>' . PHP_EOL;
		foreach ($items as $item)
		{
			$html .= '<li ' . ($item['selected']?'class="sfHoverForce"':'') . '>' . PHP_EOL .
				'<a href="' . $item['href'] . '" '. ((isset($item['new_window']) && $item['new_window'])?'target="_blank"':'') .'>' . PHP_EOL .
					$item['label'] . PHP_EOL .
				'</a>' . PHP_EOL;
			if (isset($item['submenu']) && is_array($item['submenu']) && count($item['submenu']) > 0)
			{
				$html .= $this->makeSubMenuDeprecated($item['submenu']);
			}
			$html .= '</li>' . PHP_EOL;
		}
		$html .= '</ul>' . PHP_EOL;
		...
		if (count($categories) || count($pages))
		{
			foreach ($categories as $category)
			{
				$submenu[] = array(
					...
					'href' => '#',
					'label' => $category['name'],
					'submenu' => $this->getCMSMenuItems($category['id_cms_category'], (int)$depth + 1)
				);
			}
