...
class Smarty_Internal_Data {
    ...
    {
    	if (is_null(self::$listLogs))
    	{
			self::$listLogs = array();
			if (file_exists(_PS_TOOL_DIR_.'smarty.duplicate.php'))
				self::$listLogs = include(_PS_TOOL_DIR_.'smarty.duplicate.php');
			self::$listLogs[$_SERVER['REQUEST_URI']] = array();
    	}

		foreach ($this->duplicateLog as $key => $log)
			if (count($log) > 1)
			{
				if (!isset(self::$listLogs[$_SERVER['REQUEST_URI']][$key]))
					self::$listLogs[$_SERVER['REQUEST_URI']][$key] = array();
				foreach ($log as $k => $v)
					self::$listLogs[$_SERVER['REQUEST_URI']][$key][$k] = $v;
			}
