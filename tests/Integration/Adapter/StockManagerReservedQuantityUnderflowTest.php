...
class StockManagerReservedQuantityUnderflowTest extends KernelTestCase
{
    ...
    public function testRepositoryRecomputeReplacesAStaleReservationWithZero(): void
    {
        $db = Db::getInstance();
        $stockId = (int) $db->getValue(
        ...
        // The only open line is over-refunded, so nothing is reserved: the stale 99 has to become 0,
        // and the physical quantity has to follow it.
        $row = $db->getRow(
            'SELECT reserved_quantity, physical_quantity FROM ' . _DB_PREFIX_ . 'stock_available
             WHERE id_stock_available = ' . $stockId
        );
