...
class MailPreviewOrderLayoutTest extends KernelTestCase
{
    ...
    public function testOrderConfirmationPreviewSurvivesAnOrderWithoutACart(): void
    {
        ...
        // WHY: dated after the latest order in the data rather than after the database clock - fixtures
        // written in another timezone can sit ahead of NOW(), and the preview then picks an order that
        // does have a cart, never reaching the path under test.
        $latestDate = (string) $this->connection->executeQuery(
            'SELECT MAX(date_add) FROM ' . $this->dbPrefix . 'orders'
        )->fetchOne();
        $this->connection->executeStatement(
            'UPDATE ' . $this->dbPrefix . 'orders SET id_cart = 0, date_add = DATE_ADD(:latest, INTERVAL 1 DAY) WHERE id_order = :id',
            ['id' => $targetOrderId, 'latest' => $latestDate]
        );
        $this->assertSame(
            $targetOrderId,
            (int) Order::getOrdersWithInformations(1)[0]['id_order'],
            'The preview has to be built from the order this test stripped of its cart.'
        );
