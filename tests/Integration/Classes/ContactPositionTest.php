...
class ContactPositionTest extends KernelTestCase
{
    ...
    private function presentDefaultGrid(): array
    {
        ...
        // The grid definition modifiers read the ShopContext, which only an HTTP request's context listener
        // initializes; a kernel test has to give the builder that state itself.
        $shopId = (int) Configuration::get('PS_SHOP_DEFAULT');
        $shopContextBuilder = self::getContainer()->get(ShopContextBuilder::class);
