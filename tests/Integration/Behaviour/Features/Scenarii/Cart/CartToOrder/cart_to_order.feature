...
Feature: Check cart to order data copy
  ...
  Scenario: 1 product in cart, 1 cart rule
    ...
    Given cart rule named cartrule1 has a discount code foo1
    ...
    Given product named product1 belongs to tax group named taxrule1
    Given there is a customer named customer1 whose email is fake@prestashop.com
    Given address named address1 is associated to customer named customer1
    Given there is a carrier named carrier1
    Given carrier named carrier1 ships to all groups
    Given carrier named carrier1 applies shipping fees of 5.0 in zone zone1 for quantities between 0 and 10000
  ...
  Scenario: 3 product in cart, 1 cart rule
    ...
    Given there is a tax rule named taxrule1 in country country1 and state state1 where tax tax1 is applied
    Given product named product1 belongs to tax group named taxrule1
    Given product named product2 belongs to tax group named taxrule1
    Given product named product3 belongs to tax group named taxrule1
    Given there is a customer named customer1 whose email is fake@prestashop.com
    Given address named address1 is associated to customer named customer1
    Given there is a carrier named carrier1
    Given carrier named carrier1 ships to all groups
