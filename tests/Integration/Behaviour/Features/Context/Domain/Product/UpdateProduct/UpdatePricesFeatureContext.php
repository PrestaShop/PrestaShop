...
class UpdatePricesFeatureContext extends AbstractProductFeatureContext
{
    ...
    private function updatePrices(string $productReference, TableNode $table, ShopConstraint $shopConstraint): void
    {
        ...
        $pricesInput = new ProductPricesInput();

        if (isset($data['price'])) {
            $pricesInput->setPrice($data['price']);
        }
        if (isset($data['ecotax'])) {
            $pricesInput->setEcotax($data['ecotax']);
        }
        if (isset($data['tax rules group'])) {
            ...
            $pricesInput->setTaxRulesGroupId($taxRulesGroupId);
            Cache::clean('product_id_tax_rules_group_*');
        }
        if (isset($data['on_sale'])) {
            $pricesInput->setOnSale(PrimitiveUtils::castStringBooleanIntoBoolean($data['on_sale']));
        }
        if (isset($data['wholesale_price'])) {
            $pricesInput->setWholesalePrice($data['wholesale_price']);
        }
        if (isset($data['unit_price'])) {
            $pricesInput->setUnitPrice($data['unit_price']);
        }
        if (isset($data['unity'])) {
            $pricesInput->setUnity($data['unity']);
        }

        $command->setPrices($pricesInput);
    ...
    public function updateTaxRulesGroupWithNonExistingGroup(string $productReference): void
    {
        ...
        $pricesInput->setTaxRulesGroupId(50000000);
        $command->setPrices($pricesInput);
