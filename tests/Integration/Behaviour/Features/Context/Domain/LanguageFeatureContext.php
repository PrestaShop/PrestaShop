...
class LanguageFeatureContext extends CommonDomainFeatureContext
{
    ...
    {
        $languageId = Language::getIdByIso($isoCode, true);
        if (!is_numeric($languageId)) {
            throw new RuntimeException(sprintf('No language found with the ISOCode "%s"', $isoCode));
        }
    ...
    public function setStatusLanguage(string $statusVerb, string $isoCode): void
    {
        ...
        if (!in_array($statusVerb, ['enable', 'disable'])) {
            throw new RuntimeException(sprintf('The status verb should be "enable" or "disable", not "%s"', $statusVerb));
        }

        try {
            $this->cleanLastException();
            $this->getCommandBus()->handle(
                new ToggleLanguageStatusCommand($languageId, $statusVerb === 'enable')
            );
        } catch (LanguageException $e) {
            $this->setLastException($e);
        }
    ...
    public function checkLanguageExists(string $isoCode): void
    {
        ...
        Assert::assertNotNull(
            $languageId,
            sprintf('No language found with the ISOCode "%s"', $isoCode)
        );
    }
    ...
    public function checkLanguageNotExists(string $isoCode): void
    {
        ...
        Assert::assertNull(
            $languageId,
            sprintf('Language found with the ISOCode "%s"', $isoCode)
        );
    }
