...
Feature: Currency Data
  ...
  Scenario: Get data from an unofficial currency
    ...
    When I request reference data for "BTC"
    Then I should get error that currency was not found
    When I request displayable data for "BTC"
    Then I should get currency data:
      | iso_code         | BTC                                   |
      | numeric_iso_code |                                    |
      | precision        | 0                                     |
      | names            | en-US:Bitcoin;fr-FR:BTC |
      | symbols          | en-US:₿;fr-FR:BTC                       |
      | patterns         | en-US:¤#,##0.00;fr-FR:¤#,##0.00      |
  ...
  Scenario: Get data from a customized currency
    ...
    Then I should get currency data:
      ...
      | symbols          | en-US:¥;fr-FR:¥                       |
      | patterns         | en-US:¤#,##0.00;fr-FR:#,##0.00 ¤      |
    When I request displayable data for "JPY"
    Then I should get currency data:
      | iso_code         | JPY                                   |
      | numeric_iso_code | 392                                   |
      | precision        | 0                                     |
      | names            | en-US:Custom Yen;fr-FR:yen japonais |
      | symbols          | en-US:YY;fr-FR:¥                       |
      | patterns         | en-US:¤#,##0.00;fr-FR:¤ #,##0.00      |

  @currency-data
  Scenario: Get unknown currency data
    When I request displayable data for "AUD"
    Then I should get error that currency was not found
