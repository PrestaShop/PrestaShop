...
Feature: Order payment from Back Office
  ...
  Scenario: add order payment with negative amount to get exception Property Order->total_paid_real is not valid
    When if I query order with id 1 payments I should get 0 payments
    And I add payment to order with id 1 exception is thrown with the following properties:
    ...
    Then if I query order with id 1 payments I should get 0 payments
  ...
  Scenario: change order state to Delivered to be able to add valid invoice to new Payment
    When if I query order with id 2 payments I should get 0 payments
    And I update order with id 2 to status "Delivered"
    Then if I query order with id 2 payments I should get an Order with properties:
    | payment_method    | amount | id_invoice |
    | Payments by check |        | #IN000001  |
    When I add payment to order with id 2 with the following properties:
    | date                | payment_method    | transaction_id              | id_currency | amount | id_invoice |
