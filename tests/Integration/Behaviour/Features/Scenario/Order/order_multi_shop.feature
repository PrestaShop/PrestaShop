...
Feature: Order from Back Office (BO)
  ...
  Scenario: In All Shop Context, Update product in order
    ...
    Then the remaining available stock for product "Product A" should be 0
    When shop context "Shop 2" is loaded
    And there is a product in the catalog named "Product A" with a price of 45.6 and 100 items in stock
    And product "Product A" cannot be ordered out of stock
    Then the remaining available stock for product "Product A" should be 100
    # Create Customer
    # Context : Shop 2
    ...
    # Create Order
    # Context : Shop 2
    When I create an empty cart "cart_product_A" for customer "testCustomerShop2"
    And I select "US" address as delivery and invoice address for customer "testCustomerShop2" in cart "cart_product_A"
    And I add 2 products "Product A" to the cart "cart_product_A"
    Then the remaining available stock for product "Product A" should be 98
    ## 100 - 2 (in cart)
    When I add order "order_product_a" with the following details:
      | cart                | cart_product_A             |
      | message             | test                       |
      | payment module name | dummy_payment              |
      | status              | Awaiting bank wire payment |
    Then the remaining available stock for product "Product A" should be 96
    And order "order_product_a" should contain 2 products "Product A"
    ## 100 - 2 (in cart) - 2 (in order)
    When I create an empty cart "cart_product_A" for customer "testCustomerShop2"
    And the remaining available stock for product "Product A" should be 98
    # Check Stock
    When shop context "Shop 2" is loaded
    Then the remaining available stock for product "Product A" should be 98
    When shop context "Default" is loaded
    ...
    And I edit product "Product A" to order "order_product_a" with following products details:
      | amount        | 30                      |
      | price         | 78.90                   |
