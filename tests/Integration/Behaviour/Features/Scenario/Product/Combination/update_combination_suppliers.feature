...
Feature: Update product combination suppliers in Back Office (BO)
  ...
  Scenario: Product suppliers can be updated when some combinations doesn't exist in default shop
    ...
    And I add product "product8" with following information:
      | name[en-US] | universal T-shirt |
      | type        | combinations      |
    And product product8 type should be combinations
    And I copy product product8 from shop shop1 to shop shop2
    And default shop for product "product8" is shop1
    And I generate combinations in shop "shop2" for product "product8" using following attributes:
      | Size  | [S]           |
      | Color | [White,Black] |
    And product "product8" should have no combinations for shops "shop1"
    And product "product8" should have the following combinations for shops "shop2":
      | id reference   | combination name        | reference | attributes           | impact on price | quantity | is default |
      | product8SWhite | Size - S, Color - White |           | [Size:S,Color:White] | 0               | 0        | true       |
      | product8SBlack | Size - S, Color - Black |           | [Size:S,Color:Black] | 0               | 0        | false      |
    And product "product8" should not have a default supplier
    And combination "product8SWhite" should not have any suppliers assigned
    And combination "product8SBlack" should not have any suppliers assigned
    When I associate suppliers to product "product8"
      | supplier  | combination_suppliers                                                         |
      | supplier4 | product8SWhite:product8SWhiteSupplier4;product8SBlack:product8SBlackSupplier4 |
    Then product product8 should have the following suppliers assigned:
      | supplier4 |
    Then product product8 should have following supplier values:
      | default supplier | supplier4 |
    And product product8 should have following supplier values:
      | default supplier           | supplier4 |
      | default supplier reference |           |
    And combination "product8SWhite" should have following suppliers:
      | product_supplier        | supplier  | reference | currency | price_tax_excluded |
      | product8SWhiteSupplier4 | supplier4 |           | USD      | 0                  |
    And combination "product8SBlack" should have following suppliers:
      | product_supplier        | supplier  | reference | currency | price_tax_excluded |
      | product8SBlackSupplier4 | supplier4 |           | USD      | 0                  |
