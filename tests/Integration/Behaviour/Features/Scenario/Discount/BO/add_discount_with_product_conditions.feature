# ./vendor/bin/behat -c tests/Integration/Behaviour/behat.yml -s discount --tags add-discount-with-product-conditions
@restore-all-tables-before-feature
@restore-languages-after-feature
@add-discount-with-product-conditions
Feature: Create discount with conditions
  PrestaShop allows BO users to create discounts conditions
  As a BO user
  I must be able to create conditions on discounts
  ...
  Scenario: Create discount with conditions on attributes
    When I create a "free_shipping" discount "discount_with_restricted_attributes" with following properties:
      | name[en-US]                  | Promotion |
      | productConditionQuantity     | 42        |
      | productCondition[attributes] | S, M, L   |
  ...
  Scenario: Create discount with conditions on attributes from different groups
    When I create a "free_shipping" discount "discount_with_restricted_attributes_different_groups" with following properties:
      | name[en-US]                  | Promotion            |
      | productConditionQuantity     | 42                   |
      | productCondition[attributes] | S, M, L, red, orange |
  ...
  Scenario: Create discount with multiple conditions on attributes
    When I create a "free_shipping" discount "discount_with_restricted_multiple_attributes_groups" with following properties:
      | name[en-US]                  | Promotion   |
      | productConditionQuantity     | 42          |
      | productCondition[attributes] | S, M, L     |
      | productCondition[attributes] | red, orange |
    Then discount "discount_with_restricted_multiple_attributes_groups" should have the following properties:
      | name[en-US]                  | Promotion     |
      | type                         | free_shipping |
      | productConditionQuantity     | 42            |
      | productCondition[attributes] | S, M, L       |
