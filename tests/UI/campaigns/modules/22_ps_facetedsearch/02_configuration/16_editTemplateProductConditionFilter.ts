  ...
  describe('Product condition filter', async () => {
    ...
    [
      ...
      it('should check the "All products" page', async function () {
        ...
        const isSearchFilterRadio = await foHummingbirdCategoryPage.isSearchFilterRadio(page, 'condition');
        expect(isSearchFilterRadio).to.be.eq(test.expectedIsSearchFilterRadio);

        const isSearchFilterDropdown = await foHummingbirdCategoryPage.isSearchFilterDropdown(page, 'condition');
        expect(isSearchFilterDropdown).to.be.eq(test.expectedIsSearchFilterDropdown);

        const isSearchFilterCheckbox = await foHummingbirdCategoryPage.isSearchFilterCheckbox(page, 'condition');
