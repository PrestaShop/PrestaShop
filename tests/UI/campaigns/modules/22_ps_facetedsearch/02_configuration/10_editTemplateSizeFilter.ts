...
describe('Faceted search module - Edit template - Size filter', async () => {
  ...
  [
    ...
    it('should check the "All products" page', async function () {
      ...
      const isSearchFilterRadio = await foHummingbirdCategoryPage.isSearchFilterRadio(page, 'attribute_group', 'Size');
      expect(isSearchFilterRadio).to.be.eq(test.expectedIsSearchFilterRadio);

      const isSearchFilterDropdown = await foHummingbirdCategoryPage.isSearchFilterDropdown(page, 'attribute_group', 'Size');
      expect(isSearchFilterDropdown).to.be.eq(test.expectedIsSearchFilterDropdown);

      const isSearchFilterCheckbox = await foHummingbirdCategoryPage.isSearchFilterCheckbox(page, 'attribute_group', 'Size');
