...
const baseContext: string = 'functional_BO_catalog_discounts_cartRules_CRUDCartRule_actions_applyDiscountToSpecificProduct';

describe('BO - Cart rules - Actions : Apply a discount to Specific product', async () => {
  let browserContext: BrowserContext;
  let page: Page;

  const cartRuleData: FakerCartRule = new FakerCartRule({
    name: 'Cart Rule Discount Specific Product',
    description: 'It is a test for cart rules',
    code: 'CRDSpecificProduct',
    discountType: 'Percent',
    discountPercent: 50,
    applyDiscountTo: 'Specific product',
    freeShipping: true,
    product: dataProducts.demo_14.name,
  });
  ...
  describe('Apply a discount to Specific product', async () => {
    ...
    it('should check that discount is automatically applied to the cart', async function () {
      ...
      const discount = utilsCore.percentage(dataProducts.demo_14.finalPrice, cartRuleData.getDiscountPercent());
      ...
      expect(priceATI.toString()).to.eq((dataProducts.demo_14.finalPrice - discount).toFixed(2));
  ...
  describe('Update cart rule with Discount Percent : 100', async () => {
    ...
    it('should update the discount percent to "100"', async function () {
      await testContext.addContextItem(this, 'testIdentifier', 'updateCartRulePercent', baseContext);
    ...
    it('should check that discount is automatically applied to the cart', async function () {
      ...
      const priceATI = await foClassicCartPage.getATIPrice(page);
      expect(priceATI.toFixed(2)).to.eq((dataProducts.demo_14.finalPrice - discount).toFixed(2));
  ...
  describe('Update cart rule with errors', async () => {
    ...
    it('should update the discount percent value to -50', async function () {
      await testContext.addContextItem(this, 'testIdentifier', 'updateAmountTo-50', baseContext);
    ...
    it('should update the discount percent value to ze', async function () {
      await testContext.addContextItem(this, 'testIdentifier', 'updateAmountToze', baseContext);
  ...
  describe('Update cart rule with Discount Amount : 10€ Tax included', async () => {
    it('should update the discount amount to "10€ Tax included"', async function () {
      await testContext.addContextItem(this, 'testIdentifier', 'updateCartRuleAmountValue10', baseContext);

      cartRuleData
        .setDiscountType('Amount')
        .setDiscountAmountCurrency(dataCurrencies.euro.isoCode)
        .setDiscountAmountTax('Tax included')
        .setDiscountAmountValue(10);
      const validationMessage = await boCartRulesCreatePage.createEditCartRules(page, cartRuleData);
      expect(validationMessage).to.contains(boCartRulesCreatePage.successfulUpdateMessage);
    });
    ...
    it('should check that discount is automatically applied to the cart', async function () {
      ...
      const subTotalDiscount = await foClassicCartPage.getSubtotalDiscountValue(page);
      expect(subTotalDiscount.toFixed(2)).to.eq(`-${cartRuleData.getDiscountAmount().toFixed(2)}`);

      const subTotalShipping = await foClassicCartPage.getSubtotalShippingValue(page);
      expect(subTotalShipping).to.eq('Free');

      const priceATI = await foClassicCartPage.getATIPrice(page);
      expect(priceATI.toFixed(2)).to.eq((dataProducts.demo_14.finalPrice - cartRuleData.getDiscountAmount()).toFixed(2));
  ...
  describe('Update cart rule with Discount Amount Value : 12', async () => {
    it('should go to edit cart rule page', async function () {
      await testContext.addContextItem(this, 'testIdentifier', 'goToEditCartRulePageDiscountAmountValue', baseContext);
    ...
    it('should update the discount amount value to "12"', async function () {
      await testContext.addContextItem(this, 'testIdentifier', 'updateCartRuleAmountValue12', baseContext);

      cartRuleData.setDiscountAmountValue(12);
      const validationMessage = await boCartRulesCreatePage.createEditCartRules(page, cartRuleData);
      expect(validationMessage).to.contains(boCartRulesCreatePage.successfulUpdateMessage);
    });
  ...
  describe('Update cart rule with Discount Amount Value : 100', async () => {
    it('should go to edit cart rule page', async function () {
      await testContext.addContextItem(this, 'testIdentifier', 'goToEditCartRulePageDiscountAmountValue100', baseContext);
    ...
    it('should check that discount is automatically applied to the cart', async function () {
      ...
      let discountedPrice: number = dataProducts.demo_14.finalPrice - cartRuleData.getDiscountAmount();
      discountedPrice = discountedPrice > 0 ? discountedPrice : 0;

      const subTotalDiscount = await foClassicCartPage.getSubtotalDiscountValue(page);
      expect(subTotalDiscount.toFixed(2)).to.eq(`-${dataProducts.demo_14.finalPrice.toFixed(2)}`);

      const subTotalShipping = await foClassicCartPage.getSubtotalShippingValue(page);
      expect(subTotalShipping).to.eq('Free');

      const priceATI = await foClassicCartPage.getATIPrice(page);
      expect(priceATI.toFixed(2)).to.eq(discountedPrice.toFixed(2));
  ...
  describe('Change the product', async () => {
    ...
    it('should check that discount is automatically applied to the cart', async function () {
      await testContext.addContextItem(this, 'testIdentifier', 'checkDiscountApplied5', baseContext);
