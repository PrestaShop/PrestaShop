...
class DbQueryTest extends TestCase
{
    ...
    public function providerBuild(): array
    {
        ...
        $selectWithSubQuery->from($subQuery->build(), 'p');
        ...
        return [
            ...
            [
                $selectWithSubQuery,
                'SELECT *' . self::BREAK_LINE . 'FROM `test_SELECT *' . self::BREAK_LINE . 'FROM `test_product` p' . self::BREAK_LINE . 'WHERE (p.active = 1)' . self::BREAK_LINE . '` p' . self::BREAK_LINE . 'WHERE (p.visibility in ("both", "search"))',
            ],
        ];
    }
}
