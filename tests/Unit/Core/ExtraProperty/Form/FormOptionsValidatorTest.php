...
class FormOptionsValidatorTest extends TestCase
{
    ...
    /**
     * The policy forwards a null instead of checking it, so the type is the one that decides: a
     * type refusing null says so at save time, including when it fails with a TypeError rather than
     * an options-resolver error (ChoiceType::$expanded is typed bool).
     */
    public function testANullTheFormTypeRefusesIsCaughtAtSaveTime(): void
    {
        $errors = $this->buildValidator()->validate(
            null,
            ExtraPropertyType::CHOICE,
        ...
        $this->assertCount(1, $errors);
        $this->assertStringContainsString('could not be built with these options', $errors[0]);
    }
