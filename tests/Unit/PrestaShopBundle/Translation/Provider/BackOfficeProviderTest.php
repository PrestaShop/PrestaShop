...
class BackOfficeProviderTest extends TestCase
{
    ...
    {
        self::$tempDir = implode(DIRECTORY_SEPARATOR, [sys_get_temp_dir(), 'BackOfficeProviderTest']);

        // Default catalogue
        $catalogue = new MessageCatalogue(BackOfficeProvider::DEFAULT_LOCALE);
        ...
        (new XliffFileDumper())->dump($catalogue, [
            'path' => self::$tempDir . DIRECTORY_SEPARATOR . BackOfficeProvider::DEFAULT_LOCALE,
        ]);
    ...
    {
        $catalogue = (new BackOfficeProvider(
    ...
    {
        $provider = (new BackOfficeProvider(
            $this->databaseTranslationLoader,
            self::$tempDir
        ));
