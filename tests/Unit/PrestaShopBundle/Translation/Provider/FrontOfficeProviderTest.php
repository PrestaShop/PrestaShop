...
class FrontOfficeProviderTest extends TestCase
{
    ...
    {
        self::$tempDir = implode(DIRECTORY_SEPARATOR, [sys_get_temp_dir(), 'FrontOfficeProviderTest']);

        // Default catalogue
        $catalogue = new MessageCatalogue(FrontOfficeProvider::DEFAULT_LOCALE);
        ...
        (new XliffFileDumper())->dump($catalogue, [
            'path' => self::$tempDir . DIRECTORY_SEPARATOR . FrontOfficeProvider::DEFAULT_LOCALE,
        ]);
    ...
    {
        $catalogue = (new FrontOfficeProvider(
    ...
    {
        $provider = (new FrontOfficeProvider(
            $this->databaseTranslationLoader,
            self::$tempDir
        ));
    ...
    public function testGetDatabaseCatalogue()
    {
        ...
        foreach ($dbTranslations as &$dbTranslation) {
            $dbTranslation['theme'] = FrontOfficeProvider::DEFAULT_THEME_NAME;
        }
