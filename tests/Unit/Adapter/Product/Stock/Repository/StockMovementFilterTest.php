...
class StockMovementFilterTest extends TestCase
{
    ...
    public function testItReturnsReasonIds(): void
    {
        $filter = (new StockMovementFilter())->setReasonIds(
            new MovementReasonId(1),
            new MovementReasonId(2),
            new MovementReasonId(3)
        );
        Assert::assertSame(
            '1,2,3',
            $filter->getReasonIdsAsString()
        );
        Assert::assertSame(
            '1|2|3',
            $filter->getReasonIdsAsString('|')
