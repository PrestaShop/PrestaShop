...
class DebugModeTest extends TestCase
{
    ...
    /**
     * A value this class writes is the configuration, and it has to win over the constant: right after a
     * switch the running process still holds the previous one, and SwitchDebugModeHandler compares against
     * this method to decide whether there is anything to write.
     */
    public function testAValueThisClassWritesWinsOverTheConstant(): void
    {
        $this->assertFalse($this->debugMode(true, 'false')->isDebugModeEnabled());
        $this->assertTrue($this->debugMode(false, 'true')->isDebugModeEnabled());
        $this->assertTrue($this->debugMode(false, "isset(\$_COOKIE['debug'])")->isDebugModeEnabled());
    }
}
