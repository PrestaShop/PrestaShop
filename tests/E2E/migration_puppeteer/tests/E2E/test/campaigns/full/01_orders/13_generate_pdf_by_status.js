...
scenario('Generate a PDF by status', () => {
  ...
  scenario('Generate a PDF by "Awaiting bank wire payment" status', client => {
    ...
    scenario('Get the created Orders information', client => {
      ...
      for (let i = 1; i <= 2; i++) {
        test('should go the order n°' + i, async () => {
          await client.waitForExistAndClick(OrderPage.order_view_button.replace("%ORDERNumber", i));
        });
        test('should go to "Documents" tab', async () => {
          await client.waitForExistAndClick(OrderPage.documents_tab);
          await page.waitForSelector('#documents.active');
        });
