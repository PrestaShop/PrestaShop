...
scenario('Test1: Delivery slips PDF', () => {
  ...
  scenario('Verify all the information on the deliveries slips', client => {
    ...
    test('should generate the "deliveries" pdf file', async () => {
      await client.setDownloadBehavior();
      await client.waitForExistAndClick(DeliverySlip.generate_btn, 2000);
...
scenario('Test2: Delivery slips options', () => {
  scenario('Change "Delivery slips" options', client => {
    test('should go to the Delivery Slips page', () => client.goToSubtabMenuPage(Menu.Sell.Orders.orders_menu, Menu.Sell.Orders.delivery_slips_submenu));
    test('should change the "Delivery prefix"', async () => {
      await client.getAttributeInVar(DeliverySlip.delivery_prefix, 'value', 'deliveryPrefix');
      await client.waitAndSetValue(DeliverySlip.delivery_prefix, 'PR');
    });
    test('should change the "Delivery Number"', async () => {
      await client.getAttributeInVar(DeliverySlip.delivery_number, 'value', 'delivNumber');
      await client.waitAndSetValue(DeliverySlip.delivery_number, (Number(global.tab['delivNumber']) + 1).toString(), 1000);
    });
  ...
  scenario('Create Order', client => {
    test('should open the Front Office in new window', () => client.linkAccess(URL));
    commonOrder.createOrderFO();
  }, 'common_client');
  scenario('Check the delivery slip modified option', client => {
    ...
    scenario('Check that the option name of the delivery slip is updated successfully', client => {
      ...
      test('should generate the "deliveries" pdf file', async () => {
        await client.setDownloadBehavior(true, OrderPage.delivery_slip_document);
        await client.waitForExistAndClick(OrderPage.delivery_slip_document, 2000);
