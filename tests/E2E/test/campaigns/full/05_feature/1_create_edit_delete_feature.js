...
scenario('Create, edit, delete, delete with bulk actions "Feature" in the Back Office', () => {
  ...
  scenario('Test 1: Create "Feature"', () => {
    ...
    scenario('Edit a product in the Back Office then check product in the Front Office', client => {
      ...
      test('should search for "' + secondProductData["name"] + '" product', () => client.searchByValue(AddProductPage.catalogue_filter_by_name_input, AddProductPage.catalogue_submit_filter_button, secondProductData.name));
      test('should click on "Edit" button', () => client.waitForExistAndClick(ProductList.edit_button));
      for (let f = 0; f < secondProductData['feature'].length; f++) {
        ...
        test('should select the created feature', () => client.selectFeature(AddProductPage, secondProductData['feature'][f].name + date_time, secondProductData['feature'][f].value, f));
      }
      ...
      test('Verify that there is only "Value 1" for "' + secondProductData['feature'][0].name + date_time + '"', () => client.isNotExisting(productPage.value_feature_text));
      ...
      test('should select the created feature and enter a customized value "custom 1"', () => client.selectFeatureCustomizedValue(AddProductPage, secondProductData['feature'][0].name + date_time, 'custom 1', 1));
      test('should click on "Save" button', () => client.waitForExistAndClick(AddProductPage.save_product_button, 5000));
      test('should verify the appearance of the green validation', () => client.checkTextValue(AddProductPage.validation_msg, 'Settings updated.'));
      test('should click on "Preview" button', () => client.waitForExistAndClick(AddProductPage.preview_buttons));
      ...
      commonProduct.clickOnPreviewLink(client, AddProductPage.preview_link, productPage.product_name);
      test('should click on "Product details" tab', () => client.waitForExistAndClick(productPage.product_detail_tab));
      test('should verify that "value 1" and "custom 1" exist', () => client.checkValuesFeature(productPage.product_value_text.replace('%B', 'first'), secondProductData['feature'][0].value + '\n\n' + 'custom 1'));
      test('should go back to the Back Office', () => client.switchWindow(0));
      test('should click on "Add feature" button', () => client.scrollWaitForExistAndClick(AddProductPage.add_feature_to_product_button));
      test('should select the created feature and choose pre defined value "' + myFeatureData.values[1] + '"', () => client.selectFeature(AddProductPage, secondProductData['feature'][0].name + date_time, myFeatureData.values[1], 2));
      test('should click on "Save" button', () => client.waitForExistAndClick(AddProductPage.save_product_button, 5000));
      test('should verify the appearance of the green validation', () => client.checkTextValue(AddProductPage.validation_msg, 'Settings updated.'));
      test('should click on "Preview" button', () => client.waitForExistAndClick(AddProductPage.preview_buttons));
      ...
      commonProduct.clickOnPreviewLink(client, AddProductPage.preview_link, productPage.product_name);
      test('should click on "Product details" tab', () => client.waitForExistAndClick(productPage.product_detail_tab));
      test('should verify that "' + secondProductData['feature'][0].name + date_time + '" exist', () => {
        return promise
          .then(() => client.pause(3000))
          .then(() => client.checkTextValue(productPage.product_feature_text.replace('%B', 'first'), secondProductData['feature'][0].name + date_time));
      });
      test('should verify that "value 1", "value 2" and "custom 1" exist', () => {
        return promise
          .then(() => client.pause(2000))
          .then(() => client.checkValuesFeature(productPage.product_value_text.replace('%B', 'first'), secondProductData['feature'][0].value + '\n' + 'Value 2' + '\n' + 'Custom 1'));
      });
      test('should go back to the Back Office', () => client.switchWindow(0));
      test('should click on "Add feature" button', () => client.scrollWaitForExistAndClick(AddProductPage.add_feature_to_product_button));
      ...
      test('should verify that "' + secondProductData['feature'][0].name + date_time + '" exist', () => {
        return promise
          .then(() => client.pause(3000))
          .then(() => client.checkTextValue(productPage.product_feature_text.replace('%B', 'last'), secondProductData['feature'][0].name + date_time));
      });
      test('should verify that "value 1", "value 2" and "custom 1" exist', () => {
        return promise
          .then(() => client.pause(2000))
          .then(() => client.checkValuesFeature(productPage.product_value_text.replace('%B', 'last'), secondProductData['feature'][0].value + '\n' + 'Value 2' + '\n' + 'Custom 1'));
      });
  ...
  scenario('Test 2: Edit, delete "Feature"', () => {
    require('../../high/05_feature/1_create_edit_delete_feature');
  }, 'attribute_and_feature', true);

  scenario('Test 3: Delete "Feature" with bulk actions', () => {
    scenario('Login in the Back Office', client => {
      test('should open the browser', () => client.open());
      test('should login successfully in the Back Office', () => client.signInBO(AccessPageBO));
    }, 'attribute_and_feature');
    commonFeature.createFeature(myFeatureData);
    commonProduct.createProduct(AddProductPage, firstProductData);
    commonFeature.featureBulkActions(myFeatureData, 'delete');
    scenario('Logout from the Back Office', client => {
      test('should logout successfully from the Back Office', () => client.signOutBO());
    }, 'attribute_and_feature');
    ...
    commonFeature.checkDeletedFeatureInFO(firstProductData.name);
  }, 'attribute_and_feature', true);
}, 'common_client');
