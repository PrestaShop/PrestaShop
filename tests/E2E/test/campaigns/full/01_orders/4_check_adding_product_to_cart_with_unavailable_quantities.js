...
scenario('check adding a product to the cart with unavailable quantities', () => {
  ...
  scenario('Create order in the Front Office', client => {
    ...
    scenario('check adding unavailable quantities to product in the cart', client => {
      ...
      test('should set the product quantity to 6', () => client.waitAndSetValue(CheckoutOrderPage.quantity_input, 6));
      ...
      test('should check the existence of "There are not enough products in stock" warning message', () => client.checkTextValue(productPage.product_availability_message, "There are not enough products in stock", "contain"));
    }, 'order');
