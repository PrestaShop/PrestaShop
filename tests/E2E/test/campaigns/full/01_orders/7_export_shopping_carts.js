...
scenario('Export shopping carts in the Back Office', () => {
  ...
  scenario('Search for the shopping carts to export', client => {
    ...
    test('should filter by customer "DOE" and carrier "My carrier"', () => {
      return promise
        .then(() => client.waitAndSetValue(ShoppingCart.search_input.replace('%searchParam', 'c!lastname'), 'DOE'))
        .then(() => client.waitAndSetValue(ShoppingCart.search_input.replace('%searchParam', 'ca!name'), 'My carrier'))
        .then(() => client.waitForExistAndClick(ShoppingCart.search_button))
        .then(() => client.getShoppingCartNumber(ShoppingCart.id_shopping_carts))
