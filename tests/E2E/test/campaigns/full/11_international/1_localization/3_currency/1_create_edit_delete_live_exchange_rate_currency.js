...
scenario('Create, edit, delete and live exchange rate currency', () => {
  ...
  scenario('Test1: create, check and sort "Currency"', () => {
    ...
    scenario('Enable currency', client => {
      test('should click on "Enable icon"', () => client.waitForExistAndClick(Localization.Currencies.icon_check.replace('%ID', 1)
    ...
    commonCurrency.sortCurrency(Localization.Currencies.currency_exchange_rate, 'Exchange rate');
    commonCurrency.checkCurrencyByIsoCode(firstCurrencyData);
  ...
  scenario('Test2: edit "Currency"', () => {
    ...
    scenario('Check the "Status" of the edited currency', client => {
      test('should check that the "Status" icon is well disabled', () => client.waitForExist(Localization.Currencies.icon_check.replace('%ID', 1)
        .replace('%ICON', "icon-remove")));
    }, 'common_client');
  ...
  scenario('Test3: delete "Currency"', () => {
    commonCurrency.accessToCurrencies();
    commonCurrency.checkCurrencyByIsoCode(currencyData);
    commonCurrency.deleteCurrency(false, false);
    commonCurrency.checkCurrencyByIsoCode(currencyData);
    commonCurrency.deleteCurrency(true, true, deleteSuccessMessage);
