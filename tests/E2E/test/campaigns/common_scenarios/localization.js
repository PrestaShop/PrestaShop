...
module.exports = {
  ...
  checkExistenceLanguage(language) {
    scenario('Check the existence language in the Back Office', client => {
      ...
      test('should search for "' + language.toUpperCase() + '" language', () => client.searchByValue(Localization.Localization.language_name_input, Localization.Localization.search_button, language));
      test('should check  "' + language.toUpperCase() + '" language exists', () => client.checkTextValue(Localization.Localization.language_column.replace("%ID", 4), language, 'contain'));
      test('should click on "Reset" button', () => client.waitForExistAndClick(Localization.Localization.reset_button));
    }, 'common_client');
  },
