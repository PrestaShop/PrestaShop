...
module.exports = {
  ...
  sortCurrency: function (selector, sortBy) {
    ...
    scenario('Check the sort of currencies by "' + sortBy.toUpperCase() + '"', client => {
      test('should click on "Sort by ASC" icon', () => {
        ...
        return promise
          .then(() => client.waitForExistAndClick(Localization.Currencies.sort_icon.replace("%B", sortBy)
            .replace("%W", 2)));
      });
      ...
      test('should click on "Sort by DESC" icon', () => client.waitForExistAndClick(Localization.Currencies.sort_icon.replace("%B", sortBy)
        .replace("%W", 1)));
    ...
    scenario('Search the currency in the back office by "Status"', client => {
      ...
      test('should check if all displayed currencies are enabled ', () => {
        for (let j = 1; j <= (parseInt(tab["number_currency"])); j++) {
          promise = client.waitForExist(Localization.Currencies.icon_check.replace('%ID', j)
  ...
  deleteCurrency(confirmDelete, successMessage = true, message = '') {
