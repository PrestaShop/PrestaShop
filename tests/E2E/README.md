...
By default all the tests will be run. You can ask for another campaign by setting the `TEST_CAMPAIGN` env parameter.

Example with regular suite:
```
docker run -ti --privileged -e TEST_CAMPAIGN=test prestashop/e2e
```
