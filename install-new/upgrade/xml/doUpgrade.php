...
define('UPGRADE_PATH', INSTALL_PATH.'/upgrade/');
...
if(!defined('_PS_INSTALLER_PHP_UPGRADE_DIR_'))
	define('_PS_INSTALLER_PHP_UPGRADE_DIR_',  UPGRADE_PATH.'functions/');

// No more infinite require_once on functions !
foreach (scandir(_PS_INSTALLER_PHP_UPGRADE_DIR_) as $file)
	if ($file[0] != '.' && $file != 'index.php' && preg_match('#\.php$#i', $file))
		require_once _PS_INSTALLER_PHP_UPGRADE_DIR_.$file;
...
//check DB access
include_once(UPGRADE_PATH.'classes/ToolsInstall.php');
...
$upgradeFiles = array();
if ($handle = opendir(UPGRADE_PATH.'sql'))
{
...
//refresh conf file
require_once(UPGRADE_PATH.'classes/AddConfToFile.php');
...
{
	$file = UPGRADE_PATH.'sql/'.$version.'.sql';
	if (!file_exists($file))
	{
