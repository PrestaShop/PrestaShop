...
> For the Docker environment shipped in this repository, resetting to a clean shop, switching branches, rebuilding assets and building the test database, see [DOCKER-ENV.md](DOCKER-ENV.md).
...
- Integration, Behat and UI suites need the test database: run `composer create-test-db`, never `tests/bin/create-test-db.php` on its own. It also deletes tracked fixtures under `tests/Resources/modules/`, so check `git status` afterwards. See [DOCKER-ENV.md](DOCKER-ENV.md).
