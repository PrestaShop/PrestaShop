...
- Grid views (`src/Core/Grid/View/`, feature flag `grid_views`) let an employee save/share named views (filters + sorting + page size) per grid. The panel data is injected by `GridPresenter` under the `grid_views` key; view criteria are always read server-side (`ps_admin_filter` at save time, `GridViewFiltersBuilder` applying `?grid_view={id}` at click time), never from the client. Writes go through the GridView CQRS domain and the identifiable-object CRUD form pattern — see [Domain/GridView/CONTEXT.md](../../Domain/GridView/CONTEXT.md)
- `GridFactoryProvider` (`src/Core/Grid/`) locates any grid factory by grid id: every service implementing `GridFactoryInterface` is tagged `core.grid_factory` (via `_instanceof` in `services/core/grid/grid_factory.yml`) and `GridFactoryLocatorPass` indexes them by the `GRID_ID` constant of their definition factory (or an explicit `grid_id` tag attribute). Used by `GridViewCounter` (record counts) and `GridViewCsvExporter` (CSV export) to rebuild grids server side without any page crawling or client-supplied SQL
