...
- **Association existence is checked twice, both times phase-based.** (1) The pausing `association_validation` phase between `database` and `association`: every file row exists in the DB by then, so each batch probes accessory targets with plain per-row DB lookups — stateless, cursor-resumable, no in-memory identity set, and it shares the target-decision logic (`ProductIdentityResolver::resolveProductTarget()`) with the association phase so both always agree. (2) At `association` phase resolution, defensively (DB may have changed) — failures are errors but the run completes.
