...
The non-obvious part: each extra table is **built by mirroring the primary key of the matching native base table** via Doctrine introspection (`createExtraTableFromBaseTable`), not from hardcoded columns. Base table per scope: `common`→`{entity}`, `lang`→`{entity}_lang`, `shop`→`{entity}_shop` (`ExtraPropertyDefinition::getBaseTableName()`). So `product_extra_lang`'s PK is exactly `product_lang`'s PK (`id_product,id_lang,id_shop`) — but `contact_extra_lang`'s PK is `(id_contact,id_lang)` because `contact_lang` has no `id_shop`: only the 7 multilang-multishop entities (`multilang_shop` ObjectModel flag: product, category, carrier, meta, cms, cms_category, customization_field) have a shop-aware lang table. This mirroring guarantees every grid LEFT JOIN matches **at most one row** (1:1), so no `GROUP BY` is ever needed. Whether a definition's storage table carries `id_shop` is exposed as **`ExtraPropertyDefinition::isMultiShop()`** — schema-derived by the repository (synthetic `multi_shop` row key from `SHOW COLUMNS`, same mechanism as `nullable`/`enum_values`), consumed by the reader, writer and grid modifier so none of them ever references a non-existent `id_shop` column. SHOP scope requires a native `{entity}_shop` table (registration throws `BASE_TABLE_NOT_FOUND` otherwise) — entities bound to one shop via a plain `id_shop` column (e.g. `orders`) cannot take SHOP-scoped properties, by design.
...
- **Registry failure contract: every hard failure throws `ExtraPropertyRegistryException`** (`src/Core/ExtraProperty/Exception/`) — nothing returns `false`; the reason is carried by the exception **code** (`BASE_TABLE_NOT_FOUND`, `SCOPE_CONFLICT`, `DESTRUCTIVE_SCHEMA_CHANGE`, `PERSISTENCE_FAILURE`, `SCHEMA_FAILURE`, `INVALID_FORM_OPTIONS`), one shared class instead of a class per reason (same pattern as the Domain constraint exceptions); raw DBAL driver exceptions are wrapped as `$previous`, and `getErrors()` exposes an optional list of individual errors (populated for `INVALID_FORM_OPTIONS` only so far — one entry per invalid form option). `register()` returns `int` (the row id), `unregister()` returns `void`. The CQRS handlers catch core exceptions and wrap them ALL into the Domain `ExtraPropertyRegistrationFailureException` via `fromCoreException()` — handlers never let a core exception escape — the core code maps to a matching domain code, and the controller maps each domain code to a specific BO flash message in `ExtraPropertyDefinitionController::getErrorMessages()` (code-keyed sub-array). `Module::registerExtraProperty()`/`unregisterExtraProperty()` keep their `bool` signature but always return `true` — failures throw, module install code should `catch (ExtraPropertyException $e)`.
- **Registry atomicity: DDL-first ordering, no transactions.** MySQL DDL implicitly commits, so a transaction around "row write + DDL" could never roll the row back — instead `register()` runs `ensureExtraTableAndColumn()` BEFORE the single row write: a failed creation persists nothing (the orphan-definition bug), a failed update leaves the previous row intact. If the row write itself fails after DDL on a creation, the column just added is removed (best-effort compensation, gated on `ensureExtraTableAndColumn(): bool` returning true = "column newly added", so a pre-existing column with data is never dropped). `unregister()` is inverted the same way: row delete first, column drop second — a failed drop leaves a benign unreferenced column (healed by re-registering) instead of a definition pointing at a missing column.
...
- **ShopConstraint semantics (multishop, #41568).** Every value read/write takes a `ShopConstraint` and honors all its shapes. **Writes fan out**: a shop-group / all-shops / `ShopCollection` constraint upserts one row per shop in scope for per-shop storage (`isMultiShop()` tables), batched into a single multi-row UPSERT per table — native ObjectModel parity (`ExtraPropertyWriter` resolves the scope via `Core\Shop\ShopListResolver`, whose group/all scopes only contain usable shops: `deleted = 0 AND active = 1`, like `Shop::getContextListShopID()`). **SHOP-scope rows follow the native association rule**: broad scopes (group/all) only refresh shops the entity is associated with in `{entity}_shop` (a later-associated shop reads the definition default until the next save), while explicitly named shops (single shop, `ShopCollection`) always get their row; LANG rows cover the full scope regardless of associations, like native lang-multishop writes. **Reads pin one deterministic shop**: single-shop → that shop; otherwise the constraint's *representative* shop (default shop when in scope, else lowest id) — the same rule the BO/API context listeners now use to set `ShopContext::getId()`, so grid pinning and reader agree. **Toggle** reads the representative value and writes its inverse to every shop in scope (uniformizes divergence); LANG toggles need a `langId`. The constraint's `isStrict` flag is meaningless here (no group/global storage rows) and is ignored. Legacy side: `Context::getShopConstraint()` maps `Shop::getContext()` (all/group/shop) and `ObjectModel::persistExtraProperties` follows `$id_shop_list` via `ShopCollection`; `ObjectModel::delete()` calls `deleteForShops()` on partial (per-shop) deletion and `deleteAll()` on full deletion.
...
- Multishop handling [#41568] is implemented — see the *ShopConstraint semantics* bullet above (fan-out writes, representative-shop reads, schema-derived `isMultiShop()` detection).
- Open sub-issues: advanced form placement via `property_path` [#41425], automated tests [#41541], validation hardening [#41544], docs [#41429], security review [#41640], translation scanner support [#41725], cart entity [#41424].
