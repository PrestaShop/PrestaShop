...
- **`_legacy_controller` is permanent — but only for pages actually migrated from a legacy `AdminXController`:** unlike `_legacy_feature_flag`, it stays on the route for the lifetime of the page even after full migration, because it's used for (1) permission checks — admin permission rules are keyed by the legacy controller name (`is_granted('read', request.get('_legacy_controller'))`), and (2) automatic legacy link conversion — third-party modules and old code paths still call `Link::getAdminLink('AdminFoo')` and rely on this attribute to resolve to the Symfony route. Removing it from a migrated page breaks both
- **Never add `_legacy_controller` to a fully-native route (no legacy heritage) just to make navigation work.** It used to be misused this way — declaring a `_legacy_controller` with no real legacy PHP controller behind it, purely so the sidebar/breadcrumb could resolve a `Tab` for the page. That gap is now filled by the `_parent` route attribute (see [Router/CONTEXT.md](../Router/CONTEXT.md)); permission checks on native controllers use hardcoded `is_granted('read', 'AdminFoo')` strings instead of `request.get('_legacy_controller')`
